...
if "%1"=="" goto loopend
    ...
    if /I "%1"=="--no-webapp" (
                    set MVN_PARAMS=%MVN_PARAMS% -Dno.webapp=true
            shift
                goto loopstart
    )  else (
    if /I "%1"=="--skip-tests" (
                    set MVN_PARAMS=%MVN_PARAMS% -DskipTests=true
            shift
                goto loopstart
    )  else (
...
%MAVEN_EXECUTABLE% -Djava.awt.headless=true com.atlassian.maven.plugins:maven-clover2-plugin:3.0.2:setup verify com.atlassian.maven.plugins:maven-clover2-plugin:3.0.2:clover com.atlassian.maven.plugins:maven-clover2-plugin:3.0.2:log %MVN_PARAMS%
