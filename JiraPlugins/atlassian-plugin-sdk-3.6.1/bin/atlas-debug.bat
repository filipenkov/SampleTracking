...
if "%1"=="" goto loopend
    ...
    if /I "%1"=="--jvm-debug-port" (
                    set MVN_PARAMS=%MVN_PARAMS% -Djvm.debug.port=%2
            shift
            shift
                goto loopstart
    )  else (
    if /I "%1"=="--jvm-debug-suspend" (
...
echo Executing: %MAVEN_EXECUTABLE% com.atlassian.maven.plugins:maven-amps-dispatcher-plugin:3.6.1:debug %MVN_PARAMS%
%MAVEN_EXECUTABLE% com.atlassian.maven.plugins:maven-amps-dispatcher-plugin:3.6.1:debug %MVN_PARAMS%
