...
import com.atlassian.templaterenderer.velocity.DynamicParserPoolUtil;
...
public class VelocityTemplateRendererImpl implements VelocityTemplateRenderer
{
            ...
            TemplateContextFactory templateContextFactory)
    {
        ...
        if (DynamicParserPoolUtil.canUseDynamicParserPool(compositeClassLoader))
        {
    ...
    private VelocityContext createContext(Map<String, Object> contextParams)
    {
        final VelocityContext velocityContext =
                new VelocityContext(templateContextFactory.createContext(pluginKey, contextParams));

        velocityContext.attachEventCartridge(createCartridgeFrom
                (
                        ImmutableList.of(new DisableHtmlEscapingDirectiveHandler(new TemplateRendererHtmlAnnotationEscaper())))
        );
