...
public class TestAddProject extends BaseJiraWebTest
{
    ...
    @Test
    public void testAddWithFieldsExceedingLimits()
    {
        AddProjectDialog dialog = jira.gotoLoginPage().loginAsSysAdmin(ViewProjectsPage.class)
                .openCreateProjectDialog();

        dialog.createProjectFail(StringUtils.repeat("K", 256), StringUtils.repeat("N", 151), "fred");
        Map<String, String> errors = dialog.getFormErrors();
        assertEquals("The project name must not exceed 80 characters in length.", errors.get(FIELD_NAME));
        assertEquals("The key field should have a maxlength, which is expected will prevent the need for a validation error.", null, errors.get(FIELD_KEY));
    }
