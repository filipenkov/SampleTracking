...
{
    public static final String DEFAULT_ICON_URL = "/images/icons/issue_subtask.gif";
    ...
    public void addSubtaskDialog()
    {
        ...
        final ManageSubtasksPage page = jira.gotoLoginPage().loginAsSysAdmin(ManageSubtasksPage.class);
    ...
    public void addSubtaskPage()
    {
        ...
        jira.gotoLoginPage().loginAsSysAdmin(ManageSubtasksPage.class);
    ...
    {
        ManageSubtasksPage.Subtask noDescription = new ManageSubtasksPage.Subtask("NoDescription", null, DEFAULT_ICON_URL);
        ManageSubtasksPage.Subtask description = new ManageSubtasksPage.Subtask("WithDescription", "Description of Something", DEFAULT_ICON_URL);
        ManageSubtasksPage.Subtask customIcon = new ManageSubtasksPage.Subtask("CustomIcon", null, "/image/custom.gif");
        ...
        final ManageSubtasksPage subtasksPage = addSubtaskType.setName(customIcon.getName())
                .setIconUrl(customIcon.getIconUrl()).submitSuccess();
    ...
    private void testErrors(Function<Void, AddSubtaskType> opener)
    {
        ...
        assertThat(subtask.getSubtaks(),Matchers.<ManageSubtasksPage.Subtask>hasItem(
                subtask(new ManageSubtasksPage.Subtask("Bug2", null, DEFAULT_ICON_URL))));
    }
    ...
    public void testIconPickerDialog()
    {
        ...
        final ManageSubtasksPage subtask = jira.gotoLoginPage().loginAsSysAdmin(ManageSubtasksPage.class);
        testIconPickerPicker(new Function<Void, AddSubtaskType>()
        {
            @Override
            public AddSubtaskType apply(@Nullable Void input)
            {
                return subtask.addSubtask();
            }
        });
    }
    ...
    public void testIconPickerPicker(Function<Void, AddSubtaskType> opener)
    {
        AddSubtaskType addSubtaskType = opener.apply(null);

        addSubtaskType.setIconUrl("random.gif");

        final IconPicker.IconPickerPopup iconPickerPopup = addSubtaskType.openIconPickerPopup();

        //Does clicking an icon work?
        assertTrue(iconPickerPopup.selectIcon("subtask.gif"));
        assertThat(addSubtaskType.getIconUrl(), endsWith("subtask.gif"));

        //Does typing in a value work?
        addSubtaskType.openIconPickerPopup();
        final String url = "/test/imagesit/works.png";
        iconPickerPopup.submitIconUrl(url);
        assertThat(addSubtaskType.getIconUrl(), equalTo(url));

        addSubtaskType.cancel();
    }
    ...
    public void testWebSudoPage() throws IOException
    {
        ...
        }, AddSubtaskTypePage.class, AdminSummaryPage.class);
    }

    private void testWebsudo(Function<Void, JiraWebSudo> opener,
            Class<? extends AddSubtaskType> nextPage, Class<?> cancelPage)
    {
        ManageSubtasksPage.Subtask subtask = new ManageSubtasksPage.Subtask("WithDescription", "Description of Something", DEFAULT_ICON_URL);
