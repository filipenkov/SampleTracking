...
{
    @Test
    public void testOpeningAndClosingDialogChangesPageTitleAndBackAgain()
    {
        backdoor.restoreBlankInstance();

        ProjectSummaryPageTab config = jira.gotoLoginPage().loginAsSysAdmin(ProjectSummaryPageTab.class, "HSP");
        String originalPageTitle = config.getPageTitle();
        config.execKeyboardShortcut("g", "g");
        AdministrationSearchDialog adminSearchDialog = pageBinder.bind(AdministrationSearchDialog.class);

        assertEquals("Administration Search - jWebTest JIRA installation", config.getPageTitle());
        adminSearchDialog.close();
        assertEquals(originalPageTitle, config.getPageTitle());
    }
    ...
    public void testBrowseBackButton()
    {
        ...
        jira.gotoLoginPage().loginAsSysAdmin(AdminSummaryPage.class);

        ViewFieldConfigurationSchemesPage viewPage = jira.visit(ViewFieldConfigurationSchemesPage.class);
        ...
        pageBinder.bind(AdminSummaryPage.class);
    }

    private static ViewFieldConfigurationSchemesPage.FieldConfigurationSchemeItem fcs(String name)
    {
