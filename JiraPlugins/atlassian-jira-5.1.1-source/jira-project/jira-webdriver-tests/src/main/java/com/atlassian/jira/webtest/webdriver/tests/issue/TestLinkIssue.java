...
public class TestLinkIssue extends BaseJiraWebTest
{
    ...
    private static final String STATUS_IMAGE_OPEN = "/images/icons/status_open.gif";

    private static final String PRIORITY_IMAGE_CRITICAL = "/images/icons/priority_critical.gif";
    private static final String PRIORITY_IMAGE_MAJOR = "/images/icons/priority_major.gif";
    private static final String PRIORITY_IMAGE_MINOR = "/images/icons/priority_minor.gif";

    private static final String ISSUE_IMAGE_BUG = "/images/icons/bug.gif";
    private static final String ISSUE_IMAGE_NEWFEATURE = "/images/icons/newfeature.gif";
    private static final String ISSUE_IMAGE_TASK = "/images/icons/task.gif";
    private static final String ISSUE_IMAGE_IMPROVEMENT = "/images/icons/improvement.gif";
    ...
    public void testPriorityIconIsHiddenIfPriorityFieldIsHidden()
    {
        ...
        expectedDuplicatesIssueLinks.add(IssueLink.builder().relationship(DUPLICATES_RELATIONSHIP).title("MKY-1").url(generateIssueUrl("MKY-1")).summary("new feature in project monkey with no priority set on creation as it is hidden").iconUrl(prependBaseUrl(ISSUE_IMAGE_NEWFEATURE)).statusIconUrl(prependBaseUrl(STATUS_IMAGE_OPEN)).build());
        expectedDuplicatesIssueLinks.add(IssueLink.builder().relationship(DUPLICATES_RELATIONSHIP).title("HSP-3").url(generateIssueUrl("HSP-3")).summary("this task is duplicated by hsp-1").iconUrl(prependBaseUrl(ISSUE_IMAGE_TASK)).priorityIconUrl(prependBaseUrl(PRIORITY_IMAGE_MAJOR)).statusIconUrl(prependBaseUrl(STATUS_IMAGE_OPEN)).build());
        expectedDuplicatesIssueLinks.add(IssueLink.builder().relationship(DUPLICATES_RELATIONSHIP).title("HSP-2").url(generateIssueUrl("HSP-2")).summary("improvement on project homosapien this issue will be linked to the homosapien bug and has its priority showing").iconUrl(prependBaseUrl(ISSUE_IMAGE_IMPROVEMENT)).priorityIconUrl(prependBaseUrl(PRIORITY_IMAGE_CRITICAL)).statusIconUrl(prependBaseUrl(STATUS_IMAGE_OPEN)).build());
        ...
        expectedDuplicatesIssueLinks.add(IssueLink.builder().relationship(DUPLICATED_BY_RELATIONSHIP).title("HSP-4").url(generateIssueUrl("HSP-4")).summary("this bug duplicates hsp-1").iconUrl(prependBaseUrl(ISSUE_IMAGE_BUG)).priorityIconUrl(prependBaseUrl(PRIORITY_IMAGE_MINOR)).statusIconUrl(prependBaseUrl(STATUS_IMAGE_OPEN)).build());
        expectedDuplicatesIssueLinks.add(IssueLink.builder().relationship(DUPLICATED_BY_RELATIONSHIP).title("MKY-1").url(generateIssueUrl("MKY-1")).summary("new feature in project monkey with no priority set on creation as it is hidden").iconUrl(prependBaseUrl(ISSUE_IMAGE_NEWFEATURE)).statusIconUrl(prependBaseUrl(STATUS_IMAGE_OPEN)).build());
        expectedDuplicatesIssueLinks.add(IssueLink.builder().relationship(DUPLICATED_BY_RELATIONSHIP).title("HSP-2").url(generateIssueUrl("HSP-2")).summary("improvement on project homosapien this issue will be linked to the homosapien bug and has its priority showing").iconUrl(prependBaseUrl(ISSUE_IMAGE_IMPROVEMENT)).priorityIconUrl(prependBaseUrl(PRIORITY_IMAGE_CRITICAL)).statusIconUrl(prependBaseUrl(STATUS_IMAGE_OPEN)).build());
    ...
    {
        jira.gotoLoginPage().login("noaccessuser", "noaccessuser", DashboardPage.class);
    ...
    public void testIssueLinkSortOrder()
    {
        ...
        ImmutableList.Builder<IssueLink> expectedLinks = ImmutableList.builder();
        expectedLinks.add(IssueLink.builder().relationship("blocker (mixed)").title("HSP-2").url(generateIssueUrl("HSP-2")).summary("Critical improvement on project HSP").iconUrl(prependBaseUrl(ISSUE_IMAGE_IMPROVEMENT)).priorityIconUrl(prependBaseUrl(PRIORITY_IMAGE_CRITICAL)).statusIconUrl(prependBaseUrl(STATUS_IMAGE_OPEN)).build());
        ...
        expectedLinks.add(IssueLink.builder().relationship("same name (local)").title("GRL-1").url(generateIssueUrl("GRL-1")).summary("Gorilla needs to be fed").iconUrl(prependBaseUrl(ISSUE_IMAGE_TASK)).priorityIconUrl(prependBaseUrl(PRIORITY_IMAGE_MAJOR)).statusIconUrl(prependBaseUrl(STATUS_IMAGE_OPEN)).build());
        expectedLinks.add(IssueLink.builder().relationship("same name (local)").title("HSP-2").url(generateIssueUrl("HSP-2")).summary("Critical improvement on project HSP").iconUrl(prependBaseUrl(ISSUE_IMAGE_IMPROVEMENT)).priorityIconUrl(prependBaseUrl(PRIORITY_IMAGE_CRITICAL)).statusIconUrl(prependBaseUrl(STATUS_IMAGE_OPEN)).build());

        expectedLinks.add(IssueLink.builder().relationship("zzz (local)").title("GRL-1").url(generateIssueUrl("GRL-1")).summary("Gorilla needs to be fed").iconUrl(prependBaseUrl(ISSUE_IMAGE_TASK)).priorityIconUrl(prependBaseUrl(PRIORITY_IMAGE_MAJOR)).statusIconUrl(prependBaseUrl(STATUS_IMAGE_OPEN)).build());
    ...
    private static void assertIssueLinksEquals(IssueLink expected, IssueLink actual)
    {
        ...
        if (expected.getStatusIconUrl() != null ? !expected.getStatusIconUrl().equals(actual.getStatusIconUrl()) : actual.getStatusIconUrl() != null)
        {
            fail("Status icon URLs differ. Expected: " + expected.getStatusIconUrl() + " Actual: " + actual.getStatusIconUrl());
        }
