...
public class ViewIssueTypeScreenSchemesPage extends AbstractJiraPage
{
    ...
    {
        final PageElement configureLink = issueTypeScreenSchemesTable.find
                (
                        By.cssSelector(format(CONFIGURE_LINK_CSS_SELECTOR, issueTypeScreenSchemeName))
                );
        assertTrue
                (
                        "Attempted to configure an issue type screen scheme that is not present on the page",
                        configureLink.isPresent()
                );
