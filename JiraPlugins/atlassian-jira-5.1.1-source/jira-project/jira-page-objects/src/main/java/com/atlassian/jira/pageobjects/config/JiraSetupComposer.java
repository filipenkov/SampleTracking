...
public final class JiraSetupComposer
{
    ...
    {
        private static final Set<Class<?>> alreadySetupClasses = Sets.newHashSet();
        ...
        {
            if (!alreadySetupClasses.contains(beforeClass.testClass.getJavaClass()))
            {
                if (shouldSetUp(beforeClass) && isNotSetUp())
                {
                    setup.performSetUp();
                }
                alreadySetupClasses.add(beforeClass.testClass.getJavaClass());
            }
            return null;
        }
    }
