...
public class SelectedIssue
{
    ...
    {
        row.type(IssueActions.ASSIGN_ISSUE.getShortcut());
        return binder.bind(AssignIssueDialog.class);
    }

    public EditIssueDialog editViaShortcut()
    {
        row.type(IssueActions.EDIT_ISSUE.getShortcut());
    ...
    {
        return pageElementFinder.find(By.id("issuerow" + getIssueId())).find(By.cssSelector(".nav.issuekey")).getText();
    }
