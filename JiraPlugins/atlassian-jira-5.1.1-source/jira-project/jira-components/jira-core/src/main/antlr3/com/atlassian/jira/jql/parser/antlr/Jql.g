...
terminalClause	returns [Clause clause]
	...
	{
	    if ($operator.operator  == Operator.CHANGED)
	    {
            ...
            $clause = new ChangedClauseImpl($field.field, $operator.operator, $chPred.predicate);
	    }
	    else
	    {
	       ...
	       if ($operator.operator == Operator.WAS || $operator.operator == Operator.WAS_NOT || $operator.operator == Operator.WAS_IN || $operator.operator == Operator.WAS_NOT_IN )
		   {
                ...
                $clause = new WasClauseImpl($field.field, $operator.operator, $operand.operand, $pred.predicate);
           ...
           else
           {
                ...
                $clause = new TerminalClauseImpl($field.field, $operator.operator, $operand.operand);
 ...
 */
field returns [String field]
	: str = string
	{
	    $field = checkFieldName($string.start);
    }
	| num = numberString { $field = $num.string; }
	| cf = customField { $field = $cf.field; }
	;
	catch [RecognitionException e]
	{
 ...
 */
fieldCheck returns [String field]
...
argument returns [String arg]
	...
	{
        if (e.token.getType() == COMMA || e.token.getType() == RPAREN)
        {
            //We should only see this on an empty argument.
            reportError(JqlParseErrorMessages.emptyFunctionArgument(e.token), e);
        }
        else
        {
            //We see this when we have a bad argument.
            reportError(JqlParseErrorMessages.badFunctionArgument(e.token), e);
        }
	}
