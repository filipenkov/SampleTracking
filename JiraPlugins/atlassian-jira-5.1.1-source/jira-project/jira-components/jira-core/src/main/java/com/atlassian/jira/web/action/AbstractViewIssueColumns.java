...
public abstract class AbstractViewIssueColumns extends SearchDescriptionEnabledAction implements FilterOperationsAction
{
    ...
    public List getAddableColumns()
    {
        if(addableColumns == null)
        {
            try
            {
                ...
                final Comparator<Field> nameComparator = new NameComparator(new I18nBean(getLoggedInUser()));
    ...
    public List<Issue> getSampleIssues()
    {
        ...
        try
        {
            ...
            final List<Issue> searchResults = searchProvider.search(searchRequest.getQuery(), getLoggedInUser(), pagerFilter).getIssues();
