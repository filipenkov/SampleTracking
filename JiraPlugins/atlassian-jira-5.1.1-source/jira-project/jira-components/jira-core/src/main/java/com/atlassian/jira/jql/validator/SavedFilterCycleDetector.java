...
public class SavedFilterCycleDetector
{
    ...
    public boolean containsSavedFilterReference(User searcher, final boolean overrideSecurity, SearchRequest savedFilter, Long filterId)
    {
        ...
        final Set<Long> filters = Sets.newHashSet(filterId);
        return containsFilterRef(searcher, overrideSecurity, filterId, savedFilterQuery.getWhereClause(), filters);
    }
            ...
            Set<Long> filters)
    {
        ...
        for (TerminalClause nestedFilterClause : nestedFilterClauses)
        {
            ...
            final List<SearchRequest> matchingSearchRequests = overrideSecurity
                    ? savedFilterResolver.getSearchRequestOverrideSecurity(filterValues)
                    : savedFilterResolver.getSearchRequest(searcher, filterValues);
            for (SearchRequest matchingSearchRequest : matchingSearchRequests)
            {
                ...
                if (filterId.equals(matchingSearchRequest.getId())
                        || filters.contains(matchingSearchRequest.getId()))
                {
                    return true;
                }
                else
                {
                    ...
                    filters.add(matchingSearchRequest.getId());
                    if (containsFilterRef(searcher, overrideSecurity, filterId, query.getWhereClause(), filters))
                    {
                        return true;
                    }
                }
