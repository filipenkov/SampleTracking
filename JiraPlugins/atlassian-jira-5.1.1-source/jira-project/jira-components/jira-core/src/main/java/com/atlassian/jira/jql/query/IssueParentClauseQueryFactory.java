...
public class IssueParentClauseQueryFactory implements ClauseQueryFactory
{
    ...
    List<String> getIndexValues(final com.atlassian.crowd.embedded.api.User searcher, final boolean overrideSecurity, final List<QueryLiteral> rawValues)
    {
        ...
        for (QueryLiteral rawValue : rawValues)
        {
            ...
            {
                final List<Issue> issues = overrideSecurity ? issueSupport.getIssues(rawValue.getStringValue()) : issueSupport.getIssues(rawValue.getStringValue(), searcher);
                indexValues.addAll(CollectionUtil.transform(issues, new Function<Issue, String>()
                {
                    public String get(final Issue input)
                    {
