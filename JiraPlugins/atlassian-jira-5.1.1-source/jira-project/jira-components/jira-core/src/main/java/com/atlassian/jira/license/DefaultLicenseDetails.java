...
class DefaultLicenseDetails implements LicenseDetails
{
    ...
    /**
     * @param license should never be null - see {@link NullLicenseDetails} instead
     * @param externalLinkUtil external link utils
     */
    DefaultLicenseDetails(final JiraLicense license, final String licenseString, final ApplicationProperties applicationProperties, final ExternalLinkUtil externalLinkUtil, final BuildUtilsInfo buildUtilsInfo, I18nHelper.BeanFactory i18nFactory, DateTimeFormatter dateTimeFormatter)
    {
    ...
    {
        final String msg;
        DateTimeFormatter dmyFormatter = dateTimeFormatter.withLocale(i18n.getLocale()).withStyle(DateTimeStyle.DATE);
        if (isEvaluationLicense() || isNewBuildWithOldLicense())
        {
            if (isExpired())
            {
        ...
        else if (!isMaintenanceExpired())
        {
            if (isEntitledToSupport())
            {
                msg = i18n.getText("admin.support.available.until", "<b>" + dmyFormatter.format(getMaintenanceExpiryDate()) + "</b>");
            }
            else
            {
                msg = i18n.getText("admin.upgrades.available.until", "<b>" + dmyFormatter.format(getMaintenanceExpiryDate()) + "</b>");
            }
        }
        else
        {
    ...
    public String getLicenseStatusMessage(final I18nHelper i18n, @Nullable OutlookDate outlookDate, final String delimiter)
    {
        ...
        {
            final String maintenanceEnd = "<strong>" + TextUtils.htmlEncode(dmyFormatter.format(getMaintenanceExpiryDate())) + "</strong>";
        ...
        {
            final String maintenancePeriodEnd = "<strong>" + TextUtils.htmlEncode(dmyFormatter.format(getMaintenanceExpiryDate())) + "</strong>";
            // maintenance ended
