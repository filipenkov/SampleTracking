...
public class ViewSearchRequestIssueColumns extends AbstractViewIssueColumns
{
    ...
    protected EditableColumnLayout getColumnLayout()
    {
        if (editableSearchRequestColumnLayout == null)
        {
            try
            {
                editableSearchRequestColumnLayout = getColumnLayoutManager().getEditableSearchRequestColumnLayout(getLoggedInUser(), getSearchRequest());
            }
            catch (ColumnLayoutStorageException e)
            {
                log.error(e, e);
    ...
    {
        final JiraServiceContext ctx = new JiraServiceContextImpl(getLoggedInUser());
