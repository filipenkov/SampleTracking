...
public class UpgradeTask_Build602 extends AbstractUpgradeTask
{
    ...
    public void doUpgrade(boolean setupMode) throws Exception
    {
        ...
        useSavePoints =  connection.getMetaData().supportsSavepoints() && !isORACLE(connection) && !isMSSQL(connection);
    ...
    private void flushUserCaches()
    {
        ...
        {
            log.error("Expected to find a DefaultUserPropertyManager, but got " + propertyManager.getClass().getName());
        }
...
// Get the user's name from the crowd user table
    ...
    private boolean isORACLE(final Connection connection) throws SQLException
    {
    ...
    private boolean isMSSQL(final Connection connection) throws SQLException
    {
        DatabaseType databaseType = DatabaseTypeFactory.getTypeForConnection(connection);
        return databaseType.getFieldTypeName().startsWith("mssql");
    }

    private boolean isMYSQL(final Connection connection) throws SQLException
    {
        DatabaseType databaseType = DatabaseTypeFactory.getTypeForConnection(connection);
