...
public class SavedFilterResolver
{
    ...
    {
        return _getSearchRequests(searcher, false, rawValues);
    }
    ...
    private List<SearchRequest> _getSearchRequests(final User searcher, final boolean overrideSecurity, final List<QueryLiteral> rawValues)
    {
        ...
        if (rawValues != null)
        {
            for (QueryLiteral rawValue : rawValues)
            {
                ...
                {
                    // we somehow got an Empty literal - empty is not allowed so ignore
                }
            }
        }
    ...
    private List<SearchRequest> getSearchRequestsForLong(final User searcher, final boolean overrideSecurity, final Long rawValue)
    {
    ...
    private List<SearchRequest> getSearchRequestsForString(final User searcher, final boolean overrideSecurity, final String rawValue)
    {
    ...
    private SearchRequest getSearchRequestById(final User searcher, final boolean overrideSecurity, final Long idValue)
    {
        if (!overrideSecurity)
        {
        ...
        else
        {
            ...
            final List<SearchRequest> matchedRequests = new ArrayList<SearchRequest>();
            searchRequestIterable.foreach(new Consumer<SearchRequest>()
            {
                public void consume(@NotNull final SearchRequest element)
                {
                    if (element.getId().equals(idValue))
                    {
                        matchedRequests.add(element);
                    }
                }
            });
            return matchedRequests.size() >= 1 ? matchedRequests.get(0) : null;
        }
    }

    private List<SearchRequest> getSearchRequestsByName(final User searcher, final boolean overrideSecurity, final String nameValue)
    {
        ...
        if (trimName != null)
        {
            if (!overrideSecurity)
            {
                ...
                final SharedEntitySearchResult sharedEntitySearchResult =
                        searchRequestManager.search(builder.toSearchParameters(), searcher, 0, Integer.MAX_VALUE);

                return (List<SearchRequest>) sharedEntitySearchResult.getResults();
            }
            else
            {
                final EnclosedIterable<SearchRequest> searchRequestIterable = searchRequestManager.getAll();
