...
abstract class AbstractDateTimeRelativeDatesFormatter implements DateTimeFormatStrategy
{
    ...
    {
        TimeZone userTimeZone = timeZoneInfoResolver.getUserTimeZone(new JiraServiceContextImpl(jiraAuthenticationContext.getLoggedInUser()));
        DateTime referenceDateTime = new DateTime(clock.getCurrentDate(), DateTimeZone.forTimeZone(userTimeZone));
