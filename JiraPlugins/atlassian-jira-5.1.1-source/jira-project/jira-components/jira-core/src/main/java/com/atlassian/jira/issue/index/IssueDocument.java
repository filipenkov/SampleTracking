...
public class IssueDocument
{
    ...
    public static Document getDocument(final Issue issueObject)
    {
        ...
        {
            doc.add(new Field(DocumentConstants.ISSUE_VISIBLE_FIELD_IDS, visibleDocumentFieldId, Field.Store.NO, Field.Index.NOT_ANALYZED_NO_NORMS));
        }
    ...
    private static List<Fieldable> getNonEmptyFields(final Document doc)
    {
        ...
        for (final Fieldable field : fields)
        {
            ...
            if (field.isIndexed())
            {
                allVals.add(new Field(DocumentConstants.ISSUE_NON_EMPTY_FIELD_IDS, field.name(), Field.Store.NO, Field.Index.NOT_ANALYZED_NO_NORMS));
            }
        }
        return allVals;
    }
