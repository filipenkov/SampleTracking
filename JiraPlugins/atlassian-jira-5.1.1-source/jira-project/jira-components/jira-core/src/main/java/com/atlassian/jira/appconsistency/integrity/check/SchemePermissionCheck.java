...
public class SchemePermissionCheck extends CheckImpl
{
    ...
    private List doCheck(boolean correct) throws IntegrityException
    {
        ...
        if (correct)
        {
            ...
            {
                DeleteEntityAmendment createEntityAmendment = (DeleteEntityAmendment) amendments.get(i);
                try
                {
    ...
    private List getAmendments() throws IntegrityException
    {
        ...
        try
        {
            ...
            GenericValue genericValue = (GenericValue) listIterator.next();
            while (genericValue != null)
            {
                schemePermissions = ofBizDelegator.findByAnd("SchemePermissions", EasyMap.build("scheme", genericValue.getLong("scheme"),
                        "permission", genericValue.getLong("permission"),
                        "type", genericValue.getString("type"),
                        "parameter", genericValue.getString("parameter")));
                if (schemePermissions.size() > 1)
                {
                    List containsDuplicate = EntityUtil.filterByAnd(firstDuplicate, EasyMap.build("scheme", genericValue.getLong("scheme"),
                        "permission", genericValue.getLong("permission"),
                        "type", genericValue.getString("type"),
                        "parameter", genericValue.getString("parameter")));
