...
public class ResolverManagerImpl implements ResolverManager
{
    ...
    public String getSingleIdFromName(final String name, final String field)
    {
        ...
        if (ids.size() > 1)
        {
            LOG.warn(String.format("Found more than 1 id during REST name resolution for name %s field %s.", name, field));
        }
    ...
    public static class IssueSecurityLevelResolverFacade implements NameResolver
    {
        ...
        {
            final Collection<GenericValue> levels = resolver.getIssueSecurityLevelsByName(authenticationContext.getLoggedInUser(), false, name);
            return new ArrayList<String>(Transformed.collection(levels, new Function<GenericValue, String>()
            {
