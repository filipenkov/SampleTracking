 ...
 * {@link com.atlassian.jira.imports.project.handler.ImportEntityHandler#handleEntity(String,java.util.Map)} method
 * on its registered delegates. The attributes map includes any attributes that may be nested as sub-elements.
 * <br/>
 * If you construct this with a {@link com.atlassian.jira.task.TaskProgressSink} then the progress of the XML
...
public class ChainedSaxHandler extends DefaultHandler
{
    ...
    /**
     * Examine the textBuffer set by {@link #characters(char[], int, int)} and add it to the attributes map
     * for the current inEntity entity.
     *
    ...
    /**
     * Closes the top level element (i.e. Issue). This signals that we have filled the attributesMap with all
     * attributes for this entity. This method calls the delegates {@link com.atlassian.jira.imports.project.handler.ImportEntityHandler#handleEntity(String, java.util.Map)} methods
     * with the collected information and then clears the state objects.
     *
