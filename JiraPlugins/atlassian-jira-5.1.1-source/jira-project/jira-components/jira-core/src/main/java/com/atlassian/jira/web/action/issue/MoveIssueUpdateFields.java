...
public class MoveIssueUpdateFields extends MoveIssue implements OperationContext
{
    ...
    public String doDefault()
    {
        ...
        if (!(ActionContext.getSingleValueParameters().containsKey("reset") && ("true".equals(ActionContext.getSingleValueParameters().get("reset")))))
        {
            ...
            for (Iterator iterator = targetFieldLayout.getVisibleLayoutItems(getLoggedInUser(), getTargetProjectObj(), EasyList.build(getTargetIssueType())).iterator(); iterator.hasNext();)
            {
    ...
    {
        return getTargetFieldLayout().getHiddenFields(getLoggedInUser(), getTargetProject(), EasyList.build(getTargetIssueType()));
    }
