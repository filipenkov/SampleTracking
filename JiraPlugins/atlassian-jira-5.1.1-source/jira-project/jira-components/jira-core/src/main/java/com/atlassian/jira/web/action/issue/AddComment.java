...
public class AddComment extends AbstractCommentableIssue implements OperationContext
{
    ...
    {
        return (getIssue() == null) ? permissionManager.hasPermission(Permissions.COMMENT_ISSUE, getIssueObject().getProjectObject(), getLoggedInUser())
    ...
    private void alertSystemOfComment(Comment comment) throws GenericEntityException
    {
        ...
        IssueEventDispatcher.dispatchEvent(EventType.ISSUE_COMMENTED_ID, getIssueObject(), getLoggedInUser(), comment, null, null, EasyMap.build("eventsource", IssueEventSource.ACTION));
    }
