...
public abstract class AbstractAddWorkflowTransitionDescriptor extends AbstractWorkflowTransitionAction
{
    ...
    protected String getRedirectUrl()
    {
        ...
        if (getStep() == null)
        {
            return getParamsActionName() + "?workflowName=" + URLEncoder.encode(getWorkflow().getName()) +
                   "&workflowMode=" + getWorkflow().getMode() +
                   "&workflowTransition=" + getTransition().getId() +
                   "&pluginModuleKey=" + URLEncoder.encode(type) + "&atl_token=" + getXsrfToken();
        }
        else
        {
            return getParamsActionName() + "?workflowName=" + URLEncoder.encode(getWorkflow().getName()) +
    ...
    {
        final List moduleDescriptors = getPluginAccessor().getEnabledModuleDescriptorsByType(getWorkflowModuleDescriptorType());
