...
public class MultiUserCustomFieldOperationsHandler extends AbstractCustomFieldOperationsHandler<Collection<String>>
{
    ...
    {
        @SuppressWarnings ({ "unchecked" }) Collection<User> fieldValue = (Collection<User>) field.getValue(issue);
        if (fieldValue == null)
        {
            return emptyList();
        }

        Iterable<String> users = Iterables.transform(fieldValue, new Function<User, String>()
        {
            @Override
