...
public class BrowseProjects extends JiraWebActionSupport
{
    ...
    {
        // OnDemand redirects to the first app, as opposed to the first project.
        if (!featureManager.isEnabled(CoreFeatures.ON_DEMAND))
        {
            // If there is only 1 project, let's browse it
            if (permissionManager.getProjectObjects(Permissions.BROWSE, getLoggedInUser()).size() == 1)
            {
