...
public class GlobalPermissionsCache
{
    ...
    public synchronized void refresh()
    {
        ...
        final Collection<GenericValue> allPermissions = ComponentAccessor.getOfBizDelegator().findByAnd("SchemePermissions",
            MapBuilder.build("scheme", null));
        for (final GenericValue permissionGV : allPermissions)
        {
            final JiraPermission jiraPerm = new JiraPermissionImpl(permissionGV);
