...
class PluginFactoryAndLoaderRegistrar
{
    ...
    public PluginLoader getBundledPluginsLoader(List<PluginFactory> pluginFactories)
    {
        try
        {
            final URL bundledPluginZip;
            final String override = System.getProperty("jira.dev.bundledplugins.url");
            if (override != null)
            {
                bundledPluginZip = new URL(override);
                log.warn("Bundled plugins being loaded from override " + bundledPluginZip);
            }
            else
            {
                bundledPluginZip = servletContextFactory.getServletContext().getResource(BUNDLED_PLUGIN_ZIP_LOCATION);
            }
