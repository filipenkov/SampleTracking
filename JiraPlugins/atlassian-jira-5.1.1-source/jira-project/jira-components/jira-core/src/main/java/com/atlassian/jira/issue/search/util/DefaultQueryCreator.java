...
{
    private final Collection queryHandlers = new ArrayList();
    ...
    public DefaultQueryCreator(ProjectManager projectManager, ConstantsManager constantsManager,
            ...
            JiraAuthenticationContext authenticationContext, ProjectComponentManager projectComponentManager, DateTimeFormatterFactory dateTimeFormatterFactory, TimeZoneManager timeZoneManager)
    {
    ...
    /**
     * Create a URL to be redirected to, given a search query.  The URL does not include context path, so will
     * generally start with 'IssueNavigator.jspa?reset=true...."
     * <p/>
    ...
    {
        if (searchString == null)  //Handle the null case - JRA-6084
            return NULL_QUERY;
        ...
        if (TextUtils.stringSet(quickSearchQuery))
            query = query + "&usedQuickSearch=true&originalQuickSearchQuery=" + JiraUrlCodec.encode(searchString);
    ...
    private QuickSearchResult searchQuickSearch(String searchString)
    {
        ...
        for (Iterator iterator = queryHandlers.iterator(); iterator.hasNext();)
        {
            QuickSearchHandler quickSearchHandler = (QuickSearchHandler) iterator.next();
    ...
    private String getDefaultQuery(String searchString)
    {
        if (TextUtils.stringSet(searchString))
            return "&query=" + JiraUrlCodec.encode(searchString);
        else
            return "";
    }
