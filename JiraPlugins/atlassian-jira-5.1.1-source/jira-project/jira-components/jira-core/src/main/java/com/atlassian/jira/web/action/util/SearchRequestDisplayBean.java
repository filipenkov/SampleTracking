...
public class SearchRequestDisplayBean implements SharesListHelper
{
    ...
    {
        final String userName = request.getOwnerUserName();
        if (userName != null)
        {
            return userFormatManager.formatUser(userName, UserNameUserFormat.TYPE, "search_request");
        }
        return null;
    }
    ...
    {
        return UserNames.equal(getOwnerUserName(), authCtx.getLoggedInUser());
    }
    ...
    private Collection /* <GenericValue> */getSubscriptions()
    {
        ...
        {
            subscriptions = subscriptionService.getVisibleSubscriptions(authCtx.getLoggedInUser(), request);
        }
