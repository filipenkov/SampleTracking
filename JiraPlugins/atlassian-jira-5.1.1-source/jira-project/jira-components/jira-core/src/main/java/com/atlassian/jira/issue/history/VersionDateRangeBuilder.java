...
public class VersionDateRangeBuilder extends AbstractDateRangeBuilder
{
    ...
    protected ChangeHistoryItem createInitialChangeItem(Issue issue)
    {
        ...
        String reporter = issue.getReporter() != null ? issue.getReporter().getName() : DocumentConstants.ISSUE_NO_AUTHOR;
        ChangeHistoryItem.Builder builder =  new ChangeHistoryItem.Builder().withId(-1L).inChangeGroup(-1L).forIssue(issue.getId(), issue.getKey()).
                                                    inProject(issue.getProjectObject().getId()).on(issue.getCreated()).field(fieldName).
                                                    byUser(reporter);
    ...
    private ChangeHistoryItem createChangeHistoryItem(Issue issue, Version version, boolean IncludeEmptyValue) {
        return new ChangeHistoryItem.Builder().
                     forIssue(issue.getId(), issue.getKey()).on(issue.getCreated()).
                     byUser(issue.getReporter().getName()).
                     to(version.getName(),version.getId().toString()).field(fieldName).
                     inProject(issue.getProjectObject().getId()).withId(-1L).inChangeGroup(-1L).
                     build();
