...
public class IssueCreationHelperBeanImpl implements IssueCreationHelperBean
{
    ...
    public void validateCreateIssueFields(final JiraServiceContext jiraServiceContext, final Collection<String> providedFields, final Issue issueObject, final FieldScreenRenderer fieldScreenRenderer,
            final OperationContext operationContext, final Map<String, String[]> actionParams, boolean applyDefaults, final I18nHelper i18n)
    {
        User remoteUser = jiraServiceContext.getLoggedInUser();
        ErrorCollection errors = jiraServiceContext.getErrorCollection();
        FieldLayout fieldLayout = fieldScreenRenderer.getFieldLayout();
        List visibleLayoutItems = fieldLayout.getVisibleLayoutItems(remoteUser, issueObject.getProjectObject(), Lists.newArrayList(issueObject.getIssueTypeObject().getId()));
                                ...
                                final I18nHelper i18n)
    {
        ...
        {
            errors.addErrorMessage(i18n.getText("createissue.error.invalid.license", contactLink), Reason.FORBIDDEN);
        }
        else if (licenseDetails.isExpired())
        {
            errors.addErrorMessage(i18n.getText("createissue.error.license.expired", contactLink), Reason.FORBIDDEN);
        }
        else if(userUtil.hasExceededUserLimit())
        {
            errors.addErrorMessage(i18n.getText("createissue.error.license.user.limit.exceeded", contactLink), Reason.FORBIDDEN);
        }
    }
    ...
    public List<String> getProvidedFieldNames(final User remoteUser, final Issue issueObject)
    {
        ...
        List visibleLayoutItems = fieldScreenRenderer.getFieldLayout().getVisibleLayoutItems(remoteUser, issueObject.getProjectObject(), Lists.newArrayList(issueObject.getIssueTypeObject().getId()));
        for (Iterator iterator = visibleLayoutItems.iterator(); iterator.hasNext();)
        {
            FieldLayoutItem fieldLayoutItem = (FieldLayoutItem) iterator.next();
