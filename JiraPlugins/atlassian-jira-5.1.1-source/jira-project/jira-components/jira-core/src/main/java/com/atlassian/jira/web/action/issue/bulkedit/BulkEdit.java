...
public class BulkEdit extends AbstractBulkOperationDetailsAction
{
    ...
    {
        return new FieldScreenRenderLayoutItemImpl(null, fieldManager.getFieldLayoutManager().getFieldLayout(issue).getFieldLayoutItem(field));
    }
    ...
    {
        for (Iterator iterator = getEditActions().values().iterator(); iterator.hasNext();)
        {
    ...
    public String doPerform() throws Exception
    {
        ...
        try
        {
            getBulkEditOperation().perform(getBulkEditBean(), getLoggedInUser());
        }
        catch (Exception e)
        {
    ...
    private void doValidationPerform()
    {
        try
        {
            ...
            if (!getBulkEditOperation().canPerform(getBulkEditBean(), getLoggedInUser()))
            {
    ...
    {
        return getBulkEditOperation().canPerform(getBulkEditBean(), getLoggedInUser());
    }
    ...
    public Map getEditActions()
    {
        if (editActions == null)
        {
            editActions = getBulkEditOperation().getActions(getBulkEditBean(), getLoggedInUser());
        }
        return editActions;
    }
