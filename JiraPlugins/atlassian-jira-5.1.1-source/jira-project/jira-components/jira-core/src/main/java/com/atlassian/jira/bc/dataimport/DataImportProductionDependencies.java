...
public class DataImportProductionDependencies
{
    ...
    {
        return ComponentManager.getOSGiComponentInstanceOfType(PluginUpgradeManager.class);
    }
    ...
    void globalRefresh(boolean quickImport)
    {
        if (quickImport)
        {
            ...
            // We only want to muck around with plugins if it's the system tenant
            if (MultiTenantContext.getManager().isSystemTenant())
            {
                final PluginPersistentStateStore pluginPersistentStateStore = ComponentManager.getComponentInstanceOfType(PluginPersistentStateStore.class);
                ...
                ComponentManager.getComponentInstanceOfType(ApplicationProperties.class).refresh();
    ...
    private void syncPluginStateWithNewData(final PluginPersistentStateStore pluginPersistentStateStore, final PluginPersistentState oldState)
    {
        ...
        final PluginAccessor pluginAccessor = ComponentManager.getComponentInstanceOfType(PluginAccessor.class);
        final PluginController pluginController = ComponentManager.getComponentInstanceOfType(PluginController.class);
    ...
    {
        ComponentManager.getComponentInstanceOfType(OfBizDelegator.class).refreshSequencer();
    }
}
