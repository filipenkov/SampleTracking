...
public class ExternalUtils
{
    ...
    protected void checkStatus(final Issue issue, final String status) throws WorkflowException, ExternalException
    {
        ...
        if (issue != null)
        {
            ...
            if (statusGV != null)
            {
                final JiraWorkflow workflow = workflowManager.getWorkflow(issue.getProjectObject().getId(), issue.getIssueTypeObject().getId());
    ...
    {
        return issueManager.createIssue(authenticationContext.getLoggedInUser(), fields);
    }
     ...
     *
     * @param externalAttachment the Attachment
     * @param issueGv the Issue
     * @throws ExternalException
     *
     * @deprecated Use {@link AttachmentManager#createAttachment(java.io.File, String, String, com.atlassian.crowd.embedded.api.User, Issue)} instead. Since v5.0.
     */
    ...
    /**
     * @deprecated Use {@link com.atlassian.jira.config.ConstantsManager#getConstantObject(String, String)}, or
     *          {@link com.atlassian.jira.config.ConstantsManager#getIssueConstantByName(String, String)} instead. Since v5.0.
     */
    public GenericValue getConstant(final String constantName, final String constantType)
    {
        final IssueConstant constantById = constantsManager.getConstantObject(constantType, constantName);
        if (constantById == null)
        {
            IssueConstant constantByName = constantsManager.getIssueConstantByName(constantType, constantName);
    ...
    /**
     * @deprecated Use {@link com.atlassian.jira.config.PriorityManager}, {@link com.atlassian.jira.config.IssueTypeManager},
     *              {@link com.atlassian.jira.config.ResolutionManager} instead. Since v5.0.
     */
