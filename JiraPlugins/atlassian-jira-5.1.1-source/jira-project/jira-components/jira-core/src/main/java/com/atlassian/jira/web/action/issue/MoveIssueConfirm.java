...
public class MoveIssueConfirm extends MoveIssueUpdateFields
{
    ...
    public MoveIssueConfirm(SubTaskManager subTaskManager, AttachmentManager attachmentManager,
            ...
            IssueSecurityHelper issueSecurityHelper, IssueManager issueManager, final AttachmentPathManager attachmentPathManager, UserUtil userUtil)
    {
            ...
            throws GenericEntityException
    {
        if (updateLog != null && !issueChangeHolder.getChangeItems().isEmpty())
        {
            ...
            IssueEventDispatcher.dispatchEvent(eventTypeId, newIssue, getLoggedInUser(), updateLog, true,
            ...
            throws GenericEntityException
    {
        if (subTaskUpdateLog != null && !subTaskChangeItems.isEmpty())
        {
            if (!(getProject().equals(getTargetProject())))
            {
                IssueEventDispatcher.dispatchEvent(EventType.ISSUE_MOVED_ID, subTask, getLoggedInUser(), null, null, subTaskUpdateLog);
            }
            else
            {
                IssueEventDispatcher.dispatchEvent(EventType.ISSUE_UPDATED_ID, subTask, getLoggedInUser(), null, null, subTaskUpdateLog);
            }
        }
    }
    ...
    private List moveSubTaskDetails(Issue originalSubTask, MutableIssue targetSubTask, String newKey)
    {
        ...
        if (!(getProject().equals(getTargetProject())))
        {
            ...
            List targetIssueTypeIdList = EasyList.build(targetSubTask.getIssueTypeObject().getId());
            for (Iterator iterator = targetFieldLayout.getVisibleLayoutItems(getLoggedInUser(), getTargetProjectObj(), targetIssueTypeIdList).iterator(); iterator.hasNext();)
            {
            ...
            for (Iterator iterator = targetFieldLayout.getHiddenFields(getLoggedInUser(), getTargetProject(), targetIssueTypeIdList).iterator(); iterator.hasNext();)
            {
    ...
    private String getNewKey()
    {
        ...
        long incCount = getProjectManager().getNextId(project);
        return project.getKey() + "-" + incCount;
    }
