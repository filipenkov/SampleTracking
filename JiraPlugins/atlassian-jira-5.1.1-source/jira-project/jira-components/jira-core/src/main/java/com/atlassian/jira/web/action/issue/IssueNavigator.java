...
public class IssueNavigator extends SearchDescriptionEnabledAction implements IssueSearchResultsAction, FilterOperationsAction, SearchRequestViewsAction
{
    ...
    public IssueNavigator(final SearchProvider searchProvider, final ColumnLayoutManager columnLayoutManager,
            ...
            final UserQueryHistoryManager userQueryHistoryManager, final SimpleLinkManager simpleLinkManager,
            final IssueSearchLimits issueSearchLimits, final WebResourceManager webResourceManager,
            AvatarService avatarService, EventPublisher eventPublisher)
    {
    ...
    {
        // Only saved searches have ids
        if(searchRequest != null)
        {
            boolean isUnmodifiedSearch = searchRequest.getId() != null && !searchRequest.isModified();
            if (isUnmodifiedSearch)
            {
                webResourceManager.putMetadata("filter-id", searchRequest.getId().toString());
            }
        }
        webResourceManager.putMetadata("filter-jql", getJqlQuery());
        ...
        if (avatarURL != null)
        {
            ...
            webResourceManager.putMetadata("default-avatar-url", defaultAvatarUrl);
        }
    }
    ...
    private String _doExecuteSimple()
    {
        ...
        if (actionParams.isRefreshOnly())
        {
            ...
            {
                searcher.getSearchInputTransformer().populateFromParams(getLoggedInUser(), getFieldValuesHolder(), actionParams);
            }

            eventPublisher.publish(new RefreshIssueSearchEvent(getSearchRequest()));
    ...
    {
        eventPublisher.publish(new IssueSearchEvent(getSearchRequest(), getNavigatorType().name()));
    }
    ...
    private String _doExecutePopulateFromSearchRequest()
    {
        ...
        if (actionParams.isAddParamsRequired())
        {
            if (getJqlQuery() != null)
            {
                ...
                setSearchRequest(searchRequestFactory.createFromQuery(getSearchRequest(), getLoggedInUser(), jqlQuery));
            }
            else
            {
                setSearchRequest(searchRequestFactory.createFromParameters(getSearchRequest(), getLoggedInUser(), actionParams));
            }
            updateSorts = false;
    ...
    private boolean _doUpdateParamsRequired(final IssueNavigatorActionParams actionParams)
    {
        ...
        if (getJqlQuery() != null)
        {
            ...
            setSearchRequest(searchRequestFactory.createFromQuery(searchRequest, getLoggedInUser(), jqlQuery));
        ...
        {
            populateFieldValuesHolderFromQuery(getSearchRequest().getQuery(), getFieldValuesHolder());
            validateParams(getFieldValuesHolder(), searchService.getSearchContext(getLoggedInUser(), getSearchRequest().getQuery()));
    ...
    public final String doSwitchView()
    {
        ...
        if (type != null)
        {
            ...
            {
                eventPublisher.publish(new SwitchIssueSearchEvent(type.name(), false));
                IssueNavigatorType.setInCookie(ActionContext.getResponse(), type);
            }
            else
            {
                eventPublisher.publish(new SwitchIssueSearchEvent(type.name(), true));
    ...
    {
        final IssueNavigatorActionParams actionParams = getActionParams();
        if (getLoggedInUser() != null && !isAutocompleteDisabled() && actionParams.isAutocompletePreferenceSpecified())
        {
...
//        final String jqlStr;
    ...
    private void saveJqlQueryToHistory()
    {
        ...
        {
            userQueryHistoryManager.addQueryToHistory(getLoggedInUser(), getJqlQuery());
        }
    }
    ...
    public List<String> getSavedJqlQueryHistoryItems()
    {
        ...
        final List<UserHistoryItem> history = userQueryHistoryManager.getUserQueryHistory(getLoggedInUser());
    ...
    private void updateSearchRequestWithJqlOrParams(final SearchRequest searchRequest)
    {
        ...
        if (getJqlQuery() != null)
        {
            ...
            if (newQuery != null)
            {
                SearchRequest newSearchRequest = searchRequestFactory.createFromQuery(searchRequest, getLoggedInUser(), newQuery);
        ...
        {
            SearchRequest newSearchRequest = searchRequestFactory.createFromParameters(searchRequest, getLoggedInUser(), getActionParams());
            setSearchRequest(newSearchRequest);
            validateQuery(newSearchRequest.getQuery());
        }
    ...
    private Query getNewSearchQuery()
    {
        ...
        if (getJqlQuery() != null)
        {
            final SearchService.ParseResult parseResult = searchService.parseQuery(getLoggedInUser(), getJqlQuery());
    ...
    private String validateAndGetRedirectForQuery(Query query, boolean modified)
    {
        ...
        if (getAdvancedOrSimpleView().equals(ADVANCED) || !(fits = searchService.doesQueryFitFilterForm(getLoggedInUser(), query)))
        {
            ...
            if (fits == null)
            {
                fits = searchService.doesQueryFitFilterForm(getLoggedInUser(), query);
            }
        ...
        {
            populateFieldValuesHolderFromQuery(query, getFieldValuesHolder());
            validateParams(getFieldValuesHolder(), searchService.getSearchContext(getLoggedInUser(), query));
    ...
    private boolean validateAndSetSearchRequestForParams(SearchRequest orgSearchRequest, IssueNavigatorActionParams actionParams)
    {
        ...
        {
            SearchRequest newSearchRequest = searchRequestFactory.createFromParameters(orgSearchRequest, getLoggedInUser(), actionParams);
            setSearchRequest(newSearchRequest);
        ...
        else
        {
            ...
            setSearchRequest(newSearchRequest);
            final boolean redirectingFromHide = inHideWithNonUserCreatedFilter(newSearchRequest.isModified());
            if (searchService.doesQueryFitFilterForm(getLoggedInUser(), newSearchRequest.getQuery()))
            {
                validateParams(getFieldValuesHolder(), searchService.getSearchContext(getLoggedInUser(), newSearchRequest.getQuery()));
                if (invalidInput() && !redirectingFromHide)
                {
                    setMode(MODE_SHOW);
                    return false;
    ...
    private boolean validateQuery(final Query query)
    {
        ...
        final MessageSet result = searchService.validateQuery(getLoggedInUser(), query);
        if (result != null)
        {
    ...
    private void updateSearchSorts()
    {
        ...
        if (specifiedOrderByClause != null && !specifiedOrderByClause.getSearchSorts().isEmpty())
        {
            ...
            final List<SearchSort> mergedSorts = searchSortUtil.mergeSearchSorts(getLoggedInUser(), specifiedOrderByClause.getSearchSorts(), oldSorts, 3);
    ...
    private void populateFromParams(final IssueNavigatorActionParams actionParams, final FieldValuesHolder fieldValuesHolder)
    {
        for (final IssueSearcher searcher : getSearchers())
        {
            ...
            searchInputTransformer.populateFromParams(getLoggedInUser(), fieldValuesHolder, actionParams);
        }
    }
    ...
    private void validateParams(final FieldValuesHolder fieldValuesHolder, final SearchContext searchContext)
    {
        ...
        {
            final SearchInputTransformer searchInputTransformer = searcher.getSearchInputTransformer();
            searchInputTransformer.validateParams(getLoggedInUser(), searchContext, fieldValuesHolder, this, this);
        }
    }
    ...
    public SearchResults getSearchResults() throws SearchException
    {
        ...
        if (searchResults == null)
        {
            ...
            if (isReturningToSearch)
            {
                ...
                {
                    SearchResults result = searchProvider.search(getSearchRequest().getQuery(), getLoggedInUser(), new PagerFilter(pager));
                ...
                if (searchResults == null)
                {
                    ...
                    if (searchWindowResults.isEmpty())
                    {
                        ...
                        searchResults = searchProvider.search(getSearchRequest().getQuery(), getLoggedInUser(), navigatorPager);
                    }
                    else
                    {
                        ...
                        else
                        {
                            ...
                            pager.setStart(0);
                            searchResults = searchProvider.search(getSearchRequest().getQuery(), getLoggedInUser(), pager);
                        }
                    }
                }
            }
    ...
    public boolean isHasBulkChangePermission()
    {
        ...
        else
        {
            ...
            try
            {
                return permissionManager.hasPermission(Permissions.BULK_CHANGE, getLoggedInUser());
            }
    ...
    public String getSearcherEditHtml(final IssueSearcher searcher)
    {
        ...
        if (searchRenderer.isShown(getLoggedInUser(), searchContext))
        {
            ...
            return searchRenderer.getEditHtml(getLoggedInUser(), searchContext, getFieldValuesHolder(), displayParams, this);
        }
        else
        {
            return "";
    ...
    private boolean isAdvanced()
    {
        ...
        {
            final SearchRequest searchRequest = getSearchRequest();
            isAdvanced = searchRequest != null && !searchService.doesQueryFitFilterForm(getLoggedInUser(), searchRequest.getQuery());
        }
        return isAdvanced;
    }
    ...
    private void addWarningMessages(final Collection<String> messages)
    {
        ...
        {
            warningMessages = new LinkedHashSet<String>(messages);
        }
        else
        {
    ...
    public List<SimpleLink> getPluggableItems()
    {
        ...
        return simpleLinkManager.getLinksForSection("jira.navigator.pluggable.items", loggedInUser, jiraHelper);
    }
