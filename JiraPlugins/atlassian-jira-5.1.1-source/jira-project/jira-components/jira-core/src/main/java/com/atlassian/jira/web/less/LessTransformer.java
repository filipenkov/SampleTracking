...
public class LessTransformer implements WebResourceTransformer
{
    ...
    public LessTransformer(ApplicationProperties applicationProperties, ServletContextFactory servletContextFactory,
            PluginAccessor pluginAccessor, WebResourceIntegration webResourceIntegration)
            throws IOException
    {
    ...
    {
        boolean minificationDisabled = isMinificationDisabled();
        if (minificationDisabled)
        {
            return new DevModeLessResource(pluginKeyForElement(element), servletContextFactory.getServletContext(), resourceLocation, downloadableResource, minificationDisabled);
        }

        return new LessResource(pluginKeyForElement(element), servletContextFactory.getServletContext(), resourceLocation, downloadableResource, minificationDisabled);
    }
    ...
    private class LessResource extends AbstractStringTransformedDownloadableResource
    {
        ...
        {
            return LESSC.get().compile(originalLocation.getLocation(), loader, originalContent, !minificationDisabled);
        }
    ...
    /**
     * A LessResource that adds a header to the generated CSS file with the location of the actual .less file.
     */
    private class DevModeLessResource extends LessResource
    {
        ...
        /**
         * Prepends the location of the file on disk as a comment.
         */
        @Override
