...
public class OneDimensionalObjectHitCollector extends Collector
{
     ...
     * {@link #OneDimensionalObjectHitCollector(StatisticsMapper, Map, IndexReader, boolean)}} with
     ...
     * @param indexReader the index to be searched
     */
    ...
    public OneDimensionalObjectHitCollector(StatisticsMapper statisticsMapper, Map result, IndexReader indexReader, boolean isUnique)
    {
        ...
        try
        {
            if (isUnique)
            {
                docToTerms = JiraLuceneFieldFinder.getInstance().getUniqueMatches(indexReader, statisticsMapper.getDocumentConstant());
            }
            else
            {
