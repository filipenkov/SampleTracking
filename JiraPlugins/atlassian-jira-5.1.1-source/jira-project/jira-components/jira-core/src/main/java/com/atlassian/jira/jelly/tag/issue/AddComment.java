...
public class AddComment extends UserAwareDynaBeanTagSupport
{
    ...
    public void doTag(XMLOutput xmlOutput) throws JellyTagException
    {
        ...
        if (roleLevel != null)
        {
            ...
            if (roleLevelId == null)
            {
                ProjectRole projectRole = projectRoleService.getProjectRoleByName(user, roleLevel, errorCollection);
                if (projectRole != null)
                {
        ...
        {
            CommentService commentService = ComponentManager.getComponent(CommentService.class);
            ...
            commentService.isValidAllCommentData(user, issue, commentBody, grouplevel, roleLevelId == null ? null : roleLevelId.toString(), errorCollection);
            if (errorCollection.hasAnyErrors())
            {
                throw new JellyTagException(errorCollection.toString());
            }
            ...
            // have been specified.
            CommentManager commentManager = ComponentManager.getComponent(CommentManager.class);
            commentManager.create(issue, author, updateAuthor, commentBody, grouplevel, roleLevelId == null ? null : roleLevelId, created, updated, true, tweakIssueUpdateDate);
        }
        else
        {
            throw new JellyTagException(errorCollection.toString());
    ...
    private User findUser(String author)
            throws JellyTagException
    {
        User user = UserUtils.getUser(author);
        if (user == null)
        {
