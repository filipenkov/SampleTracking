...
{
    /**
     * Returns an action configuration {@link Entry} describing the action to be executed for a given alias by querying the current configuration.
     * If there is no explicit alias defined for an action in the configuration, we consider that the alias for that
     * action is the action's name.
     *
     * @param alias The alias to use to lookup the action configuration entry.
     * @return An action configuration entry instance specifying the action, command, and
     *      permissions required to execute it.
     *      <p>If there's no entry for the specified alias <code>null</code> is returned.</p>
     */
        ...
        public Entry getActionCommand(final String alias)
        {
            try
            {
                return Entry.newBuilder().
                        ...
                        rolesRequired(getRolesRequiredMappingEntryFor(alias)).
                        build();
            }
    ...
    /**
     * An entry providing the webwork configuration for an action alias.  The entry
     * contains the action class, the command, and any permissions required in order
    ...
    public static class Entry
    {
        ...
        public Entry(final String name, final String command, final Iterable<Integer> permissionsRequired)
        {
            this.name = name;
        ...
        /**
         * Returns a String representing this action configuration entry if the format expected by the
         * action factory chain, i.e. <code>ManageSubTasks!enableSubTasks</code> where <code>ManageSubTasks</code>
         * is the action class name and <code>enableSubTasks</code> is the command method.
         * If no command method is supplied, then the string will only contain the action name,
         * i.e. <code>ViewProjects</code>.
         *
        ...
        public static class EntryBuilder
        {
            ...
            /**
             * Supplies a comma-separated list of the roles that are required to execute
             * this action.  When multiple roles are given, they are <strong>all</strong>
             * required.  When {@link #build()} is invoked, this list of roles is
             * transformed to a list of integer permissions as specified by {@link Permissions#getType(String)}.
             *
            ...
            public Entry build()
            {
                return new Entry(name, command, transformRolesToPermissions(rolesRequired));
            }
