...
public abstract class IssueConstantsSearchRenderer<T extends IssueConstant> extends AbstractSearchRenderer implements SearchRenderer
{
    ...
    public String getEditHtml(final User searcher, final SearchContext searchContext, final FieldValuesHolder fieldValuesHolder, final Map displayParameters, final Action action)
    {
        ...
        Collection selectedValues = (Collection) fieldValuesHolder.get(constants.getUrlParameter());
        velocityParams.put("selectedValues", selectedValues);
        Collection<T> validSelectListOptions = getSelectListOptions(searchContext);

        SortedSet<T> invalidSelections = getInvalidSelections(selectedValues, validSelectListOptions);
    ...
    private SortedSet<T> getInvalidSelections(Collection selectedValues, Collection<T> validSelectListOptions)
    {
        Collection<T> allSelectListOptions = getSelectListOptions(new SearchContextImpl(emptyList(), emptyList(), emptyList()));

        SortedSet<T> invalidSelections = new TreeSet<T>(ConstantsComparator.COMPARATOR);
        if (selectedValues != null) {
            for (T selectListOption : allSelectListOptions)
            {
                if (!validSelectListOptions.contains(selectListOption) && selectedValues.contains(selectListOption.getId()))
                {
                    invalidSelections.add(selectListOption);
                }
            }
        }
    ...
    public String getViewHtml(final User searcher, final SearchContext searchContext, final FieldValuesHolder fieldValuesHolder, final Map displayParameters, final Action action)
    {
        ...
        SortedSet<T> invalidSelections = getInvalidSelections(selectedValues, getSelectListOptions(searchContext));
