...
public abstract class AbstractIssuePickerSearchProvider implements IssuePickerSearchProvider
{
    ...
    public IssuePickerResults getResults(final JiraServiceContext context, final IssuePickerSearchService.IssuePickerParameters issuePickerParams, final int maxIssueCount)
    {
        ...
        try
        {
            final SearchRequest sr = getNoSortRequest(currentSearch);
            ...
            final SearchResults searchResults = searchProvider.search(sr.getQuery(), context.getLoggedInUser(), pagerFilter, filterQuery);
     ...
     *
     * @param searchRequest - the search request to wrap
     * @return a wrapped SearchRequest with no sorting
     */
    static SearchRequest getNoSortRequest(final SearchRequest searchRequest)
    {
        ...
        return new SearchRequest(new QueryImpl(query.getWhereClause(), null, null));
    }
