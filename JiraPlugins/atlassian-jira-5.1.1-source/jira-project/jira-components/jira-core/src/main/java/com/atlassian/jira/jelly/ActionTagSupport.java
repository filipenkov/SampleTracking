...
public abstract class ActionTagSupport extends JiraDynaBeanTagSupport
{
    ...
    public WebWorkAdaptor getWebWorkAdaptor()
    {
        if (webWorkAdaptor == null)
        {
            if (isBackendAction())
            {
                webWorkAdaptor = new WebWorkBackendAdaptor();
            }
            else
            {
    ...
    {
        return isBackendAction;
    }

    public void setBackendAction(boolean backendAction)
    {
        isBackendAction = backendAction;
    }
    ...
    {
        boolean valid = true;
        for (int i = 0; i < requiredContextVariables.length; i++)
        {
            if (!map.containsKey(requiredContextVariables[i]))
            {
                valid = false;
            }
            else if ((map.get(requiredContextVariables[i]) == null))
            {
                map.remove(requiredContextVariables[i]);
                valid = false;
            }
        }
        return valid;
