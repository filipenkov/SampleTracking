...
public class WorklogSystemField extends AbstractOrderableField implements OrderableField, RequirableField, HideableField, RenderableField, RestFieldOperations, RestAwareField
{
    ...
    public WorklogSystemField(VelocityTemplatingEngine templatingEngine, ApplicationProperties applicationProperties,
            ...
            CommentSearchHandlerFactory searchHandlerFactory, ComponentLocator componentLocator, GroupManager groupManager, DateTimeFormatterFactory dateTimeFormatterFactory, JiraBaseUrls jiraBaseUrls)
    {
    ...
    protected Object getRelevantParams(Map inputParameters)
    {
        ...
        final WorklogValue.Builder builder = new WorklogValue.Builder()
                .setId(asArray(inputParameters, WORKLOG_ID))
        ...
        String specificComment = fromArray(asArray(inputParameters, WORKLOG_DESCRIPTION));
        if (StringUtils.isNotBlank(specificComment))
        {
            builder.setComment(asArray(inputParameters, WORKLOG_DESCRIPTION));
        }
        else if (isCreateIssue || getTimeTrackingConfiguration().copyCommentToWorkDescriptionOnTransition())
        {
            ...
            builder.setComment(asArray(inputParameters, PARAM_COMMENT));
        }

        String specificSecurity = fromArray(asArray(inputParameters, WORKLOG_VISIBILITY_LEVEL));
        if (StringUtils.isNotBlank(specificSecurity))
        {
            builder.setCommentLevel(asArray(inputParameters, WORKLOG_VISIBILITY_LEVEL));
        }
        else if (isCreateIssue || getTimeTrackingConfiguration().copyCommentToWorkDescriptionOnTransition())
        {
            // on Create Issue, there is no Comment field, so we always use the comment as the Work Description
            builder.setCommentLevel(asArray(inputParameters, PARAM_COMMENT_LEVEL));
        }
    ...
    public FieldJsonRepresentation getJsonFromIssue(Issue issue, boolean renderedVersionRequested, final @Nullable FieldLayoutItem fieldLayoutItem)
    {
        ...
        final List<Worklog> worklogs = getWorklogService().getByIssueVisibleToUser(serviceContext, issue);
        if (!serviceContext.getErrorCollection().hasAnyErrors())
        {
            ...
            worklogWithPaginationBean.setWorklogs(WorklogJsonBean.asBeans(worklogs, jiraBaseUrls, ComponentAccessor.getUserManager(), ComponentAccessor.getComponent(TimeTrackingConfiguration.class)));
            ...
            if (renderedVersionRequested)
            {
                ...
                renderedWorklogBean.setWorklogs(WorklogJsonBean.asRenderedBeans(worklogs, jiraBaseUrls, fieldLayoutItem != null ? fieldLayoutItem.getRendererType() : null, issue.getIssueRenderContext()));
    ...
    {
        if (authenticationContext.getLoggedInUser() == null || !getCommentService().isGroupVisiblityEnabled())
        {
    ...
    {
        if (authenticationContext.getLoggedInUser() != null && issue != null && getCommentService().isProjectRoleVisiblityEnabled())
        {
