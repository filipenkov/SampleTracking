...
public class FieldHtmlFactoryImpl implements FieldHtmlFactory
{
            ...
            final Action action, final MutableIssue newIssueObject, boolean retainValues, final List<String> fieldsToRetain)
    {
        ...
        final List<FieldRenderItemWithTab> items = getRenderableItems(user, newIssueObject, CREATE_ISSUE_OPERATION);
            ...
            final Action action, final Issue issue, final boolean retainValues)
    {
        ...
        final List<FieldRenderItemWithTab> items = getRenderableItems(user, issue, EDIT_ISSUE_OPERATION);
        ...
        if (permissionManager.hasPermission(Permissions.COMMENT_ISSUE, issue, user))
        {
            ...
            final FieldLayoutItem commentFieldLayoutItem = getFieldScreenRenderer(user, issue, EDIT_ISSUE_OPERATION).getFieldScreenRenderLayoutItem(commentField).getFieldLayoutItem();
    ...
    private List<FieldRenderItemWithTab> getRenderableItems(final User user, final Issue issue, ScreenableIssueOperation operation)
    {
        final List<FieldRenderItemWithTab> items = new ArrayList<FieldRenderItemWithTab>();
        final FieldScreenRenderer fieldScreenRenderer = getFieldScreenRenderer(user, issue, operation);
        ...
        for (final FieldScreenRenderTab fieldScreenRenderTab : fieldScreenRenderTabs)
        {
            ...
            for (final FieldScreenRenderLayoutItem fieldScreenRenderLayoutItem : fieldScreenRenderTab.getFieldScreenRenderLayoutItems())
            {
                ...
                //Add the field if it's shown, it's not the project and it's not the issuetype (during create issue)
    ...
    private FieldScreenRenderer getFieldScreenRenderer(final User user, final Issue issue, final ScreenableIssueOperation operation)
    {
        return fieldScreenRendererFactory.getFieldScreenRenderer(user, issue, operation, false);
    }
