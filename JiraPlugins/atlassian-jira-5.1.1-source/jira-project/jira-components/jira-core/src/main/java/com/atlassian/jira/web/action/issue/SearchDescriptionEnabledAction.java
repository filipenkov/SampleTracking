...
public class SearchDescriptionEnabledAction extends IssueActionSupport
{
    ...
    {
        return favouritesService.isFavourite(getLoggedInUser(), getSearchRequest());
    }
    ...
    public FilterOperationsBean getFilterOperationsBean()
    {
        ...
        {
            final String userName = getLoggedInUser() == null ? null : getLoggedInUser().getName();
    ...
    {
        final SearchRequest searchRequest = getSearchRequest();
        if (searchRequest != null && searchService.validateQuery(getLoggedInUser(), searchRequest.getQuery()).hasAnyErrors())
        {
            return false;
        }
        return true;
