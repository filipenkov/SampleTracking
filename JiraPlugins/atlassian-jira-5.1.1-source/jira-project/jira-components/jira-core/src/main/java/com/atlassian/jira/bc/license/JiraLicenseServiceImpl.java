...
public class JiraLicenseServiceImpl implements JiraLicenseUpdaterService
{
    ...
    public JiraLicenseServiceImpl(JiraLicenseManager licenseManager, final BuildUtilsInfo buildUtilsInfo, final UserUtil userUtil)
    {
    ...
    public ValidationResult validate(final I18nHelper i18nHelper, final String licenseString)
    {
        ...
        else
        {
            ...
            if (checkLicenseVersion(i18nHelper, validationResult, licenseDetails))
            {
                final String buildPartnerName = buildUtilsInfo.getBuildPartnerName();
                final String licensePartnerName = licenseDetails.getPartnerName();

                if (StringUtils.isNotBlank(licensePartnerName) && !licensePartnerName.equals(buildPartnerName))
                {
                    validationResult.addError(i18nHelper.getText("setup.error.invalidlicensekey.regional.license", licensePartnerName));
                }
                else if (StringUtils.isBlank(licensePartnerName) && StringUtils.isNotBlank(buildPartnerName))
                {
