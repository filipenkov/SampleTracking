...
public class DefaultReindexMessageManager implements ReindexMessageManager
{
    ...
    public DefaultReindexMessageManager(final JiraPropertySetFactory jiraPropertySetFactory, final UserFormatManager userFormatManager,
            final I18nHelper.BeanFactory i18nFactory, final OutlookDateManager outlookDateManager, final VelocityRequestContextFactory velocityRequestContextFactory, PermissionManager permissionManager)
    {
    ...
    {
        if (propertiesReference.get().exists(PS_KEY_USER) && permissionManager.hasPermission(Permissions.ADMINISTER, user))
        {
