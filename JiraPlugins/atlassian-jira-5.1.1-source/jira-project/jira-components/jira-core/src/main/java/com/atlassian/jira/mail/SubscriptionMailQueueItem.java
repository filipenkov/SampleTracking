...
public class SubscriptionMailQueueItem extends AbstractMailQueueItem
{
    ...
    public void send() throws MailException
    {
        ...
        {
            final String userName = subscription.getString("username");
            final User user = getSubscriptionUser();
            if (user == null)
            {
                log.warn(
                        format
                                (
                                        ...
                                        userName, subscription.getLong("id"), subscription.getLong("filterID")
                                )
                );
            }
            else
    ...
    private void sendSearchRequestEmail(final User user) throws Exception
    {
        ...
        final User originalUser = jiraAuthenticationContext.getLoggedInUser();
    ...
    public IssueTableLayoutBean getTableLayout(final User user) throws Exception
    {
    ...
    public List <ColumnLayoutItem> getColumns(final User user) throws Exception
    {
        final SearchRequest searchRequest = getSearchRequest();
        ...
        if (searchRequest != null && searchRequest.isLoaded() && searchRequest.useColumns())
        {
            ...
            columnLayout = getColumnLayoutManager().getColumnLayout(user, getSearchRequest());
        }
        else
        {
            ...
            columnLayout = getColumnLayoutManager().getColumnLayout(user);
        }
    ...
    private Map<String, Object> getContextParams(final GenericValue sub, final SearchRequest sr, final User u) throws Exception
    {
