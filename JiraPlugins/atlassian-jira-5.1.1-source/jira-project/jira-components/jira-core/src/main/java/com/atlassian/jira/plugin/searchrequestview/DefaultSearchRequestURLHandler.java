...
public class DefaultSearchRequestURLHandler implements SearchRequestURLHandler
{
    ...
    public String getURLWithoutContextPath(final SearchRequestViewModuleDescriptor moduleDescriptor, final SearchRequest searchRequest)
    {
        final StringBuilder sb = new StringBuilder();
        sb.append("/sr/");
        sb.append(moduleDescriptor.getCompleteKey());

        final Long searchRequestId = searchRequest.getId();
        // generate the link for saved search requests: (eg. /sr/jira.issueviews:searchrequest-xml/10010/SearchRequest-10010.xml)
        if ((searchRequestId != null) && !searchRequest.isModified())
        {
            sb.append("/");
    ...
    private String getQueryString(final SearchRequest searchRequest)
    {
    ...
    public void handleRequest(final HttpServletRequest request, final HttpServletResponse response) throws IOException
    {
        ...
        else
        {
            ...
            searchRequest = ComponentManager.getInstance().getSearchRequestService().getFilter(ctx, searchRequestId);
        ...
        final MessageSet messageSet = searchService.validateQuery(loggedInUser, searchRequest.getQuery());
        ...
        final SearchRequestParams searchRequestParams = new SearchRequestParamsImpl(request.getSession(true), getPagerFilter(request),
    ...
    {
        final String[] jqlArr = parameters.get(IssueNavigator.JQL_QUERY_PARAMETER);
