...
public final class IssueTabPanelInvokerImpl implements IssueTabPanelInvoker
{
    ...
    @Override
    public ShowPanelReply invokeShowPanel(ShowPanelRequest request, IssueTabPanelModuleDescriptor descriptor)
    {
        final String timerName = "Calling showPanel for " + descriptor;
        UtilTimerStack.push(timerName);
        try
        {
            IssueTabPanel issueTabPanel = descriptor.getModule();
            if (issueTabPanel instanceof IssueTabPanel2)
            {
                // use the new method from IssueTabPanel2
                return ((IssueTabPanel2) issueTabPanel).showPanel(request);
            }

            try
            {
                return ShowPanelReply.create(issueTabPanel.showPanel(request.issue(), request.remoteUser()));
            }
        ...
        {
            log.error("Exception thrown while trying to call showPanel() for " + descriptor, e);
    ...
    @Override
    public GetActionsReply invokeGetActions(final GetActionsRequest request, final IssueTabPanelModuleDescriptor descriptor)
    {
        ...
        try
        {
            ...
            // try to use the new method from IssueTabPanel2
            if (issueTabPanel instanceof IssueTabPanel2)
            {
                IssueTabPanel2 issueTabPanel2 = (IssueTabPanel2) issueTabPanel;
                return issueTabPanel2.getActions(request);
            }

            try
            {
                return GetActionsReply.create(issueTabPanel.getActions(request.issue(), request.remoteUser()));
            }
            catch (AbstractMethodError e)
            {
                // This could happen because we changed the getActions(Issue,User) in a binary-incompatible way, but the JVM
                ...
                return GetActionsReply.create(new RenderingErrorAction(descriptor));
            }
        }
        catch (RuntimeException e)
        {
            log.error("Exception thrown while trying to call getActions() for " + descriptor, e);
    ...
    class RenderingErrorAction implements IssueAction
    {
        ...
        {
            return i18Factory.getInstance(jiraAuthenticationContext.getLoggedInUser()).getText("modulewebcomponent.exception", moduleDescriptor.getCompleteKey());
        }
