...
public class DefaultSessionSearchObjectManagerFactory implements SessionSearchObjectManagerFactory
{
    ...
    {
        return new DefaultSessionSearchRequestManager(request, new AbstractSessionSearchObjectManager.HttpSessionWrapper(request.getSession()));
    }

    public SessionSearchRequestManager createSearchRequestManager(final VelocityRequestSession session)
    {
        return new DefaultSessionSearchRequestManager(ExecutingHttpRequest.get(), new AbstractSessionSearchObjectManager.VelocityRequestSessionWrapper(session));
    }
