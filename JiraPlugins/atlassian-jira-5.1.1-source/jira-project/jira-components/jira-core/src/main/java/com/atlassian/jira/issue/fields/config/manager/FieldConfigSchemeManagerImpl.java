...
public class FieldConfigSchemeManagerImpl implements FieldConfigSchemeManager
{
    ...
    // ---------------------------------------------------------------------------------------------------- Constructors
    public FieldConfigSchemeManagerImpl(final FieldConfigSchemePersister configSchemePersister, final FieldConfigContextPersister contextPersister, final JiraContextTreeManager treeManager, final FieldConfigManager configManager)
    {
    ...
    public FieldConfigScheme updateFieldConfigScheme(final FieldConfigScheme newScheme, final List<JiraContextNode> contexts, final ConfigurableField field)
    {
        ...
        contextPersister.removeContextsForConfigScheme(newScheme.getId());
        storeAssociateContexts(configScheme, contexts, field);
        return configScheme;
    }
    ...
    public void removeFieldConfigScheme(final Long fieldConfigSchemeId)
    {
         ...
         */
        contextPersister.removeContextsForConfigScheme(fieldConfigSchemeId);
    ...
    public FieldConfig getRelevantConfig(final IssueContext issueContext, final ConfigurableField field)
    {
        ...
        if (scheme != null)
        {
            final Map configs = scheme.getConfigs();
            if (configs != null)
            {
                final FieldConfig configForAnyIssueType = (FieldConfig) configs.get(null);
                ...
                final FieldConfig config = (FieldConfig) configs.get(issueTypeId);
                if (config != null)
                {
                    return config;
                }
    ...
    private void storeAssociateContexts(final FieldConfigScheme config, final List<JiraContextNode> contexts, final ConfigurableField field)
    {
        for (final JiraContextNode contextNode : contexts)
        {
            setValue(contextNode, field.getId(), config);
        }
    }
}
