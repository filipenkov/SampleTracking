 ...
 * <p/>
 * The appender may be called before JIRA.HOME is ready to be used. In this situtation the appender will buffer the log
...
public final class JiraHomeAppender implements Appender, OptionHandler, RollOverLogAppender
{
    ...
    private static enum State
    {
        ...
        /**
         * JIRA.HOME is not configured. All log messages will be silently droped.
         */
        ERROR
    }
    ...
    private synchronized void doAppendSync(final LoggingEvent event)
    {
        if (state == State.WAITING)
        {
            ...
            else if (configState == State.READY)
            {
            ...
            {
                //Once in error state we don't need these either.
                events = null;
                fileName = null;
            }
            state = configState;
