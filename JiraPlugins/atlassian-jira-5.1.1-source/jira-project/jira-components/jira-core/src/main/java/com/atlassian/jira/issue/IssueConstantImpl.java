...
public class IssueConstantImpl extends AbstractOfBizValueWrapper implements IssueConstant
{
    ...
    // perf: cache this as it gets called a lot in lucene sorts
    ...
    {
        synchronized (propertySetLock)
        {
            if (ps == null)
            {
                // PropertySet expects GenericValue to have a Long for its id - IssueConstants have Strings for the id
                String id = genericValue.getString("id");
                genericValue.set("id", new Long(id));
                PropertySet ofbizPs = OFBizPropertyUtils.getPropertySet(genericValue);
                genericValue.set("id", id);

                HashMap<String,Object> args = new HashMap<String,Object>();
                args.put("PropertySet", ofbizPs);
                args.put("bulkload", Boolean.TRUE);
                // Create a cached property set
                ps = PropertySetManager.getInstance("cached", args);
            }
        }
        return ps;
