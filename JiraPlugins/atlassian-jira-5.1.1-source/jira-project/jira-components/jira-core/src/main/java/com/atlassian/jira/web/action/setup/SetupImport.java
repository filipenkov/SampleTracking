...
public class SetupImport extends AbstractSetupAction
{
    ...
    public SetupImport(final IndexPathManager indexPathManager, final ExternalLinkUtil externalLinkUtil,
            final BuildUtilsInfo buildUtilsInfo, FileFactory fileFactory, final DataImportService dataImportService,
            final ImportResultHandler importResultHandler, final VelocityRequestContextFactory velocityRequestContextFactory)
    {
        super(fileFactory);
    ...
    protected void doValidation()
    {
        ...
        if(lastResult == null)
        {
            final DataImportParams params = new DataImportParams.Builder(filename).setLicenseString(license).setUseDefaultPaths(useDefaultPaths).setupImport().build();
