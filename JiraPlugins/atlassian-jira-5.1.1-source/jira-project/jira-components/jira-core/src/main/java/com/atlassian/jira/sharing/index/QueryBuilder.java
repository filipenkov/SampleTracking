...
public class QueryBuilder
{
    ...
    static final class Analyzers
    {
        ...
        static final Analyzer STEMMER = new Analyzer()
        {
            ...
            {
                return new PorterStemFilter(new LowerCaseFilter(LUCENE_VERSION, new StandardFilter(LUCENE_VERSION, new StandardTokenizer(LUCENE_VERSION, reader))));
            }
        };
        ...
        static final Analyzer WILDCARD_STEMMER = new Analyzer()
        {
            public TokenStream tokenStream(final String arg0, final Reader reader)
            {
                return new WildcardFilter(new PorterStemFilter(new LowerCaseFilter(LUCENE_VERSION, new StandardFilter(LUCENE_VERSION, new StandardTokenizer(LUCENE_VERSION, reader)))));
            }
        };
        ...
        static final Analyzer WILDCARD = new Analyzer()
        {
            public TokenStream tokenStream(final String arg0, final Reader reader)
            {
                return new WildcardFilter(new LowerCaseFilter(LUCENE_VERSION, new StandardFilter(LUCENE_VERSION, new StandardTokenizer(LUCENE_VERSION, reader))));
            }
        };
          ...
          static final Analyzer LOWERCASE = new Analyzer()
        {
            public TokenStream tokenStream(final String arg0, final Reader reader)
            {
                return new LowerCaseFilter(LUCENE_VERSION, new StandardFilter(LUCENE_VERSION, new StandardTokenizer(LUCENE_VERSION, reader)));
            }
        };
        ...
        static final Analyzer STANDARD = new Analyzer()
        {
            public TokenStream tokenStream(final String arg0, final Reader reader)
            {
                return (new StandardFilter(new StandardTokenizer(LUCENE_VERSION, reader)));
            }
        };
    }
    ...
    static Query parseQuery(final SharedEntityColumn column, final String value) throws SearchParseException
    {
        ...
        final QueryParser parser = new QueryParser(LUCENE_VERSION, getNameFor(column), Analyzers.LOWERCASE);
    ...
    static Query parseWildcardQuery(final SharedEntityColumn column, final String value) throws SearchParseException
    {
        ...
        final QueryParser wildCardParser = new QueryParser(LUCENE_VERSION, getNameFor(column), Analyzers.WILDCARD);
        final QueryParser parser = new QueryParser(LUCENE_VERSION, getNameFor(column),Analyzers.STANDARD);
        parser.setAllowLeadingWildcard(false);
        try
        {
