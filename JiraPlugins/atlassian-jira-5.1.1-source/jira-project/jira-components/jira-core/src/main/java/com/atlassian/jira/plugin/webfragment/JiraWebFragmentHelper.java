...
public class JiraWebFragmentHelper implements WebFragmentHelper
{
    ...
    protected String getHtml(final String fragment, final Map<String, Object> startingParams)
    {
        try
        {
            if (isNotBlank(fragment))
            {
                return templatingEngine.render(fragment(fragment)).applying(startingParams).asHtml();
            }
        }
