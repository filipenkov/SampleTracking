...
public class ViewVoters extends AbstractIssueSelectAction
{
    ...
    public String doDefault() throws Exception
    {
        if (!isIssueValid())
        {
            ...
            if (!permissionManager.hasPermission(Permissions.VIEW_VOTERS_AND_WATCHERS, getProject(), getLoggedInUser()))
            {
    ...
    public boolean isVotedAlready()
    {
        if (votedAlready == null)
        {
            ...
            {
                votedAlready = Boolean.valueOf(voteManager.hasVoted(getLoggedInUser(), getIssue()));
            }
            else
            {
