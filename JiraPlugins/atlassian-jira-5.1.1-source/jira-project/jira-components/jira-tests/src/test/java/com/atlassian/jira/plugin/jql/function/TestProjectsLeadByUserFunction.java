...
public class TestProjectsLeadByUserFunction extends MockControllerTestCase
{
    ...
    public void testGetValuesAnonymous()
    {
        ...
        for (Project project : projectsList2)
        {
            EasyMock.expect(permissionManager.hasPermission(Permissions.BROWSE, project, (User) null)).andReturn(true);
        }
        replay(userUtil, projectManager, permissionManager);
        ...
        for (Project project : projectsList2)
        {
            EasyMock.expect(permissionManager.hasPermission(Permissions.BROWSE, project, (User) null)).andReturn(false);
        }
        replay(userUtil, projectManager, permissionManager);
