...
public class IssueTabPanelInvokerImplTest
{
    ...
    public void setUp() throws Exception
    {
        ...
        getActionsRequest = new GetActionsRequest(issue, admin, false);
    }
    ...
    {
        ShowPanelReply reply = invoker().invokeShowPanel(new ShowPanelRequest(issue, admin), descriptorFor(new BrokenIssueTabPanel()));
        assertThat(reply.isShow(), equalTo(true));

        ShowPanelReply reply2 = invoker().invokeShowPanel(new ShowPanelRequest(issue, admin), descriptorFor(new BrokenIssueTabPanel2()));
        assertThat(reply2.isShow(), equalTo(true));
    }

    @Test
    public void getActionsShouldRenderAMessageWhenPanelThrowsAnException() throws Exception
    {
        GetActionsReply reply = invoker().invokeGetActions(getActionsRequest, descriptorFor(new BrokenIssueTabPanel()));
        assertThat(reply.actions().get(0).getHtml(), equalTo(NoopI18nHelper.makeTranslation("modulewebcomponent.exception", "completeKey-" + BrokenIssueTabPanel.class.getName())));

        GetActionsReply reply2 = invoker().invokeGetActions(getActionsRequest, descriptorFor(new BrokenIssueTabPanel2()));
        assertThat(reply2.actions().get(0).getHtml(), equalTo(NoopI18nHelper.makeTranslation("modulewebcomponent.exception", "completeKey-" + BrokenIssueTabPanel2.class.getName())));
    }
    ...
    public void showPanelShouldReturnTrueWhenDescriptorThrowsAnException() throws Exception
    {
        ...
        ShowPanelReply reply = invoker().invokeShowPanel(showPanelRequest, descriptor);
        assertThat(reply.isShow(), equalTo(true));
    }
    ...
    public void getActionsShouldRenderAMessageWhenDescriptorThrowsAnException() throws Exception
    {
        ...
        GetActionsReply reply = invoker().invokeGetActions(getActionsRequest, descriptor);
        assertThat(reply.actions().get(0).getHtml(), equalTo(NoopI18nHelper.makeTranslation("modulewebcomponent.exception", "keyOfBadDescriptor")));
    }
    ...
    {
        IssueTabPanelModuleDescriptor descriptor = mock(IssueTabPanelModuleDescriptor.class);
        when(descriptor.getModule()).thenReturn(tabPanel);
