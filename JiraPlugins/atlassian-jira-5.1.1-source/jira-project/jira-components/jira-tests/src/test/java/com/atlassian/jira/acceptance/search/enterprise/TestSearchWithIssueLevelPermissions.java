...
public class TestSearchWithIssueLevelPermissions extends AbstractUsersIndexingTestCase
{
    ...
    protected void setUp() throws Exception
    {
        ...
        Group allGroup = createMockGroup("group1");

        j = createMockUser("j");
        addUserToGroup(j, allGroup);

        k = createMockUser("k");
        addUserToGroup(k, allGroup);
        Group lGroup = createMockGroup("L Group");
        addUserToGroup(k, lGroup);

        l = createMockUser("l");
        addUserToGroup(l, allGroup);
        m = createMockUser("m");
        addUserToGroup(m, allGroup);
        n = createMockUser("n");
        addUserToGroup(n, allGroup);
