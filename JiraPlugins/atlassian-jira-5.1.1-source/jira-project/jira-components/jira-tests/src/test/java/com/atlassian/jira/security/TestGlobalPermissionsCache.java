...
public class TestGlobalPermissionsCache extends LegacyJiraMockTestCase
{
    ...
    public void testRefreshHasGetPermission()
    {
        ...
        assertTrue(!pc.hasPermission(new JiraPermissionImpl(perm1)));

        pc.refresh();
        assertTrue(pc.hasPermission(new JiraPermissionImpl(perm1)));
        assertEquals(perm1, pc.getPermission(new JiraPermissionImpl(perm1)));
        ...
        assertTrue(!pc.hasPermission(new JiraPermissionImpl(perm2)));

        pc.refresh();
        assertTrue(pc.hasPermission(new JiraPermissionImpl(perm1)));
        assertTrue(pc.hasPermission(new JiraPermissionImpl(perm2)));
        assertEquals(perm1, pc.getPermission(new JiraPermissionImpl(perm1)));
        assertEquals(perm2, pc.getPermission(new JiraPermissionImpl(perm2)));
    }
}
