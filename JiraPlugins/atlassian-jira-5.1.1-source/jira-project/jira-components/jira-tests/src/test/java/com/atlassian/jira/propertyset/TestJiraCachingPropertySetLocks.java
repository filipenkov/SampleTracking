...
public class TestJiraCachingPropertySetLocks extends ListeningTestCase
{
    @Test
    ...
    public void testGetSetDoubleLocks() throws Exception
    {
        ...
        assertEquals(new Double(123.456).doubleValue(), propertySet.getDouble("test"), 0);
        // The value is now cached, so no lock is needed.
        lock.assertLockedCount(3);
    }
    ...
    public void testGetSetObjectLocks() throws Exception
    {
        ...
        assertEquals(new Integer(891), propertySet.getObject("test"));
        // The value is now cached, so no lock is needed.
        lock.assertLockedCount(3);
    }
    ...
    public void testGetKeysDoesNotLock() throws Exception
    {
        ...
        assertEquals(EasyList.build("test"), propertySet.getKeys());
        lock.assertLockedCount(2);

        assertEquals(EasyList.build("test"), propertySet.getKeys(PropertySet.TEXT));
        lock.assertLockedCount(2);

        assertEquals(EasyList.build("test"), propertySet.getKeys("tes"));
        lock.assertLockedCount(2);

        assertEquals(EasyList.build("test"), propertySet.getKeys("tes", PropertySet.TEXT));
        lock.assertLockedCount(2);
    }
    ...
    private void init(final JiraCachingPropertySet propertySet)
    {
