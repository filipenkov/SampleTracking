...
public class TestDefaultProjectManager extends AbstractUsersIndexingTestCase
{
    ...
    protected void setUp() throws Exception
    {
        ...
        User u1 = createMockUser("Owen Fellows");
        Group g1 = createMockGroup("Test Group");
        addUserToGroup(u1, g1);
        createMockUser("Watcher 1");
    ...
    public void testGetNextIdWithDuplicateKey() throws GenericEntityException
    {
        ...
        Project project1Obj = pm.getProjectObj(100L);
        assertEquals(103, pm.getNextId(project1Obj));
        assertEquals(104, pm.getNextId(project1Obj));
        assertEquals(105, pm.getNextId(project1Obj));
    ...
    public void testUpdateProject()
    {
        ...
        projectManager.updateProject(projectManager.getProjectObjByKey("ABC"), "Snookums", "Snook snooky", "vez", null, AssigneeTypes.PROJECT_LEAD);
        final Project project = projectManager.getProjectObjByKey("ABC");

        assertEquals("ABC", project.getKey());
        assertEquals("Snookums", project.getName());
        assertEquals("Snook snooky", project.getDescription());
        assertEquals("vez", project.getLead().getName());
        assertEquals(new Long(2), project.getAssigneeType());
        assertEquals(null, project.getUrl());

        assertNotNull(projectManager.getProjectObjByName("Snookums"));
        assertNotNull(projectManager.getProjectObjByKey("ABC"));
    }
    ...
    {
        componentLead = createMockUser("componentLead");
        projectLead = createMockUser("projectLead");
    ...
    public void testDefaultAssigneeWithNoUnassigned()
            throws DefaultAssigneeException, OperationNotPermittedException, InvalidUserException, InvalidCredentialException
    {
    ...
    public void testDefaultAssigneeWithUnassigned()
            throws DefaultAssigneeException, GenericEntityException, OperationNotPermittedException, InvalidUserException, InvalidCredentialException
    {
        setupDefaultAssigneeTests();
