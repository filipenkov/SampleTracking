...
public class TestFilterOperationsBean extends ListeningTestCase
{
    ...
    {
        final SearchRequest searchRequest = new SearchRequest(null, null, "v5.1 Bugs", "Bugs not done in v5.1");

        FilterOperationsBean bean;

        // null SR owner : ownFilter
        bean = FilterOperationsBean.create(searchRequest, true, "fred", true);
        assertTrue(bean.isShowSaveNew());

        // SR owner = loggedInUser (case-insensitive) : ownFilter
        searchRequest.setOwnerUserName("Fred");
        bean = FilterOperationsBean.create(searchRequest, true, "fred", true);
        assertTrue(bean.isShowSaveNew());
