...
public class TestCloneIssueDetails extends LegacyJiraMockTestCase
{
    ...
    {
        mockIssueLinkTypeManager.expectAndReturn("getIssueLinkTypesByName", P.args(new IsEqual("Clones")), EasyList.build(new IssueLinkTypeImpl(cloneLink)));
        applicationProperties = new MyApplicationProperties("Clones");
        CloneIssueDetails cloneIssueDetails = getCloneIssueDetails(applicationProperties);
        assertFalse(cloneIssueDetails.isDisplayCloneLinkWarning());
        mockIssueLinkTypeManager.verify();
    ...
    private CloneIssueDetails getCloneIssueDetails(ApplicationPropertiesImpl applicationProperties)
    {
        ...
        if (applicationProperties != null)
            cloneIssueDetails = new CloneIssueDetails(applicationProperties, myPermissionManager, null, null, (IssueLinkTypeManager) mockIssueLinkTypeManager.proxy(), (SubTaskManager) mockSubTaskManager.proxy(), null, null, issueCreationHelperBean, null, null);
        else
            cloneIssueDetails = new CloneIssueDetails(null, myPermissionManager, null, null, (IssueLinkTypeManager) mockIssueLinkTypeManager.proxy(), (SubTaskManager) mockSubTaskManager.proxy(), null, null, issueCreationHelperBean, null, null);
