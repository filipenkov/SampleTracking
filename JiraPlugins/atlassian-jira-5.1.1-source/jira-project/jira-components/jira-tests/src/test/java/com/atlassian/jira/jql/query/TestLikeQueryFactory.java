...
public class TestLikeQueryFactory extends ListeningTestCase
{
    ...
    public void testCreateQueryNullRawValues() throws Exception
    {
        try
        {
            ...
            componentAccessorWorker.addMock(IssueIndexManager.class, issueIndexManager);
            ComponentAccessor.initialiseWorker(componentAccessorWorker);
            when(issueIndexManager.getAnalyzerForSearching()).thenReturn(JiraAnalyzer.ANALYZER_FOR_SEARCHING);
    ...
    public void testCreateQueryEmptyString() throws Exception
    {
        try
        {
            ...
            ComponentAccessor.initialiseWorker(componentAccessorWorker);
            when(issueIndexManager.getAnalyzerForSearching()).thenReturn(JiraAnalyzer.ANALYZER_FOR_SEARCHING);
            final QueryFactoryResult result = new LikeQueryFactory().createQueryForSingleValue("testField", Operator.LIKE, Lists.newArrayList(createLiteral("")));
            assertEquals(QueryFactoryResult.createFalseResult(), result);
        }
        finally
        {
    ...
    public void testCreateQueryHappyPathLong() throws Exception
    {
        final LikeQueryFactory likeQueryFactory = new LikeQueryFactory()
        {
            ...
            {
                return new EnglishAnalyzer(DefaultIndexManager.LUCENE_VERSION, false);
            }
        };
    ...
    public void testCreateQueryHappyPathComplexStringWithEscaping() throws Exception
    {
        final LikeQueryFactory likeQueryFactory = new LikeQueryFactory()
        {
            ...
            Analyzer getAnalyzer()
            {
                return new EnglishAnalyzer(Version.LUCENE_29, false);
            }
        };
