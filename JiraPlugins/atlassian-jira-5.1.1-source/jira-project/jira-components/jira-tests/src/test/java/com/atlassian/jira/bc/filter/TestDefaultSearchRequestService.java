...
public class TestDefaultSearchRequestService extends ListeningTestCase
{
    ...
    public void setUp() throws Exception
    {
        ...
        loggedInUser = ImmutableUser.newUser().name("admin").toUser();

        filter1 = new SearchRequest(new QueryImpl(), loggedInUser.getName(), "filter1", "filterter1Desc", 1L, 0L);
        filter2 = new SearchRequest(new QueryImpl(), loggedInUser.getName(), "filter2", "filterter2Desc", 2L, 0L);
        filter3 = new SearchRequest(new QueryImpl(), loggedInUser.getName(), "filter3", "filterter3Desc", 3L, 0L);

        ComponentAccessor.initialiseWorker(new MockComponentWorker().addMock(TimeZoneManager.class, mock(TimeZoneManager.class)));
    }
    ...
    {
        return createContext(loggedInUser);
    }
    ...
    public void testFavouriteFiltersNullUser()
    {
        ...
        final Collection results = service.getFavouriteFilters(null);
    ...
    {
        when(mockSearchRequestManager.getSearchRequestOwnerUserName(filter1.getId())).
                thenReturn(filter1.getOwnerUserName());
    ...
    public void testDeleteFilterNoFilter()
    {
        ...
        when(mockSearchRequestManager.getSearchRequestOwnerUserName(filter1.getId())).thenReturn(null);
    ...
    {
        when(mockSearchRequestManager.getSearchRequestOwnerUserName(filter1.getId())).thenReturn("other");
    ...
    {
        final User anAnonymousUser = null;
        when(mockSearchRequestManager.getSearchRequestById(anAnonymousUser, filter1.getId())).thenReturn(filter1);

        final SearchRequestService service = createSearchRequestService();
        final JiraServiceContext serviceContext = createContext(anAnonymousUser);
        final SearchRequest result = service.getFilter(serviceContext, filter1.getId());
    ...
    {
        final SearchRequest filter1 = new SearchRequest(new QueryImpl(), loggedInUser.getName(), "filter1", "filterter1Desc", 1L, 3L);
        final SearchRequest filter2 = new SearchRequest(new QueryImpl(), loggedInUser.getName(), "filter2", "filterter2Desc", 2L, 3L);
        final SearchRequest filter3 = new SearchRequest(new QueryImpl(), loggedInUser.getName(), "filter3", "filterter3Desc", 3L, 3L);
    ...
    {
        final SearchRequest filter1 = new SearchRequest(new QueryImpl(), loggedInUser.getName(), "filter1", "filterter1Desc", 1L, 1L);
        final SearchRequest filter2 = new SearchRequest(new QueryImpl(), loggedInUser.getName(), "filter2", "filterter2Desc", 2L, 1L);
        final SearchRequest filter3 = new SearchRequest(new QueryImpl(), loggedInUser.getName(), "filter3", "filterter3Desc", 3L, 0L);
    ...
    {
        final SearchRequest filter1 = new SearchRequest(new QueryImpl(), loggedInUser.getName(), "filter1", "filterter1Desc", 1L, 2L);
        final SearchRequest filter2 = new SearchRequest(new QueryImpl(), loggedInUser.getName(), "filter2", "filterter2Desc", 2L, 1L);
        final SearchRequest filter3 = new SearchRequest(new QueryImpl(), loggedInUser.getName(), "filter3", "filterter3Desc", 3L, 3L);

        final List<SearchRequest> searchRequests = ImmutableList.of(filter2, filter1, filter3);
        final List<SearchRequest> compareList = ImmutableList.of(filter1, filter3);
        filter1.setOwnerUserName(loggedInUser.getName());
    ...
    public void testGetFiltersFavouritedByOthersNullUser()
    {
        ...
        final Collection<SearchRequest> filters = service.getFiltersFavouritedByOthers(null);
    ...
    {
        final User owner = ImmutableUser.newUser().name("owen").toUser();

        filter1.setOwnerUserName(owner.getName());
        ...
        when(mockSearchRequestManager.getSearchRequestOwnerUserName(filter1.getId())).thenReturn(owner.getName());
    ...
    public void testValidateFilterForUpdateWithValidFilter()
    {
        ...
        when(mockSearchRequestManager.getSearchRequestOwnerUserName(filter1.getId())).thenReturn(filter1.getOwnerUserName());
    ...
    public void testValidateFilterForUpdateWithBadNameAndSaved()
    {
        ...
        final SearchRequest newRequest = new SearchRequest(new QueryImpl(), loggedInUser.getName(), "testValidateFilterWithBadNameAndSaved", null, 1L, 0L);
        final SearchRequest oldRequest = new SearchRequest(new QueryImpl(), loggedInUser.getName(), "crapName", null, 2L, 0L);

        when(mockSearchRequestManager.getSearchRequestOwnerUserName(newRequest.getId())).
                thenReturn(newRequest.getOwnerUserName());
    ...
    public void testValidateFilterForUpdateUpdateName()
    {
        ...
        final SearchRequest oldRequest = new SearchRequest(new QueryImpl(), loggedInUser.getName(), "crapName", null, 1L, 0L);
    ...
    {
        final User otherUser = ImmutableUser.newUser().name("other").toUser();

        final JiraServiceContext ctx = createServiceContext();

        filter1.setOwnerUserName(otherUser.getName());
        filter1.setPermissions(SharePermissions.PRIVATE);
    ...
    {
        final JiraServiceContext ctx = new MockJiraServiceContext(loggedInUser);
    ...
    public void testValidateFilterForCreateWithBadNameAndNotSaved()
    {
        ...
        final SearchRequest notSavedFilter = new SearchRequest(new QueryImpl(), loggedInUser.getName(), "NotSaved", null);
    ...
    public void testValidateFilterForCreateWithNameThatsTooLong()
    {
        ...
        final SearchRequest newRequest = new SearchRequest(new QueryImpl(), loggedInUser.getName(), name, null, 1L, 0L);
        when(mockShareTypeValidatorUtils.isValidSharePermission(ctx, newRequest)).thenReturn(true);
    ...
    {
        final User otherUser = ImmutableUser.newUser().name("otherUser").toUser();

        when(mockSearchRequestManager.getSearchRequestOwnerUserName(filter1.getId())).thenReturn(otherUser.getName());
    ...
    public void testUpdateFilterNotOwnerInDb()
    {
        ...
        when(mockSearchRequestManager.getSearchRequestOwnerUserName(filter1.getId())).thenReturn("notMe");
    ...
    public void testUpdateSearchParametersFilter()
    {
        ...
        when(mockSearchRequestManager.getSearchRequestOwnerUserName(newFilter.getId())).thenReturn(newFilter.getOwnerUserName());

        when(mockSearchRequestManager.getSearchRequestById(ctx.getLoggedInUser(), newFilter.getId())).thenReturn(filter1);
    ...
    public void testUpdateSearchParametersFilterDoesNotExist()
    {
        ...
        when(mockSearchRequestManager.getSearchRequestById(ctx.getLoggedInUser(), newFilter.getId())).thenReturn(null);
    ...
    public void testSearchNullUser()
    {
        ...
        final JiraServiceContext ctx = createContext(aNullUser);
        final SearchRequestService service = createSearchRequestService();
        final SharedEntitySearchResult actualResult = service.search(ctx, sharedEntitySearchParameters, 0, 10);
    ...
    @Test
    public void testValidateSearchAnyShareTypeInvalidUserWhenLoggedIn() throws Exception
    {
        final String searchUserName = "fred";
        when(mockUserUtil.userExists(searchUserName)).thenReturn(false);
        when(mockPermissionManager.hasPermission(Permissions.USER_PICKER, loggedInUser)).thenReturn(true);
        ...
        service.validateForSearch(ctx, searchTemplate.toSearchParameters());
        assertTrue(ctx.getErrorCollection().hasAnyErrors());
        assertNotNull(ctx.getErrorCollection().getErrors().get("searchOwnerUserName"));
    }
    ...
    private static final class Stubs
    {
        ...
        private static final class SearchRequests
        {
            ...
            {
                return new SearchRequest(new QueryImpl(), null, "filter1", "filterter1Desc", null, 0L);
            }
        }
