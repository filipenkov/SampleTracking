...
public class TestAddActorsToProjectRole extends AbstractProjectRolesTest
{
    ...
    public void testAddActorsToProjectRole() throws Exception
    {
        ...
        projectRoleService.expectVoid("addActorsToProjectRole", new Constraint[] { new IsEqual(u), new IsEqual(actors), new IsEqual(
            MockProjectRoleManager.PROJECT_ROLE_TYPE_1), new IsEqual(mockProject), new IsEqual("atlassian-user-role-actor"), new IsEqual(
            new SimpleErrorCollection()) });
        projectRoleService.expectAndReturn("getProjectRole", new Constraint[] { new IsEqual(u), new IsEqual(new Long(1)), new IsEqual(
