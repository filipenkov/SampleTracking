...
public class TestIssueDocument extends LegacyJiraMockTestCase
{
    ...
    public void testNonEmptyFieldIds() throws Exception
    {
        ...
        TopDocs hits = indexSearcher.search(notHaveEnvironmentQuery, Integer.MAX_VALUE);

        // Make sure we only got one out
        assertEquals(1, hits.totalHits);
        // Make sure it is the right one
        String[] values = indexSearcher.doc(hits.scoreDocs[0].doc).getValues(DocumentConstants.ISSUE_KEY);
        ...
        hits = indexSearcher.search(hasEnvironmentQuery, Integer.MAX_VALUE);

        // Make sure we only got one out
        assertEquals(1, hits.totalHits);
        // Make sure it is the right one
        values = indexSearcher.doc(hits.scoreDocs[0].doc).getValues(DocumentConstants.ISSUE_KEY);
