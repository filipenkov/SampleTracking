...
public class TestSelectComponentAssignees extends AbstractJellyTestCase
{
    ...
    protected void setUp() throws Exception
    {
        ...
        ManagerFactory.getPermissionManager().addPermission(Permissions.ASSIGNABLE_USER, scheme, createMockGroup(AbstractSetupAction.DEFAULT_GROUP_DEVELOPERS).getName(), GroupDropdown.DESC);
    }
    ...
    public void testSelectInvalidComponentLead()
    {
        try
        {
            ManagerFactory.getPermissionManager().removeGroupPermissions(createMockGroup(AbstractSetupAction.DEFAULT_GROUP_DEVELOPERS).getName());
            assertTrue(!ManagerFactory.getPermissionManager().hasPermission(Permissions.ASSIGNABLE_USER, project, user));
            _testSelectComponentAssignee("select-component-assignees.test.select-assigneetype-componentLead.jelly", new Long(ComponentAssigneeTypes.COMPONENT_LEAD));
        }
        catch (Exception e)
