 ...
 * @see AJS.UncomplicatedInlineLayer
 * @see JIRA.ProgressiveDataSet
 */
AJS.InlineLayer = AJS.Control.extend({
    ...
    setPosition: function () {
        ...
        if (AJS.$(".aui-blanket").is(":visible")) {
            var scrollLeft = AJS.$(window).scrollLeft();
            if (scrollLeft > 0) {
                positioning.left -= scrollLeft;
            }
            positioning.maxHeight = AJS.$(window).height() - positioning.top - this.options.cushion();
        }
    ...
    setWidth: function (width, showhorizontalScroll) {
        ...
        var contentScrollWidth = this.content().prop("scrollWidth");
        ...
        if (width && contentScrollWidth <= width) {
            this.layer().css({
                width: width,
                whiteSpace: ""
            });
        } else if (width && (showhorizontalScroll || this.options.allowDownsize())) {
            this.layer().css({
                width: width,
                overflowX: "auto",
                whiteSpace: ""
