 ...
 *
 * @constructor JIRA.CreateProjectField
 */
JIRA.CreateProjectField = Class.extend({
    ...
    init: function (options) {
        ...
        this.keygen = new JIRA.ProjectKeyGenerator({desiredKeyLength: 4, maxKeyLength: maxKeyLength});
        this.lastKeyValidated = "";
        if (AJS.DarkFeatures.isEnabled("addproject.project.sample")) {
            this.sample = new JIRA.ProjectSample({element: this.$element.find("#sample-project-container"), events: this.eventBus});
        }
        ...
        if ($keyHelpElement.length) {
            new AJS.InlineDialog($keyHelpElement, "project-key-help-popup",
