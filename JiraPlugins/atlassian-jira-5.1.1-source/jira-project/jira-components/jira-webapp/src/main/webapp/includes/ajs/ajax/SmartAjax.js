...
(function ($) {
    ...
    function buildRawHttpErrorMessage(smartAjaxResult, options) {
        ...
        if (smartAjaxResult.statusText == JIRA.SmartAjax.SmartAjaxResult.TIMEOUT) {
            ...
            errMsg = (options.alert ? AJS.I18n.getText('common.forms.ajax.unauthorised.alert') : AJS.params.ajaxUnauthorised);
        }
    ...
    function buildDialogAjaxErrorMessage(errorMessage, noHeader) {
        var errorContent = '<div class="aui-message error"><span class="aui-icon icon-warning"></span>' +
                '<p>' + errorMessage + '</p>' +
