...
JIRA.createProjectAvatarPickerDialog = function (options) {
    var projectAvatarDialog = new JIRA.FormDialog({
        ...
        content: function (ready) {
            ...
            AJS.$(avatarPicker.imageEditor).bind(JIRA.AvatarPicker.ImageEditor.DISMISSED, function () {
                // When in dialog, adds scrollbars if the image is too big
                projectAvatarDialog._positionInCenter();
                projectAvatarDialog.getButtonsContainer().show();
            });

            AJS.$(avatarPicker.imageEditor).bind(JIRA.AvatarPicker.ImageEditor.LOADED, function () {
                // When in dialog, adds scrollbars if the image is too big
                projectAvatarDialog._positionInCenter();
                projectAvatarDialog.getButtonsContainer().hide();
            });
...
JIRA.createUserAvatarPickerDialog = function (options) {
    ...
    var userAvatarDialog = new JIRA.FormDialog({
        ...
        content: function (ready) {
            ...
            AJS.$(avatarPicker.imageEditor).bind(JIRA.AvatarPicker.ImageEditor.DISMISSED, function () {
                // When in dialog, adds scrollbars if the image is too big
                userAvatarDialog._positionInCenter();
                userAvatarDialog.getButtonsContainer().show();
            });

            AJS.$(avatarPicker.imageEditor).bind(JIRA.AvatarPicker.ImageEditor.LOADED, function () {
                // When in dialog, adds scrollbars if the image is too big
                userAvatarDialog._positionInCenter();
                userAvatarDialog.getButtonsContainer().hide();
            });
