...
test("Firefox Agent Strings", function () {
    ...
    ok(!/-gt-/gi.test(jQuery("html").attr("class")), "Expected no version greater than classes expected (IE only)");
});
...
test("No browser string supplied", function () {
    AJS.describeBrowser();
    ok(/opera|webkit|mozilla|msie/gi.test(jQuery("html").attr("class")), "Expected to fall back to running browser string if not supplied");
});