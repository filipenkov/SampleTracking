...
JIRA.EditPreferencesDialog = JIRA.UserProfileDialog.extend({
    ...
    _updateAutowatch : function() {
        var autowatchValue = AJS.$("#update-user-preferences-autowatch option:selected").val();
        if (autowatchValue !== 'false') {
            AJS.$("#up-p-autowatch-enabled").show();
            AJS.$("#up-p-autowatch-disabled").hide();
        } else {
            AJS.$("#up-p-autowatch-enabled").hide();
            AJS.$("#up-p-autowatch-disabled").show();
        }
    }
});
