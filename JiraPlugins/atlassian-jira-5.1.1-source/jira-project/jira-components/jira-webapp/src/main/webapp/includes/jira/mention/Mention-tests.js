(function() {
    ...
    test("non-valid syntaxes do not trigger autocomplete", function() {
        equal(match("["), false, "matching [");
        equal(match("[a"), false, "matching [a");
        equal(match('~@'), false);
    });
    ...
    test("the @ syntax does not match if preceded by alpha-numeric characters", function() {
        equal(match("test@a"), false, "matching test@a");

        equal(match("the quick brown fox jumped over the lazy@dog"), false, "there's an alphanumeric character before the @, so shouldn't match");
    ...
    test("the query is limited to three words maximum", function() {
        ...
        equal(match(content, content.length), false, "caret is at end of 5th word after the @, so should return false");
        equal(match(content, content.length-4), false, "caret is at end of 4th word after the @, so should return false");
        equal(match(content, content.length-7), false, "caret is inside 4th word after the @, so should return false");
    ...
    test("carriage return and newline break the query", function() {
        ...
        equal(match(content, content.length), false, "when the user's caret is on the next line, it returns false"); //
        equal(match(content, content.length-3), false, "when the user's caret is just after the new line, it returns false"); //
