...
test("schemes are correct for projects", function () {
    equals(this.projectIssueTypeSelect.getIssueTypeSchemeForProject("10240"), "10452",
            "Project with [id=10240] should be mapped to scheme with [id=10452]");

    equals(this.projectIssueTypeSelect.getIssueTypeSchemeForProject("10000"), "10000",
            "Project with [id=10000] should be mapped to scheme with [id=10000]");
});

test("issueType defaults are correct for schemes", function () {
    equals(this.projectIssueTypeSelect.getDefaultIssueTypeForScheme("10452"), "4",
            "Issue type scheme with id [10452] should have a default issue type of [id=4]");

    equals(this.projectIssueTypeSelect.getDefaultIssueTypeForScheme("10000"), "",
...
test("Selecting issue type scheme WITHOUT a default issue type selects first", function () {
    ...
    equals(this.projectIssueTypeSelect.$refIssueTypeSelect.val(), "1");
});

test("Selecting issue type scheme WITH a default issue type, selects it", function () {
    this.projectIssueTypeSelect.setIssueTypeScheme("10452");
    equals(this.projectIssueTypeSelect.$issueTypeSelect.val(), "4");
});
...
test("If an issue type is already selected it takes priority over default", function () {
    ...
    equals(this.projectIssueTypeSelect.$issueTypeSelect.val(), "9",
            "issuetype with [id=9] should be selected, not the first issuetype");

    this.projectIssueTypeSelect.setIssueTypeScheme("10452");

    equals(this.projectIssueTypeSelect.$issueTypeSelect.val(), "3",
...
test("Selecting project displays correct issue types", function () {
    ...
    equals(this.issueType.attr("data-project"), "10000");
    equals(options.eq(0).val(), "1");
    equals(options.eq(1).val(), "2");
    equals(options.eq(2).val(), "3");
    equals(options.eq(3).val(), "4");
    equals(options.eq(4).val(), "6");
    equals(options.eq(5).val(), "9");
    equals(options.eq(6).val(), "10");
    equals(this.issueType.val(), "1");
    equals(options.length, 7);

    this.project.val("10240").trigger("change");
    var options = this.issueType.find("option");
    equals(this.issueType.data().project, "10240");
    equals(options.eq(0).val(), "4");
    equals(options.eq(1).val(), "3");
    equals(this.issueType.val(), "4");
    equals(options.length, 2);
...
test("If user has selected an issuetype and changes project, the same issue type remains selected", function () {
    ...
    this.project.val("10240").trigger("change");
    equals(this.issueType.val(), "4", "project [id=10240] doesn't have an issue type [id=10] so the default should be selected");
    this.project.val("10000").trigger("change");
    equals(this.issueType.val(), "4", "project [id=10000] has an issue type [id=4] so should be selected");
});
