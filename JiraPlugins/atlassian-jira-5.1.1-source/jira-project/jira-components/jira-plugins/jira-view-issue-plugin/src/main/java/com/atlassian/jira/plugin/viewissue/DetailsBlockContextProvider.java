...
public class DetailsBlockContextProvider implements ContextProvider
{
    ...
    public Map<String, Object> getContextMap(Map<String, Object> context)
    {
        ...
        if (workflow != null)
        {
            ...
            final StepDescriptor step = workflow.getLinkedStep(issue.getStatusObject());
            if (step != null)
            {
                paramsBuilder.add("currentWorkflowStep", Integer.toString(step.getId()));
            }
        }
        ...
        {
            paramsBuilder.add("renderedEnvironmentHtml", getRenderedEnvironmentFieldValue(user, issue, action));
            paramsBuilder.add("environment", issue.getEnvironment());
        }
        final FieldScreenRenderer fieldScreenRenderer = fieldScreenRendererFactory.getFieldScreenRenderer(user, issue, IssueOperations.VIEW_ISSUE_OPERATION, FieldPredicates.isStandardViewIssueCustomField());
     ...
     */
    private String getRenderedEnvironmentFieldValue(final User user, final Issue issue, final Action action)
    {
