...
public class JiraPluginSchedulerService extends AbstractService
{
    ...
    private static final Map<String, ObjectConfigurationProperty> params = new HashMap<String, ObjectConfigurationProperty>();
    ...
    public void run()
    {
        ...
        // Find the descriptor
        final JiraPluginScheduler scheduler = getPluginScheduler();
        if (scheduler == null)
        {
            log.error(String.format("Unable to get JiraPluginScheduler. Cannot run job '%s'.", jobName));
            return;
        }
        ...
        // If this is the initial fire, we have to reconfigure the service's interval.
        if (!Boolean.valueOf(props.getString(JiraPluginScheduler.INITIALLY_FIRED)))
        {
            scheduler.reconfigureAfterFirstFire(jobName);
        }
    ...
    {
        return (JiraPluginScheduler) ComponentAccessor.getOSGiComponentInstanceOfType(PluginScheduler.class);
    }
