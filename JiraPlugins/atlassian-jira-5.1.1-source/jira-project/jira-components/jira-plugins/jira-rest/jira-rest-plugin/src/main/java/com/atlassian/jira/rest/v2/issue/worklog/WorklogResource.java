...
public class WorklogResource
{
    ...
    public WorklogResource(final WorklogService worklogService, final JiraAuthenticationContext authenticationContext, final UserManager userManager, ContextI18n i18n, TimeTrackingConfiguration timeTrackingConfiguration, JiraBaseUrls jiraBaseUrls)
    {
    ...
    public Response getIssueWorklogs(Issue issue)
    {
        ...
        return Response.ok(convertToBean(worklogs)).cacheControl(never()).build();
    }
    ...
    public Response getWorklog(String worklogId)
    {
        try
        {
            ...
            if (worklog != null)
            {
                return Response.ok(WorklogJsonBean.getWorklog(worklog, jiraBaseUrls, userManager, timeTrackingConfiguration)).cacheControl(never()).build();
            }
            else
            {
    ...
    {
        return WorklogJsonBean.getWorklog(worklog, jiraBaseUrls, userManager, timeTrackingConfiguration);
    }
