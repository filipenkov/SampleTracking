...
public class IssueBeanBuilder
{
    ...
    {
        final IssueBean bean = new IssueBean(issue.getId(), issue.getKey(), uriBuilder.build(contextUriInfo, IssueResource.class, String.valueOf(issue.getId())));
    ...
    private void addTransitions(Issue issue, IssueBean bean)
    {
        ...
        {
            List<ActionDescriptor> sortedAvailableActions = issueWorkflowManager.getSortedAvailableActions(issue);
    ...
    private void addFields(final Issue issue, final IssueBean bean)
    {
        ...
        final List<FieldLayoutItem> fieldLayoutItems = layout.getVisibleLayoutItems(authContext.getLoggedInUser(), issue.getProjectObject(), CollectionBuilder.list(issue.getIssueTypeObject().getId()));
