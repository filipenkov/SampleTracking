...
public class PermissionsResource
{
    ...
    public PermissionsResource(PermissionManager permissionManager, JiraAuthenticationContext jiraAuthenticationContext, ContextUriInfo contextUriInfo, GlobalPermissionManager globalPermissionManager, ProjectManager projectManager, IssueManager issueManager)
    {
                                    ...
                                    @QueryParam ("issueId") final String issueId)
    {
        ...
        for (Permissions.Permission permission : Permissions.Permission.values())
        {
            ...
            if (Permissions.Type.GLOBAL == permission.getType())
            {
                ...
                {
                    hasPermission = globalPermissionManager.hasPermission(permission.getId(), loggedInUser);
                }
                else
                {
                    hasPermission = globalPermissionManager.hasPermission(permission.getId());
                }
            }
            else
            {
    ...
    private Issue getIssueByIdOrKey(String issueKey, String issueId, ErrorCollection errorCollection)
    {
        if (StringUtils.isNotBlank(issueId))
        {
            try
            {
                ...
                if (issueObj == null)
                {
                    errorCollection.addErrorMessage("Could not find issue with id " + issueObj);
                    errorCollection.reason(com.atlassian.jira.util.ErrorCollection.Reason.NOT_FOUND);
                }
                return issueObj;
            }
