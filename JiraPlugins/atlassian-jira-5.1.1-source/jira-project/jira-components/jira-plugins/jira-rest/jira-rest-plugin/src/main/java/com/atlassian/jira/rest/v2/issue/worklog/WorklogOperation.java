...
public enum WorklogOperation
{
    ADD
    {
        ...
        public WorklogInputParameters prepareData(JiraServiceContextImpl serviceContext, Issue issue, WorklogJsonBean request, WorklogResource.WorklogAdjustmentRequest adjustment)
        {
            ...
            if (!errors.hasAnyErrors())
            {
                ...
                addCommentVisibilityParams(request, builder);
        ...
        public WorklogInputParameters prepareData(JiraServiceContextImpl serviceContext, Issue issue, WorklogJsonBean request, WorklogResource.WorklogAdjustmentRequest adjustment)
        {
            ...
            if (!errors.hasAnyErrors())
            {
                ...
                if (!errors.hasAnyErrors())
                {
                    ...
                    {
                        builder.roleLevelId(null);
                        builder.groupLevel(null);
                    }
                    else
                    {
                        builder.groupLevel(existingWorklog.getGroupLevel());
                        builder.roleLevelId(existingWorklog.getRoleLevelId() != null ? existingWorklog.getRoleLevelId().toString() : null);
                    }
                    return builder.buildAll();
                }
            }
    ...
    private static void addCommentVisibilityParams(WorklogJsonBean request, WorklogInputParametersImpl.Builder builder)
    {
        if (request.getVisibility() != null)
        {
            ...
            {
                role = getProjectRoleManager().getProjectRole(visibility.value).getId();
                group = null;
            }
            else
            {
            ...
            builder.roleLevelId(role != null ? role.toString() : null);
            builder.groupLevel(group != null ? group : null);
        }
    }
