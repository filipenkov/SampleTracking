...
public abstract class SearchQueryBackedResource extends AbstractResource
{
    ...
    private void validateParams(Collection<ValidationError> errors, Map<String, Object> params)
    {
        ...
        else if (queryString.startsWith(PROJECT_PREFIX))
        {
            ...
            {
                if (!permissionManager.hasPermission(Permissions.BROWSE, (Project) params.get(PROJECT), authenticationContext.getLoggedInUser()))
                {
                    errors.add(new ValidationError(QUERY_STRING, "gadget.common.invalid.project"));
                }
            }
    ...
    protected String getFilterUrl(final Map<String, Object> params)
    {
        if (params.containsKey(PROJECT))
        {
            ...
            return "/secure/IssueNavigator.jspa?reset=true&mode=hide" + searchService.getQueryString(authenticationContext.getLoggedInUser(), query);
        }
        else if (params.containsKey(SEARCH_REQUEST))
        {
            ...
            {
                return "/secure/IssueNavigator.jspa?reset=true&mode=hide" + searchService.getQueryString(authenticationContext.getLoggedInUser(), (request == null) ? new QueryImpl() : request.getQuery());
            }
        }
        else
        {
