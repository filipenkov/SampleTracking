...
jira.plugin.charting = function () {
    ...
    getPostData = function(formData) {
        ...
        if(AJS.params.filterId) {
            postData.filterId = AJS.params.filterId;
        } else {
            postData.jql = AJS.params.jql || "";
        }
    ...
    loadSaveToDashboardForm = function() {
        if(!saveFormLoaded) {
            ...
            if(AJS.params.filterId) {
                saveFormUrl = saveFormUrl + "?filterId=" + encodeURIComponent(AJS.params.filterId);
            } else {
                saveFormUrl = saveFormUrl + "?jql=" + encodeURIComponent(AJS.params.jql);
            }
    ...
    addThrobber = function () {
        jQuery().ajaxSend(function(a,xhr) {
            if (dialog.popup.element.is(":visible")) {
                jQuery(xhr).throbber({target: jQuery(".dialogThrobber")});
            }
        });
    },
    ...
    createDialog = function (width, height, data) {
        ...
        dialog.page[0].addHeader(AJS.I18n.getText('navigator.results.currentview.gadgets') + "<div class=\"dialogThrobber\">&nbsp;</div><button class='close'>" + AJS.I18n.getText('admin.common.words.close') + "</button>");
        resetPanels(data);
        dialog.page[0].addButton(AJS.I18n.getText('portletSearchRequestView.save.to.dashboard'),
                function(dialog) {
        ...
        dialog.page[1].addHeader(AJS.I18n.getText('portletSearchRequestView.save.to.dashboard.title')+"<div class=\"dialogThrobber\">&nbsp;</div><button class='close'>" + AJS.I18n.getText('admin.common.words.close') + "</button>");
        jQuery("#charts-dialog button.close").click(function(e) {
            dialog.hide();
            e.preventDefault();
        });
        addThrobber();
                ...
                panel.body.show = function (panel, show) {
                    return function() {
                        if(!panel.loaded) {
                            ...
                            if(AJS.params.filterId) {
                                rendererUrl = rendererUrl + "&filterId=" + encodeURIComponent(AJS.params.filterId);
                            } else {
                                rendererUrl = rendererUrl + "&jql=" + encodeURIComponent(AJS.params.jql);
                            }
    ...
    return function () {
        ...
        if (jQuery("#charts").length > 0) {
            jQuery("#charts").click(function(e) {
        ...
        if (jQuery("#onDashboard").length > 0) {
            jQuery("#onDashboard").click(function(e) {
