...
AJS.gadget.plugin.installer = function(key, setupPath) {
    ...
    function installPlugin(response) {
        ...
        //
        window.top.JIRA.SmartAjax.makeRequest({
            type: "head",
    ...
    function updatePluginInstallStatus(response) {
        if (response.status) {
            ...
            if (response.status.amountDownloaded && response.status.amountDownloaded > 0) {
                ...
                if (percent < 100) {
                    statusText = AJS.format(AJS.I18n.getText("gadget.admin.plugin.downloading"), response.status.filename);
                } else {
                    statusText = AJS.format(AJS.I18n.getText("gadget.admin.plugin.installing"), response.status.filename);
                }
            }
