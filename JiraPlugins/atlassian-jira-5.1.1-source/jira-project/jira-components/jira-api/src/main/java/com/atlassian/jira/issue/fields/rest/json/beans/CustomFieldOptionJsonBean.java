...
public class CustomFieldOptionJsonBean
{
    ...
    {
        return transform(allowedValues, new Function<Option, CustomFieldOptionJsonBean>()
        {
            @Override
            public CustomFieldOptionJsonBean apply(Option from)
            {
                return CustomFieldOptionJsonBean.shortBean(from, baseUrls);
            }
        });
