...
public class TestJqlAutoComplete extends AbstractTestJqlAutoComplete
{
    ...
    private static final String[] ORDERBY_SUGGESTIONS =  new String[] {"ORDER BY"};
    ...
    public void testCompletionWorks() throws Exception
    {
        ...
        _testOrderByAfterWasAndChanged();
        _testPredicatesSuggestEmpty();
...
//        client.getEval("this.browserbot.getCurrentWindow().onbeforeunload = null");
    ...
    private void _testSuggestionsKeyNavigation()
    {
        ...
        // Testing waiting for a bit as we have a locally un-reproducable bug.
        waitFor(2500);
    ...
    private void _testOrderByAfterWasAndChanged()
    {
        client.typeWithFullKeyEvents(JQLTEXT_INPUT, "status was open after");
        // currentLogin()
        typeInStringAndCompleteFunction(" c", 0);
        // order by
        typeInStringAndComplete(" ord", 0);
        typeInStringAndComplete(" a",0);
         assertEquals("status was open after currentLogin() ORDER BY affectedVersion", client.getValue(JQLTEXT_INPUT));
        // Lets verify that the query does  parse
        SUCCESS.check();

    }

    private void _testPredicatesSuggestEmpty()
    {
        client.typeWithFullKeyEvents(JQLTEXT_INPUT, "status was open by");
        //empty
        typeInStringAndComplete(" e", 0);
        assertEquals("status was open by EMPTY", client.getValue(JQLTEXT_INPUT));
        SUCCESS.check();
    }
