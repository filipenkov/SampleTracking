...
public class AdministrationImpl extends AbstractSeleniumUtil implements Administration
{
    ...
    public void addPermissionForUser(String username, String permissionScheme, String permission)
    {
        ...
        selenium.click("id=add_submit", true);
    }
    ...
    public String getCurrentAttachmentPath()
    {
        ...
        // Check that  'Attachment Path' is in the second row where we expect it:
        if (selenium.getTable("id=table-AttachmentSettings.1.0").startsWith("Attachment Path"))
        {
    ...
    private void setAttachmentsEnabled(boolean isEnable, String size)
    {
        navigator.gotoAdmin();
        selenium.click("attachments", true);
        selenium.click("id=edit-attachments", false);
        assertThat.visibleByTimeout("jquery=#edit-attachments-dialog", 3000);

        if (isEnable)
        {
            selenium.click("id=edit-attachemnts-attachmentPathOption_DEFAULT");
            if(size != null)
            {
        ...
        {
            selenium.click("id=edit-attachemnts-attachmentPathOption_DISABLED");
        }
