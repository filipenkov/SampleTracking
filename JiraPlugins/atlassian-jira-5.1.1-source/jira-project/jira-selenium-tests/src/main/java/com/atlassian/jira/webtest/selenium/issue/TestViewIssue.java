...
public class TestViewIssue extends JiraSeleniumTest
{
    public void testCommentBoxExpand()
    {
        ...
        client.type("jquery=#comment", "This is some test, just enough to make sure that the the height changes when I reach 4 rows. This is some test, just enough to make sure that the the height changes when I reach 4 rows. This is some test, just enough to make sure that the the height changes when I reach 4 rows. This is some test, just enough to make sure that the the height changes when I reach 4 rows.This is some test, just enough to make sure that the the height changes when I reach 4 rows.");
        client.typeWithFullKeyEvents("jquery=#comment", "a", false);
    ...
    public void testFieldsShortened()
    {
        ...
        backdoor.plugins().disablePlugin("com.atlassian.jira.jira-issue-nav-plugin");
        client.getEval("window.resizeTo(1024, 800); window.moveTo(0,0);"); // sort out selenium running on CI problems
    ...
    public void testCollapsingSections()
    {
        ...
        client.click("id=manage-attachment-link");
