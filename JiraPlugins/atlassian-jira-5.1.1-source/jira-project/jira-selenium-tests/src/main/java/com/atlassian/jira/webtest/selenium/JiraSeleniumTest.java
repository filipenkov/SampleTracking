...
public abstract class JiraSeleniumTest extends SeleniumTest implements EnvironmentAware
{
    ...
    private void captureScreenshot()
    {
        ...
        {
            logger.warn("Something really bad happened when capturing a screenshot", e);
        }
    }
    ...
    public void runBare() throws Throwable
    {
        ...
        {
            environmentData = SeleniumAcceptanceTestHarness.getLocalTestEnvironmentData();
        }
    ...
    protected void runBareTestCase() throws Throwable
    {
        ...
        finally
        {
            ...
            client.deleteCookie("jira.toggleblocks.cong.cookie", getEnvironmentData().getContext());

            // clear local storage
            client.getEval("this.browserbot.getUserWindow().localStorage.clear();");
//            client.deleteCookie("AJS.conglomerate.cookie", getEnvironmentData().getContext());
//            client.deleteCookie("JSESSIONID", getEnvironmentData().getContext());
    ...
    {
        restoreViaBackdoor(file);
        backdoor.websudo().disable();
        client.deleteAllVisibleCookies();
    ...
    private void doRestore(String filename, boolean reloadPluginsSystem)
    {
        ...
        waitForOtherRequestsToDieAndBeLoggedIn();

        // By now we are guaranteed to be logged in.
        getNavigator().login(ADMIN_USERNAME, ADMIN_PASSWORD);
    ...
    /**
     * Restores an XML import file via the Backdoor after confirming that the Selenium session is
     * sane.
     *
     * @param filename the XML file to restore
     */
    ...
    /**
     * Restores an XML import file via the Backdoor after confirming that the Selenium session is
     * sane. Then goes to the dashboard because many tests assume that it's where they start.
     * <p>
     *     Don't use this method! Just go to the first URL needed by your test!
     * </p>
     *
     * @deprecated Use the {@link #backdoor#} restore method instead, and login as whichever
     *             user is needed for the test. Since v5.0.
     * @param filename the XML file to restore
     */
    protected void restoreViaBackdoorAndGoToDashboard(String filename)
    {
        restoreViaBackdoor(filename);

        // A lot of tests assume that they start logged in as admin at the dashboard, but they don't
        // need to be. That's why this method is deprecated at its point of creation. dT
        getNavigator().gotoPage("/", true);
    }
     ...
     *
     */
    private void waitForOtherRequestsToDieAndBeLoggedIn()
    {
        int retryCount = 0;
    ...
    private boolean isLoggedInAndOnlyOneRequest()
    {
        try
        {
            // We really really want to login
            getNavigator().gotoPage("login.jsp", true);
            client.type("os_username", ADMIN_USERNAME);
            client.type("os_password", ADMIN_PASSWORD);
            getNavigator()
                    .clickAndWaitForPageLoad("login-form-submit")
                    .gotoPage("rest/func-test/1.0/concurrentRequestInfo", true);
    ...
    {
        return client.getEval("with(selenium.browserbot.getCurrentWindow()){JIRA.Issues && JIRA.Issues.InlineEdit}") != null;
    }

    protected void waitForContentUpdate()
    {
        // kickass is currently ajaxifying dialog consequences but not in issue navigator.
        if (isKickAssEnabled()) {
            client.waitForAjaxWithJquery(PAGE_LOAD_WAIT_TIME);
        } else {
            client.waitForPageToLoad(PAGE_LOAD_WAIT_TIME);
        }
    }
    ...
    /**
     * @deprecated Use {@link #backdoor#} instead. Since v5.0.
     * @return a backdoor
     */
