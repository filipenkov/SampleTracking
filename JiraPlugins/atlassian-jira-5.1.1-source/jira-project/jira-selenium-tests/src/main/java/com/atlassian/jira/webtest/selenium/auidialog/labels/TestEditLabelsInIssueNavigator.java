...
public class TestEditLabelsInIssueNavigator extends AbstractAuiLabelsDialogTest
{
    ...
    public void testAddMultipleLabelsShowsOneMessage()
    {
        ...
        int numNotifications = Integer.parseInt(client.getEval("dom=this.browserbot.getCurrentWindow().jQuery(\"#affectedIssueMsg .aui-message.success\").length"));
        ...
        assertThat.visibleByTimeout("jquery=#affectedIssueMsg", DROP_DOWN_WAIT);
        numNotifications = Integer.parseInt(client.getEval("dom=this.browserbot.getCurrentWindow().jQuery(\"#affectedIssueMsg .aui-message.success\").length"));
        assertEquals(1, numNotifications);
