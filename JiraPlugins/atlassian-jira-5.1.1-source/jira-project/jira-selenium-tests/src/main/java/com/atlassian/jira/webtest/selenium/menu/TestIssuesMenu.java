...
public class TestIssuesMenu extends JiraSeleniumTest
{
    ...
    private static final String[] LOGGED_IN_ISSUES_LIST = { CREATE_FILTER_LINK, MANAGE_FILTER_LINK };
    ...
    private void _testSubtaskAppearsInRecentIssues()
    {
        ...
        quickCreateSubtask.setFieldValue("summary", "a1");
        quickCreateSubtask.submit(SubmitType.BY_CLICK);
        client.waitForPageToLoad();

        checkRecentIssues("TPA-1",
                "TPA-7",
                "TPA-6",
                "TPA-5",
                "TPA-3");
    ...
    private void _testNoFiltersCurrentFilter()
    {
        ...
        assertThat.elementVisibleContainsText(UNSAVED_FILTER_LINK, "Unsaved filter (6 issues)");
    ...
    private void checkFilters(String... filterName)
    {
        ...
        for (int i = 0; i < FILTERS_OVERLOAD_NO && i < filterName.length; i++)
        {
            ...
            client.click(replaced, true);
            assertThat.elementVisibleContainsText("jquery=section#content > header > h1", filterName[i]);
        }
