...
public class TestHideShowToggleOnViewIssueScreen extends JiraSeleniumTest
{
    ...
    {
        assertThat.visibleByTimeout(notCollapsedSelector(moduleName));
    ...
    private void assertModuleHidden(final String moduleName)
    {
        ...
        assertThat.visibleByTimeout(collapsedSelector(moduleName));
    }

    private void assertFieldShown(final String fieldName)
    {
        assertThat.visibleByTimeout(verboseSelector(fieldName));
        assertThat.elementNotVisible(conciseSelector(fieldName));
    }

    private void assertFieldHidden(final String fieldName)
    {
        assertThat.visibleByTimeout(conciseSelector(fieldName));
        assertThat.elementNotVisible(verboseSelector(fieldName));
    }

    private void showModule(final String fieldName)
    {
        final String moduleSelector = collapsedSelector(fieldName) + " .toggle-title";
        assertThat.elementPresentByTimeout(moduleSelector);
        client.click(moduleSelector);
    }

    private void hideModule(final String moduleName)
    {
        final String moduleSelector = notCollapsedSelector(moduleName) + " .toggle-title";
        assertThat.elementPresentByTimeout(moduleSelector);
        client.click(moduleSelector);
    }
    ...
    {
        final String fieldSelector = conciseSelector(fieldName) + " .twixi, #" + fieldName + " .inverted-twixi";
        assertThat.elementPresentByTimeout(fieldSelector);
        client.click(fieldSelector);
    }

    private void hideField(final String fieldName)
    {
        final String fieldSelector = verboseSelector(fieldName) + " .twixi, #" + fieldName + " .inverted-twixi";
        assertThat.elementPresentByTimeout(fieldSelector);
        client.click(fieldSelector);
    }
