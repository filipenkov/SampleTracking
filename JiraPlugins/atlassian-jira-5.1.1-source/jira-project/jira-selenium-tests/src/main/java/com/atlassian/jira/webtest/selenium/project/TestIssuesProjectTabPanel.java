...
public class TestIssuesProjectTabPanel extends JiraSeleniumTest
{
    ...
    private static final String FRAG_UNRESOLVED_BY_ISSUE_TYPE_DIV = "//div[@id='fragunresolvedissuesbyissuetype']";
    private static final String ISSUETYPES_TABLE = "//table[@id='issueTypes']";
    private static final String ISSUE_TYPE_TOGGLE_DIV = "//div[@id='fragunresolvedissuesbyissuetype_toggle']";

    private static final String ADMIN = "admin";
    public static final String UNRESOLVED_ISSUES_BY_ISSUE_TYPE_XML = "TestIssuesProjectTabPanel_UnresolvedIssuesByIssueType.xml";
    public static final String BROWSE_ISSUES_URL = "browse/HSP?selectedTab=com.atlassian.jira.plugin.system.project%3Aissues-panel";
    ...
    public void testUnresolvedIssuesByAssignee()
    {
        ...
        getNavigator().gotoPage(BROWSE_ISSUES_URL, true);
    ...
    public void testUnresolvedByIssueType()
    {
        restoreData(UNRESOLVED_ISSUES_BY_ISSUE_TYPE_XML);

        safeLogin(ADMIN, ADMIN);
        getNavigator().gotoPage(BROWSE_ISSUES_URL, true);

        assertTrue("Unresolved Issues by Issue Type div should be visible", client.isVisible(FRAG_UNRESOLVED_BY_ISSUE_TYPE_DIV));

        // initially, toggle should be in "Show all/and X more" state
        assertEquals("and 1 more", client.getText(FRAG_UNRESOLVED_BY_ISSUE_TYPE_DIV + ISSUE_TYPE_TOGGLE_DIV));

        // also, only 10 rows should be visible
        assertNumberOfRowsVisible(FRAG_UNRESOLVED_BY_ISSUE_TYPE_DIV + ISSUETYPES_TABLE, 10);

        // toggle all rows to be displayed
        client.click(ISSUE_TYPE_TOGGLE_DIV);
        assertEquals("show first 10", client.getText(FRAG_UNRESOLVED_BY_ISSUE_TYPE_DIV + ISSUE_TYPE_TOGGLE_DIV));
        assertNumberOfRowsVisible(FRAG_UNRESOLVED_BY_ISSUE_TYPE_DIV + ISSUETYPES_TABLE, 11);
