...
public class TestCustomFields extends JIRAWebTest
{
    ...
    public void testSearchNumberCustomField()
    {
        ...
        assertions.getTextAssertions().assertTextPresentHtmlEncoded("'SOMETEXT' is an invalid number");
    }
    ...
    private void viewIssueInNavigator()
    {
        ...
        setFormElement("pid", project_Homosap_Id);
        submit("show");
        viewAndValidateIssue(true);
...
//            assertFormElementEquals(CUSTOM_FIELD_PREFIX + cascadingSelectId + ":1", "10010");
    ...
    public void testCustomFieldDescriptionsCanBeRenderedAsRawHtmlOrWikiMarkup() throws Exception
    {
        ...
        final String onDemandFeature = "com.atlassian.jira.config.CoreFeatures.ON_DEMAND";
        gotoAdmin();

        // description and its equivalent wiki-rendered content
        String RAW_DESC = "description *wiki* markup";
        String HTML_DESC = "<div class=\"secondary-text description\">description *wiki* markup</div>";
        String WIKI_DESC = "<div class=\"secondary-text description\"><p>description <b>wiki</b> markup</p></div>";

        // test that the description is rendered as raw HTML
        String fieldId = addCustomField("multicheckboxes", "global", CUSTOM_FIELD_NAME_MULTI_CHECKBOX, RAW_DESC, null, null, null);
        clickOnAdminPanel("admin.issuefields", "view_custom_fields");
        assertEquals("ON_DEMAND is disabled so the description should be rendered as raw HTML for " + fieldId, HTML_DESC, locatorForDescription(fieldId).getHTML());

        // now test that the description is rendered as Wiki markup
        getBackdoor().darkFeatures().enableForSite(onDemandFeature);
        clickOnAdminPanel("admin.issuefields", "view_custom_fields");
        assertEquals("ON_DEMAND is enabled so the description should be rendered as wiki markup for " + fieldId, WIKI_DESC, locatorForDescription(fieldId).getHTML());
    }
    ...
    public void testEditIssueTypeDoesNotCauseCustomFieldToDisappear()
    {
        ...
        String issueTypeId = addIssueType(issueTypeName, "a todo item", "/images/icons/genericissue.gif");
        ...
        editIssueType(issueTypeId, issueTypeName, "a todo item", "/jiratrashdb.gif");
...
//
    ...
    {
        restoreBlankInstance();
        String fieldId = addCustomField(CUSTOM_FIELD_TYPE_TEXTFIELD, "freestylin");
        getNavigation().issueNavigator().displayAllIssues();
        ...
        // try to assert that the error is on the correct field - VERIFIES THAT JRA-13430 IS FIXED
    ...
    public void testFreeTextSearcherForCustomFieldInvalidFirstChar()
    {
        ...
        assertions.getTextAssertions().assertTextPresentHtmlEncoded("Invalid start character '*'");
        // try to assert that the error is on the correct field - VERIFIES THAT JRA-13430 IS FIXED
        assertTextPresentBeforeText("<label for=\"searcher-customfield_10000\">", "freestylin");
        assertTextPresentBeforeText("freestylin", "<div class=\"error\">");
    }
    ...
    {
        return locator.css(String.format("#custom-fields-customfield_%s-name div.description", fieldId));
    }
}
