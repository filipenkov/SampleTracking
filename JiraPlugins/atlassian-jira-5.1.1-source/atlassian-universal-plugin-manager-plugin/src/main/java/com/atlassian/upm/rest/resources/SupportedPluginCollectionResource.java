...
/**
 * Provides a collection of supported plugins that are available to install and are not currently installed.
 */
@Path("/available/supported")
public class SupportedPluginCollectionResource
{
    private final RepresentationFactory factory;
    private final PacClient client;
    private final PermissionEnforcer permissionEnforcer;

    public SupportedPluginCollectionResource(RepresentationFactory factory, PacClient client,
    ...
    @GET
    @Produces(SUPPORTED_PLUGINS_JSON)
        ...
        @QueryParam("start-index") Integer startIndex)
    {
        ...
        try
        {
            pluginVersions = client.getSupported(maxResults, startIndex);
        }
        ...
        return Response.ok(factory.createSupportedPluginCollectionRepresentation(pluginVersions, pacUnreachable)).build();
    }
}
