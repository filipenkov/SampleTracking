...
public class TestDefaultJiraDataImporter {
	...
	public void testImportUnusedUsers()
		...
		ImportDataBean mockBean = mock(ImportDataBean.class, RETURNS_MOCKS);
	...
	public void testMissingLinks() {
		...
		DefaultJiraDataImporter importer = new DefaultJiraDataImporter(utils, worklogManager, fieldManager,
				...
				searchProviderFactory, userUtil, jiraLicenseService, componentManager, customFieldManager, fieldScreenManager, fieldConfigSchemeManager, fieldConfigManager, historyImporter);
    ...
    public void testImportSkipUsersWhenExternalUserManagementIsOn()
        ...
        ImportDataBean dataBean = mock(ImportDataBean.class);
        ...
        when(dataBean.getAllUsers(Matchers.<ImportLogger>anyObject())).thenReturn(allUsers);
        when(dataBean.getSelectedProjects(Matchers.<ImportLogger>anyObject())).thenReturn(projects);
        when(dataBean.getRequiredUsers(eq(projects), Matchers.<ImportLogger>anyObject())).thenReturn(requiredUsers);
