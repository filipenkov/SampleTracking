...
public class TestExternalUserUtils {
    ...
    @Mock
    private CreateUserHandlerProvider createUserHandlerProvider;

	@Before
	public void setup() {
		MockitoAnnotations.initMocks(this);

		utils = new ExternalUserUtils(userMock, groupMock, globalMock, projectRoleService, createUserHandlerProvider);
	}
	...
	public void testCreateRoleIfItDoesntExist() throws Exception {
        ...
        final ProjectRoleActors pRA = mock(ProjectRoleActors.class, RETURNS_MOCKS);
        ...
        when(projectRoleService.getProjectRoleActors(eq(user), any(ProjectRoleImpl.class), eq(project), any(SimpleErrorCollection.class))).thenReturn(pRA);
        when(pRA.getUsers()).thenReturn(Sets.<User>newHashSet());

		utils.addUsersToProjectRole(user, project, "Developers", Lists.<String>newArrayList("test"));
