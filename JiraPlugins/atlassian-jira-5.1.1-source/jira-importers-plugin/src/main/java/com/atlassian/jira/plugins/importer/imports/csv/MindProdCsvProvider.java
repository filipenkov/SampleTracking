...
public class MindProdCsvProvider implements CsvProvider {
	...
	public void startSession() throws ImportException {
		try {
			this.csvReader = new CSVReader(
					new InputStreamReader(new JiraFileInputStream(file), encoding),
					getDelimiter(), '\"', "#", true, true, true, true, false);

			csvMapper.init(csvReader.getAllFieldsInLine());
		}
		catch (IOException e) {
	...
	public ListMultimap<String, String> getNextLine() throws ImportException {
		try {
			...
			try {
				String[] dataRow = csvReader.getAllFieldsInLine();
				if (dataRow != null) {
	...
	public List<String> getNextLineRaw() throws ImportException {
		try {
			...
			try {
				String[] dataRow = csvReader.getAllFieldsInLine();
				if (dataRow != null) {
					nextLine = Arrays.asList(dataRow);
				}
			}
			catch (EOFException e) {
				// Just return null if eof;
