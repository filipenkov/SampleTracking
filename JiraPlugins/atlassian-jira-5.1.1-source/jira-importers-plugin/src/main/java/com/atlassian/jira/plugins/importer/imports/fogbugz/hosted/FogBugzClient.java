...
public class FogBugzClient {
	...
	private URI getApiUri() throws FogBugzRemoteException {
		if (rootUri.getHost().endsWith("fogbugz.com")) {
			return UriBuilder.fromUri(rootUri).path("/api.asp").build();
		}

		final URI fbApiUri = UriBuilder.fromUri(rootUri).path("/fogbugz/api.asp").build();
		final HttpHead head = new HttpHead(fbApiUri);
		head.getParams().setBooleanParameter(ClientPNames.HANDLE_REDIRECTS, false);
		try {
			final HttpResponse response = getHttpClient().execute(head);
			try {
	...
	public void login() throws FogBugzRemoteException {
		...
		final HttpPost post = new HttpPost(UriBuilder.fromUri(apiUri).queryParam("cmd", "logon").build());
		try {
			...
			if (response.getStatusLine().getStatusCode() == HttpStatus.SC_MOVED_TEMPORARILY) {
				...
				if (location != null && location.getValue() != null) {
					...
					if (targetHost.equals(apiUri.getHost()) && !targetUri.getScheme().equals(apiUri.getScheme())) {
						throw new FogBugzRemoteException("The server requested redirection. Please consider using \""
								+ targetUri.getScheme() + "\" instead of \"" + apiUri.getScheme() + "\".");
					}
				}
			}
			if (response.getStatusLine().getStatusCode() != HttpStatus.SC_OK) {
	...
	public void logout() {
		try {
			getWithToken(UriBuilder.fromUri(apiUri).queryParam("cmd", "logoff"));
		} catch (FogBugzRemoteException e) {
			// don't care
		}
	...
	private Element getWithToken(UriBuilder uri) throws FogBugzRemoteException {
		...
		try {
            ...
            final SAXBuilder builder = XmlUtil.getSAXBuilder();
            /*
            https://support.atlassian.com/browse/JST-35844
            builder.setFactory(new DefaultJDOMFactory() {
                @Override
                public CDATA cdata(String text) {
                    return super.cdata(text != null ? text.replace("]]>", "]]&gt;") : null);
                }
            });*/
			final Document document = builder.build(is);
	...
	public Collection<ExternalProject> getAllProjects(ImportLogger log) throws FogBugzRemoteException {
		ensureLoggedInToWeb();
		final Element projects = getWithToken(UriBuilder.fromUri(apiUri).queryParam("cmd", "listProjects"));
	...
	public Collection<ExternalVersion> getFixFors(ExternalProject project, ImportLogger log) throws FogBugzRemoteException {
		ensureLoggedInToWeb();
		final UriBuilder uri = UriBuilder.fromUri(apiUri).queryParam("cmd", "listFixFors")
	...
	public Collection<ExternalComponent> getAreas(ExternalProject project, ImportLogger log) throws FogBugzRemoteException {
		ensureLoggedInToWeb();
		final Element xmlResult = getWithToken(UriBuilder.fromUri(apiUri).queryParam("cmd", "listAreas")
				.queryParam("ixProject", project.getId()));
							...
							public ExternalIssue apply(@Nullable ExternalIssue input) {
								if (input != null) {
									...
									customFields.add(new ExternalCustomFieldValue(DefaultJiraDataImporter.EXTERNAL_ISSUE_URL,
											...
											UriBuilder.fromUri(apiUri).replacePath("default.asp")
	...
	public Collection<ExternalUser> getAllUsers(ImportLogger importLogger) throws
		...
		ensureLoggedInToWeb();
		final Element memberships = getWithToken(UriBuilder.fromUri(apiUri).queryParam("cmd", "listPeople"));
	...
	private void download(String url, File destination) throws IOException {
		...
		ensureLoggedInToWeb();
		final InputStream inputStream = getWithToken0(UriBuilder.fromUri(apiUri)
	...
	public Set<String> getStatuses() throws FogBugzRemoteException {
		final UriBuilder uri = UriBuilder.fromUri(apiUri).queryParam("cmd", "listStatuses");
