...
public class PivotalSchemeManager {
	...
	private static final Map<String, String> pivotalStatuses = ImmutableMap.<String, String>builder()
			.put(ICE_BOX, "/images/icons/status_generic.gif")
	...
	public PivotalSchemeManager(IssueTypeSchemeManager issueTypeSchemeManager,
		...
		this(issueTypeSchemeManager, subTaskManager, constantsManager, configSchemeManager,	fieldManager, workflowManager,
				...
				ComponentManager.getComponentInstanceOfType(JiraContextTreeManager.class), statusManager);
	}
	...
	public void setPTSchemesForProject(Project project) throws Exception {
		...
		final List<String> issueTypeIds = createIssueTypes();
		assignPTIssueTypeScheme(project, issueTypeIds);
	...
	protected List<String> createIssueTypes() {
		...
		for (String issueTypeName : defaultTypesAndIcons.keySet()) {
			...
			if (issueConstant != null) {
				...
				try {
					if (issueTypeName.equals(SUBTASK)) {
						issueType = subTaskManager.createSubTaskIssueType(issueTypeName,
								Long.valueOf(subTaskManager.getSubTaskIssueTypeObjects().size()), "", icon);
					} else {
						issueType = constantsManager.createIssueType(issueTypeName, null, null, "", icon);
					}
				} catch (CreateException e) {
					throw new RuntimeException("Cannot create issue type: " + issueTypeName, e);
				}
	...
	void assignPTIssueTypeScheme(Project project, List<String> issueTypeIds) {
		final FieldConfigScheme ptScheme = getPTScheme(issueTypeIds);
	...
	FieldConfigScheme getPTScheme(List<String> issueTypeIds) {
		...
		final FieldConfigScheme scheme = issueTypeSchemeManager
				.create(issueTypeSchemeName, issueTypeSchemeDesc, issueTypeIds);
	...
	Scheme getPTWorkflowScheme() {
		...
		try {
			final Scheme scheme = workflowSchemeManager.createSchemeObject(workflowSchemeName, workflowSchemeDescription);
			final GenericValue schemeGV = workflowSchemeManager.getScheme(scheme.getId());
	...
	Map<String, String> getPTStatusNameToIdMapping() {
		...
		for (String statusName : pivotalStatuses.keySet()) {
			...
			if (statusByName == null) {
		 		final ViewStatuses vs = new ViewStatuses(null, statusManager, workflowManager) {
					@Override
