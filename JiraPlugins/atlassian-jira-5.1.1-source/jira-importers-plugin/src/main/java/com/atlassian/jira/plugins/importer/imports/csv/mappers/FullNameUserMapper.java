...
public class FullNameUserMapper implements ExternalUserMapper {
	...
	public ExternalUser buildFromMultiMap(Multimap<String, String> bean) {
		...
		if (!bean.get(fullNameField).isEmpty()) {
			final String fullName = (String) ((Collection) bean.get(fullNameField)).iterator().next();
			// Remove illegal chars for the username
			final String username = extractUserName(fullName);
	...
	public String extractUserName(String fullName) {
		return StringUtils.lowerCase(StringUtils.replaceChars(fullName, " '()", ""));
	}
