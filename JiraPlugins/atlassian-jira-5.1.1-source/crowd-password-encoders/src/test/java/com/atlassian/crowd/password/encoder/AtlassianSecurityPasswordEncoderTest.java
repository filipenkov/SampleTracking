...
public class AtlassianSecurityPasswordEncoderTest extends TestCase
{
    ...
    public void testNonDeterministicEncoding()
    {
        PasswordEncoder enc = new AtlassianSecurityPasswordEncoder();

        Set<String> hashed = new HashSet<String>();
        for (int i = 0; i < 10; i++)
        {
            hashed.add(enc.encodePassword("password", null));
        }

        assertThat("Passwords should hash with different salts", hashed.size(), OrderingComparison.greaterThan(1));
    }
}
