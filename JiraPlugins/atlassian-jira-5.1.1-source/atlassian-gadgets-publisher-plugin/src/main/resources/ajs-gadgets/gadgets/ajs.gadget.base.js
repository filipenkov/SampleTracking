...
var Gadget = function (options) {
    ...
    gadget = {
        ...
        ajaxErrorHandler: {
             ...
             * @param data - data sent back from server
             * @param settings - ajax options used to make request
             */
            handle500: function (evt, data, settings) {
                var msg = AJS.format(gadget.getMsg("gadget.common.error.500", settings.url, settings.url));
                AG.sysMsg.addError(msg);
            },
            ...
            handle503: function(evt, data, settings) {
                if (data && data.errorMessages) {
                    AJS.$.each(data.errorMessages, function () {
                        AG.sysMsg.addError({
                            message: this
                        });
                    });
            ...
            handle404: function (evt, data, settings) {
                var msg = AJS.format(gadget.getMsg("gadget.common.error.404"), settings.url, settings.url);
                AG.sysMsg.addError(msg);
            },
    ...
    (function(){
        ...
        function overflowsIframeHeight () {
            return parseInt(AJS.$("body").prop("scrollHeight")) > AJS.$("body").outerHeight();
        }
        ...
        var iframeAsContainerHandler = {
            show: function () {
                if (overflowsIframeHeight() && gadgets.window.adjustHeight) {
                    if (AJS.$("body").prop("scrollHeight") !== AJS.$("body").height())
                        gadgets.window.adjustHeight(AJS.$("body").prop("scrollHeight"));
                }
            },
            hide: function () {
