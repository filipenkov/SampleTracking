...
public class UserComparatorTest
{
    ...
    {
        Function<User, UserComparator.Key> km = UserComparator.KEY_MAKER;
        User u = mock(User.class);
        when(u.getName()).thenReturn("name");
        when(u.getDirectoryId()).thenReturn(1L);
        UserComparator.Key k = km.apply(u);

        assertEquals(0, k.compareTo(k));
        assertNotSame(k, km.apply(u));
        assertEquals(0, km.apply(u).compareTo(k));
        assertEquals(0, k.compareTo(km.apply(u)));
    }

    @Test
    public void inequalWhenDifferent()
    {
        Function<User, UserComparator.Key> km = UserComparator.KEY_MAKER;
        User u = mock(User.class);
        when(u.getName()).thenReturn("name");
        ...
        when(u.getDirectoryId()).thenReturn(2L);
        UserComparator.Key k2 = km.apply(u);

        assertEquals(-1, k.compareTo(k2));
        assertEquals(1, k2.compareTo(k));

        when(u.getName()).thenReturn("name2");
        when(u.getDirectoryId()).thenReturn(1L);
        UserComparator.Key k3 = km.apply(u);
        assertEquals(-1, k.compareTo(k3));
        assertEquals(1, k3.compareTo(k));
    }

    @Test(expected = ClassCastException.class)
    public void failsWhenComparedToSomethingElse()
    {
        Function<User, UserComparator.Key> km = UserComparator.KEY_MAKER;
        User u = mock(User.class);
        when(u.getName()).thenReturn("name");
    ...
    /**
     * When sorting keys, username is considered first with directory ID as a tiebreaker.
     */
    @Test
    public void sortIsByUsernameAndThenDirectoryId()
    {
        Function<User, UserComparator.Key> km = UserComparator.KEY_MAKER;

        User u = mock(User.class);
        when(u.getName()).thenReturn("a");
        when(u.getDirectoryId()).thenReturn(2L);
        UserComparator.Key k_a_2 = km.apply(u);

        when(u.getName()).thenReturn("b");
        when(u.getDirectoryId()).thenReturn(1L);
        UserComparator.Key k_b_1 = km.apply(u);

        assertEquals(-1, k_a_2.compareTo(k_b_1));
        assertEquals(1, k_b_1.compareTo(k_a_2));
    }
}
