...
public class TranslatingApplicationServiceTest
{
    ...
    @Test
    public void testSearchUsers_SortedCaseInsensitively() throws Exception
    {
        EntityQuery<User> query = QueryBuilder.queryFor(User.class, EntityDescriptor.user()).returningAtMost(EntityQuery.ALL_RESULTS);

        when(application.isAliasingEnabled()).thenReturn(false);
        // Alias manager returns the same username when aliasing is disabled
        when(aliasManager.findAliasByUsername(application, USER4B_USERNAME)).thenReturn(USER4B_USERNAME);
        when(aliasManager.findAliasByUsername(application, USER1_USERNAME)).thenReturn(USER1_USERNAME);
        when(applicationService.searchUsers(application, query)).thenReturn(ImmutableList.of(USER4B, USER1));

        final List<User> result = translatingApplicationService.searchUsers(application, query);

        assertEquals(ImmutableList.of(USER1, USER4B), result);
    }
