...
function detectPrototype(){
    var prototypeText = "This component is in Prototype status - it is still being evaluated within Atlassian. Plugin developers are advised to avoid using this component until its stability has been confirmed.",
    prototypeStatus = "Prototype!"
    
    if(AJS.$("meta[name='status']").attr("content")=="prototype"){
        if(AJS.$("meta[name='product']").size()>0){
            prototypeText = "This component is in Prototype status - it is still being evaluated within Atlassian "+AJS.$("meta[name='product']").attr("content")+", all other Atlassian products should avoid using it until it has been approved for generalised prototyping. Plugin developers are advised to avoid using this component until its stability has been confirmed."
