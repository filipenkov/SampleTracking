// todo move this back into JIRA master once we no longer have to support 5.0
...
(function () {
    ...
    function getNewScrollTop($scrollTarget, options) {
        ...
        var offsetTop    = Math.max(0, getPageY(this[0], $scrollTarget) - options.marginTop);
    ...
    function getPageY(element, $scrollTarget) {

        var currElement = element,
                offsetTop = 0;


        if ($scrollTarget[0]  === window.top) {
            do {
                offsetTop += currElement.offsetTop;
            } while (currElement = currElement.offsetParent);
        } else {
            offsetTop = currElement.offsetTop + $scrollTarget.scrollTop();
        }
    ...
    window.jQuery.fn.scrollIntoView = function(options) {
        ...
        if (options.scrollTarget || !fixedParent) {

            var $scrollTarget = options.scrollTarget ? options.scrollTarget.call(this, options) : jQuery(window.top);
            ...
            var scrollTop    = $scrollTarget.scrollTop();
            var newScrollTop = getNewScrollTop.call(this, $scrollTarget, options);
            var $stalker     = jQuery("#stalker");
            ...
            if (newScrollTop !== scrollTop) {
                ...
                $scrollTarget.trigger("moveToStarted", $target);

                var $moveTarget = $scrollTarget[0] === window.top ? jQuery("body, html") : $scrollTarget;

                if (options.duration) {



                    $moveTarget.stop(true).animate(
                            {
                    ...
                    $moveTarget.attr("scrollTop", newScrollTop);
                }
