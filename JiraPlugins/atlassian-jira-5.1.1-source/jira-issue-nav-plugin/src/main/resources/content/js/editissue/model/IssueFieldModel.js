...
JIRA.Issues.IssueFieldModel = JIRA.Issues.BaseModel.extend({
     ...
     */
        "editingCancelled",
    /**
     * Triggered when a save is required. This lets the JIRA.Issues.EditIssueController know to trigger a save
     */
        "save",
    /**
     ...
     */
        "required",
    /**
     * The key the field type for custom fields (for system fields this value is undefined).
    ...
    initialize: function () {
        ...
        this.bindModelDestroyed(function () {
     ...
     *
     * @see JIRA.Issues.IssueFieldCollection._handleEditingStarted to see how this works
     */
    blurEdit: function () {
        if (this.getEditing()) {
     ...
     */
    setValidationError: function (errorHtml, error) {
        ...
        this.triggerValidationError(errorHtml);
    },
    ...
    cancelEdit: function (reason) {
        if (!this.cancelLocked) {
            ...
            JIRA.trigger(event, [this.getId(), this.getFieldType(), reason]);
    ...
    getCurrentParams: function() {
        ...
        if (this.getParams()) {
            ...
            this.setInitParams($el.serializeObject());
            params = this.getInitParams();
        }
        var fp = {};
    ...
    switchElToEdit: function($el) {
        ...
        $el.html(JIRA.Templates.ViewIssue.Fields.field({
            issue: this.toJSON(),
    ...
    update: function($el) {
        if(this.getEditing()) {
            this.setParams($el.serializeObject());
        }
    },
    ...
    /**
     * Initiates a save. This lets the JIRA.Issues.EditIssueController know to trigger a save.
     */
    save: function () {
        if (!this.getSaving()) {
    ...
    matchesFieldSelector: function() {
        return JIRA.Issues.IssueFieldUtil.matchesFieldSelector(this.id);
    }
}, {
    /**
