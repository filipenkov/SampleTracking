...
public class JiraEntryBuilderFactory
{
     ...
     * @return
     */
    public StreamsEntry.Parameters<HasId, HasPostedDate, HasAlternateLinkUri, HasApplicationType, NeedsRenderer, NeedsVerb, NeedsAuthors> newParams(JiraActivityItem item,
                                                                                                                                                                         URI itemUri)
    {
        return newParams(item, itemUri, false);
    }

    public StreamsEntry.Parameters<HasId, HasPostedDate, HasAlternateLinkUri, HasApplicationType, NeedsRenderer, NeedsVerb, NeedsAuthors> newLinkedIssueParams(JiraActivityItem item,
                                                                                                                                                                                    URI itemUri)
    {
        return newParams(item, itemUri, true);
    }

    private StreamsEntry.Parameters<HasId, HasPostedDate, HasAlternateLinkUri, HasApplicationType, NeedsRenderer, NeedsVerb, NeedsAuthors> newParams(JiraActivityItem item,
                                                                                                                                                                         URI itemUri,
                                                                                                                                                                         boolean issueLinked)
    {
    ...
    private Iterable<Link> buildLinks(JiraActivityItem item, boolean issueLinked)
    {
        ...
        links.add(getIconLink(applicationProperties.getBaseUrl(), issue));
        links.add(new Link(uriProvider.getWikiRendererCssUri(), CSS_LINK_REL, none(String.class)));
        ...
        if (!issueLinked)
        {
            ...
            {
                links.add(new Link(getReplyToUri(issue.getKey()), REPLY_TO_LINK_REL, none(String.class)));
            }
            ...
            if (isUserLoggedIn())
            {
                if (canWatch(issue))
                {
                    links.add(new Link(getIssueWatchUri(issue.getKey()), WATCH_LINK_REL, none(String.class)));
                }

                if (canVote(item))
                {
                    links.add(new Link(getIssueVoteUri(issue.getKey()), ISSUE_VOTE_REL, none(String.class)));
                }
            }
        }
        return links.build();
    ...
    private boolean canWatch(Issue issue)
    {
        try
        {
            return watcherManager.isWatchingEnabled() && !watcherManager.isWatching(authenticationContext.getLoggedInUser(), issue);
        }
    ...
    private boolean canVote(JiraActivityItem item)
    {
        ...
        try
        {
            return voteManager.isVotingEnabled()
                    && !voteManager.hasVoted(authenticationContext.getLoggedInUser(), issue)
