...
class GeneralUpdateEntryBuilder
{
    ...
    private Option<StreamsEntry> buildSingleActivityItem(JiraActivityItem item)
    {
        ...
        {
            return buildRemoteLink(item, changeItems);
        }
        else
        {
    ...
    private Option<StreamsEntry> buildRemoteLink(JiraActivityItem item, Iterable<GenericValue> changeItems)
    {
        ...
        if (remoteLink.isDefined())
        {
            return some(new StreamsEntry(helper.newBuilder(item).
                ...
                addActivityObject(helper.buildActivityObject(item.getIssue(), item.getDisplaySummary())).
                verb(update()).
                renderer(issueUpdateRendererFactory.newRemoteIssueLinkEntryRenderer(item, remoteLink.get())), i18nResolver));
        }
        else
        {
