...
public class TestFileOptionsValidator extends TestCase
{
	...
	public void testEmptyRequestError() {
		...
		validator.validate(this.context, new SafeHttpServletRequestImpl(this.emptyReq), log);
	...
	public void testEmptyRequestWarning() {
		...
		validator2.validate(this.context, new SafeHttpServletRequestImpl(this.emptyReq), log2);
	...
	public void testValidationNoErrorsOrWarnings()
	{
		...
		DefaultApplicationFileBundle validFileBundle = new DefaultApplicationFileBundle(BundleManifest.APPLICATION_LOGS, "test", "test");

		this.info.addApplicationFileBundle(validFileBundle);
		HttpServletRequest validFileReq = mock(HttpServletRequest.class);
		when(validFileReq.getParameter(BundleManifest.APPLICATION_LOGS.getKey())).thenReturn("true");

		ValidationLog log3 = new ValidationLog(this.info);
		FileOptionsValidator validator3 = new FileOptionsValidator(this.info);
		validator3.validate(this.context, new SafeHttpServletRequestImpl(validFileReq), log3);
		assertFalse("Validation Log contains warnings.", log3.hasWarnings());
		assertFalse("Validation Log contains errors:" + log3.getErrors().toString(), log3.hasErrors());
	}
}
