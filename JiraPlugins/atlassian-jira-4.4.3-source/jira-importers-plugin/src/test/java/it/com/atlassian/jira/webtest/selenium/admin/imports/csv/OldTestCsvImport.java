 ...
 * @todo these tests still run against the old (built-in) CSV importers. We need to change them so that they run against the JIM one
 */
@SuppressWarnings("deprecation")
public class OldTestCsvImport extends JIRAWebTest {
	...
	public OldTestCsvImport(final String name)
    {
        super(name);
    }
    ...
    public void testTrivialWithEmails()
    {
        ...
        clickLinkWithText(reporter);
        assertTextSequence(new String[] { "Username", reporter, "Full Name", reporter, "Email", reporter });
        assertTextNotPresent(reporter + "@atlassian.com");
    }
        ...
        Collection<String> options = Collections2.transform(fieldMappingsPage.getSelectOptions(
                ...
                new Function<WebElement, String>() {
                    ...
                    public String apply(@Nullable WebElement webElement) {
                        return webElement != null ? webElement.getValue() : "";
                    }
                });
    ...
    public void testImportTimeTracking()
    {
        ...
        fieldMappingsPage.setFieldMapping("Description", "description");
        fieldMappingsPage.setFieldMapping("Reporter", "reporter");
        fieldMappingsPage.setFieldMapping("OriginalEstimate", "timeoriginalestimate");
        fieldMappingsPage.setFieldMapping("RemainingEstimate", "timeestimate");
        fieldMappingsPage.setFieldMapping("TimeSpent", "timespent");
    ...
    public void testImportWizardValidatesSummaryFieldIsMapped() throws SAXException
    {
        ...
        setupPage.setCsvFile(ITUtils.getCurrentWorkingDirectory() + "/src/test/resources/csv/redundant-field-mappings.csv");
        setupPage.setConfigurationFile(ITUtils.getCurrentWorkingDirectory() + "/src/test/resources/csv/redundant-field-mappings.properties");
		...
		assertNotNull(Iterables.find(issue.getFields(), new Predicate<Field>() {
			@Override
			public boolean apply(Field input) {
				return "userNewCF".equals(input.getName()) && input.getValue() != null &&
						"com.atlassian.jira.plugin.system.customfieldtypes:userpicker".equals(input.getType());
			}
		}, null));
		...
		assertNotNull(Iterables.find(issue.getFields(), new Predicate<Field>() {
			@Override
			public boolean apply(Field input) {
				return "Version Custom Field".equals(input.getName()) && input.getValue() != null &&
						"com.atlassian.jira.plugin.system.customfieldtypes:multiversion".equals(input.getType());
			}
		}, null));
    ...
    public void testPropertiesFileValidation()
    {
        ...
        setupPage = product.visit(LoginPage.class)
				.loginAsSysAdmin(CsvSetupPage.class)
				.webSudo();
    ...
    public void testConfigWizard() {
		...
		fieldMappingsPage.setFieldMapping("opendate", IssueFieldConstants.CREATED);
        fieldMappingsPage.setFieldMapping("bug_severity", IssueFieldConstants.PRIORITY);
        ...
        fieldMappingsPage.setFieldMapping("rep_platform", IssueFieldConstants.ENVIRONMENT);
        fieldMappingsPage.setFieldMapping("assigned_to_realname", IssueFieldConstants.ASSIGNEE);
        fieldMappingsPage.setFieldMapping("reporter_realname", IssueFieldConstants.REPORTER);
		fieldMappingsPage.setFieldMapping("bug_status", IssueFieldConstants.STATUS);
        fieldMappingsPage.setMapValues("bug_status", true);
        fieldMappingsPage.setMapValues("bug_severity", true);
        fieldMappingsPage.setFieldMapping("resolution", IssueFieldConstants.RESOLUTION);
        fieldMappingsPage.setMapValues("resolution", true);
        fieldMappingsPage.newCustomField("product").setFieldName("product").setFieldType(CustomFieldConstants.TEXT_FIELD_TYPE);
        fieldMappingsPage.setFieldMapping("component", IssueFieldConstants.COMPONENTS);
        fieldMappingsPage.setFieldMapping("version", IssueFieldConstants.AFFECTED_VERSIONS);
        fieldMappingsPage.newCustomField("target_milestone").setFieldName("target_milestone").setFieldType(CustomFieldConstants.SELECT_FIELD_TYPE);
        fieldMappingsPage.newCustomField("status_whiteboard").setFieldName("status_whiteboard").setFieldType(CustomFieldConstants.TEXT_FIELD_TYPE);
        fieldMappingsPage.setFieldMapping("keywords", IssueFieldConstants.LABELS);
        fieldMappingsPage.setFieldMapping("short_desc", IssueFieldConstants.SUMMARY);
    ...
    public void testImportWithExternalUserManagement()
    {
		...
		assertContains(expectedError + " chai", finishPage.getLog());
    }
    ...
    public void testImportWizardWithTabDelimiter()
    {
		...
		fieldMappingsPage.setFieldMapping("Issue type", IssueFieldConstants.ISSUE_TYPE);
		fieldMappingsPage.setMapValues("Issue type", true);

		fieldMappingsPage.setFieldMapping("Assigned to", IssueFieldConstants.ASSIGNEE);
		fieldMappingsPage.setFieldMapping("Priority", IssueFieldConstants.PRIORITY);
		fieldMappingsPage.setMapValues("Priority", true);
		fieldMappingsPage.setFieldMapping("Resolution", IssueFieldConstants.RESOLUTION);
		fieldMappingsPage.setMapValues("Resolution", true);
				fieldMappingsPage.setFieldMapping("Status", IssueFieldConstants.STATUS);
		fieldMappingsPage.setMapValues("Status", true);
				fieldMappingsPage.setFieldMapping("Summary", IssueFieldConstants.SUMMARY);
		CsvValueMappingsPage valueMappingsPage = fieldMappingsPage.next();
		...
		assertEquals("1", valueMappingsPage.getValue("value.4")); // mapping for Status.New
