...
public class TestIssueTransformer {
	...
	public void testSqlQueryContainsProjectId() {
		...
		IssueTransformer transformer = new IssueTransformer("http://localhost", configBean, project, dateTimeMock, importLogger);
	...
	public void testNullComponent() throws SQLException {
		...
		Assert.assertNotNull(issue.getExternalComponents());
		Assert.assertEquals(0, issue.getExternalComponents().size());
	...
	public void testComponentIsSet() throws SQLException {
		...
		ExternalIssue issue = transformer.transform(rsMock);
		Assert.assertNotNull(issue);
		Assert.assertEquals(1, issue.getExternalComponents().size());
		Assert.assertEquals(component, issue.getExternalComponents().get(0));
    ...
    public void testEmptyDateCustomField() throws SQLException {
        ...
        IssueTransformer transformer = new IssueTransformer("http://localhost", configBean,
				externalProject, dateTimePickerConverter, log);
