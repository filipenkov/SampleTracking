...
public class StoryParserTest {
	private final StoryParser storyParser = new StoryParser();
	...
	public void testParseStory() throws Exception {
		...
		assertEquals("Pawel Niewiadomski", externalIssue.getAssignee());
	...
	public void testParseStoryWithNotes() throws Exception {
		...
		verifyComment(externalComments.get(0), "wseliga", "2011/02/14 14:32:48 UTC", "a comment added while uploading a file");
		verifyComment(externalComments.get(1), "wseliga", "2011/02/14 14:33:46 UTC", "another comment here");
	}
	...
	public void testParseAttachments() throws Exception {
		...
		assertEquals("1209615", attachment.getId());
		assertEquals("1.png", attachment.getFileName());
		...
		assertEquals(expectedDate, attachment.getAttachedDate());
	...
	public void testParseLabels() throws Exception {
		...
		final Set<Label> labels = storyParser.parseLabels((Element) o);
		assertEquals(4, labels.size());
		assertEquals(ImmutableSet.of("this_is_a_long_text", "coma", "xcs", "impossible"), ImmutableSet.copyOf(Collections2.transform(labels,
		...
		final Set<Label> emptyLabels = storyParser.parseLabels(null);
