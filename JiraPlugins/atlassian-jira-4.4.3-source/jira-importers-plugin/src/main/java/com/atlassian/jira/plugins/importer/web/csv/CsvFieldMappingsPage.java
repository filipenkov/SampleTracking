...
public class CsvFieldMappingsPage extends ImporterProcessSupport.Csv {
	...
	protected void validateProjectRequiredFields() {
		// check if all required fields have a mapping defined
		// then check if values are valid

		final boolean hasProjectKey = getConfigBean().containsFieldWithValue("project.key");
		final boolean hasProjectName = getConfigBean().containsFieldWithValue("project.name");
		...
		if (hasProjectKey && hasProjectName) {
			try {
				ImportDataBean dataBean = getController().createDataBean(dateTimeFormatterFactory);
	...
	public Collection<CustomField> getCustomFields() {
		...
		Collection<CustomField> availableCustomFields = Lists.newArrayList();

		if (!getConfigBean().isReadingProjectsFromCsv()
				&& utils.getProjectManager().getProjectObjByKey(getConfigBean().getProjectKey("CSV")) != null) {
			...
			for (CustomField customFieldObject : allCustomFields) {
				if (customFieldObject.isAllProjects() || customFieldObject.getAssociatedProjects().contains(
	...
	public String getModel() {
		...
		try {
			mapper.writeValue(stringWriter, getModelImpl());
