...
public class IssueTransformerVer118OrOlder extends AbstractIssueTransformer<MantisConfigBean> {
	...
	public ExternalIssue transform(final ResultSet rs) throws SQLException {
		...
		externalIssue.setExternalComponents(Lists.newArrayList(rs.getString("category")));
		...
		externalIssue.setCreated(configBean.getTimestamp(rs, "date_submitted"));

		externalIssue.setLabels(createLabels(configBean.getJdbcConnection().queryDb(
		...
		final Date closed = configBean.getTimestamp(rs, "last_updated");
		final Date resolved = configBean.getTimestamp(rs, "last_updated");
