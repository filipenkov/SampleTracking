...
private class ArtifactsCategory
{
  ...
  static final LANG_PACK_PREFIX = JIRA_PREFIX + "jira-lang"
          ...
          use(ArtifactsCategory) {
            ...
            element(id: 'directory', name: 'lib') {
              ...
              artifacts.thirdPartyJars().each {
                element(id: 'library', level: 'project', name: "Maven: ${it.groupId}:${it.artifactId}:${it.version}")
              }

              artifacts.languagePacks().each {
                def language = it.artifactId
                element(id: 'archive', name: "${language}.jar") {
                  element(id: 'module-output', name: "${language}")
                }
              }
