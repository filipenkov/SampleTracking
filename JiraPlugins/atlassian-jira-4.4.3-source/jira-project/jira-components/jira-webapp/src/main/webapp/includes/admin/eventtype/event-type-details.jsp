...
<table id="eventTypeTable" class="aui aui-table-rowhover">
    ...
    <tr>
        <!-- Name -->
        <td><ww:if test="./systemEventType == true">
        ...
        <td >
            <ww:property value="/eventTypeManager/associatedNotificationSchemes(.)" >
            <ww:if test="./empty == false">
                <ul>
                    <ww:iterator value="./keySet">
                        <li><a href="<ww:url page="/secure/admin/EditNotifications!default.jspa">
                            <ww:param name="'schemeId'"><ww:property value="." /></ww:param>
                        </ww:url>"><ww:property value="../(.)" /></a>
                        </li>
                    </ww:iterator>
                </ul>
            </ww:if>
        ...
        <% int wfCount = 0; %>
        <td >
            <ww:property value="/eventTypeManager/associatedWorkflows(., 'false')" >
                ...
                <ww:iterator value="./keySet">
                    <li>
                         <div id="short_<%=evtCount%>_<%=wfCount%>" onclick="toggle('expand', '<%=evtCount%>_<%=wfCount%>');">
                            ...
                            </ww:url>"><ww:property value="." /></a>&nbsp
                            <!-- Transition Link -->
                                <span class="small">[
                                <ww:iterator value="/shortList(../(.))" status="'status'">
                                    <a title="<ww:text name="'admin.event.type.transitionlink'" />" href="<ww:url page="/secure/admin/workflows/ViewWorkflowTransition.jspa">
                                        ...
                                        <ww:param name="'workflowName'"><ww:property value=".." /></ww:param>
                                        <ww:param name="'workflowMode'" value="'live'"/>
                                    </ww:url>"> <ww:property value="./name"/>
                                    (<ww:property value="./id" />)</a>
                                    <ww:if test="@status/last == false">,</ww:if>
                                </ww:iterator>
                                <ww:if test="../(.)/size > 3"><span style="cursor:pointer;" class="smallgrey" title="<ww:text name="'admin.event.type.list.fulllist'" />">...</span></ww:if>&nbsp;]</span>
                        </div>
                        <ww:if test="../(.)/size > 3">
                        <div style="display:none; cursor:pointer;" id="long_<%=evtCount%>_<%=wfCount%>" onclick="toggle('hide', '<%=evtCount%>_<%=wfCount%>');">
                            ...
                            <span class="smallgrey">(<ww:text name="'admin.event.type.list.hide'" />)</span>&nbsp;]</span>
                        </div>
                        </ww:if>
                    </li>
                    <% wfCount++; %>
