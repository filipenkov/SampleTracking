    ...
    */
	...
	for (var i in tmp) {
		if (typeof params[tmp[i]] == "string") {
            var id = "#" + params[tmp[i]].escapejQuerySelector();
            if (params.context) {
                params[tmp[i]] = AJS.$(params.context).find(id).get(0);
            } else {
                params[tmp[i]] = AJS.$(id).get(0);
            }
		}
	}
    ...
    jQuery(triggerEl).bind(params.eventName, function (e) {
		...
		if (!(cal && params.cache)) {
            ...
            if (params.useISO8601WeekNumbers) {
                // ISO8061 assumes that first day of week is Monday
