...
(function($) {
    ...
    /**
     * Checks whether the given jQuery collection contains unsaved changes.
     *
     * @return {boolean}
     */
    $.fn.isDirty = function() {
    ...
    // is a bad idea (e.g. staying on an issue page after it is deleted). Skip the dirty check!Ê
    $(document).bind('page-unload.location-change.from-dialog', function (e) {
    ...
    $(document).bind('page-unload.refresh.from-dialog', function (e) {
    ...
    JIRA.DirtyForm = {
        ...
        getDirtyWarning: function () {
            ...
            for (var i = 0, ii = $textareas.length; i < ii; i++) {
                if ($textareas[i].form !== activeForm && isElementDirty($textareas[i])) {
