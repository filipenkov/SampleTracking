...
public class TestOfBizConsumerTokenStore extends TestCase
{
    public void testGetConsumerTokens() throws Exception
    {
        ...
        expect(propertySet.getKeys()).andReturn(Collections.emptyList());
        expect(jiraPropertySetFactory.buildCachingPropertySet("OAuthConsumerToken", new Long(789l), true)).andReturn(propertySet);
        expect(jiraPropertySetFactory.buildCachingPropertySet("OAuthConsumerToken", new Long(123l), true)).andReturn(propertySet);
        ...
        final Iterator<ConsumerToken> tokenIterator = tokenMap.values().iterator();
