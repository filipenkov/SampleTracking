...
public class TestStatHeadingMarkupBuilder extends TestCase
{
    ...
    I18nBean.BeanFactory i18nFactory = createMock(I18nBean.BeanFactory.class);
    ...
    protected void setUp() throws Exception
    {
        ...
        transformer = new StatsMarkupFieldValueToDisplayTransformer(authenticationContext, constantsManager, cfm, i18nFactory, null);

        expect(authenticationContext.getLoggedInUser()).andReturn(user);
        replay(authenticationContext);

        expect(i18nFactory.getInstance(user)).andReturn(i18n);
        replay(i18nFactory);
    ...
    public void testHeadingMarkupForStatusesGV()
    {
        final IssueConstant issueConstant = createMock(IssueConstant.class);
        ...
        String s = ObjectToFieldValueMapper.transform("statuses", new MockGenericValue("foo"), FULL_URL, transformer).getHtml();

        assertEquals("<img src=\"http://jira/icon.gif\" height=\"16\" width=\"16\" alt=\"theNameTranslated\" title=\"theNameTranslated - theDescTranslated\"/><a href='" + FULL_URL + "' title='theDescTranslated'>theNameTranslated</a>", s);
    }


    public void testHeadingMarkupForStatuses()
    {
        final IssueConstant issueConstant = createMock(IssueConstant.class);
        ...
        String s = ObjectToFieldValueMapper.transform("statuses", issueConstant, FULL_URL, transformer).getHtml();
