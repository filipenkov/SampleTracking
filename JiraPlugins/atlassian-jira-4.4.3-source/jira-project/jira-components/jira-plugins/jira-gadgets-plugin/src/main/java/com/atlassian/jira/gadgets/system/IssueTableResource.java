...
public class IssueTableResource extends AbstractResource
{
            ...
            @QueryParam (ADD_DEFAULTS) @DefaultValue ("false") boolean addDefaults)
    {
        ...
        List<ColumnLayoutItem> columns = getColumns(context, columnNames, addDefaults);
        return createResponse(request, linkedLabelledQuery, columns, isPaging, start, validatedNumberToShow, enableSorting, displayHeader, showActions);
    }
            ...
            @QueryParam (ADD_DEFAULTS) @DefaultValue ("false") boolean addDefaults)
    {
        ...
        List<ColumnLayoutItem> columns = getColumns(context, columnNames, addDefaults);
        return createResponse(request, query, columns, isPaging, start, validatedNumberToShow, enableSorting, displayHeader, showActions);
    }
            ...
            @QueryParam (SHOW_ACTIONS) @DefaultValue ("false") boolean showActions)
    {
        ...
        List<ColumnLayoutItem> columns = getColumns(context, columnNames, false);
        return createResponse(request, linkedLabelledQuery, columns, isPaging, start, validatedNumberToShow, enableSorting, displayHeader, showActions);
    }
    ...
    private IssueTable createIssueTable(final SearchResults results, final LinkedLabelledQuery linkedLabelledQuery, final IssueTableLayoutBean layout, final boolean isPaging, final int numberToShow)
    {// only get the table html if there are search results.
        ...
        return new IssueTable(html, results.getIssues().size(), results.getTotal(), results.getNiceStart(), results.getEnd(), 0, numberToShow, url, name, description);
    }

    Response createResponse(HttpServletRequest request, LinkedLabelledQuery linkedLabelledQuery, List<ColumnLayoutItem> columns, boolean isPaging, int start, int numberToShow, boolean enableSorting, boolean displayHeader, boolean showActions)
    {
        final Collection<ValidationError> errors = new ArrayList<ValidationError>();
        ...
        try
        {
            ...
            final IssueTable table = createIssueTable(results, linkedLabelledQuery, layout, isPaging, numberToShow);
    ...
    private List<ColumnLayoutItem> getColumns(String context, List<String> columnNames, boolean addDefaults)
    {
        User user = authenticationContext.getLoggedInUser();
        try
        {
    ...
    private SearchRequest getSearchRequestAndValidate(String filterId, Collection<ValidationError> errors)
    {
        final User user = authenticationContext.getLoggedInUser();
        ...
        else
        {
            ...
            if (filterId.startsWith(JQL_PREFIX))
            {
                ...
                else
                {
                    ...
                    {
                        errors.add(new ValidationError(FILTER_ID, "gadget.common.invalid.filter.validationfailed", CollectionBuilder.newBuilder(filterId, errorMessage).asList()));
                    }
                }
            }
            else
            ...
            {
                errors.add(new ValidationError(FILTER_ID, "gadget.common.invalid.filter.id", filterId));
            }
            else
            {
                final MessageSet messageSet = searchService.validateQuery(user, searchRequest.getQuery());
        ...
        if (columnNames != null && !columnNames.isEmpty())
        {
            try
            {
                ...
                for (String columnName : columnNames)
                {
                    ...
                    if (ColumnNamesValuesGenerator.Property.DEFAULT_COLUMNS.equalsIgnoreCase(columnName))
                    {
                        continue;
                    }
    ...
    public static class IssueTable
    {
        ...
        IssueTable(String table, int displayed, int total, int startIndex, int end, int page, int pageSize, String url, String title, String description)
        {
            this.table = table;
