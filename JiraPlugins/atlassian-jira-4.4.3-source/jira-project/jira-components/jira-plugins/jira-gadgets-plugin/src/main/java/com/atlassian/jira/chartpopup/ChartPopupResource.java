...
public class ChartPopupResource
{
    ...
    public ChartPopupResource(final JiraAuthenticationContext authenticationContext, final GadgetViewFactory gadgetViewFactory,
            ...
            final SessionSearchObjectManagerFactory sessionSearchObjectManagerFactory, final VelocityManager velocityManager,
    ...
    public Response getGadgetToDashboardForm(@QueryParam ("filterId") Long filterId, @QueryParam (PREF_JQL) String jql)
    {
        ...
        final MapBuilder<String, Object> contextBuilder = MapBuilder.<String, Object>newBuilder().
                add("portals", portalPageService.getOwnedPortalPages(authenticationContext.getUser())).
    ...
    public Response addGadgetToDashboard(final Gadget gadget)
    {
        ...
        final String remoteUsername = remoteUser == null ? null : remoteUser.getName();
        ...
        if (filterId == null)
        {
            ...
            {
                final SearchService.ParseResult parseResult = searchService.parseQuery(remoteUser, gadget.getJql());
                searchRequest = new SearchRequest(parseResult.getQuery(), remoteUsername, filterName, "");
            }
            else
            {
                searchRequest = new SearchRequest(new QueryImpl(null, new OrderByImpl(), null), remoteUsername, filterName, "");
            }
    ...
    private Response addGadgetToDashboard(final GadgetState newGadget, Long portalId)
    {
        ...
        final String remoteUsername = authenticationContext.getUser() == null ? null : authenticationContext.getUser().getName();
    ...
    private Long createDefaultDashboardClone()
    {
        ...
        PortalPage newPage = PortalPage.name(name).description(desc).owner(userName).permissions(SharedEntity.SharePermissions.PRIVATE).build();
    ...
    private String renderTemplate(final String template, final Map velocityParams)
    {
        try
        {
