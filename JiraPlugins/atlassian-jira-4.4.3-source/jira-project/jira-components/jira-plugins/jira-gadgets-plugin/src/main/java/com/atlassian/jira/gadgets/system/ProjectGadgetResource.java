...
public class ProjectGadgetResource extends AbstractResource
{
    ...
    public ProjectGadgetResource(final JiraAuthenticationContext authenticationContext, final PermissionManager permissionManager,
                                 final ProjectManager projectManager, final PluginAccessor pluginAccessor, final ConstantsManager constantsManager,
    ...
    private void validateCategories(Collection<ValidationError> errors, List<String> projectAndCategoryIds)
    {
        for (String projectId : projectAndCategoryIds)
        {
            ...
            if (projectcat == null || permissionManager.getProjects(Permissions.BROWSE, authenticationContext.getUser(), projectcat).isEmpty())
            {
                             ...
                             @QueryParam("showStats") @DefaultValue("false") final boolean showStats)
    {
        ...
        return Response.ok(new Projects(categories, permissionManager.hasPermission(Permissions.ADMINISTER, user))).cacheControl(CacheControl.NO_CACHE).build();
    ...
    private DetailedProjectData getProjectDataNoPermissionCheck(User user, Project project, boolean showStats)
    {
        ...
        final User lead = project.getLead();
    ...
    {
        DefaultVelocityRequestContextFactory.cacheVelocityRequestContext("", null);
        final ProjectHelper jiraHelper = new ProjectHelper(null, new BrowseProjectContext(user, project));
    ...
    private List<OpenIssuesData> getOpenIssuesData(final Project project)
    {
        ...
        try
        {
            final StatisticMapWrapper<GenericValue, Integer> stats = sab.getAllFilterBy("priorities");
            final List<OpenIssuesData> openIssues = new ArrayList<OpenIssuesData>();
            I18nBean i18nBean = new I18nBean(authenticationContext.getUser());
            for (Object o : stats.keySet())
            {
                ...
                GenericValue priorityGV = (GenericValue) o;
                ...
                {
                    clauseBuilder.and().priority(priorityGV.getString("id"));
                }
                String priorityId = priorityGV == null ? "-1" : priorityGV.getString("id");
                openIssues.add(new OpenIssuesData(
                        priorityGV == null ? "none" : priorityGV.getString("statusColor"),
                        stats.getPercentage(priorityGV),
                        constantsManager.getPriorityName(priorityId) + " - " +
                                stats.getPercentage(priorityGV) + "% (" + stats.get(priorityGV) + " " +
                                i18nBean.getText("gadget.common.issues") + ")",
                        priorityId, searchService.getQueryString(authenticationContext.getUser(), clauseBuilder.buildQuery())
                ));
            }
            return openIssues;
        }
    ...
    {
        return new StatisticAccessorBean(authenticationContext.getUser(), project.getId());
    }
    ...
    {
        return !new FieldVisibilityBean().isFieldHidden(project.getId(), IssueFieldConstants.PRIORITY, FieldVisibilityManager.ALL_ISSUE_TYPES);
    }
    ...
    public static class Projects
    {
        ...
        public Projects(List<Category> categories, boolean isAdmin)
        {
