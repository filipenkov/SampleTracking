...
public class IssueLinkTypeResource
{
    ...
    public Response getIssueLinkTypes()
    {
        ...
        final Iterable<IssueLinkTypeBean> iterable = Iterables.transform(linkTypes, new Function<IssueLinkType, IssueLinkTypeBean>()
        {
            public IssueLinkTypeBean apply(@Nullable IssueLinkType from)
            {
                ...
                return new IssueLinkTypeBean(from.getId(), from.getName(), from.getInward(), from.getOutward(), uri);
            }
        });
     ...
     *
     * @response.representation.200.example
     *      {@link IssueLinkTypeBean#ISSUE_LINK_TYPE_EXAMPLE}
     *
     * @response.representation.404.doc
    ...
    public Response getIssueLinkType(@PathParam ("issueLinkTypeId") final String issueLinkTypeIdString)
    {
        ...
        return Response.ok(IssueLinkTypeBean.create(linkType, uri)).cacheControl(never()).build();
    }
