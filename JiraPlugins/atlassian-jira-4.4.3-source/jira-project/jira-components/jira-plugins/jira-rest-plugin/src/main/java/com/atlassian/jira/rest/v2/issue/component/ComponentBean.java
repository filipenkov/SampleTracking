...
public class ComponentBean
{
    ...
    public static Collection<ComponentBean> asBeans(final Collection<? extends ProjectComponent> components, final UriInfo uriInfo)
    {
        final ArrayList<ComponentBean> list = new ArrayList<ComponentBean>();
        for (ProjectComponent component : components)
        {
            list.add(ComponentBean.shortComponent(component, uriInfo));
        }
        return list;
    }

    public static Collection<ComponentBean> asFullBeans(final Collection<? extends ProjectComponent> components, final UriInfo uriInfo, String projectLeadUserName, long projectAssigneeType, final UserManager userManager, final AvatarService avatarService, final PermissionManager permissionManager, final ProjectManager projectManager)
    {
        final ArrayList<ComponentBean> list = new ArrayList<ComponentBean>();
        for (ProjectComponent component : components)
        {
            list.add(ComponentBean.fullComponent(component, uriInfo, projectLeadUserName, projectAssigneeType, userManager, avatarService, permissionManager, projectManager));
        }
        return list;
    }
    ...
    public static ComponentBean shortComponent(final ProjectComponent component, final UriInfo uriInfo)
    {
        final ComponentBean bean = new ComponentBean();
        bean.self = uriInfo.getBaseUriBuilder().path(ComponentResource.class).path(component.getId().toString()).build();
        bean.id = component.getId();
        bean.description = component.getDescription();
        bean.name = component.getName();
        return bean;
    }

    public static ComponentBean fullComponent(final ProjectComponent component, final UriInfo uriInfo, String projectLeadUserName, long projectAssigneeType, final UserManager userManager, final AvatarService avatarService, final PermissionManager permissionManager, final ProjectManager projectManager)
    {
        final ComponentBean bean = shortComponent(component, uriInfo);
        bean.description = component.getDescription();

        final String lead = component.getLead();
        final User user = userManager.getUserEvenWhenUnknown(lead);


        if (user != null)
        {
            final URI avatarURL = avatarService.getAvatarURL(user, user.getName(), Avatar.Size.SMALL);
            bean.lead = new UserBeanBuilder().user(user).avatarUrl(avatarURL).context(uriInfo).buildShort();
        }

        populateAssignee(bean, component, projectLeadUserName, projectAssigneeType, uriInfo, userManager, permissionManager, projectManager);

        return bean;
    }

    private static void populateAssignee(ComponentBean bean, ProjectComponent component, String projectLeadUserName, long projectAssigneeType, UriInfo uriInfo, UserManager userManager, PermissionManager permissionManager, ProjectManager projectManager)
    {
        ...
        switch ((int) component.getAssigneeType())
        {
            ...
            {
                configuredAssigneeName = projectLeadUserName;
                break;
            }
        ...
        switch ((int) realAssigneeType)
        {
            ...
            case (int) AssigneeTypes.PROJECT_LEAD:
            {
                realAssigneeName = projectLeadUserName;
                break;
            }
            case (int) AssigneeTypes.PROJECT_DEFAULT:
            {
        ...
        {
            final User user = userManager.getUserEvenWhenUnknown(configuredAssigneeName);
            bean.assignee = user != null ? new UserBeanBuilder().user(user).context(uriInfo).buildShort() : null;
        }

        bean.realAssigneeType = AssigneeType.getAssigneeType(realAssigneeType);
        if (realAssigneeName != null)
        {
            final User user = userManager.getUserEvenWhenUnknown(realAssigneeName);
            bean.realAssignee = user != null ? new UserBeanBuilder().user(user).context(uriInfo).buildShort() : null;
