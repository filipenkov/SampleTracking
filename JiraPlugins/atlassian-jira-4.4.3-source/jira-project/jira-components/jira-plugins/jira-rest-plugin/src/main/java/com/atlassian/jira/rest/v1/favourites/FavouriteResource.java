...
public class FavouriteResource
{
    ...
    public Response isFavourite()
    {
        try
        {
            ...
            return Response.ok(new BooleanResult(favService.isFavourite(authContext.getLoggedInUser(), entity))).cacheControl(NO_CACHE).build();
        }
    ...
    private Response setFavourite(final SharedEntity.TypeDescriptor entityType, final Long entityId, final boolean enable, final List<Long> originalOrder)
    {
        ...
        if (enable)
        {
            ...
            {
                final long positionForFavourite = getPositionForFavourite(ctx.getLoggedInUser(), entity, originalOrder);
    ...
    long getPositionForFavourite(final User user, final SharedEntity entityToUndo, final List<Long> orginalOrder)
    {
