...
public class IssueResource
{
    ...
    public IssueResource(
            final JiraAuthenticationContext authContext, final I18nHelper.BeanFactory i18nBeanFactory,
            ...
            final WorkflowManager workflowManager, final SimpleLinkManager simpleLinkManager,
            final XsrfTokenGenerator xsrfTokenGenerator, final VoteService voteService, final WatcherManager watcherManager)
    {
    ...
    {
        return new IssuePickerResource(authContext, i18nBeanFactory, service, issueManager, applicationProperties, projectManager);
    }
    ...
    {
        return new IssueActionsAndOperationsResource(authContext, issueManager, i18nBeanFactory, workflowManager, issueId, simpleLinkManager, xsrfTokenGenerator);
    }
    ...
    public Response addWatcher(@PathParam ("issueId") Long issueId)
    {
        ...
        watcherManager.startWatching(user, result.getIssue().getGenericValue());
        final List<String> watchers = watcherManager.getCurrentWatcherUsernames(result.getIssue());
        return Response.ok(new VoteWatchResult(watchers.size())).cacheControl(NO_CACHE).build();
    }
