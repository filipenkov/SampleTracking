...
public class DateBlockContextProvider implements CacheableContextProvider
{
    ...
    private List<DateBlockField> getDateCustomFields(Issue issue, Action action)
    {
        ...
        FieldScreenRenderer screenRenderer = fieldScreenRendererFactory.getFieldScreenRenderer(authenticationContext.getLoggedInUser(), issue,
                IssueOperations.VIEW_ISSUE_OPERATION, FieldPredicates.isCustomDateField());
        ...
        for (FieldScreenRenderLayoutItem renderItem : dateRenderItems)
        {
            ...
            customfields.add(new DateBlockField(customField.getId() + "-val", customField.getName(), null, displayHtml, null, null));
        }
    ...
    {
        return escapeHtml(dateTimeFormater.withSystemZone().withStyle(DateTimeStyle.DATE).format(date));
    }
    ...
    /**
     * A date that is backed by a Timestamp.
     */
    class DateField extends DateBlockField
    {
        DateField(String id, String key, Timestamp date)
        {
            super(id, getI18nText(key), null, formatDate(date), formatDateIso8601(date), formatDate(date));
        }
    }

    /**
     * A date/time that is backed by a Timestamp.
     */
    class DateTimeField extends DateBlockField
    {
        public DateTimeField(String id, String key, Timestamp dateTime)
        {
            super(id, getI18nText(key), "user-tz", formatDateTime(dateTime), formatDateTimeIso8601(dateTime), formatDateTimeTitle(dateTime));
        }
    }
