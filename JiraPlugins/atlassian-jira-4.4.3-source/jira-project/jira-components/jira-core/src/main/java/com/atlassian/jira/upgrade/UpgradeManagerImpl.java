...
public class UpgradeManagerImpl implements UpgradeManager, Startable
{
    ...
    public UpgradeManagerImpl(final JiraLicenseService jiraLicenseService, final BuildUtilsInfo buildUtilsInfo,
            ...
            final BuildVersionRegistry buildVersionRegistry, final EventPublisher eventPublisher, OfBizDelegator ofBizDelegator)
    {
        this(jiraLicenseService, buildUtilsInfo, i18HelperFactory, applicationProperties, buildVersionRegistry,
                eventPublisher, UPGRADES_XML, ofBizDelegator);
    }

    UpgradeManagerImpl(final JiraLicenseService jiraLicenseService, final BuildUtilsInfo buildUtilsInfo,
            final I18nHelper.BeanFactory i18HelperFactory, final ApplicationProperties applicationProperties,
            final BuildVersionRegistry buildVersionRegistry, final EventPublisher eventPublisher,
            final String upgradeTaskFileName, OfBizDelegator ofBizDelegator)
    {
    ...
    UpgradeManagerImpl(JiraLicenseService jiraLicenseService, BuildUtilsInfo buildUtilsInfo,
            ...
            BuildVersionRegistry buildVersionRegistry, OfBizDelegator ofBizDelegator,
    ...
    public Collection<String> doUpgradeIfNeededAndAllowed(@Nullable final String backupPath) throws IllegalXMLCharactersException
    {
        ...
        if (licenseDetails.isLicenseSet())
        {
            if (!licenseDetails.isMaintenanceValidForBuildDate(buildUtilsInfo.getCurrentBuildDate()) && !licenseDetails.hasLicenseTooOldForBuildConfirmationBeenDone())
            {
                ...
                final String logMsg2 = "The maintenance period of your license expired on " + licenseDetails.getMaintenanceEndString(new OutlookDate(Locale.ENGLISH));
                log.info(logMsg1);
                log.info(logMsg2);
                log.info("Cannot proceed with the upgrade.");
                return CollectionBuilder.<String>newBuilder(logMsg1 + "\n" + logMsg2).asList();
            }
        ...
        return doUpgradeIfNeeded(backupPath);
    }

    private Collection<String> createErrorMessages(final JiraLicenseService.ValidationResult validationResult)
    {
    ...
    private Collection<String> doUpgradeIfNeeded(final String defaultBackupPath) throws IllegalXMLCharactersException
    {
    ...
    {
        return !applicationProperties.getOption(APKeys.JIRA_AUTO_EXPORT) || JiraSystemProperties.isDevMode();
    }
    ...
    private Collection<String> doUpgrade()
    {
        ...
        try
        {
            ...
            errors = runUpgradeTasks(upgradeNumbers, standardUpgrades, false);
     ...
     */
    private Collection<String> runUpgradeTasks(final Collection<String> upgradeNumbers, final Map<String, UpgradeTask> upgradeTasks, boolean setupMode)
            throws Exception
    {
        final Collection<String> errors = new ArrayList<String>();
        ...
        for (final String number : upgradeNumbers)
        {
            ...
            {
                log.info("Setting current build number to " + number);
    ...
    public Collection<String> doSetupUpgrade()
    {
        ...
        try
        {
            ...
            errors = runUpgradeTasks(upgradeNumbers, setupUpgrades, true);
    ...
    private Map<String, GenericValue> getPreviouslyRunUpgrades()
    {
        ...
        if (upgradeHistoryMap == null)
        {
            ...
            upgradeHistoryMap = new HashMap<String, GenericValue>();
    ...
    SortedMap<String, UpgradeTask> getRelevantUpgradesFromList(final Map<String, UpgradeTask> upgradeMap)
    {
        ...
        {
            log.error("Exception getting upgrades " + e.getMessage(), e);
    ...
    private void createUpgradeVersionHistory() throws GenericEntityException
    {
        ...
        try
        {
            ...
            if (record == null)
            {
                EntityUtils.createValue(ENTITY_UPGRADE_VERSION_HISTORY, MapBuilder.build(
                        FIELD_TIMEPERFORMED, timePerformed, FIELD_TARGETBUILD, currentBuildNumber, FIELD_TARGETVERSION, version));
            }
            else
            {
