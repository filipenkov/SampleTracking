...
public class UserFitsNavigatorHelper
{
     ...
     * in the navigator. It first checks if the user exists under
     * the given name as the username but lowercased. If that fails and the user full name search
     * option is on, it checks if the user exists with the given name as the fullname or email, if it does, null is returned because
     * that means that the query will not fit in the simple navigator.
     *
     ...
     * @return the username of the user if found by username, null if found by the full name or the passed in name if not found.
     */
    public String checkUser(final String name)
    {
        ...
        else if (isFullNameAndEmailSearchingEnabled() && userExistsByFullNameOrEmail(name))
        {
            return null;
        }
        return name;
    ...
    {
        return userPickerSearchService.isAjaxSearchEnabled();
    }
