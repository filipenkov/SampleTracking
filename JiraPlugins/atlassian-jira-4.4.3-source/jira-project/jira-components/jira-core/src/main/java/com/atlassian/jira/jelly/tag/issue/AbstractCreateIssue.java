        ...
        implements ProjectContextAccessor, IssueContextAccessor, CustomFieldValuesAwareTag
{
    ...
    protected boolean isDuplicateIssue(XMLOutput output) throws GenericEntityException, SAXException
    {
        ...
        if (!DUPLICATE_SUMMARY_IGNORE.equals(duplicateSummary))
        {
            ...
            final List matchingIssues = CoreFactory.getGenericDelegator().findByAnd("Issue", EasyMap.build("project", getProjectId(), "summary", getProperty(KEY_SUMMARY)));
    ...
    private void convertVersionNameToId(String key, GenericValue project)
    {
        ...
        if (StringUtils.isNotEmpty(versionNames))
        {
            ...
            while (tokenizer.hasMoreTokens())
            {
                ...
                Version version = versionManager.getVersion(project, versionName);
                if (version != null)
                {
    ...
    public Map getIssueTypes()
    {
        if (issueTypeMap == null)
        {
            ...
            final Collection issueTypes = ManagerFactory.getConstantsManager().getIssueTypes();
    ...
    public Map getPriorities()
    {
        if (priorityMap == null)
        {
            ...
            final Collection priorities = ManagerFactory.getConstantsManager().getPriorities();
    ...
    protected void postTagExecution(XMLOutput output) throws JellyTagException
    {
        try
        {
            ...
            if (getProperties().containsKey(KEY_ISSUE_ID))
            {
                ...
                try
                {
                    issue = ManagerFactory.getIssueManager().getIssue(key);
