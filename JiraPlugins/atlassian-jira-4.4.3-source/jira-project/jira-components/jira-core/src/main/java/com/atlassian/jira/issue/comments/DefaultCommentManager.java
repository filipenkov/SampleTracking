...
public class DefaultCommentManager implements CommentManager
{
    ...
    public DefaultCommentManager(IssueManager issueManager, TextAnalyzer textAnalyzer, ProjectRoleManager projectRoleManager,
    ...
    public MutableComment getMutableComment(Long commentId)
    {
        ...
        Map fields = EasyMap.build(COMMENT_ID, commentId);
        GenericValue gv = delegator.findByPrimaryKey(COMMENT_ENTITY, fields);
        if (gv != null)
        {
            return convertToComment(gv, issueManager.getIssueObject(gv.getLong("issue")));
        }
    ...
    public List<Comment> getCommentsForUser(Issue issue, com.opensymphony.user.User user)
    {
        return getCommentsForUser(issue, (User) user);
    }
            ...
            throws DataAccessException
    {
        ...
        textAnalyzer.analyseContent(issue, comment.getBody(), commentGV);
    ...
    public void update(Comment comment, boolean dispatchEvent)
    {
        ...
        try
        {
            commentGV = delegator.findByPrimaryKey(COMMENT_ENTITY, EasyMap.build("id", comment.getId()));
        ...
        // TODO: this should live in the service and should probably use the IssueUpdateBean
        // analyze and dispatch
        textAnalyzer.analyseContent(comment.getIssue(), comment.getBody(), commentGV);
