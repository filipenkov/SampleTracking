...
public class TimeTrackingSystemField extends AbstractOrderableField implements HideableField, RequirableField
{
    ...
    public TimeTrackingSystemField(VelocityManager velocityManager, ApplicationProperties applicationProperties, IssueManager issueManager, JiraAuthenticationContext authenticationContext, PermissionManager permissionManager, final JiraDurationUtils jiraDurationUtils)
    {
        super(IssueFieldConstants.TIMETRACKING, TIME_TRACKING_NAME_KEY, velocityManager, applicationProperties, authenticationContext, permissionManager, null);
    ...
    protected Object getRelevantParams(Map inputParameters)
    {
        ...
        {
            builder.setInLegacyMode(true).setEstimate(asArray(inputParameters, getId()));
        }
        else
        {
            // handle the special case of targeting only one field
            final String targetSubField = String.valueOf(inputParameters.get(TIMETRACKING_TARGETSUBFIELD));
            builder.setTargetSubField(targetSubField);
            if (TIMETRACKING_ORIGINALESTIMATE.equals(targetSubField))
            {
                builder
                        .setOriginalEstimate(asArray(inputParameters, TIMETRACKING_ORIGINALESTIMATE));
            }
            else if (TIMETRACKING_REMAININGESTIMATE.equals(targetSubField))
            {
                builder
                        .setRemainingEstimate(asArray(inputParameters, TIMETRACKING_REMAININGESTIMATE));
            }
            else
            {
                builder
        ...
        builder.setLogWorkActivated(asArray(inputParameters, WorklogSystemField.WORKLOG_ACTIVATE));
    ...
    private Object convertToIssueFormInModernMode(final TimeTrackingValue value)
    {
        ...
        if (TIMETRACKING_ORIGINALESTIMATE.equals(targetedSubField))
        {
            ...
            {
                builder.setOriginalEstimate(convertDurationIntoMillis(oeDisplayValue));
            }
        }
        else if (TIMETRACKING_REMAININGESTIMATE.equals(targetedSubField))
        {
            ...
            {
                builder.setRemainingEstimate(convertDurationIntoMillis(reDisplayValue));
            }
        }
        else
        {
    ...
    private Object convertToIssueFormInLegacyMode(final TimeTrackingValue value)
    {
        ...
        if (StringUtils.isNotBlank(estimate))
        {
            builder.setEstimate(convertDurationIntoMillis(estimate));
        }
        return builder.build();
    }

    private Long convertDurationIntoMillis(final String estimate)
    {
        try
        {
