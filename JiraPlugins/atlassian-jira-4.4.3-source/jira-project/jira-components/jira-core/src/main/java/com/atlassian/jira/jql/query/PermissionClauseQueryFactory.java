...
class PermissionClauseQueryFactory implements ClauseQueryFactory
{
    ...
    PermissionClauseQueryFactory(final ClauseQueryFactory clauseQueryFactory, final PermissionQueryFactory permissionQueryFactory, final JqlOperandResolver operandResolver, final int permissionId, final CurrentUser currentUser, final IndexInfoResolver<com.atlassian.crowd.embedded.api.User> infoResolver)
    {
    ...
    {
        final com.atlassian.crowd.embedded.api.User user = queryCreationContext.getQueryUser();
        ...
        return currentUser.rewrite(permissionQueryFactory.getQuery(OSUserConverter.convertToOSUser(user), permissionId), userPresence, user, infoResolver);
    }
    ...
    {
        Query rewrite(Query query, Presence userInLiterals, User user, IndexInfoResolver<User> resolver);
        ...
        enum Presence
        {
            ...
            static Presence get(final User user, final Collection<QueryLiteral> values, final IndexInfoResolver<User> resolver)
            {
    ...
    static class OverrideField implements CurrentUser
    {
        ...
        public Query rewrite(final Query query, final Presence hasUser, final User user, final IndexInfoResolver<User> resolver)
        {
            switch (hasUser)
            {
                case NOT:
                ...
                case CONTAINS:
                {
                    ...
                    result.add(new TermQuery(new Term(field, resolver.getIndexedValue(user))), Occur.SHOULD);
    ...
    static class UserEquals implements Predicate<QueryLiteral>
    {
        ...
        UserEquals(final User user, final IndexInfoResolver<User> infoResolver)
        {
            this.user = user;
        ...
        public boolean evaluate(final QueryLiteral literal)
        {
            ...
            for (final String userName : infoResolver.getIndexedValues(literal.asString()))
            {
                if (userNameEqualsUtil.equals(userName, user))
                {
                    return true;
                }
            }
