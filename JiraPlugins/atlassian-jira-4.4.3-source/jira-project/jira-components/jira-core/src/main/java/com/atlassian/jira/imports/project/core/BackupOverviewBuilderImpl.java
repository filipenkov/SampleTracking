...
public class BackupOverviewBuilderImpl implements BackupOverviewBuilder
{
    ...
    public void populateInformationFromElement(final String elementName, final Map attributes) throws ParseException
    {
        ...
        entityCount++;
        for (final Iterator iterator = overviewPopulators.iterator(); iterator.hasNext();)
        {
            final BackupOverviewPopulator populator = (BackupOverviewPopulator) iterator.next();
    ...
    List /*<BackupProject>*/getProjects()
    {
        ...
        {
            final ExternalProject externalProject = (ExternalProject) iterator.next();
    ...
    List getCustomFieldConfigurations(final String projectId)
    {
        ...
        // custom field.
        for (final Iterator iterator = customFields.iterator(); iterator.hasNext();)
        {
            final ExternalCustomField externalCustomField = (ExternalCustomField) iterator.next();
            ...
            if (configurationContext != null)
            {
                ...
                // Convert the issue type constraints to the string id's of the issue types
    ...
    private ConfigurationContext getRelevantConfiguratonContext(final String projectId, final String customFieldId)
    {
        ...
        for (final Iterator iterConfigContexts = configContexts.iterator(); iterConfigContexts.hasNext();)
        {
            final ConfigurationContext configurationContext = (ConfigurationContext) iterConfigContexts.next();
    ...
    private List getIssueTypesList(final List fieldConfigIssueTypes)
    {
        ...
        for (final Iterator iter = fieldConfigIssueTypes.iterator(); iter.hasNext();)
        {
            final FieldConfigSchemeIssueType fieldConfigSchemeIssueType = (FieldConfigSchemeIssueType) iter.next();
