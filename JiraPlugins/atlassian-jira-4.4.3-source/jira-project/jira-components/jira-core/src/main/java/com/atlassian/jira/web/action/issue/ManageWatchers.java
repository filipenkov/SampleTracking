...
public class ManageWatchers extends AbstractIssueSelectAction implements ParameterAware
{
    ...
    public ManageWatchers(WatcherManager watcherManager, VelocityManager velocityManager,
            UserPickerSearchService searchService, WatcherService watcherService, PermissionManager permissionManager)
    {
    ...
    public String getUserPickerHtml() throws GenericEntityException
    {
        ...
        return new UserPickerWebComponent(velocityManager, getApplicationProperties(), searchService)
    ...
    {
        return watcherManager.isWatching(getRemoteUser(), getIssue());
    }
    ...
    public String doStartWatching() throws GenericEntityException
    {
        try
        {
            watcherManager.startWatching(getRemoteUser(), getIssue());
        }
        catch (IssueNotFoundException e)
        {
            return ISSUE_PERMISSION_ERROR;
    ...
    public String doStopWatching() throws GenericEntityException
    {
        try
        {
            watcherManager.stopWatching(getRemoteUser(), getIssue());
        }
        catch (IssueNotFoundException e)
        {
            return ISSUE_PERMISSION_ERROR;
    ...
    {
        if (username == null || !TextUtils.stringSet(username))
        {
    ...
    @RequiresXsrfCheck
    public String doStopWatchers() throws EntityNotFoundException, GenericEntityException
    {
        try
        {
        ...
        Collection/*<String>*/ userNames = UserPickerWebComponent.getUserNamesToRemove(params, REMOVE_WATCHERS_PREFIX);
    ...
    {
        return (permissionManager.hasPermission(Permissions.MANAGE_WATCHER_LIST, getIssue(), getRemoteUser()));
    }

    // Check permission to view watcher list
    public boolean isCanViewWatcherList() throws GenericEntityException
    {
        return (permissionManager.hasPermission(Permissions.VIEW_VOTERS_AND_WATCHERS, getIssue(), getRemoteUser())
                || isCanEditWatcherList());
    }

    public boolean isCanStartWatching()
    {
        return isWatchingEnabled() && getRemoteUser() != null && !isWatching();
    }
