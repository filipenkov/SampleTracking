...
public class JiraAppLinksHostApplication extends AbstractInternalHostApplication implements Startable
{
    ...
    public Iterable<EntityReference> getLocalEntities()
    {
        final Iterable<Project> projects = Iterables.filter(projectManager.getProjectObjects(), new Predicate<Project>()
        {
            ...
            {
                return (input != null) && checkProjectPermissions(input);
            }
        });
    ...
    public boolean doesEntityExist(String key, Class<? extends EntityType> type)
    {
        if (JiraProjectEntityType.class.isAssignableFrom(type))
        {
            ...
            if (project != null)
            {
                return checkProjectPermissions(project);
            }
        }
        return false;
    }
    ...
    {
        if (entityReference.getType() instanceof JiraProjectEntityType)
        {
     ...
     * @return true if the has either the BROWSE project, JIRA Administrator or PROJECT ADMIN permission.
     */
    private boolean checkProjectPermissions(Project project)
    {
      ...
      if( permissionManager.hasPermission(Permissions.BROWSE, project, jiraAuthenticationContext.getLoggedInUser()) ||
          permissionManager.hasPermission(Permissions.ADMINISTER, jiraAuthenticationContext.getLoggedInUser())      ||
