...
public class MoveIssueUpdateFields extends MoveIssue implements OperationContext
{
    ...
    public MoveIssueUpdateFields(IssueLinkManager issueLinkManager, SubTaskManager subTaskManager,
                                 ConstantsManager constantsManager, WorkflowManager workflowManager,
                                 FieldManager fieldManager, FieldLayoutManager fieldLayoutManager,
                                 IssueFactory issueFactory, FieldScreenRendererFactory fieldScreenRendererFactory,
    ...
    public String doDefault()
    {
        ...
        if (!(ActionContext.getSingleValueParameters().containsKey("reset") && ("true".equals(ActionContext.getSingleValueParameters().get("reset")))))
        {
            ...
            for (Iterator iterator = targetFieldLayout.getVisibleLayoutItems(getRemoteUser(), getTargetProjectObj(), EasyList.build(getTargetIssueType())).iterator(); iterator.hasNext();)
            {
    ...
    protected MutableIssue getTargetIssueObjectWithSecurityLevel()
    {
        ...
        // TODO: Should this just use getIssueObject() ?
        targetIssue = getIssueObject(getIssueManager().getIssue(getId()));
        targetIssue.setProject(getTargetProject());
        targetIssue.setIssueType(getTargetIssueTypeGV());
    ...
    protected void doValidation()
    {
        if (getMoveIssueBean() != null)
        {
            try
            {
                ...
                for (Iterator iterator = getMoveFieldLayoutItems().iterator(); iterator.hasNext();)
                {
                    FieldLayoutItem fieldLayoutItem = (FieldLayoutItem) iterator.next();
                    OrderableField orderableField = fieldLayoutItem.getOrderableField();
                    ...
                    else
                    {
                        ...
                        if (errorCollection.getErrors() != null && !errorCollection.getErrors().isEmpty())
                        {
                            ...
                            {
                                addErrorMessage(getFieldName(orderableField) + ": " + iterator1.next());
                            }
                        }
    ...
    public String doExecute() throws Exception
    {
        ...
        for (Iterator iterator = getTargetHiddenFields().iterator(); iterator.hasNext();)
        {
    ...
    {
        return getTargetFieldLayout().getHiddenFields(getRemoteUser(), getTargetProject(), EasyList.build(getTargetIssueType()));
    }
     ...
     * @return A collection of custom field objects associated with the specified issue type.
     */
    protected Collection getCurrentCustomFieldObjects(String issueTypeId)
    {
        return getCustomFieldManager().getCustomFieldObjects(getProject().getLong("id"), issueTypeId);
    }
     ...
     * @param currentIssueTypeId the id of the current issue type.
     * @param targetIssueTypeId the id of the target issue type.
     * @return A collection of the custom fields from the current issue that are not applicable in the target destination.
     */
    protected Collection getInvalidCustomFields(String currentIssueTypeId, String targetIssueTypeId)
    {
        Collection invalidCustomFields = new ArrayList();
        Collection targetCustomFields = getTargetCustomFieldObjects(targetIssueTypeId);
        for (Iterator it = getCurrentCustomFieldObjects(currentIssueTypeId).iterator(); it.hasNext();)
        {
            CustomField currentCustomField = (CustomField) it.next();
            if (!targetCustomFields.contains(currentCustomField))
            {
