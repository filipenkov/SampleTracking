...
public class WorklogTabPanel extends AbstractIssueTabPanel
{
    ...
    public List getActions(Issue issue, User remoteUser)
    {
        ...
        for (Worklog userWorklog : userWorklogs)
        {
            ...
            worklogs.add(new WorklogAction(descriptor, userWorklog, jiraDurationUtils, canEditWorklog, canDeleteWorklog, fieldLayoutManager, rendererManager));
        }

        // This is a bit of a hack to indicate that there are no comments to display
        if (worklogs.isEmpty())
        {
            GenericMessageAction action = new GenericMessageAction(descriptor.getI18nBean().getText("viewissue.nowork"));
