...
public class Setup extends AbstractSetupAction
{
    ...
    private String forceRedirectToInput()
    {
        ...
        try
        {
            if (title != null)
            {
                params.put("title", VelocityHelper.encode(title, ManagerFactory.getApplicationProperties().getEncoding()));
            }
            if (baseURL != null)
            {
                params.put("baseURL", VelocityHelper.encode(baseURL, ManagerFactory.getApplicationProperties().getEncoding()));
            }
            if (mode != null)
            {
                params.put("mode", VelocityHelper.encode(mode, ManagerFactory.getApplicationProperties().getEncoding()));
            }
            if (licenseString != null)
            {
                params.put("license", VelocityHelper.encode(licenseString, ManagerFactory.getApplicationProperties().getEncoding()));
            }
            if (attachmentPath != null)
            {
                params.put("attachmentPath", VelocityHelper.encode(attachmentPath, ManagerFactory.getApplicationProperties().getEncoding()));
            }
            if (indexPath != null)
            {
                params.put("indexPath", VelocityHelper.encode(indexPath, ManagerFactory.getApplicationProperties().getEncoding()));
            }
            if (mode != null)
            {
                params.put("mode", VelocityHelper.encode(mode, ManagerFactory.getApplicationProperties().getEncoding()));
        ...
        final StringBuffer redirectStr = new StringBuffer("Setup!input.jspa");
        int i = 0;
