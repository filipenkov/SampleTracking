...
public abstract class DefaultIssueConversionService implements IssueConversionService
{
    ...
    public DefaultIssueConversionService(PermissionManager permissionManager, WorkflowManager workflowManager, FieldLayoutManager fieldLayoutManager,
                                         IssueTypeSchemeManager issueTypeSchemeManager, JiraAuthenticationContext jiraAuthenticationContext, FieldManager fieldManager)
    {
    ...
    {
        return permissionManager.hasPermission(getPermissionNeeded(), issue, context.getUser());
    }
    ...
    {
        List linkedStatusObjects = getWorkflowForProjectAndIssueType(projectId, issueTypeId).getLinkedStatusObjects();
        for (Iterator it = linkedStatusObjects.iterator(); it.hasNext();)
        {
            Status wfStatus = (Status) it.next();
    ...
    public Collection /* <FieldLayoutItem> */ getFieldLayoutItems(JiraServiceContext context, Issue originalIssue, Issue targetIssue)
    {
        ...
        List convertFieldLayoutItems = new ArrayList();
        ...
        final List /* <String> */ issueTypeIds = EasyList.build(targetIssue.getIssueTypeObject().getId());
        final List /* <FieldLayoutItem> */ visibleLayoutItems = targetFieldLayout.getVisibleLayoutItems(context.getUser(), targetProject, issueTypeIds);
    ...
    {
        Collection removedFields = new ArrayList();

        FieldLayout targetFieldLayout = getFieldLayout(targetIssue);

        Collection hiddenFields = targetFieldLayout.getHiddenFields(context.getUser(), targetIssue.getProjectObject(), EasyList.build(targetIssue.getIssueTypeObject().getId()));
        ...
        for (Iterator iterator = hiddenFields.iterator(); iterator.hasNext();)
        {
            ...
            {
                boolean doValueCheck = isShouldCheckFieldValue(context, origIssue, field);
     ...
     * @param context The service context needed for the remote user
    ...
    public void convertIssue(JiraServiceContext context, Issue currentIssue, MutableIssue updatedIssue)
    {
        ...
        GenericValue updateLog = ChangeLogUtils.createChangeGroup(context.getUser(), currentIssue, updatedIssue,
                issueChangeHolder.getChangeItems(), false);

        if (updateLog != null && !issueChangeHolder.getChangeItems().isEmpty())
        {
            // Presumably issueChangeHolder.isSubtasksUpdated() is always false, but better to just pass it through.
            IssueEventDispatcher.dispatchEvent(EventType.ISSUE_UPDATED_ID, updatedIssue, context.getUser(),
                    updateLog, true, issueChangeHolder.isSubtasksUpdated());
        }
