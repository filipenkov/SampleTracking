...
public class DefaultRepositoryManager implements RepositoryManager, Startable
{
    ...
    public DefaultRepositoryManager(AssociationManager associationManager, DelegatorInterface genericDelegator,
            ...
            final EventPublisher eventPublisher) throws GenericEntityException
    {
    ...
    {
        this(CoreFactory.getAssociationManager(), CoreFactory.getGenericDelegator(), ManagerFactory.getServiceManager(),
                ManagerFactory.getPermissionManager(),
                ComponentManager.getComponentInstanceOfType(ChangeHistoryManager.class),
                ComponentAccessor.getProjectManager(), ComponentManager.getComponentInstanceOfType(EventPublisher.class));
    }
    ...
    {
        @SuppressWarnings ({ "unchecked" }) List<GenericValue> vcsRepositories = CoreFactory.getGenericDelegator().findAll("VersionControl");
    ...
    public Collection<Repository> getRepositoriesForProject(GenericValue project) throws GenericEntityException
    {
        ...
        List<GenericValue> vcsRepositories = getAssociationManager().getSinkFromSource(project, "VersionControl", ProjectRelationConstants.PROJECT_VERSIONCONTROL, false);
    ...
    public Collection<GenericValue> getProjectsForRepository(Repository repository) throws GenericEntityException
    {
        ...
        @SuppressWarnings ({ "unchecked" }) List<GenericValue> projectGVs = associationManager.getSourceFromSink(repositoryGV, "Project", ProjectRelationConstants.PROJECT_VERSIONCONTROL, false);
    ...
    private GenericValue getRepositoryGV(Long id) throws GenericEntityException
    {
        return EntityUtil.getOnly(getGenericDelegator().findByAnd("VersionControl", EasyMap.build("id", id)));
    }
    ...
    public Map<Long, Set<CVSCommit>> getCommits(Issue issue, com.opensymphony.user.User remoteUser)
    {
        return getCommits(issue, (User) remoteUser);
    }
    ...
    public Map<Long, Set<CVSCommit>> getCommits(Issue issue, User remoteUser)
    {
        ...
        if (!permissionManager.hasPermission(Permissions.VIEW_VERSION_CONTROL, issue, remoteUser))
        {
    ...
    public Repository createRepository(String type, String name, String description, Properties properties) throws Exception
    {
        if (CVS_TYPE.equals(type))
        {
            ...
            /*
            propertySet.setString(CvsRepository.KEY_CVS_ROOT, properties.getProperty(CvsRepository.KEY_CVS_ROOT));
            propertySet.setString(CvsRepository.KEY_MODULE_NAME, properties.getProperty(CvsRepository.KEY_MODULE_NAME));
            propertySet.setString(CvsRepository.KEY_PASSWORD, properties.getProperty(CvsRepository.KEY_PASSWORD));
            propertySet.setBoolean(CvsRepository.KEY_FETCH_LOG, Boolean.valueOf(properties.getProperty(CvsRepository.KEY_FETCH_LOG)).booleanValue());

            // Check if the repository browser type is specified
            if (properties.containsKey(Repository.KEY_REPOSITTORY_BROWSER_TYPE))
            {
                final String repositoryBrowserType = properties.getProperty(Repository.KEY_REPOSITTORY_BROWSER_TYPE);
                if (RepositoryBrowser.VIEW_CVS_TYPE.equals(repositoryBrowserType))
                {
                    // Record the type against this cvs
                    propertySet.setString(Repository.KEY_REPOSITTORY_BROWSER_TYPE, repositoryBrowserType);

                    // Save the Base URL for ViewCVS
                    propertySet.setString(ViewCvsBrowser.KEY_BASE_URL, properties.getProperty(ViewCvsBrowser.KEY_BASE_URL));
                    propertySet.setString(ViewCvsBrowser.ROOT_PARAMETER, properties.getProperty(ViewCvsBrowser.ROOT_PARAMETER));
                }
            }
            */
    ...
    {
        serviceManager.addService(VCS_SERVICE_NAME, "com.atlassian.jira.service.services.vcs.VcsService", VCS_SERVICE_DELAY);
    }

    private void removeRepositoryUpdateService() throws Exception
    {
        getServiceManager().removeServiceByName(VCS_SERVICE_NAME);
    }
    ...
    protected void markVcsServiceToRun()
    {
        ...
        try
        {
            service = serviceManager.getServiceWithName(RepositoryManager.VCS_SERVICE_NAME);
        }
        catch (Exception e)
        {
            log.warn("Failed to get the service with '" + RepositoryManager.VCS_SERVICE_NAME + "' name");
            return;
        }

        // only execute if service usable, i.e. fully loaded
        if (service != null && service.isUsable())
        {
            serviceManager.getScheduleSkipper().addService(service.getId());
            ...
            throws GenericEntityException
    {
        ...
        List<GenericValue> oldAssociations = associationManager.getSinkFromSource(project, "VersionControl", ProjectRelationConstants.PROJECT_VERSIONCONTROL, false);
        for (GenericValue oldAssociation : oldAssociations)
        {
            associationManager.removeAssociation(project, oldAssociation, ProjectRelationConstants.PROJECT_VERSIONCONTROL);
        }

        // Create new project to repository associations
        for (GenericValue repoEntity : newRepositoryGVs)
        {
            associationManager.createAssociation(project, repoEntity, ProjectRelationConstants.PROJECT_VERSIONCONTROL);
        }
    }
    ...
    protected DelegatorInterface getGenericDelegator()
    {
