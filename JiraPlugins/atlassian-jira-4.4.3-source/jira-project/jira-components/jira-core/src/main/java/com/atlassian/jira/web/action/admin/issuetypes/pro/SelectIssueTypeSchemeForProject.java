...
public class SelectIssueTypeSchemeForProject extends AssociateIssueTypeSchemes
{
        ...
        if (CREATE_SCHEME.equals(getCreateType()))
        {
            ...
            if (getSelectedOptions() != null)
            {
                ...
                for (int i = 0; i < getSelectedOptions().length; i++)
                {
                    ...
                    IssueType issueType = ManagerFactory.getConstantsManager().getIssueTypeObject(id);
    ...
    {
        final Collection constantObjects = ManagerFactory.getConstantsManager().getConstantObjects(getManageableOption().getFieldId());
    ...
    public long getSubTaskIssues() throws SearchException
    {
        if (subTaskCount == -1)
        {
            ...
            subTaskCount = searchProvider.searchCount(query, getRemoteUser());
        }
    ...
    public long getStandardIssues() throws SearchException
    {
        if (standardIssuesCount == -1)
        {
            ...
            standardIssuesCount = searchProvider.searchCount(query, getRemoteUser());
        }
