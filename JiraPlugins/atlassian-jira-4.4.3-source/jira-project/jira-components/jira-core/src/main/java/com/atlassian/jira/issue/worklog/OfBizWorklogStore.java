...
public class OfBizWorklogStore implements WorklogStore
{
    ...
    public Worklog update(Worklog worklog)
    {
        ...
        GenericValue worklogGV = ofBizDelegator.findByPrimaryKey(WORKLOG_ENTITY, EasyMap.build("id", worklog.getId()));
    ...
    public Worklog getById(Long id)
    {
        ...
        GenericValue worklogGV = ofBizDelegator.findByPrimaryKey(WORKLOG_ENTITY, EasyMap.build("id", id));
    ...
    public List<Worklog> getByIssue(Issue issue)
    {
        ...
        List worklogGVs = ofBizDelegator.findByAnd(WORKLOG_ENTITY, EasyMap.build("issue", issue.getId()), EasyList.build("created ASC"));

        for (Iterator iterator = worklogGVs.iterator(); iterator.hasNext();)
        {
            GenericValue worklogGV = (GenericValue) iterator.next();
    ...
    public long getCountForWorklogsRestrictedByGroup(String groupName)
    {
        ...
        if (worklogCount != null && worklogCount.size() == 1)
        {
            ...
            return worklogCountGV.getLong("count").longValue();
        }
        else
        {
    ...
    Worklog convertToWorklog(Issue issue, GenericValue gv)
    {
        ...
        Worklog worklog = new WorklogImpl(getWorklogManager(),
                issue,
                gv.getLong("id"),
                gv.getString("author"),
                gv.getString("body"),
    ...
    private WorklogManager getWorklogManager()
    {
        ...
        {
            worklogManager = (WorklogManager) ComponentManager.getComponentInstanceOfType(WorklogManager.class);
        }
