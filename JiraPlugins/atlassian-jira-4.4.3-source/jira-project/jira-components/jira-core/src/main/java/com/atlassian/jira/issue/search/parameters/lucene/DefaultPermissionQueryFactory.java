        ...
        for (final GenericValue scheme : schemes)
        {
            ...
            for (final GenericValue entity : entities)
            {
                ...
                try
                {
                    ...
                    {
                        final Query tempQuery = securityType.getQuery(OSUserConverter.convertToOSUser(searcher), projectGV, entity.getString("parameter"));
                        if (tempQuery != null)
                        {
        ...
        for (final GenericValue securityLevel : usersSecurityLevels)
        {
            ...
            for (final GenericValue entity : securities)
            {
                ...
                if (userHasPermissionForProjectAndSecurityType(searcher, project, entity, securityType))
                {
                    ...
                    final Query tempQuery = securityType.getQuery(OSUserConverter.convertToOSUser(searcher), projectObject, securityLevel, entity.getString("parameter"));
                    if (tempQuery != null)
                    {
                        queries.add(tempQuery);
                    }
