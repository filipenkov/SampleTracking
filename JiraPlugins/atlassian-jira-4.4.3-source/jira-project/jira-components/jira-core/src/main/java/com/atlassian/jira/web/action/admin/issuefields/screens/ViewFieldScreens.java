...
public class ViewFieldScreens extends AbstractFieldScreenAction
{
    ...
    public String doAddFieldScreen()
    {
        ...
        if (!invalidInput())
        {
            ...
            fieldScreen.store();
            // TODO: What is with the single quotes here?
            fieldScreen.addTab(getComponentManager().getJiraAuthenticationContext().getI18nHelper().getText("'admin.field.screen.default'"));
    ...
    private boolean hasWorkflowsIncludingDrafts(FieldScreen fieldScreen) {
        for (JiraWorkflow workflow: workflowManager.getWorkflowsIncludingDrafts())
        {
            Collection<ActionDescriptor> actions = workflow.getActionsForScreen(fieldScreen);
            if (actions != null && !actions.isEmpty()) {
                return true;
