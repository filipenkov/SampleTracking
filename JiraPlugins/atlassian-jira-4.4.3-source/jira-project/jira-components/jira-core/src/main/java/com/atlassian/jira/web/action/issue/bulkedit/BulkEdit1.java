...
public class BulkEdit1 extends AbstractBulkOperationAction
{
    ...
    private void initialiseBulkEditBean() throws SearchException
    {
        ...
        List issues = searchProvider.search((getSearchRequest() != null) ? getSearchRequest().getQuery() : null, getRemoteUser(), pager).getIssues();
    ...
    private SimpleColumnLayoutItem getCheckboxColumn()
    {
        return new SimpleColumnLayoutItem()
        {
            ...
            {
                return "<input type='checkbox' name='" + getBulkEditBean().getCheckboxName(issue) +
                               "' " + (getBulkEditBean().isChecked(issue) ? "checked='true'" : "");

            }

            public String getHeaderHtml()
            {
                return "<input type='checkbox' name='all' onClick='setCheckboxes()'>";
            }
        };
    }
