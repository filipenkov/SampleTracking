...
public class JiraEntityUtils
{
    ...
    public static List updateDependentEntitiesCheckId(GenericValue issue, Collection newEntities, String relationType, String changeItemField) throws GenericEntityException
    {
        ...
        Collection oldEntities = ManagerFactory.getIssueManager().getEntitiesByIssue(relationType, issue);
        ...
        {
            final GenericValue newEntity = (GenericValue) iterator.next();
            if (!oldEntityIds.contains(newEntity.getLong("id")))
            {
                CoreFactory.getAssociationManager().createAssociation(issue, newEntity, relationType);
        ...
        for (Iterator iterator = oldEntities.iterator(); iterator.hasNext();)
        {
            GenericValue oldEntity = (GenericValue) iterator.next();
            if (!newEntityIds.contains(oldEntity.getLong("id")))
            {
                CoreFactory.getAssociationManager().removeAssociation(issue, oldEntity, relationType);
    ...
    public static List updateDependentEntities(GenericValue issue, Collection newEntities, String relationType, String changeItemField) throws GenericEntityException
    {
        ...
        for (Iterator iterator = newEntities.iterator(); iterator.hasNext();)
        {
            GenericValue newEntity = (GenericValue) iterator.next();
