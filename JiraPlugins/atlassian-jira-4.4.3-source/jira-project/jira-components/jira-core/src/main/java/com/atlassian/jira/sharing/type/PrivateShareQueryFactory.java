...
public class PrivateShareQueryFactory implements ShareQueryFactory<PrivateShareTypeSearchParameter>
{
    ...
    public Field getField(final SharedEntity entity, final SharePermission permission)
    {
        ...
        return new Field(Name.FIELD, (ownerUserName == null) ? "" : ownerUserName, Field.Store.YES, Field.Index.UN_TOKENIZED);
    }

    public Term[] getTerms(final User user)
    {
        final Term term = getTerm(null, user);
    ...
    private Term getTerm(final ShareTypeSearchParameter searchParameter, final User user)
    {
        return (user == null) ? null : new Term(Name.FIELD, user.getName());
    }
}
