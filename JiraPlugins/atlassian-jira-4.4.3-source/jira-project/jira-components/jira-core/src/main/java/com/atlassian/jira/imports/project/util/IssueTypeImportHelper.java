...
public class IssueTypeImportHelper
{
    ...
    public IssueType getIssueTypeForName(final String name)
    {
        ...
        for (final Iterator iterator = constantsManager.getAllIssueTypeObjects().iterator(); iterator.hasNext();)
        {
            final IssueType issueType = (IssueType) iterator.next();
    ...
    public boolean isIssueTypeValidForProject(final String projectKey, final String newIssueTypeId)
    {
        ...
        final Collection issueTypesForProject;
        if (project == null)
        {
    ...
    private IssueType getExistingIssueTypeForId(final Collection issueTypesForProject, final String newId)
    {
        IssueType existingIssueType = null;
        // Run through all the issue types for this project to see if we have a matching issue type
        for (final Iterator iter = issueTypesForProject.iterator(); iter.hasNext();)
        {
            final IssueType issueType = (IssueType) iter.next();
