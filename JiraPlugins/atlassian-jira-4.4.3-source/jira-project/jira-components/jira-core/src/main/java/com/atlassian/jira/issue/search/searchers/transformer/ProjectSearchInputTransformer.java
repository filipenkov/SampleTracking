...
public class ProjectSearchInputTransformer implements SearchInputTransformer
{
    ...
    public void populateFromParams(final User searcher, final FieldValuesHolder fieldValuesHolder, final ActionParams actionParams)
    {
        ...
        fieldValuesHolder.put(url, actionParams.getValuesForKey(url));
    }
    ...
    public Set<String> getIdValuesAsStrings(final User searcher, final Query query, final SearchContext searchContext)
    {
        IndexedInputHelper helper = createIndexedInputHelper();
        return helper.getAllIndexValuesForMatchingClauses(searcher, SystemSearchConstants.forProject().getJqlClauseNames(), query, searchContext);
    }

    public Set<String> getNavigatorValuesAsStrings(User searcher, Query query, SearchContext searchContext)
    {
        IndexedInputHelper helper = createIndexedInputHelper();
        return helper.getAllNavigatorValuesForMatchingClauses(searcher, SystemSearchConstants.forProject().getJqlClauseNames(), query, searchContext);
    }
    ...
    void setProjectIdInSession(Set<String> selectedProjectIds)
    {
        if (selectedProjectIds != null && selectedProjectIds.size() == 1)
        {
            ...
            if (id != null)
            {
                ...
                if (project != null)
                {
                    projectHistoryManager.addProjectToHistory(authenticationContext.getUser(), project);
                }
            }
        }
    }
    ...
    {
        return new NavigatorStructureChecker<Project>(SystemSearchConstants.forProject().getJqlClauseNames(), false, fieldFlagOperandRegistry, operandResolver, projectIndexInfoResolver, searchContextVisibilityChecker);
    }

    IndexedInputHelper createIndexedInputHelper()
    {
        return new DefaultIndexedInputHelper<Project>(projectIndexInfoResolver, operandResolver, fieldFlagOperandRegistry, searchContextVisibilityChecker);
    }
