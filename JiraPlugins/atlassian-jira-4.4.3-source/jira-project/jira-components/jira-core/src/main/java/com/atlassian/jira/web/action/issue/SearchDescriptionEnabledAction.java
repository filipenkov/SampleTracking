...
public class SearchDescriptionEnabledAction extends IssueActionSupport
{
    ...
    {
        return favouritesService.isFavourite(getRemoteUser(), getSearchRequest());
    }

    protected void populateFieldValuesHolderFromQuery(final Query query, final FieldValuesHolder fieldValuesHolder)
    {
        final SearchContext context = searchService.getSearchContext(getRemoteUser(), query);
        final Collection<IssueSearcher<?>> searchers = getSearchers();
        for (IssueSearcher<?> searcher : searchers)
        {
            searcher.getSearchInputTransformer().populateFromQuery(getRemoteUser(), fieldValuesHolder, query, context);
        }
    }

    public Collection getSearcherGroups()
    {
        return issueSearcherManager.getSearcherGroups(getSearchContext());
    }
    ...
    public String getSearcherViewHtml(IssueSearcher searcher)
    {
        ...
        if (searchRequest != null && searchRenderer.isRelevantForQuery(getRemoteUser(), searchRequest.getQuery()))
        {
            final SearchContext searchContext = getSearchContext();
            return searchRenderer.getViewHtml(getRemoteUser(), searchContext, getFieldValuesHolder(), new HashMap(), this);
        }
        else
        {
            return "";
    ...
    {
        return searchService.doesQueryFitFilterForm(getRemoteUser(), (getSearchRequest() != null) ? getSearchRequest().getQuery() : null);
    }

    public boolean isShown(SearcherGroup searcherGroup)
    {
        return searcherGroup.isShown(getRemoteUser(), getSearchContext());
    }
    ...
    private SearchContext getSeachContextFromQueryString(final String query)
    {
        ...
        {
            final SearchService.ParseResult jqlQuery = searchService.parseQuery(getRemoteUser(), query);
            if (jqlQuery.isValid())
            {
                return searchService.getSearchContext(getRemoteUser(), jqlQuery.getQuery());
            }
        }
    ...
    {
        Collection visibleProjects = permissionManager.getProjects(Permissions.BROWSE, authenticationContext.getUser());
    ...
    public FilterOperationsBean getFilterOperationsBean()
    {
        ...
        {
            final String userName = getRemoteUser() == null ? null : getRemoteUser().getName();
            filterOperationsBean = createFilterOperationsBean(userName);
        }
    ...
    {
        return FilterOperationsBean.create(getSearchRequest(), isFilterValid(), userName, false);
    }
    ...
    {
        final SearchRequest searchRequest = getSearchRequest();
        if (searchRequest != null && searchService.validateQuery(getRemoteUser(), searchRequest.getQuery()).hasAnyErrors())
        {
            return false;
        }
        return true;
