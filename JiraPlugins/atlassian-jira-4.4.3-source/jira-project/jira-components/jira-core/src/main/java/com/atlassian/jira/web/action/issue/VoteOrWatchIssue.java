...
public class VoteOrWatchIssue extends AbstractIssueSelectAction
{
    ...
    protected String doExecute() throws Exception
    {
        try
        {
            ...
            {
                final VoteService.VoteValidationResult validationResult = voteService.validateAddVote(getRemoteUser(), getRemoteUser(), getIssueObject());
                if (validationResult.isValid())
                {
                    voteService.addVote(getRemoteUser(), validationResult);
                }
                else
                {
            ...
            {
                final VoteService.VoteValidationResult validationResult = voteService.validateRemoveVote(getRemoteUser(), getRemoteUser(), getIssueObject());
                if (validationResult.isValid())
                {
                    voteService.removeVote(getRemoteUser(), validationResult);
                }
                else
                {
                    addErrorCollection(validationResult.getErrorCollection());
