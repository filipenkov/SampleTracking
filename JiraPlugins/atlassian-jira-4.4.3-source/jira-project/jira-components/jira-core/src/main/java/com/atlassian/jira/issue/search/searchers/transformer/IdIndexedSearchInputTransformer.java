...
public abstract class IdIndexedSearchInputTransformer<T> implements SearchInputTransformer
{
    ...
    public IdIndexedSearchInputTransformer(ClauseNames id, IndexInfoResolver<T> indexInfoResolver, JqlOperandResolver operandResolver,
            FieldFlagOperandRegistry fieldFlagOperandRegistry, final SearchContextVisibilityChecker searchContextVisibilityChecker)
    {
    ...
    IdIndexedSearchInputTransformer(ClauseNames clauseNames, String urlParameterName, IndexInfoResolver<T> indexInfoResolver,
            ...
            final NavigatorStructureChecker navigatorStructureChecker, final SearchContextVisibilityChecker searchContextVisibilityChecker)
    {
        this.clauseNames = clauseNames;
        this.urlParameterName = urlParameterName;
        this.fieldFlagOperandRegistry = fieldFlagOperandRegistry;
    ...
    {
        final Collection<String> valuesAsStrings = getIndexedInputHelper().getAllNavigatorValuesForMatchingClauses(searcher, clauseNames, query, searchContext);
    ...
    {
        return navigatorStructureChecker.checkSearchRequest(query, searchContext);
    }
     ...
     * @param fieldValuesHolder contains values populated by the searchers  @return Clause that represents the raw values.
    ...
    {
        return new NavigatorStructureChecker<T>(clauseNames, true, fieldFlagOperandRegistry, operandResolver, indexInfoResolver, searchContextVisibilityChecker);
    }
    ///CLOVER:ON
    ...
    protected DefaultIndexedInputHelper getDefaultIndexedInputHelper()
    {
        ...
        {
            defaultIndexedInputHelper = new DefaultIndexedInputHelper<T>(indexInfoResolver, operandResolver, fieldFlagOperandRegistry, searchContextVisibilityChecker);
        }
