...
public class DataUtils
{
    ...
    /**
     * @deprecated Use {@link #normaliseDateRangeCount(java.util.Map, int, Class, java.util.TimeZone)} instead. Since v4.4.
     */
    @Deprecated
    public static void normaliseDateRangeCount(Map<RegularTimePeriod, Number> dateMap, int days, Class period)
    {
        normaliseDateRangeCount(dateMap, days, period, TimeZone.getDefault());
    }
    ...
    /**
     * @deprecated Use {@link #normaliseDateRange(java.util.Map, int, Class, java.util.TimeZone)} instead. Since v4.4.
     */
    @Deprecated
    public static void normaliseDateRange(Map<RegularTimePeriod, List<Long>> dateMap, int days, Class period)
    {
        normaliseDateRange(dateMap, days, period, TimeZone.getDefault());
    }
    ...
    public static void normaliseDateRange(Map<RegularTimePeriod, List<Long>> dateMap, int days, Class period, TimeZone timeZone)
    {
        ...
        while (cursor != null && cursor.compareTo(end) <= 0)
        {
            if (!dateMap.containsKey(cursor))
            {
                dateMap.put(cursor, EasyList.build((long) 0));
            }
            cursor = cursor.next();
            cursor.peg(cal);
        }
