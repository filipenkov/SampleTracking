...
public class DefaultSearchRequestFactory implements SearchRequestFactory
{
    ...
    {
        // we need to check if both queries imply the same query context, as this will effect population        
        ...
        // we need to check if the simple search contexts are the same, as this will effect population
        ...
        for (IssueSearcher<?> issueSearcher : issueSearchers)
        {
            ...
            issueSearcher.getSearchInputTransformer().populateFromQuery(osuser, newHolder, newQuery, newSearchContext);
            issueSearcher.getSearchInputTransformer().populateFromQuery(osuser, oldHolder, oldQuery, oldSearchContext);
    ...
    List<Clause> getClausesFromSearchers(final User searchUser, final ActionParams actionParams)
    {
        ...
        // Need an OSUser object for now
        com.opensymphony.user.User osuser = OSUserConverter.convertToOSUser(searchUser);
        ...
        for (IssueSearcher<?> searcher : searchers)
        {
            ...
            searchInputTransformer.populateFromParams(osuser, fieldValuesHolder, actionParams);
            final Clause clause = searchInputTransformer.getSearchClause(osuser, fieldValuesHolder);
            if (clause != null)
            {
                clauses.add(clause);
            }
    ...
    @Override
    public SearchRequest createFromParameters(SearchRequest oldSearchRequest, com.opensymphony.user.User searchUser, ActionParams parameterMap)
    {
        return createFromParameters(oldSearchRequest, (User) searchUser, parameterMap);
    }

    @Override
    public SearchRequest createFromQuery(SearchRequest oldSearchRequest, com.opensymphony.user.User searchUser, Query query)
    {
        return createFromQuery(oldSearchRequest, (User) searchUser, query);
    }
}
