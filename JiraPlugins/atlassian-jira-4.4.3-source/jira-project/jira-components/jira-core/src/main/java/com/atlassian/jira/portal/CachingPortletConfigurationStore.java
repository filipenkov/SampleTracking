 ...
 * @since 3.13
 */
public class CachingPortletConfigurationStore implements FlushablePortletConfigurationStore, Startable
{
    ...
    public CachingPortletConfigurationStore(final PortletConfigurationStore delegateStore, final EventPublisher eventPublisher)
    {
    ...
    @edu.umd.cs.findbugs.annotations.SuppressWarnings(value="UL_UNRELEASED_LOCK_EXCEPTION_PATH", justification="This appears to be doing exactly the right thing with the finally-clause to release the lock")
    public PortletConfiguration addLegacyPortlet(final Long pageId, final Long portletConfigurationId, final Integer column, final Integer row,
            final URI gadgetXml, final Color color, final Map<String, String> userPreferences, final String portletKey)
    {
        ...
        try
        {
            returnConfig = delegateStore.addLegacyPortlet(pageId, portletConfigurationId, column, row, gadgetXml, color, userPreferences, portletKey);
        }
        finally
        {
            if (returnConfig != null)
        ...
        if (portletConfiguration != null)
        {
            try
            {
                ...
                return new PortletConfigurationImpl(portletConfiguration.getId(), portletConfiguration.getDashboardPageId(),
                    portletConfiguration.getKey(), portletConfiguration.getPortlet(), portletConfiguration.getColumn(),
                    portletConfiguration.getRow(), clonePropertySet(portletConfiguration.getProperties()),
    ...
    ///CLOVER:OFF
    /**
     * Copying the existing propertyset in to an in memory property set (as opposed to the traditional JIRA property set
     * that is directly backed by the database)
     *
     * @param srcPropertySet the property set to clone
     * @return a new instance of an in memory property set.
     */
    PropertySet clonePropertySet(final PropertySet srcPropertySet)
    {
        final PropertySet newPropertySet = PropertySetManager.getInstance("memory", new HashMap<String, Object>());
        if (srcPropertySet != null)
        {
            PropertySetManager.clone(srcPropertySet, newPropertySet);
        }
