...
import com.atlassian.jira.issue.index.indexers.impl.ResolutionDateIndexer;
...
public class ResolutionDateSearcher extends AbstractDateSearcher
{
    public ResolutionDateSearcher(final DateConverter dateConverter, final DateTimeConverter dateTimeConverter, final JqlOperandResolver operandResolver,
            final ApplicationProperties applicationProperties, final VelocityRequestContextFactory velocityRenderContext,
            final VelocityManager velocityManager, final CalendarLanguageUtil calendarUtils,
    ...
    {
        super(SystemSearchConstants.forResolutionDate(), "navigator.filter.resolutiondate", ResolutionDateIndexer.class, dateConverter,
                dateTimeConverter, new JqlDateSupportImpl(timeZoneManager), operandResolver, applicationProperties, velocityRenderContext, velocityManager,
