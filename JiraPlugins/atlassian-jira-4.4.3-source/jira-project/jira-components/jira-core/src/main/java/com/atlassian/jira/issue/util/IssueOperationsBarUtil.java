 ...
 * <p/>
 * Does a lot of caching (only request scope) as we need constantly need to check the sizes of different lists and it is hard to keep track of a lot
 * of these things in the view layer.
 * <p/>
...
public class IssueOperationsBarUtil
{
    ...
    public IssueOperationsBarUtil(JiraHelper helper, User user,
                                  SimpleLinkManager simpleLinkManager, ApplicationProperties applicationProperties)
    {
        this.helper = helper;
        this.user = user;
     ...
     * <p/>
     * Though this can be overridden for individual groups:
     ...
     * We display the more link if the group has a a label or if there are more items to show in a drop down.
     *
    ...
    private List<SimpleLink> getLinksForSection(SimpleLinkSection section)
    {
        if (!linksBySection.containsKey(section.getId()))
        {
            ...
            linksBySection.put(section.getId(), sections);
        }
