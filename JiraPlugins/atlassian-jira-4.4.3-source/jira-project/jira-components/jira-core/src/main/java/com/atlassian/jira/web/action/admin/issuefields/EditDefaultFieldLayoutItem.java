...
public class EditDefaultFieldLayoutItem extends AbstractEditFieldLayoutItem
{
    ...
    protected EditableFieldLayout getFieldLayout()
    {
        if (editableDefaultFieldLayout == null)
        {
            try
            {
                editableDefaultFieldLayout = ManagerFactory.getFieldManager().getFieldLayoutManager().getEditableDefaultFieldLayout();
            }
    ...
    protected void store()
    {
        try
        {
            ManagerFactory.getFieldManager().getFieldLayoutManager().storeEditableDefaultFieldLayout((EditableDefaultFieldLayout) getFieldLayout());
        }
        catch (DataAccessException e)
        {
            log.error(ACCESS_EXCEPTION, e);
