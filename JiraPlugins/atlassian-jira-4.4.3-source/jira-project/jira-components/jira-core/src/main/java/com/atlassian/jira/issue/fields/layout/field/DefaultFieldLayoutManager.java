...
public class DefaultFieldLayoutManager extends AbstractFieldLayoutManager implements FieldLayoutManager, Startable
{
    ...
    public DefaultFieldLayoutManager(FieldManager fieldManager, OfBizDelegator ofBizDelegator, final ConstantsManager constantsManager,
            final SubTaskManager subTaskManager, final ProjectManager projectManager, final EventPublisher eventPublisher, I18nHelper.BeanFactory i18n)
    {
    ...
    public Collection<GenericValue> getProjects(FieldConfigurationScheme fieldConfigurationScheme)
    {
        ...
        try
        {
            return CoreFactory.getAssociationManager().getSourceFromSink(fieldConfigurationSchemeGV, "Project", SchemeManager.PROJECT_ASSOCIATION, false);
        }
    ...
    public Collection<GenericValue> getProjects(FieldLayoutScheme fieldLayoutScheme)
    {
        try
        {
            return CoreFactory.getAssociationManager().getSourceFromSink(fieldLayoutScheme.getGenericValue(), "Project", SchemeManager.PROJECT_ASSOCIATION, false);
        }
        catch (GenericEntityException e)
        {
            throw new DataAccessException(e);
    ...
    public FieldLayoutScheme createFieldLayoutScheme(FieldLayoutScheme fieldLayoutScheme)
    {
        try
        {
            GenericValue genericValue = EntityUtils.createValue(SCHEME, MapBuilder.build("name", fieldLayoutScheme.getName(), "description", fieldLayoutScheme.getDescription()));
    ...
    public FieldConfigurationScheme getFieldConfigurationScheme(final Long schemeId)
    {
        ...
        final GenericValue fieldLayoutSchemeGV = ofBizDelegator.findByPrimaryKey(SCHEME, MapBuilder.build("id", schemeId));
    ...
    {
        return buildFieldLayoutScheme(ofBizDelegator.findByPrimaryKey(SCHEME, MapBuilder.build("id", schemeId)));
    }
    ...
    public FieldConfigurationScheme getFieldConfigurationScheme(GenericValue project)
    {
        ...
        try
        {
            ...
            GenericValue fieldLayoutSchemeGV = EntityUtil.getOnly(CoreFactory.getAssociationManager().getSinkFromSource(project, FIELD_LAYOUT_SCHEME_ASSOCIATION, SchemeManager.PROJECT_ASSOCIATION, false));
            if (fieldLayoutSchemeGV != null)
            {
    ...
    public void addSchemeAssociation(GenericValue project, Long fieldLayoutSchemeId)
    {
        ...
        try
        {
            ...
            if (fieldLayoutSchemeId != null)
            {
                ...
                CoreFactory.getAssociationManager().createAssociation(project, gvFieldLayoutScheme, SchemeManager.PROJECT_ASSOCIATION);
            }
            clearCaches();
        }
        catch (GenericEntityException ex)
    ...
    public void removeSchemeAssociation(GenericValue project, Long fieldLayoutSchemeId)
    {
        ...
        try
        {
            CoreFactory.getAssociationManager().removeAssociation(project, gvFieldLayoutScheme, SchemeManager.PROJECT_ASSOCIATION);
        }
        catch (GenericEntityException ex)
        {
            throw new DataAccessException(ex);
