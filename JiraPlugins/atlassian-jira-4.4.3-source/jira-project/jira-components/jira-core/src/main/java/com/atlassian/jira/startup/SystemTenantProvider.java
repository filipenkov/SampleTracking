    ...
    public DatabaseConfig getSystemDatabaseConfig()
    {
        ...
        else
        {
            ...
            if (datasourceInfo != null)
            {
                ...
                {
                    datasource = new JndiDatasource(datasourceInfo.getJndiDatasource().getJndiName());
                }
                else if(datasourceInfo.getJdbcDatasource() != null)
                {
                    JdbcDatasourceInfo jdbc = datasourceInfo.getJdbcDatasource();
                    final ConnectionPoolInfo connectionPool = jdbc.getConnectionPoolInfo();
                    datasource = new JdbcDatasource(jdbc.getUri(), jdbc.getDriverClassName(), jdbc.getUsername(), jdbc.getPassword(), jdbc.getConnectionProperties(),
                            connectionPool.getMaxSize(), connectionPool.getValidationQuery(), connectionPool.getMinEvictableTimeMillis(), connectionPool.getTimeBetweenEvictionRunsMillis());
                }
                else
                {
