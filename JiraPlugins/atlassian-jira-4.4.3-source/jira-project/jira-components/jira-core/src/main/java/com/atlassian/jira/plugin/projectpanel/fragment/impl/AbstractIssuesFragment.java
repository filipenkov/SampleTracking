...
public abstract class AbstractIssuesFragment extends AbstractMultiFragment
{
    ...
    public AbstractIssuesFragment(final JiraAuthenticationContext jiraAuthenticationContext, final VelocityManager velocityManager, final SearchProvider searchProvider, final ApplicationProperties applicationProperites, DateTimeFormatterFactory dateTimeFormatterFactory)
    {
    ...
    {
        return getDisplayIssueCount() > 0 && !getIssues(ctx).isEmpty();
    }
    ...
    {
        String displayIssueCount = applicationProperites.getDefaultBackedString(APKeys.JIRA_PROJECT_SUMMARY_MAX_ISSUES);
        try
        {
    ...
    List<Issue> getIssues(final BrowseContext ctx)
    {
        try
        {
            ...
            SearchResults results = searchProvider.search(sr.getQuery(), jiraAuthenticationContext.getUser(), pagerFilter);
    ...
    {
        final VelocityRequestContext velocityRequestContext = new DefaultVelocityRequestContextFactory(applicationProperites).getJiraVelocityRequestContext();
        final StringBuilder link = new StringBuilder();
        link.append(SearchRequestViewUtils.getLink(sr, velocityRequestContext.getBaseUrl(), jiraAuthenticationContext.getUser()))
                .append("&mode=hide");
