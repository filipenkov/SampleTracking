...
public class QuickSearch extends ProjectActionSupport
{
    ...
    public QuickSearch(ProjectManager projectManager, QueryCreator queryCreator, PermissionManager permissionManager)
    {
        super(projectManager, permissionManager);
        this.queryCreator = queryCreator;
    }

    public QuickSearch(QueryCreator queryCreator)
    {
        this.queryCreator = queryCreator;
    }
    ...
    public String getKey() throws Exception
    {
        ...
        else
        {
            try
            {
                ...
                GenericValue searchProject = getSearchProject();

                //if we find a project - return a key
                if (searchProject != null)
                {
                    return searchProject.getString("key") + "-" + (potentialKey.startsWith("-") ? potentialKey.substring(1, potentialKey.length()) : potentialKey);
                }
    ...
    private GenericValue getSearchProject() throws PermissionException
    {
        // try the issue with the current project key on the front (maybe they just put in a number)
        final GenericValue project = getSelectedProject();
        if (project != null)
        {
            return project;
        }
        else if (getBrowseableProjects().size() == 1)
        {
            // otherwise if there is only one project - try that one
            return (GenericValue) getBrowseableProjects().iterator().next();
        }
        else
        {
