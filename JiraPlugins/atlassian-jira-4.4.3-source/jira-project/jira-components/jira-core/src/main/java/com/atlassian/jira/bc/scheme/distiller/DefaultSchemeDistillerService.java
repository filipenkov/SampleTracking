...
public class DefaultSchemeDistillerService implements SchemeDistillerService
{
    ...
    @Override
    public DistilledSchemeResults distillSchemes(com.opensymphony.user.User user, Collection schemes, ErrorCollection errorCollection)
    {
        return distillSchemes((User) user, schemes, errorCollection);
    }
    ...
    @Override
    public Scheme persistNewSchemeMappings(com.opensymphony.user.User user, DistilledSchemeResult distilledSchemeResult, ErrorCollection errorCollection)
            throws DataAccessException
    {
        return persistNewSchemeMappings((User) user, distilledSchemeResult, errorCollection);
    }
    ...
    private void validateDistilledSchemeResultIntegrity(User user, DistilledSchemeResult distilledSchemeResult, ErrorCollection errorCollection)
    {
        ...
        if (!errorCollection.hasAnyErrors())
        {
            ...
            StringBuffer originalSchemesWithErrors = new StringBuffer();
            for (Iterator iterator = distilledSchemeResult.getOriginalSchemes().iterator(); iterator.hasNext();)
            {
                Scheme originalScheme = (Scheme) iterator.next();
    ...
    @Override
    public SchemeRelationships getSchemeRelationships(com.opensymphony.user.User user, DistilledSchemeResults distilledSchemeResults, ErrorCollection errorCollection)
    {
        return getSchemeRelationships((User) user, distilledSchemeResults, errorCollection);
    }
    ...
    @Override
    public void isValidNewSchemeName(com.opensymphony.user.User user, String fieldName, String newSchemeName, String schemeType, ErrorCollection errorCollection)
    {
        isValidNewSchemeName((User) user, fieldName, newSchemeName, schemeType, errorCollection);
    }
