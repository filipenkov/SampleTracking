    ...
    protected String doExecute() throws Exception
    {
        if (taskId != null)
        {
            ...
            if ((descriptor != null) && descriptor.isFinished())
            {
                ...
                if (currentUser != null)
                {
                    if (!authenticationContext.getUser().equals(descriptor.getUser()))
                    {
                        addErrorMessage(getText("common.tasks.cant.acknowledge.task.you.didnt.start", descriptor.getUser().getName()));
                        return ERROR;
                    }
                }
