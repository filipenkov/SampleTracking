...
public class UserBean
{
    ...
    {
        final List userBeans = new ArrayList();
        final UserUtil userUtil = ComponentAccessor.getUserUtil();
        for (final Iterator it = usernames.iterator(); it.hasNext();)
        {
            ...
            {
                userBeans.add(new UserBean(username, getUserFullName(username)));
            }
        }
    ...
    public static List/*<UserBean>*/convertUsersToUserBeans(final Locale userLocale, final Collection/*<User>*/users)
    {
        ...
        for (final Iterator it = users.iterator(); it.hasNext();)
        {
            ...
            userBeans.add(new UserBean(user.getName(), getUserFullName(user.getName())));
        }
        Collections.sort(userBeans, new UserBeanComparator(userLocale));
        return userBeans;
    }
    ...
    private static String getUserFullName(final String username)
    {
        final UserFormatManager userFormatManager = ComponentManager.getComponentInstanceOfType(UserFormatManager.class);
        return userFormatManager.formatUser(username, FullNameUserFormat.TYPE, "user-bean");
    }
