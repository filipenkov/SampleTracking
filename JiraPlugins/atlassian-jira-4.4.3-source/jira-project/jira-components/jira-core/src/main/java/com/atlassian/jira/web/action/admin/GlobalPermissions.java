...
public class GlobalPermissions extends ProjectActionSupport
{
    ...
    public void doValidation()
    {
        if (permType >= 0)
        {
            ...
            else
            {
                ...
                if (permType == Permissions.USE && Actions.ADD.equals(action))
                {
    ...
    protected void validateDelete()
    {
        if (Actions.DEL.equals(action) || Actions.CONFIRM.equals(action))
        {
            ...
            {
                boolean removingAllPerms = globalPermissionGroupAssociationUtil.isRemovingAllMyAdminGroups(EasyList.build(groupName), getRemoteUser())
                                           && !globalPermissionManager.hasPermission(Permissions.SYSTEM_ADMIN, getRemoteUser());
            ...
            {
                if (!globalPermissionManager.hasPermission(Permissions.SYSTEM_ADMIN, getRemoteUser()))
                {
                    addErrorMessage(getText("admin.errors.permissions.no.permission.sys.admin.only"));
                }
                else if (globalPermissionGroupAssociationUtil.isRemovingAllMySysAdminGroups(EasyList.build(groupName), getRemoteUser()))
                {
    ...
    private Collection<Group> getAdministrativeGroups() {
        final Collection<Group> groups = new ArrayList<Group>(globalPermissionManager.getGroups(Permissions.ADMINISTER));
        groups.addAll(globalPermissionManager.getGroups(Permissions.SYSTEM_ADMIN));
