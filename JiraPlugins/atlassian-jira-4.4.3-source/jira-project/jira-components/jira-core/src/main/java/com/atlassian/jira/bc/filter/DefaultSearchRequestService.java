...
public class DefaultSearchRequestService implements SearchRequestService
{
    ...
    @Override
    public Collection<SearchRequest> getFavouriteFilters(com.opensymphony.user.User user)
    {
        return getFavouriteFilters((User) user);
    }
    ...
    @Override
    public Collection<SearchRequest> getOwnedFilters(com.opensymphony.user.User user)
    {
        return getOwnedFilters((User) user);
    }
    ...
    @Override
    public Collection<SearchRequest> getNonPrivateFilters(com.opensymphony.user.User user)
    {
        return getNonPrivateFilters((User) user);
    }
    ...
    @Override
    public Collection<SearchRequest> getFiltersFavouritedByOthers(com.opensymphony.user.User user)
    {
        return getFiltersFavouritedByOthers((User) user);
    }
    ...
    public void deleteFilter(final JiraServiceContext serviceCtx, final Long filterId)
    {
        ...
        final User user = serviceCtx.getLoggedInUser();
    ...
    @Override
    public void deleteAllFiltersForUser(JiraServiceContext serviceCtx, com.opensymphony.user.User user)
    {
        deleteAllFiltersForUser(serviceCtx, (User) user);
    }
    ...
    public SearchRequest getFilter(final JiraServiceContext serviceCtx, final Long filterId)
    {
        ...
        final SearchRequest filter = searchRequestManager.getSearchRequestById(serviceCtx.getLoggedInUser(), filterId);
        if (filter == null)
        {
    ...
    public void validateFilterForUpdate(final JiraServiceContext serviceCtx, final SearchRequest request)
    {
        ...
        if (this.validateFilterName(serviceCtx, request))
        {
            ...
            {
                final SearchRequest filterByName = searchRequestManager.getOwnedSearchRequestByName(serviceCtx.getLoggedInUser(), request.getName());
    ...
    public boolean validateUpdateSearchParameters(final JiraServiceContext serviceCtx, final SearchRequest request)
    {
        ...
        {
            final String currentOwnerUsername = searchRequestManager.getSearchRequestOwnerUserName(request.getId());
            if (currentOwnerUsername == null)
            {
                serviceCtx.getErrorCollection().addErrorMessage(serviceCtx.getI18nBean().getText("admin.errors.filters.not.saved"));
            }
            else if (!userNameEqualsUtil.equals(currentOwnerUsername, serviceCtx.getLoggedInUser()) || request.getOwnerUserName() == null ||
                     !userNameEqualsUtil.equals(request.getOwnerUserName(), serviceCtx.getLoggedInUser()))
            {
    ...
    public void validateFilterForCreate(final JiraServiceContext serviceCtx, final SearchRequest request)
    {
        ...
        {
            if ((request.getOwnerUserName() == null) || !userNameEqualsUtil.equals(request.getOwnerUserName(), serviceCtx.getLoggedInUser()))
            {
                serviceCtx.getErrorCollection().addErrorMessage(serviceCtx.getI18nBean().getText("admin.errors.filters.must.be.owner"));
            }
        }
    ...
    public void validateForDelete(final JiraServiceContext serviceCtx, final Long filterId)
    {
        ...
        {
            final String ownerUsername = searchRequestManager.getSearchRequestOwnerUserName(filterId);
            if (ownerUsername == null)
            {
                serviceCtx.getErrorCollection().addErrorMessage(serviceCtx.getI18nBean().getText("admin.errors.filters.not.saved"));
            }
            else if (!userNameEqualsUtil.equals(ownerUsername, serviceCtx.getLoggedInUser()) &&
                    !permissionManager.hasPermission(Permissions.ADMINISTER, serviceCtx.getLoggedInUser()))
            {
                serviceCtx.getErrorCollection().addErrorMessage(serviceCtx.getI18nBean().getText("admin.errors.filters.must.be.owner"));
            }
        }
    ...
    public SearchRequest createFilter(final JiraServiceContext serviceCtx, final SearchRequest request, final boolean isFavourite)
    {
        ...
        if (!serviceCtx.getErrorCollection().hasAnyErrors())
        {
            if (isFavourite)
            {
                try
                {
                    favouritesManager.addFavourite(serviceCtx.getLoggedInUser(), filter);
                }
            ...
            {
                favouritesManager.removeFavourite(serviceCtx.getLoggedInUser(), filter);
            }
        }
        return filter;
    }
    ...
    public SearchRequest updateSearchParameters(final JiraServiceContext serviceCtx, final SearchRequest request)
    {
        ...
        {
            final SearchRequest databaseRequest = searchRequestManager.getSearchRequestById(serviceCtx.getLoggedInUser(), request.getId());
            if (databaseRequest != null)
            {
                final SearchRequest newRequest = new SearchRequest(request.getQuery(), databaseRequest.getOwnerUserName(), databaseRequest.getName(),
                        databaseRequest.getDescription(), databaseRequest.getId(), databaseRequest.getFavouriteCount());
    ...
    public SharedEntitySearchResult<SearchRequest> search(final JiraServiceContext serviceCtx, final SharedEntitySearchParameters searchParameters, final int pagePosition, final int pageWidth)
    {
        ...
        return searchRequestManager.search(searchParameters, serviceCtx.getLoggedInUser(), pagePosition, pageWidth);
    }
    ...
    private boolean checkPermissionsForUpdateSearchParameters(final JiraServiceContext ctx, final SearchRequest request)
    {
        ...
        else if ((request.getOwnerUserName() == null) || !userNameEqualsUtil.equals(request.getOwnerUserName(), ctx.getLoggedInUser()))
        {
        ...
        else
        {
            ...
            else if (!userNameEqualsUtil.equals(currentOwnerUsername, ctx.getLoggedInUser()))
            {
                ctx.getErrorCollection().addErrorMessage(ctx.getI18nBean().getText("admin.errors.filters.must.be.owner"));
            }
        }
    ...
    private boolean checkPermissionsForOwnerUpdateSearchParameters(final JiraServiceContext ctx, User user, final SearchRequest request)
    {
        if (request.getId() == null)
        {
            ctx.getErrorCollection().addErrorMessage(ctx.getI18nBean().getText("admin.errors.filters.not.saved"));
        ...
        else if ((request.getOwnerUserName() == null) || !permissionManager.hasPermission(Permissions.ADMINISTER, user))
        {
    ...
    private boolean checkPermissionsForCreate(final JiraServiceContext ctx, final SearchRequest request)
    {
        ...
        if ((request.getOwnerUserName() == null) || !userNameEqualsUtil.equals(request.getOwnerUserName(), ctx.getLoggedInUser()))
        {
            ctx.getErrorCollection().addErrorMessage(ctx.getI18nBean().getText("admin.errors.filters.must.be.owner"));
        }
