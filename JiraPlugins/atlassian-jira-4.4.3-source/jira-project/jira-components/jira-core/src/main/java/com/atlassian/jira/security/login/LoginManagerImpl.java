...
public class LoginManagerImpl implements LoginManager
{
    ...
    ///CLOVER:OFF
    public LoginManagerImpl(final PermissionManager permissionManager, final LoginStore loginStore, final JiraAuthenticationContext jiraAuthenticationContext, final CrowdService crowdService, final JiraCaptchaService jiraCaptchaService, VelocityRequestContextFactory velocityRequestContextFactory)
    {
    ...
    public LoginManagerImpl(final PermissionManager permissionManager, final LoginStore loginStore, final JiraAuthenticationContext jiraAuthenticationContext, final CrowdService crowdService, final JiraCaptchaService jiraCaptchaService, final StaticDependencies staticDependencies, VelocityRequestContextFactory velocityRequestContextFactory)
    {
    ...
    public boolean authorise(final User user, final HttpServletRequest httpServletRequest)
    {
        notNull("user", user);

        httpServletRequest.removeAttribute(AUTHORISED_FAILURE);

        final boolean authorised = permissionManager.hasPermission(Permissions.ADMINISTER, user) || permissionManager.hasPermission(Permissions.USE, user);
        if (!authorised)
        {
            //
            // because of the call back nature of Seraph we cant tell later the difference between elevated failure / authentication / authorisation failure
    ...
    public LoginResult authenticate(final User user, final String password)
    {
        ...
        //
        logSecurityEvents(userName, loginInfo, reason);
    ...
    public void logout(final HttpServletRequest request, final HttpServletResponse response)
    {
        ...
        String userName = jiraAuthenticationContext.getUser() == null ? "unknown" : jiraAuthenticationContext.getUser().getName();
    ...
    private void logSecurityEvents(final String userName, final LoginInfo loginInfo, final LoginReason reason)
    {
