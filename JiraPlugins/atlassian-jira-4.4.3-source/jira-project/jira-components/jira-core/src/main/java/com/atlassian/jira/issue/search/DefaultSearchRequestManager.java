...
public class DefaultSearchRequestManager implements SearchRequestManager
{
    ...
    {
        return Transformed.enclosedIterable(searchRequestStore.get(descriptor), createPermissionSanitisingResolver(user));
    }
    ...
    @Override
    public Collection<SearchRequest> getAllOwnedSearchRequests(com.opensymphony.user.User user)
    {
        return getAllOwnedSearchRequests((User) user);
    }
    ...
    @Override
    public SearchRequest getOwnedSearchRequestByName(com.opensymphony.user.User author, String name)
    {
        return getOwnedSearchRequestByName((User) author, name);
    }
    ...
    @Override
    public SearchRequest getSearchRequestById(com.opensymphony.user.User user, Long id)
    {
        return getSearchRequestById((User) user, id);
    }
    ...
    public SearchRequest create(final SearchRequest request)
    {
        ...
        return setSanitisedQuery(userUtil.getUserObject(searchRequest.getOwnerUserName()), searchRequest);
    }
     ...
     */
    SearchRequest setSanitisedQuery(final User searcher, final SearchRequest filter)
    {
        if (filter != null)
        {
    ...
    private boolean isSharedWith(final SearchRequest entity, final User user)
    {
    ...
    @Override
    public List<SearchRequest> getVisibleRequests(final com.opensymphony.user.User user)
    {
        final EnclosedIterable<SearchRequest> requestList = searchRequestStore.getAll();
        final List<SearchRequest> returnList = new ArrayList<SearchRequest>();

        requestList.foreach(new Consumer<SearchRequest>()
        {
            public void consume(final SearchRequest searchRequest)
            {
                if (isSharedWith(searchRequest, user))
                {
                    returnList.add(setSanitisedQuery(user, searchRequest));
                }
            }
        });
        Collections.sort(returnList, FilterNameComparator.COMPARATOR);
        return returnList;
    }

    @Override
    public SearchRequest getRequest(final com.opensymphony.user.User searcher, final Long id)
    {
        return getSearchRequestById(searcher, id);
    }

    @Override
    public SearchRequest getSearchRequest(final com.opensymphony.user.User searcher, final Long id)
    {
        return getSearchRequestById(searcher, id);
    }

    @Override
    public SharedEntitySearchResult<SearchRequest> search(SharedEntitySearchParameters searchParameters, com.opensymphony.user.User user, int pagePosition, int pageWidth)
    {
