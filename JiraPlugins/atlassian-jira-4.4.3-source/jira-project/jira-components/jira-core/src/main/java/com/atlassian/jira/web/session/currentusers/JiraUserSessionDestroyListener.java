...
public class JiraUserSessionDestroyListener implements HttpSessionListener
{
    ...
    private void destroySession(final HttpSession session)
    {
        ...
        Object userName = session.getAttribute(DefaultAuthenticator.LOGGED_IN_KEY);
        loggerSecurityEvents.info("HttpSession [" + encodeSessionId(session.getId()) + "] destroyed for '" + (userName == null ? "anonymous" : userName) + "'");
    }
