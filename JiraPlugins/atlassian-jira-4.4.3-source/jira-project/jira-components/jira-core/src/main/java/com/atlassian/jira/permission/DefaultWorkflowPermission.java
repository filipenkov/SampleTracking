...
public class DefaultWorkflowPermission implements WorkflowPermission
{
    ...
    public final boolean allows(int permission, Issue issue, com.opensymphony.user.User user)
    {
    ...
    {
        if (user == null) return grantType.hasPermission(issue.getGenericValue(), getValueFor(issue));
        else return grantType.hasPermission(issue.getGenericValue(), getValueFor(issue), user, false);
    }
