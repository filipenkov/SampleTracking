...
public class UpgradeTask_Build601 extends AbstractUpgradeTask
{
    ...
    public void doUpgrade(boolean setupMode) throws Exception
    {
        ...
        DirectoryDao directoryDao = ComponentManager.getComponentInstanceOfType(DirectoryDao.class);
    ...
    private boolean validatePresentConfiguration(final InputStream osuserStream) throws SQLException
    {
        ...
        if (areExternalUsersPresent())
        {
            ...
            addError(getI18nBean().getText("admin.errors.upgrade.601.error.missing.osuser.xml", upgradeGuideUrl, upgradeGuideTitle));
            return false;
        }
        else if (isExternalUserManagement())
        {
            // External password management, but we don't have an osuser.xml to migrate the config.
            addError(getI18nBean().getText("admin.errors.upgrade.601.error.missing.extrenal.user.management", upgradeGuideUrl, upgradeGuideTitle));
            return false;
        }
        else if (isExternalPasswordManagement())
        {
            // External password management, but we don't have an osuser.xml to migrate the config.
            addError(getI18nBean().getText("admin.errors.upgrade.601.error.missing.extrenal.password.management", upgradeGuideUrl, upgradeGuideTitle));
            return false;
        }
        else
        {
