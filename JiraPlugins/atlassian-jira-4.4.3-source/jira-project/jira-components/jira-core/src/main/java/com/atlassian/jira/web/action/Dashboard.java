...
public class Dashboard extends JiraWebActionSupport
{
    ...
    protected String doExecute() throws Exception
    {
        ...
        {
            userHistoryManager.addItemToHistory(UserHistoryItem.DASHBOARD, getLoggedInUser(),
        ...
        {
            userHistoryManager.addItemToHistory(UserHistoryItem.DASHBOARD, getLoggedInUser(), String.valueOf(selectPageId));
        }
    ...
    public DashboardState getCurrentDashboardState() throws PermissionException
    {
        if (currentDashboardState == null)
        {
            ...
            if (getRemoteUser() == null && isSystemDashboardId(getCurrentDashboardId()) && !applicationProperties.getOption(APKeys.JIRA_DISABLE_LOGIN_GADGET))
            {
            ...
            currentDashboardState = gadgetPermissionManager.filterGadgets(currentDashboardState, getRemoteUser());
        }
    ...
    {
        final LoginProperties loginProperties = loginService.getLoginProperties(getRemoteUser(), request);
        return MapBuilder.<String, String>newBuilder().
                ...
                add("loginFailedByPermissions", String.valueOf(loginProperties.getLoginFailedByPermissions())).toMap();
    }

    public boolean isShowWhitelistUpgradeWarning()
    {
        return gadgetApplinkUpgradeUtil.isUpgradeRequired(getLoggedInUser());
    }
    ...
    public String getWarningMessage()
    {
        ...
        if (selectPageId != null)
        {
            ...
            boolean currentPortalPageFavourite = portalPageService.isFavourite(getRemoteUser(), portalPage);
        ...
        if (newWarningMessage == null)
        {
            if (!isUserCurrentPageOwner(portalPage))
            {
                ...
                {
                    if (portalPageService.isFavourite(getRemoteUser(), portalPage))
                    {
    ...
    {
        return portalPageService.isFavourite(getRemoteUser(), portalPageService.getPortalPage(getJiraServiceContext(), pageId));
    }
    ...
    private static String createFavouriteLink(final PortalPage portalPage)
    {
        StringBuffer buffer = new StringBuffer("AddFavourite.jspa?entityType=").append(portalPage.getEntityType().getName());
    ...
    private static String createUnfavouriteLink(final PortalPage portalPage)
    {
        StringBuffer buffer = new StringBuffer("RemoveFavourite.jspa?entityType=").append(portalPage.getEntityType().getName());
        buffer.append("&entityId=").append(portalPage.getId());
    ...
    {
        return currentPortalPage != null && currentPortalPage.getOwnerUserName() != null && userNameEqualsUtil.equals(currentPortalPage.getOwnerUserName(), getRemoteUser());
    }
    ...
    private void initialiseCurrentDashboardId()
    {
        ...
        else
        {
            ...
            {
                final List<UserHistoryItem> history = userHistoryManager.getHistory(UserHistoryItem.DASHBOARD, getLoggedInUser());
                if (history.isEmpty())
                {
                    pageId = getPageIdForUser();
                    userHistoryManager.addItemToHistory(UserHistoryItem.DASHBOARD, getLoggedInUser(), String.valueOf(pageId));
                }
                else
                {
                    pageId = Long.valueOf(history.get(0).getEntityId());
                    final PortalPage portalPage = portalPageService.getPortalPage(new JiraServiceContextImpl(getLoggedInUser(), new SimpleErrorCollection()), pageId);
                    //if this page doesn't exist any longer or it's not a favourite the history must be stale!
                    if(portalPage == null || !portalPageService.isFavourite(getLoggedInUser(), portalPage))
                    {
                        pageId = getPageIdForUser();
                        userHistoryManager.addItemToHistory(UserHistoryItem.DASHBOARD, getLoggedInUser(), String.valueOf(pageId));
                    }
    ...
    private List<DashboardTab> getUserTabs()
    {
        if (userTabs == null)
        {
            userTabs = transform(portalPageService.getFavouritePortalPages(getRemoteUser()), new Function<PortalPage, DashboardTab>()
            {
