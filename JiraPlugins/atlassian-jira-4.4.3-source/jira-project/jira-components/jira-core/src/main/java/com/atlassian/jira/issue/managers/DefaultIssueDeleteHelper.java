...
public class DefaultIssueDeleteHelper implements IssueDeleteHelper
{
    ...
    public DefaultIssueDeleteHelper(IssueIndexManager indexManager, SubTaskManager subTaskManager,
            IssueLinkManager issueLinkManager, MailThreadManager mailThreadManager, CustomFieldManager customFieldManager,
    ...
    private void removeIssueLinks(User user, GenericValue issueGV) throws RemoveException
    {
        ...
        if (subTaskManager.isSubTask(issueGV))
        {
            // Get the parent issue before removing the links, as we need the link to determine the parent issue
            Issue parentIssue = issueManager.getIssueObject(subTaskManager.getParentIssueId(issueGV));
            // Remove the links
            issueLinkManager.removeIssueLinks(issueGV, user);
    ...
    private void removeAttachments(MutableIssue issue) throws RemoveException
    {
            ...
            throws Exception
    {
        ...
        {
            MutableIssue subTaskIssue = subTaskIssueLink.getDestinationObject();
    ...
    private void deindex(MutableIssue issue, GenericValue issueGV)
    {
        try
        {
    ...
    /**
     * <p/>
     * Holds the state of the deleted issue object more consistent before we it gets deleted. Makes parentId (and thus
     * information, if the issue is a sub-task) accessible in thread local cache and collects custom fields values (also
     * making them accessible in thread-local cache).
     * <p/>
     * <p/>
     * See also:<br> http://jira.atlassian.com/browse/JRA-12091<br> http://jira.atlassian.com/browse/JRA-24331<br>
