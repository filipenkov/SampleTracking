...
public class DevModeSecretSauce
{
    ...
    public DevModeSecretSauce()
    {
        final File secretSauce = new File(System.getProperty("user.home") + "/.jiradev/secretsauce.properties");
    ...
    *
    ...
    /**
     * This will return an redirect to a specific URL if the secret sauce is in play
     *
     * @param result the desired action result
     * @return the same result or a redirect URL
     */
    String redirectUrlOnSetupComplete(String result)
    {
        if (isBoneFideJiraDeveloper() && Action.SUCCESS.equals(result))
        {
            final String replacementURL = getSecretSauceProperty("redirecturl");
            if (StringUtils.isNotBlank(replacementURL))
            {
                final String requestURI = ActionContext.getRequest().getRequestURI();
                return requestURI.replaceAll("/.*jspa$", replacementURL);
            }
        }
        return result;
    }
