...
public class OfBizCustomFieldValuePersister implements CustomFieldValuePersister
{
    ...
    public List getValues(CustomField field, Long issueId, PersistenceFieldType persistenceFieldType)
    {
        return getValues(field, issueId, persistenceFieldType, null);
    }


    public List getValues(CustomField field, Long issueId, PersistenceFieldType persistenceFieldType, String parentKey)
    {
        List genericValues = getValuesForTypeAndParent(field, issueId, parentKey);
    ...
    private static List getValuesFromGenericValues(List<GenericValue> genericValues, PersistenceFieldType persistenceFieldType)
    {
        List values = new ArrayList(genericValues.size());
        for (GenericValue genericValue : genericValues)
        {
    ...
    {
        List GVs = getValuesForType(field, issueId);
    ...
    {
        List GVs = getValuesForTypeAndParent(field, issueId, parentKey);
        delegator.removeAll(GVs);
        if (values != null && !values.isEmpty())
        {
    ...
    public Set<Long> removeValue(CustomField field, Long issueId, PersistenceFieldType persistenceFieldType, Object value)
    {
        final Map<String, Object> limitClause =
               EasyMap.build(ENTITY_CUSTOMFIELD_ID, CustomFieldUtils.getCustomFieldId(field.getId()),
    ...
    public Set removeAllValues(final CustomField customField)
    {
        return removeAllValues(customField.getId());
    }
    ...
    public Set<Long> removeAllValues(String customFieldId)
    {
        ...
        final Map<String, Object> limitClause = EasyMap.build(ENTITY_CUSTOMFIELD_ID, CustomFieldUtils.getCustomFieldId(customFieldId));
        List<GenericValue> values = delegator.findByAnd(TABLE_CUSTOMFIELD_VALUE, limitClause);
    ...
    {
        final Map<String, Object> limitClause = EasyMap.build(ENTITY_ISSUE_ID, issueId,
