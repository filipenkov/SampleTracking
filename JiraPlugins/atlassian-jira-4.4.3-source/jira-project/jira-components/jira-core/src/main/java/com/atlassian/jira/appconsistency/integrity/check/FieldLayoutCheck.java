...
public class FieldLayoutCheck extends CheckImpl
{
    ...
    private List doCheck(boolean correct) throws IntegrityException
    {
        ...
        HashSet fieldsToRemove = new HashSet();
        ...
        try
        {
            ...
            {
                // Fix the problem
                for (Iterator iterator = fieldsToRemove.iterator(); iterator.hasNext();)
                {
                    String fieldLayoutItemId = (String) iterator.next();
                    ofBizDelegator.removeByAnd("FieldLayoutItem", EasyMap.build("fieldidentifier", fieldLayoutItemId));
