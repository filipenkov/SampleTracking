        ...
        implements SharedEntitySearchAction, FilterOperationsAction, DropDownModelProvider<SearchRequestDisplayBean>
{
    ...
    public ManageFilters(final JiraAuthenticationContext authCtx, final IssueSearcherManager issueSearcherManager,
            ...
            final SearchRequestDisplayBean.Factory beanFactory, final SearchService searchService, final SearchSortUtil searchSortUtil,
    ...
    public DropDownModel getDropDownModel(final SearchRequestDisplayBean displayBean, final int listIndex)
    {
        ...
        if (displayBean.isCurrentOwner())
        {
            ...
            if (displayBean.canEditColumns())
            {
                builder.addItem(
                        builder.item()
                                .setText(getText("managefilters.columns"))
                                .setAttr("id", "colOrder_" + displayBean.getId())
                                .setAttr("href", toUrl(displayBean, "ViewSearchRequestIssueColumns!default.jspa", false))
                );
            }
            builder.endSection();
        }
    ...
    private FilterViewTabs getTabs()
    {
        if (validTabs == null)
        {
            ...
            {
                validTabs = new FilterViewTabs(EasyList.build(FilterViewTabs.POPULAR, FilterViewTabs.SEARCH), FilterViewTabs.POPULAR);
            }
            else
            {
                validTabs = new FilterViewTabs(EasyList.build(FilterViewTabs.FAVOURITES, FilterViewTabs.MY, FilterViewTabs.POPULAR,
    ...
    public List<SearchRequestDisplayBean> getFiltersWithSubscriptions()
    {
        if (subscribedFilters == null)
        {
            if (filters == null)
            {
                subscribedFilters = Collections.emptyList();
            }
            else
            {
                subscribedFilters = new ArrayList<SearchRequestDisplayBean>();
                for (Object search : filters)
                {
                    SearchRequestDisplayBean bean = (SearchRequestDisplayBean) search;
                    if (bean.getSubscriptionCount() > 0)
                    {
                        subscribedFilters.add(bean);
                    }
                }
            }
    ...
    {
        final Collection<SearchRequest> favouriteFilters = searchRequestService.getFavouriteFilters(getLoggedInUser());
    ...
    {
        final Collection<SearchRequest> myFilters = searchRequestService.getOwnedFilters(getLoggedInUser());
