...
public abstract class AbstractVersionsFunction extends AbstractJqlFunction implements ClauseSanitisingJqlFunction
{
    ...
    public List<QueryLiteral> getValues(final QueryCreationContext queryCreationContext, final FunctionOperand operand, final TerminalClause terminalClause)
    {
        ...
        {
            final Collection<Version> versions = getAllVersions();
            for (Version version : versions)
            {
                // only use versions the searcher has access to view
                User user = queryCreationContext.getQueryUser();
                if (queryCreationContext.isSecurityOverriden() || permissionManager.hasPermission(Permissions.BROWSE, version.getProjectObject(), user))
                {
