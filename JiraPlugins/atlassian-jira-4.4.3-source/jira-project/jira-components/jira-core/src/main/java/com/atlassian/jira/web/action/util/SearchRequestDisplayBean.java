...
import com.atlassian.jira.web.action.admin.GlobalPermissions;
...
public class SearchRequestDisplayBean implements SharesListHelper
{
    ...
    {
        return request.getOwnerUserName();
    }

    public String getOwnerFullName()
    {
        final String userName = getOwnerUserName();
        if (userName != null)
        {
            return userFormatManager.formatUser(userName, FullNameUserFormat.TYPE, "search_request");
        }
        return null;
    }
    ...
    {
        return userNameEqualsUtil.equals(getOwnerUserName(), authCtx.getUser());
    }
    ...
    public long getIssueCount()
    {
        if (issueCount == -1)
        {
            try
            {
                issueCount = searchProvider.searchCount((request != null) ? request.getQuery() : null, authCtx.getUser());
            }
    ...
    {
        return permissionManager.hasPermission(Permissions.CREATE_SHARED_OBJECTS, authCtx.getUser());
    }
            ...
            else
            {
                ...
                for (final SharePermission sharePermission : request.getPermissions())
                {
                    ...
                    if (type != null)
                    {
                        ...
                        if (permissionChecker.hasPermission(authCtx.getUser(), sharePermission))
                        {
    ...
    {
        if (permissionManager.hasPermission(Permissions.ADMINISTER, authCtx.getUser()))
        {
