...
public class CommentDocument
{
    public static Document getDocument(Comment comment, Issue issue)
    {
        ...
        if (body != null)
        {
            ...
            doc.add(new Field(DocumentConstants.PROJECT_ID, String.valueOf(issue.getProjectObject().getId()), Field.Store.YES, Field.Index.UN_TOKENIZED));
            doc.add(new Field(DocumentConstants.ISSUE_ID, String.valueOf(issue.getId()), Field.Store.YES, Field.Index.UN_TOKENIZED));
            doc.add(new Field(DocumentConstants.COMMENT_ID, String.valueOf(comment.getId()), Field.Store.YES, Field.Index.UN_TOKENIZED));

            String author = comment.getAuthor();
            if (author != null) //can't add null keywords
            {
                author = CaseFolding.foldString(comment.getAuthor(), Locale.ENGLISH);
            ...
            {
                updateAuthor = CaseFolding.foldString(comment.getUpdateAuthor(), Locale.ENGLISH);
                doc.add(new Field(DocumentConstants.COMMENT_UPDATE_AUTHOR, updateAuthor, Field.Store.YES, Field.Index.NOT_ANALYZED_NO_NORMS));
            }

            doc.add(new Field(DocumentConstants.COMMENT_BODY, body, Field.Store.YES, Field.Index.TOKENIZED));

            doc.add(new Field(DocumentConstants.COMMENT_CREATED, LuceneUtils.dateToString(comment.getCreated()), Field.Store.YES, Field.Index.UN_TOKENIZED));
            Date updated = comment.getUpdated();
            if (updated != null)
            {
                doc.add(new Field(DocumentConstants.COMMENT_UPDATED, LuceneUtils.dateToString(updated), Field.Store.YES, Field.Index.UN_TOKENIZED));
            }
