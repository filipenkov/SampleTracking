...
public class IssuePicker extends JiraWebActionSupport
{
    ...
    {
        return searchRequestService.getFavouriteFilters(getRemoteUser());
    }
    ...
    {
        return filterIssues(userHistoryManager.getShortIssueHistory(getRemoteUser()));
    ...
    private Collection<Issue> executeSearch(SearchRequest searchRequest)
    {
        try
        {
            SearchResults searchResults = searchProvider.search((searchRequest == null) ? null : searchRequest.getQuery(), getRemoteUser(), PAGER_FILTER);
