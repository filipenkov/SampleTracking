 ...
 * @since 4.2
 */
public class LabelsSystemField extends AbstractOrderableNavigableFieldImpl implements HideableField, RequirableField
{
    ...
    public LabelsSystemField(VelocityManager velocityManager, ApplicationProperties applicationProperties, JiraAuthenticationContext authenticationContext,
            PermissionManager permissionManager, LabelsSearchHandlerFactory searchHandlerFactory, final LabelManager labelManager, final IssueManager issueManager)
    {
        super(IssueFieldConstants.LABELS, LABELS_NAME_KEY, velocityManager, applicationProperties, authenticationContext, permissionManager, searchHandlerFactory);
    ...
    protected LabelUtil getLabelUtil()
    {
        if (labelUtil == null)
        {
            labelUtil = ComponentManager.getComponentInstanceOfType(LabelUtil.class);
        }
    ...
    public void createValue(final Issue issue, final Object value)
    {
        ...
        labelManager.setLabels(authenticationContext.getUser(), issue.getId(), labels, false, false);
    }
            ...
            final IssueChangeHolder issueChangeHolder)
    {
        ...
        {
            final Set<Label> newLabels = labelManager.setLabels(authenticationContext.getUser(), issue.getId(), labels, false, false);
    ...
    public String getColumnViewHtml(final FieldLayoutItem fieldLayoutItem, final Map displayParams, final Issue issue)
    {
        ...
        velocityParams.put("remoteUser", authenticationContext.getUser());
        velocityParams.put("canEdit", issueManager.isEditable(issue, authenticationContext.getUser()));
    ...
    @Override
    protected Object getRelevantParams(final Map params)
    {
