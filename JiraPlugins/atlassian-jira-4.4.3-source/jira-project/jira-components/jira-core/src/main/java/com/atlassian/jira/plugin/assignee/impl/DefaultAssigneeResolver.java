...
public class DefaultAssigneeResolver implements AssigneeResolver
{
    ...
    public ErrorCollection validateDefaultAssignee(Issue issue, Map fieldValuesHolder)
    {
        ...
        catch (DefaultAssigneeException ex)
        {
            ...
            errorCollection.addError(IssueFieldConstants.ASSIGNEE, authenticationContext.getI18nHelper().getText("assign.error.unassignabledefault"));
        }
     ...
     */
    @Override
    public com.opensymphony.user.User getDefaultAssignee(Issue issue, Map fieldValuesHolder)
    {
       return OSUserConverter.convertToOSUser(getDefaultAssigneeObject(issue, fieldValuesHolder));
    }
    ...
    {
        return (ComponentsSystemField) ComponentManager.getInstance().getFieldManager().getField(IssueFieldConstants.COMPONENTS);
    }
}
