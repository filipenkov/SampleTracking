...
public class MoveIssueUpdateWorkflow extends MoveIssue
{
    ...
    public MoveIssueUpdateWorkflow(IssueLinkManager issueLinkManager, SubTaskManager subTaskManager,
    ...
    {
        Collection types = getSubTaskTypesUsed();
        Collection workflows = new ArrayList();

        for (Iterator iterator = types.iterator(); iterator.hasNext();)
        {
            GenericValue type = (GenericValue) iterator.next();
    ...
    private void populatesubTaskTargetStatusHolder() throws WorkflowException
    {
        ...
        Collection subTaskTypesUsed = getSubTaskTypesUsed();

        for (Iterator iterator = subTaskTypesUsed.iterator(); iterator.hasNext();)
        {
            GenericValue subTaskTypeGV = (GenericValue) iterator.next();
            if (subTaskTypeGV != null)
            {
                ...
                Collection statuses = getTaskInvalidStatuses(taskTypeId);
                String subTaskTypeKey = null;
                String targetStatusId = null;

                for (Iterator iterator1 = statuses.iterator(); iterator1.hasNext();)
                {
                    GenericValue status = (GenericValue) iterator1.next();
