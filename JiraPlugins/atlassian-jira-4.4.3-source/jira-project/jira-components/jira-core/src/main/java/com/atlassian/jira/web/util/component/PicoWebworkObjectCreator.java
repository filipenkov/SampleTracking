...
public class PicoWebworkObjectCreator implements ObjectCreator
{
    ...
    public Object instantiate(Class actionClass) throws IllegalAccessException, InstantiationException
    {
        ...
        DefaultPicoContainer requestContainer = new RequestComponentManager().getContainer(applicationContainer);

        //register the actionClass that we need.
        requestContainer.registerComponentImplementation(actionClass);

        try
        {
            return requestContainer.getComponentInstance(actionClass);
