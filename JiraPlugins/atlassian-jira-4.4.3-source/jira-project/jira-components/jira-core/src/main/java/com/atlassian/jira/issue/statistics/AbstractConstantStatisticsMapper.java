...
public abstract class AbstractConstantStatisticsMapper implements StatisticsMapper<GenericValue>
{
    ...
    public boolean isValidValue(final GenericValue value)
    {
        return true;
    }

    public GenericValue getValueFromLuceneField(final String documentValue)
    {
        if (TextUtils.stringSet(documentValue))
        {
            return constantsManager.getConstant(getConstantType(), documentValue);
        }
    ...
    public SearchRequest getSearchUrlSuffix(final GenericValue value, final SearchRequest searchRequest)
    {
        ...
        else
        {
            ...
            if (value != null)
            {
                final String searchValue = (value).getString("name");
