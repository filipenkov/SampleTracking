...
public class DefaultTimeTrackingIssueUpdater implements TimeTrackingIssueUpdater
{
    ...
    @Override
    public void updateIssueOnWorklogCreate(com.opensymphony.user.User user, Worklog worklog, Long newEstimate, boolean dispatchEvent)
    {
        updateIssueOnWorklogCreate((User) user, worklog, newEstimate, dispatchEvent);
    }
    ...
    @Override
    public void updateIssueOnWorklogUpdate(com.opensymphony.user.User user, Worklog originalWorklog, Worklog newWorklog, Long originalTimeSpent, Long newEstimate, boolean dispatchEvent)
    {
        updateIssueOnWorklogUpdate((User) user, originalWorklog, newWorklog, originalTimeSpent, newEstimate, dispatchEvent);
    }
    ...
    @Override
    public void updateIssueOnWorklogDelete(com.opensymphony.user.User user, Worklog worklog, Long newEstimate, boolean dispatchEvent)
    {
        updateIssueOnWorklogDelete((User) user, worklog, newEstimate, dispatchEvent);
    }
    ...
    List constructChangeItemBeansForWorklogDelete(Worklog worklog)
    {
        ...
        ChangeItemBean worklogIdBean = new ChangeItemBean(ChangeItemBean.STATIC_FIELD, "WorklogId", worklog.getId().toString(), worklog.getId().toString(), null, null);
        ChangeItemBean worklogDurationBean = new ChangeItemBean(ChangeItemBean.STATIC_FIELD, "WorklogTimeSpent", timeSpentString, message, null, null);
    ...
    void updateIssue(GenericValue issueGV, User user, Worklog originalWorklog, Worklog worklog, Long eventId, List changeItems, boolean dispatchEvent)
    {
    ...
    void doUpdate(IssueUpdateBean iub)
    {
        ...
        GenericValue changeGroup = ChangeLogUtils.createChangeGroup(iub.getUser(), iub.getOriginalIssue(), changedIssue, iub.getChangeItems(), true);
    ...
    {
        IssueEventDispatcher.dispatchEvent(iub.getEventTypeId(), issueFactory.getIssue(changedIssue), iub.getUser(), iub.getComment(), iub.getWorklog(), changeGroup, iub.getParams(), iub.isSendMail());
    }
