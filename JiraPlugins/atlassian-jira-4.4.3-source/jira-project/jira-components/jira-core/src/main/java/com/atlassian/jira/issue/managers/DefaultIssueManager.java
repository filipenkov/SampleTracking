...
public class DefaultIssueManager implements IssueManager
{
    ...
    public DefaultIssueManager(final WorkflowManager workflowManager, final AssociationManager associationManager, final TextAnalyzer textAnalyzer,
    ...
    public GenericValue getIssue(final Long id) throws DataAccessException
    {
        ...
        try
        {
            return CoreFactory.getGenericDelegator().findByPrimaryKey("Issue", EasyMap.build("id", id));
        }
    ...
    public GenericValue getIssue(final String key) throws GenericEntityException
    {
        ...
        return EntityUtil.getOnly(CoreFactory.getGenericDelegator().findByAnd("Issue", EasyMap.build("key", key)));
    }
    ...
    public Collection<Long> getIssueIdsForProject(final Long projectId) throws GenericEntityException
    {
        ...
        try
        {
            issueIterator = CoreFactory.getGenericDelegator().findListIteratorByCondition("Issue",
    ...
    {
        return EntityUtil.getOnly(CoreFactory.getGenericDelegator().findByAnd("Issue", EasyMap.build("workflowId", wfid)));
    }
            ...
            throws GenericEntityException
    {
        ...
        else if (relationName.equals(IssueRelationConstants.WORKFLOW_HISTORY))
        {
            ...
            return workflowManager.makeWorkflow(null).getHistorySteps(issue.getLong("workflowId"));
        }
        ...
        {
            return CoreFactory.getGenericDelegator().findByAnd("IssueLink", MapBuilder.build("destination", issue.getLong("id")));
        }
        else if (relationName.equals(IssueRelationConstants.LINKS_OUTWARD))
        {
            return CoreFactory.getGenericDelegator().findByAnd("IssueLink", MapBuilder.build("source", issue.getLong("id")));
        }
        else if (relationName.equals(IssueRelationConstants.CUSTOM_FIELDS_VALUES))
        {
            return CoreFactory.getGenericDelegator().findByAnd("CustomFieldValue", MapBuilder.build("issue", issue.getLong("id")));
        }
    ...
    {
        return createIssue((User) remoteUser, issue);
    }
    ...
    {
        return updateIssue((User) user, issue, eventDispatchOption, sendMail);
    }
    ...
    public GenericValue createIssue(com.opensymphony.user.User remoteUser, Map<String, Object> fields)
            throws CreateException
    {
        return createIssue((User) remoteUser, fields);
    }

    public void notifyTrackbacks(final GenericValue issue)
    {
        textAnalyzer.analyseContent(issue, issue.getString("description") + " " + issue.getString("environment"));
    }

    public List<Issue> getVotedIssues(final com.opensymphony.user.User user) throws GenericEntityException
    {
    ...
    {
        return OSUserConverter.convertToOSUserList(userAssociationStore.getUsersFromSink("WatchIssue", issue));
    }
            ...
            throws UpdateException
    {
        ...
        textAnalyzer.analyseContent(issue, modifiedText.toString());
    ...
    {
        deleteIssue((User) user, issue, eventDispatchOption, sendMail);
    }
    ...
    protected void doUpdate(MutableIssue issue, GenericValue originalIssueGV, EventDispatchOption eventDispatchOption, User user, boolean sendMail,
            DefaultIssueChangeHolder issueChangeHolder) throws UpdateException
    {
