...
public class DefaultSubTaskToIssueConversionService extends DefaultIssueConversionService implements SubTaskToIssueConversionService
{
    ...
    public void validateTargetIssueType(JiraServiceContext context, Issue issue, IssueType issueType, final String fieldNameIssueTypeId)
    {
        ...
        final Collection issueTypes = issueTypeSchemeManager.getIssueTypesForProject(issue.getProjectObject());
        boolean found = false;

        for (Iterator it = issueTypes.iterator(); it.hasNext() && !found;)
        {
    ...
    {
        Collection inwardLinks = issueLinkManager.getInwardLinks(currentIssue.getId());
        for (Iterator iterator = inwardLinks.iterator(); iterator.hasNext();)
        {
            ...
            if (issueLink.getIssueLinkType().isSubTaskLinkType())
            {
                try
                {
                    issueLinkManager.removeIssueLink(issueLink, context.getUser());
                }
