...
public class IssueSecurityLevelResolver
{
    ...
    public List<GenericValue> getAllSecurityLevels(User searcher)
    {
        try
        {
            return new ArrayList<GenericValue>(issueSecurityLevelManager.getAllUsersSecurityLevels(searcher));
        }
        catch (GenericEntityException e)
        {
            log.warn("Unable to find issue levels for user '" + searcher.getName() + "': " + e.getMessage(), e);
    ...
    private List<GenericValue> _getIssueSecurityLevels(User searcher, boolean overrideSecurity, List<QueryLiteral> rawValues)
    {
        ...
        List<GenericValue> matchingLevels = new ArrayList<GenericValue>();
    ...
    private Collection<GenericValue> getIssueSecurityLevelsForString(final User searcher, final boolean overrideSecurity, final String rawValue)
    {
        Collection<GenericValue> levels = getIssueSecurityLevelsByName(searcher, overrideSecurity, rawValue);
        if (levels == null)
        {
        ...
        if (levels.isEmpty())
        {
            ...
            if (valueAsLong != null)
            {
                ...
                final GenericValue securityLevel = getIssueSecurityLevelById(searcher, overrideSecurity, valueAsLong);
    ...
    private List<GenericValue> getIssueSecurityLevelsForLong(final User searcher, final boolean overrideSecurity, final Long rawValue)
    {
        final List<GenericValue> levels = new ArrayList<GenericValue>();
        final GenericValue level = getIssueSecurityLevelById(searcher, overrideSecurity, rawValue);
        if (level != null)
        {
            levels.add(level);
        }
        else
        {
            ...
            final Collection<GenericValue> securityLevelsByName = getIssueSecurityLevelsByName(searcher, overrideSecurity, rawValue.toString());
    ...
    GenericValue getIssueSecurityLevelById(final User searcher, final boolean overrideSecurity, final Long valueAsLong)
    {
        try
        {
            final Collection<GenericValue> usersSecurityLevels = overrideSecurity ? issueSecurityLevelManager.getAllSecurityLevels() : issueSecurityLevelManager.getAllUsersSecurityLevels(searcher);
            for (GenericValue level : usersSecurityLevels)
            {
                if (valueAsLong.equals(level.getLong("id")))
                {
                    return level;
                }
            }
     ...
     * @return the {@link org.ofbiz.core.entity.GenericValue}s representing the levels with the specified name that the
     * user can see; null if there was an exception.
     */
    Collection<GenericValue> getIssueSecurityLevelsByName(final User searcher, final boolean overrideSecurity, final String nameValue)
    {
        try
        {
            return overrideSecurity ? issueSecurityLevelManager.getSecurityLevelsByName(nameValue) : issueSecurityLevelManager.getUsersSecurityLevelsByName(searcher, nameValue);
        }
        catch (GenericEntityException e)
        {
            log.error(e, e);
