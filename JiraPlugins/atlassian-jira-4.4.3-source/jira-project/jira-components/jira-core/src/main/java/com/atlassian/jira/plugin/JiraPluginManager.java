...
public class JiraPluginManager extends DefaultPluginManager implements Startable
{
    ...
    private void resetCaches()
    {
        ...
        ComponentManager.getInstance().getFieldManager().refresh();
        ComponentManager.getInstance().getFieldScreenManager().refresh();

        //this cache stores direct references to portlets. Clear the cache so these references are not kept. This is SOOOOOOO evil.
        final PortletConfigurationStore store = ComponentManager.getComponentInstanceOfType(PortletConfigurationStore.class);
