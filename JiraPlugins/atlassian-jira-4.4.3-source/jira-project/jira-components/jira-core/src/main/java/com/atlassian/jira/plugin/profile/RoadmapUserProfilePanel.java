    ...
    public boolean showPanel(User profileUser, User currentUser)
    {
        if (profileUser.equals(currentUser))
        {
            for (Project project : getApplicableProjects(currentUser))
            {
                ...
                {
                    if ((!fieldVisibilityManager.isFieldHidden(project.getId(), IssueFieldConstants.FIX_FOR_VERSIONS, FieldVisibilityManager.ALL_ISSUE_TYPES)) &&
                            !fieldVisibilityManager.isFieldHidden(project.getId(), IssueFieldConstants.ASSIGNEE, FieldVisibilityManager.ALL_ISSUE_TYPES))
                    {
                        return true;
                    }
                }
