...
public class UpdateIssueFieldFunction extends AbstractJiraFunctionProvider
{
    ...
    public void execute(Map transientVars, Map args, PropertySet ps) throws WorkflowException
    {
        ...
        else if (IssueFieldConstants.TIME_ORIGINAL_ESTIMATE.equals(fieldName) || IssueFieldConstants.TIME_ESTIMATE.equals(fieldName))
        {
            ...
            final Map<String, Object> map = MapBuilder.<String, Object>newBuilder()
                    ...
                    .add(TimeTrackingSystemField.TIMETRACKING_TARGETSUBFIELD, targetSubField)
    ...
    private void updateTimeSpent(MutableIssue issue, String fieldValue, List changeItems, Map transientVars)
    {
        try
        {
            ...
            if (!valuesEqual(oldTimeSpent, newTimeSpent))
            {
    ...
    private void updateStatus(MutableIssue issue, String fieldValue, List changeItems)
    {
        ...
        if (!valuesEqual(oldStatus, newStatus))
        {
            if (oldStatus != null)
            {
                changeItems.add(new ChangeItemBean(ChangeItemBean.STATIC_FIELD, IssueFieldConstants.STATUS, oldStatus.getString("id"), oldStatus.getString("name"), newStatus.getString("id"), newStatus.getString("name")));
            }
            else
            {
                changeItems.add(new ChangeItemBean(ChangeItemBean.STATIC_FIELD, IssueFieldConstants.STATUS, null, null, newStatus.getString("id"), newStatus.getString("name")));
            }
        }
    }

    private void processField(MutableIssue issue, String fieldName, Map inputParameters, String username, List changeItems, Map transientVars)
            throws WorkflowException
    {
        ...
        try
        {
            ...
            {
                fieldLayoutItem = ComponentAccessor.getFieldLayoutManager().getFieldLayout(issue.getGenericValue()).getFieldLayoutItem(field);
            }
            else
            {
                fieldLayoutItem = ComponentAccessor.getFieldLayoutManager().getFieldLayout(issue.getProject(), issue.getIssueType().getString("id")).getFieldLayoutItem(field);
            }
