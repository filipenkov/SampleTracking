...
public class SystemInfoUtilsImpl implements SystemInfoUtils
{
    ...
    {
        // TODO - utilise some Crowd API which returns intelligent information about any Crowd instance
        // see CWD-718 for more information

        final UserManager manager = ComponentManager.getComponentInstanceOfType(UserManager.class);
        for (final Iterator iterator = manager.getCredentialsProviders().iterator(); iterator.hasNext();)
        {
            final CredentialsProvider p = (CredentialsProvider) iterator.next();
            if (p.getClass().getName().indexOf("Crowd") >= 0)
            {
                return true;
            }
        }
