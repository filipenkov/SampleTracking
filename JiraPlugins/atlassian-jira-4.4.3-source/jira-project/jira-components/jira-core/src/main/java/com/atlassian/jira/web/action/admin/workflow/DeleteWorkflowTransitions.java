...
@WebSudoRequired
public class DeleteWorkflowTransitions extends JiraWebActionSupport
{
    ...
    protected String doExecute() throws Exception
    {
        ...
        {
            int transitionId = transitionIds[i];
    ...
    public Collection getSelectedTransitions()
    {
        ...
        if (transitionIds != null)
        {
            for (int i = 0; i < transitionIds.length; i++)
            {
                transitions.add(step.getAction(transitionIds[i]));
            }
        }
    ...
    public String getCancelUrl()
    {
        ...
        {
            return "ViewWorkflowStep.jspa?workflowStep=" + step.getId() +
                   "&workflowMode=" + getWorkflow().getMode() +
                   "&workflowName=" + URLEncoder.encode(workflow.getName());
        }

        return "ViewWorkflowSteps.jspa?workflowName=" + URLEncoder.encode(workflow.getName()) +
                    "&workflowMode=" + getWorkflow().getMode();
    }
}