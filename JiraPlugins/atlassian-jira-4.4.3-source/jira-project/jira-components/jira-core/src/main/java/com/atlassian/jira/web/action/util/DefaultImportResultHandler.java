    ...
    public boolean handleErrorResult(ServletContext context, DataImportService.ImportResult lastResult, I18nHelper i18n, ErrorCollection errorCollection)
    {
        switch (lastResult.getImportError())
        {
            case UPGRADE_EXCEPTION:
                ...
                if (eventCont != null)
                {
                    ...
                    if (!licenseJohnsonEventRaiser.checkLicenseIsTooOldForBuild(context, licenseDetails)
                        && !licenseJohnsonEventRaiser.checkLicenseIsInvalid(context, licenseDetails))
                    {
            ...
            case CUSTOM_PATH_EXCEPTION:
                ...
                return false;
            default:
                if(lastResult.getErrorCollection().hasAnyErrors())
                {
                    errorCollection.addErrorCollection(lastResult.getErrorCollection());
                    return false;
                }
                else
                {
                    //finally if the import was successful check if the current license we got now is too old for this build!
                    return checkLicenseIsInvalidOrTooOldForBuild(context);
                }
        }
    }
