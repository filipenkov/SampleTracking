...
public class DefaultPortalPageService implements PortalPageService
{
    ...
    public Collection<PortalPage> getFavouritePortalPages(final User user)
    {
        final Collection<Long> ids = getFavouriteIds(user);
        final List<PortalPage> results = new ArrayList<PortalPage>(ids.size());
        for (final Long id : ids)
        {
            final PortalPage portalPage = portalPageManager.getPortalPage((com.opensymphony.user.User) user, id);
            if (portalPage != null)
            {
    ...
    @Override
    public Collection<PortalPage> getFavouritePortalPages(com.opensymphony.user.User user)
    {
        return getFavouritePortalPages((User) user);
    }

    public boolean isFavourite(final User user, final PortalPage portalPage)
    {
    ...
    @Override
    public boolean isFavourite(com.opensymphony.user.User user, PortalPage portalPage)
    {
        return isFavourite((User) user, portalPage);
    }

    public Collection<PortalPage> getOwnedPortalPages(final User user)
    {
        if (user == null)
        {
    ...
    @Override
    public Collection<PortalPage> getOwnedPortalPages(com.opensymphony.user.User user)
    {
        return getOwnedPortalPages((User) user);
    }

    public Collection<PortalPage> getNonPrivatePortalPages(final User user)
    {
    ...
    @Override
    public Collection<PortalPage> getNonPrivatePortalPages(com.opensymphony.user.User user)
    {
        return getNonPrivatePortalPages((User) user);
    }

    public Collection<PortalPage> getPortalPagesFavouritedByOthers(final User user)
    {
    ...
    @Override
    public Collection<PortalPage> getPortalPagesFavouritedByOthers(com.opensymphony.user.User user)
    {
        return getPortalPagesFavouritedByOthers((User) user);
    }
    ...
    public PortalPage getPortalPage(final JiraServiceContext context, final Long portalPageId)
    {
        ...
        final PortalPage portalPage = portalPageManager.getPortalPage(context.getUser(), portalPageId);
        if (portalPage == null)
        {
    ...
    public boolean validateForCreate(final JiraServiceContext serviceCtx, final PortalPage portalPage)
    {
        ...
        else if (serviceCtx.getUser() != null)
        {
            final PortalPage byName = portalPageManager.getPortalPageByName(serviceCtx.getUser(), portalPage.getName());
            if (byName != null)
            {
    ...
    public PortalPage createPortalPage(final JiraServiceContext serviceCtx, final PortalPage portalPage, final boolean isFavourite)
    {
        ...
        if ((newPortalPage != null) && isFavourite)
        {
            try
            {
                favouritesManager.addFavourite(serviceCtx.getUser(), newPortalPage);
            }
    ...
    private PortalPage checkForClone(final JiraServiceContext serviceCtx, final PortalPage portalPage, final Long clonePortalPageId)
    {
        ...
        if (ok && (serviceCtx.getUser() != null))
        {

            final PortalPage tempClonePortalPage = portalPageManager.getPortalPage(serviceCtx.getUser(), clonePortalPageId);
    ...
    public PortalPage createPortalPageByClone(final JiraServiceContext serviceCtx, final PortalPage portalPage, final Long clonePortalPageId, final boolean isFavourite)
    {
        ...
        final PortalPage newPortalPage = portalPageManager.createBasedOnClone(serviceCtx.getUser(), portalPage, clonePortalPage);
        if ((newPortalPage != null) && isFavourite)
        {
            try
            {
    ...
    public boolean validateForDelete(final JiraServiceContext serviceCtx, final Long portalPageId)
    {
        ...
        final User user = serviceCtx.getUser();
        ...
        else
        {
            ...
            else if (!userNameEqualsUtil.equals(page.getOwnerUserName(), user))
            {
    ...
    @Override
    public void deleteAllPortalPagesForUser(com.opensymphony.user.User user)
    {
    ...
    public boolean validateForUpdate(final JiraServiceContext serviceCtx, final PortalPage portalPage)
    {
        ...
        else
        {
            ...
            if (ok && (serviceCtx.getUser() != null))
            {
                final PortalPage portalPageByName = portalPageManager.getPortalPageByName(serviceCtx.getUser(), portalPage.getName());
    ...
    public PortalPage updatePortalPage(final JiraServiceContext serviceCtx, final PortalPage portalPage, final boolean isFavourite)
    {
        ...
        if (validateForUpdate(serviceCtx, portalPage))
        {
            ...
            if (isFavourite)
            {
                try
                {
                    favouritesManager.addFavourite(serviceCtx.getUser(), updatedPortalPage);
                }
                catch (final PermissionException e)
                {
                    serviceCtx.getErrorCollection().addErrorMessage(serviceCtx.getI18nBean().getText("common.favourites.not.added"));
            ...
            {
                favouritesManager.removeFavourite(serviceCtx.getUser(), updatedPortalPage);
            }
    ...
    public void validateForChangeOwner(final JiraServiceContext serviceCtx, final PortalPage page)
    {
        ...
        {
            final PortalPage portalPageByName = portalPageManager.getPortalPageByName(serviceCtx.getLoggedInUser(), page.getName());
            if ((portalPageByName != null) && !page.getId().equals(portalPageByName.getId()))
            {
                serviceCtx.getErrorCollection().addErrorMessage( serviceCtx.getI18nBean().getText("admin.errors.portalpages.already.owns.same.name", serviceCtx.getLoggedInUser().getDisplayName()));
            }
    ...
    public void increasePortalPageSequence(final JiraServiceContext serviceCtx, final Long portalPageId)
    {
        ...
        if ((portalPage != null) && !serviceCtx.getErrorCollection().hasAnyErrors())
        {
            try
            {
                favouritesManager.increaseFavouriteSequence(serviceCtx.getUser(), portalPage);
            }
            catch (final PermissionException e)
            {
                // /CLOVER:OFF
    ...
    public void decreasePortalPageSequence(final JiraServiceContext serviceCtx, final Long portalPageId)
    {
        ...
        if ((portalPage != null) && !serviceCtx.getErrorCollection().hasAnyErrors())
        {
            try
            {
                favouritesManager.decreaseFavouriteSequence(serviceCtx.getUser(), portalPage);
            }
            catch (final PermissionException e)
            {
                // /CLOVER:OFF
    ...
    public void moveToStartPortalPageSequence(final JiraServiceContext serviceCtx, final Long portalPageId)
    {
        ...
        if ((portalPage != null) && !serviceCtx.getErrorCollection().hasAnyErrors())
        {
            try
            {
                favouritesManager.moveToStartFavouriteSequence(serviceCtx.getUser(), portalPage);
            }
            catch (final PermissionException e)
            {
                // /CLOVER:OFF
    ...
    public void moveToEndPortalPageSequence(final JiraServiceContext serviceCtx, final Long portalPageId)
    {
        ...
        if ((portalPage != null) && !serviceCtx.getErrorCollection().hasAnyErrors())
        {
            try
            {
                favouritesManager.moveToEndFavouriteSequence(serviceCtx.getUser(), portalPage);
            }
            catch (final PermissionException e)
            {
                // /CLOVER:OFF
    ...
    public SharedEntitySearchResult<PortalPage> search(final JiraServiceContext serviceCtx, final SharedEntitySearchParameters searchParameters, final int pagePosition, final int pageWidth)
    {
        ...
        return portalPageManager.search(searchParameters, serviceCtx.getUser(), pagePosition, pageWidth);
    }
    ...
    */
    ...
    boolean checkSystemDefaultPageForUpdate(final JiraServiceContext serviceCtx, final PortalPage portalPage)
    {
        if (portalPage.isSystemDefaultPortalPage())
        {
            ...
            if (!permissionManager.hasPermission(Permissions.ADMINISTER, serviceCtx.getUser()))
            {
    ...
    boolean checkOwnerShipAndExistence(final JiraServiceContext serviceCtx, final PortalPage portalPage)
    {
        ...
        else if (portalPage.getOwnerUserName() == null || !userNameEqualsUtil.equals(portalPage.getOwnerUserName(), serviceCtx.getUser()))
        {
        ...
        else
        {
            ...
            else if (!userNameEqualsUtil.equals(readPage.getOwnerUserName(), serviceCtx.getUser()))
            {
                serviceCtx.getErrorCollection().addErrorMessage(serviceCtx.getI18nBean().getText("admin.errors.portalpages.must.be.owner"));
            }
        }
    ...
    private PortalPage checkPermissionsForMove(final JiraServiceContext serviceCtx, final Long portalPageId)
    {
        ...
        else
        {
            ...
            if (portalPage != null)
            {
                if (!isFavourite(serviceCtx.getUser(), portalPage))
                {
    ...
    private boolean checkOwnership(final JiraServiceContext serviceCtx, final PortalPage portalPage)
    {
        ...
        {
            if (portalPage.getOwnerUserName() == null || !userNameEqualsUtil.equals(portalPage.getOwnerUserName(), user))
            {
                errorCollection.addErrorMessage(serviceCtx.getI18nBean().getText("admin.errors.portalpages.must.be.owner"));
            }
        }
    ...
    private Collection<Long> getFavouriteIds(final User user)
    {
