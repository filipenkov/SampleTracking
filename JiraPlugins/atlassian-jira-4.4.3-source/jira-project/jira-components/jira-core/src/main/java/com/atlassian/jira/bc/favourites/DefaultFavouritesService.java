...
public class DefaultFavouritesService implements FavouritesService
{
    ...
    public void addFavourite(final JiraServiceContext ctx, final SharedEntity entity)
    {
        try
        {
            favouritesManager.addFavourite(ctx.getUser(), entity);
        }
    ...
    public void addFavouriteInPosition(final JiraServiceContext ctx, final SharedEntity entity, final long position)
    {
        try
        {
            favouritesManager.addFavouriteInPosition(ctx.getUser(), entity, position);
        }
        catch (PermissionException e)
        {
    ...
    {
        favouritesManager.removeFavourite(ctx.getUser(), entity);
    }
    ...
    @Override
    public boolean isFavourite(com.opensymphony.user.User user, SharedEntity entity)
    {
