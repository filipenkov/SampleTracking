        ...
        try
        {
            ...
            if (value instanceof Collection)
            {
                ...
                for (final Object o : col)
                {
                    if (o != null)
                    {
                        final String indexValue = CaseFolding.foldString(o.toString());
                        doc.add(new Field(getDocumentFieldId(), indexValue, Field.Store.YES, indexType));
                        doc.add(new Field(getDocumentFieldId()+ SelectStatisticsMapper.RAW_VALUE_SUFFIX, o.toString(), Field.Store.YES, indexType));
                    }
                }
            }
            else if (value instanceof String || value instanceof Option)
            {
                final String indexValue = CaseFolding.foldString(value.toString());
                doc.add(new Field(getDocumentFieldId(), indexValue, Field.Store.YES, indexType));
                doc.add(new Field(getDocumentFieldId()+ SelectStatisticsMapper.RAW_VALUE_SUFFIX, value.toString(), Field.Store.YES, indexType));
            }
        }
