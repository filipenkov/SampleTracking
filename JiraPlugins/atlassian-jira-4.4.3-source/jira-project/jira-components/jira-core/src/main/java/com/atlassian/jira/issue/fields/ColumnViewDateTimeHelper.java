...
public class ColumnViewDateTimeHelper
{
    ...
    String render(NavigableFieldImpl field, FieldLayoutItem fieldLayoutItem, Map displayParams, Issue issue, @Nullable Date date)
    {
        ...
        if (date != null)
        {
            ...
            dateVelocityParams = MapBuilder.<String, Object>build(
                    "title", dateTimeFormatter.withStyle(DateTimeStyle.COMPLETE).format(date),
                    "iso8601", dateTimeFormatter.withStyle(DateTimeStyle.ISO_8601_DATE_TIME).format(date),
                    "value", dateTimeFormatter.withStyle(displayStyle).format(date)
            );
        }
