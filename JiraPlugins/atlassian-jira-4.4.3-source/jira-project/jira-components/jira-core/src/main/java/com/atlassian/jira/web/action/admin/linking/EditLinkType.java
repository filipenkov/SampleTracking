...
public class EditLinkType extends JiraWebActionSupport
{
    ...
    private boolean duplicateLinkName()
    {
        ...
        Collection existingIssueLinkTypes = issueLinkTypeManager.getIssueLinkTypesByName(getName());

        if (existingIssueLinkTypes != null)
        {
            for (Iterator iterator = existingIssueLinkTypes.iterator(); iterator.hasNext();)
            {
