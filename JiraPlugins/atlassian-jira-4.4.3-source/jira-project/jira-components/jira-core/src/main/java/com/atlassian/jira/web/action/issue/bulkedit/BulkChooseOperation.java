...
public class BulkChooseOperation extends AbstractBulkOperationAction
{
    ...
    {
        return bulkOperation.canPerform(getBulkEditBean(), getRemoteUser());
    }

    public boolean isHasAvailableOperations() throws Exception
    {
        for (Iterator iterator = bulkOperations.iterator(); iterator.hasNext();)
        {
            BulkOperation bulkOperation = (BulkOperation) iterator.next();
