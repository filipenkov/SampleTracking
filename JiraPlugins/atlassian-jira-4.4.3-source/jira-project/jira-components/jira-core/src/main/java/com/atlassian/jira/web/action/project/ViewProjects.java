...
public class ViewProjects extends JiraWebActionSupport
{
    ...
    {
        return ComponentAccessor.getPermissionManager().hasPermission(Permissions.ADMINISTER, getRemoteUser());
    }

    public boolean isProjectAdmin(GenericValue project) throws GenericEntityException
    {
        return ComponentAccessor.getPermissionManager().hasPermission(Permissions.PROJECT_ADMIN, project, getRemoteUser());
    }

    public boolean hasAdminPermission(GenericValue project) throws GenericEntityException
    {
        return ComponentAccessor.getPermissionManager().hasPermission(Permissions.ADMINISTER, project, getRemoteUser());
    }
    ...
    public List getProjects()
    {
        ...
        for (Iterator iterator = projects.iterator(); iterator.hasNext();)
        {
            ...
            if (ComponentAccessor.getPermissionManager().hasPermission(Permissions.PROJECT_ADMIN, project, getRemoteUser())
                    || ComponentAccessor.getPermissionManager().hasPermission(Permissions.ADMINISTER, getRemoteUser()))
            {
                returns.add(project);
            }
        }
    ...
    public User getUser(GenericValue project)
    {
        try
        {
            return UserUtils.getUser(project.getString("lead"));
        }
    ...
    public boolean getAdmin(GenericValue project)
    {
        ...
        if (admins.size() > 0)
        {
            User u = (User) admins.iterator().next();
            project.setString("lead", u.getName());
            ManagerFactory.getProjectManager().updateProject(project);
            return true;
        }
        return false;
    }
