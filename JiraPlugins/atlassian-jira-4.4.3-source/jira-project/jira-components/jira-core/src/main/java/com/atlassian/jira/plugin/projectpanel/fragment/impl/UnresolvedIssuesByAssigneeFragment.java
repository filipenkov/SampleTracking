...
public class UnresolvedIssuesByAssigneeFragment extends AbstractUnresolvedIssuesFragment implements ProjectTabPanelFragment
{
    ...
    public UnresolvedIssuesByAssigneeFragment(final VelocityManager velocityManager, final ApplicationProperties applicationProperites, final JiraAuthenticationContext jiraAuthenticationContext,
            final UserFormatManager userFormatManager, final AvatarManager avatarManager)
    {
    ...
    protected Map<String, Object> createVelocityParams(final BrowseContext ctx)
    {
        ...
        velocityParams.put("urlUtil", new AssigneeUrlUtil(getSearchRequest(ctx), jiraAuthenticationContext.getUser(), applicationProperites));
        velocityParams.put("userAvatarEnabled", avatarManager.isUserAvatarsEnabled());
