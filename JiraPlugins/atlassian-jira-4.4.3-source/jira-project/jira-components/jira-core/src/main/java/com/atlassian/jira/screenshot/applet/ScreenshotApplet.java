...
public class ScreenshotApplet extends JApplet implements ActionListener
{
    ...
    private boolean parseParameters()
    {
        ...
        final String after = getParameter("after");
        if (after == null)
        {
            return false;
        }
        ...
        {
            final URL base = getDocumentBase();
            final URL url = new URL(base, after);
            afterURL = url.toString();
        }
        catch (final MalformedURLException e)
        {
            return false;
    ...
    private String stripSelectedIssue(final String url)
    {
    ...
    private URL getAfterUrl(String url, String action) throws MalformedURLException
    {
        try
        {
            return new URL(getCodeBase() + "views/issue/screenshot-redirecter.jsp?afterURL=" + URLEncoder.encode(url, "UTF8") + "&action="+action);
        }
    ...
    private void runJavascript(final String action)
    {
        try
        {
            ...
            {
                url = getAfterUrl(stripSelectedIssue(afterURL),"cancel");
            }
            else
            {
                url = getAfterUrl(afterURL,"submit");
            }
