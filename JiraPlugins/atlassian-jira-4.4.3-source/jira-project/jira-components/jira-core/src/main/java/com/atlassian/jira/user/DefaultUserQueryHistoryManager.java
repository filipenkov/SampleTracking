...
public class DefaultUserQueryHistoryManager implements UserQueryHistoryManager
{
    ...
    @Override
    public void addQueryToHistory(com.opensymphony.user.User user, String query)
    {
        addQueryToHistory((User) user, query);
    }
    ...
    @Override
    public List<UserHistoryItem> getUserQueryHistory(com.opensymphony.user.User user)
    {
        return getUserQueryHistory((User) user);
    }
}
