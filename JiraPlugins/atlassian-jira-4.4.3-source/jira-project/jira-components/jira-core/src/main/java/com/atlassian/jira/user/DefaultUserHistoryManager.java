...
public class DefaultUserHistoryManager implements UserHistoryManager
{
    ...
    public void addItemToHistory(@NotNull final UserHistoryItem.Type type, @NotNull final com.opensymphony.user.User user, @NotNull final String entityId)
    {
        addItemToHistory(type, user, entityId, null);
    }
    ...
    public void addItemToHistory(@NotNull final UserHistoryItem.Type type, @Nullable final com.opensymphony.user.User user, @NotNull final String entityId, @Nullable final String data)
    {
        notNull("type", type);
        notNull("entityId", entityId);
        store.addHistoryItem(user, new UserHistoryItem(type, entityId, data));
    ...
    public boolean hasHistory(@NotNull final UserHistoryItem.Type type, @NotNull final com.opensymphony.user.User user)
    {
        notNull("type", type);
        notNull("user", user);
        final List<UserHistoryItem> history = store.getHistory(type, user);
    ...
    @NotNull
    public List<UserHistoryItem> getHistory(@NotNull final UserHistoryItem.Type type, @Nullable final com.opensymphony.user.User user)
    {
        notNull("type", type);
        return store.getHistory(type, user);
    }
    ...
    public void removeHistoryForUser(@NotNull final com.opensymphony.user.User user)
    {
        notNull("user", user);
        store.removeHistoryForUser(user);
    }
