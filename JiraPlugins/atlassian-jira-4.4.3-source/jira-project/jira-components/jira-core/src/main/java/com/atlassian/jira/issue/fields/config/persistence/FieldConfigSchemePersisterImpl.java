...
public class FieldConfigSchemePersisterImpl implements FieldConfigSchemePersister
{
    ...
    // ---------------------------------------------------------------------------------------------------- Constructors
    public FieldConfigSchemePersisterImpl(final OfBizDelegator delegator, final ConstantsManager constantsManager, final FieldConfigPersister fieldConfigPersister, final FieldConfigContextPersister contextPersister)
    {
    ...
    public Collection<FieldConfigScheme> getInvalidFieldConfigSchemeAfterIssueTypeRemoval(final IssueType issueType)
    {
        ...
        final List<GenericValue> gvs = delegator.findByAnd(ENTITY_RELATED_TABLE_NAME, MapBuilder.build(ENTITY_ISSUE_TYPE, issueType.getId()));
        ...
        for (final GenericValue fieldConfigSchemeIssueTypeGV : gvs)
        {
            ...
            final List specificSchemes = delegator.findByAnd(ENTITY_RELATED_TABLE_NAME, MapBuilder.build(ENTITY_SCHEME_ID, fieldConfigSchemeId));
    ...
    {
        Null.not("issueType", issueType);
        delegator.removeByAnd(ENTITY_RELATED_TABLE_NAME, MapBuilder.build(ENTITY_ISSUE_TYPE, issueType.getId()));
    }
    ...
    public FieldConfigScheme update(final FieldConfigScheme configScheme)
    {
        try
        {
            ...
            final Map fields = transformToFieldsMap(configScheme);
    ...
    {
        final List<GenericValue> configs = delegator.findByAnd(ENTITY_TABLE_NAME, MapBuilder.build(ENTITY_FIELD, field.getId()));
    ...
    public FieldConfigScheme getConfigSchemeForFieldConfig(final FieldConfig fieldConfig)
    {
        ...
        final List<GenericValue> configs = delegator.findByAnd(ENTITY_RELATED_TABLE_NAME, MapBuilder.build(ENTITY_CONFIG_ID, fieldConfigId));
    ...
    public List<Long> getConfigSchemeIdsForCustomFieldId(final String customFieldId)
    {
        ...
        final List<GenericValue> configs = delegator.findByAnd(ENTITY_TABLE_NAME, MapBuilder.build(ENTITY_FIELD, customFieldId));

        return CollectionUtil.transform(configs, new Function<GenericValue, Long>()
        {
            public Long get(final GenericValue input)
            {
                return input.getLong(ENTITY_ID);
            }
        });
    }
    ...
    {
        return delegator.findByPrimaryKey(ENTITY_TABLE_NAME, MapBuilder.build(ENTITY_ID, configId));
    }
    ...
    private void storeConfigAssociations(final Map<String, FieldConfig> configs, final Long createdId) throws GenericEntityException
    {
        if (configs != null)
        {
            ...
            for (final Map.Entry<String, FieldConfig> entry : entries)
            {
                ...
                final Map<String, Object> relatedFields = MapBuilder.<String, Object>newBuilder(ENTITY_ISSUE_TYPE, issueType).add(ENTITY_SCHEME_ID, createdId).add(ENTITY_CONFIG_ID, config.getId()).toMap();
