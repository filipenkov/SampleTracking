 ...
 * @since v4.3
 */
public abstract class AbstractSingleVersionFunction extends AbstractJqlFunction implements ClauseSanitisingJqlFunction
{
    ...
    public AbstractSingleVersionFunction(final NameResolver<Project> projectResolver, final PermissionManager permissionManager)
    {
    ...
    public MessageSet validate(com.opensymphony.user.User searcher, FunctionOperand operand, final TerminalClause terminalClause)
    {
        final MessageSet messages = new NumberOfArgumentsValidator(1, getI18n()).validate(operand);
        if (!messages.hasAnyErrors())
        {
            ...
            for (String projectArg : args)
            {
                ...
                final List<String> projectIds = getFilteredIndexValues(projectArg, false, searcher);
    ...
    public List<QueryLiteral> getValues(final QueryCreationContext queryCreationContext, final FunctionOperand operand, final TerminalClause terminalClause)
    {
        ...
        if (args.size() == 1)
        {
            String projectArg = args.get(0);
            // try to resolve project
            final List<String> projectIds;
            try
            {
                projectIds = getFilteredIndexValues(projectArg, queryCreationContext.isSecurityOverriden(), queryCreationContext.getQueryUser());
            }
            ...
            version = getVersionForProject(new Long(project));
        }
     ...
     */
    private List<String> getFilteredIndexValues(final String projectArg, final boolean overrideSecurity, final User searcher)
    {
     ...
     */
    protected abstract Version getVersionForProject(Long projectId);
