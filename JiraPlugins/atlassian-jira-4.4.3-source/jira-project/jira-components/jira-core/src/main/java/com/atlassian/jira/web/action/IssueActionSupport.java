...
public class IssueActionSupport extends ProjectActionSupport
{
    ...
    public SearchRequest getSearchRequest()
    {
        if (searchRequest == null)
        {
            ...
            {
                final SearchRequest requestFromDB = ComponentManager.getInstance().getSearchRequestService().getFilter(getJiraServiceContext(),
                    searchRequest.getId());
                if ((requestFromDB != null) && searchRequest.isModified())
                {
    ...
    public SessionSearchObjectManagerFactory getSessionSearchRequestManagerFactory()
    {
        ...
        {
            sessionSearchObjectManagerFactory = ComponentManager.getComponentInstanceOfType(SessionSearchObjectManagerFactory.class);
        }
    ...
    private List<VersionProxy> getPossibleVersions(final GenericValue project, final boolean unreleasedFirst) throws Exception
    {
        ...
        final Iterator<Version> unreleasedIter = versionManager.getVersionsUnreleased(project, false).iterator();
        ...
        final List<Version> releasedIter = new ArrayList<Version>(versionManager.getVersionsReleased(project, false));
    ...
    public Workflow getWorkflow()
    {
        if (workflow == null)
        {
            workflow = ComponentAccessor.getWorkflowManager().makeWorkflow(getRemoteUser() != null ? getRemoteUser().getName() : null);
        }
    ...
    {
        return ComponentManager.getInstance().getJiraDurationUtils().getFormattedDuration(v);
    }

    /**
     * Checks if the field is hidden or visible across all the projects that the user can see (has the {@link
     * Permissions#BROWSE} permission).
     *
     * @param id id of the FieldConfiguration
     *
     * @deprecated Use {@link FieldVisibilityManager#isFieldHidden(com.opensymphony.user.User, java.lang.String)}
     *             instead
     */
    @Deprecated
    public boolean isFieldHidden(final String id)
    {
        return getFieldVisibilityManager().isFieldHidden(getRemoteUser(), id);
    }
    ...
    {
        return ComponentManager.getComponentInstanceOfType(LoginService.class);
    }
