...
public class SchemeMergeAction extends AbstractMergeAction
{
    ...
    protected void doValidation()
    {
        ...
        List newSchemeNames = validateInputFields(params);
    ...
    {
        List newSchemeNames = new ArrayList();
        for (Iterator iterator = getDistilledSchemeResults().getDistilledSchemeResults().iterator(); iterator.hasNext();)
        {
            DistilledSchemeResult distilledSchemeResult = (DistilledSchemeResult) iterator.next();
    ...
    private void checkForDuplicates(List newSchemeNames)
    {
        for (Iterator iterator = getDistilledSchemeResults().getDistilledSchemeResults().iterator(); iterator.hasNext();)
        {
            ...
            if (getSelectedDistilledSchemesAsList().contains(scheme.getName()))
            {
                ...
                schemeDistiller.isValidNewSchemeName(getRemoteUser(), scheme.getName(), resultingSchemeTempName, getSelectedSchemeType(), this);
            }
        }
    }
