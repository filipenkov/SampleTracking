...
public class ProjectShareTypeValidator implements ShareTypeValidator
{
    ...
    public boolean checkSharePermission(final JiraServiceContext ctx, final SharePermission permission)
    {
        ...
        Assertions.notNull("ctx.user", ctx.getUser());
        ...
        final boolean hasPermission = permissionManager.hasPermission(Permissions.CREATE_SHARED_OBJECTS, ctx.getUser());
        if (!hasPermission)
        {
            ...
            ctx.getErrorCollection().addError(ShareTypeValidator.DELEGATED_ERROR_KEY,
                                              ctx.getI18nBean().getText("common.sharing.exception.delegated.user.no.share.permission",
                                                                              ctx.getLoggedInUser().getDisplayName()));
        }
        else
        {
            ...
            else
            {
                ...
                {
                    if (!permissionManager.hasPermission(Permissions.BROWSE, project, ctx.getUser()))
                    {
                        ctx.getErrorCollection().addError(ShareTypeValidator.ERROR_KEY,
                            ctx.getI18nBean().getText("common.sharing.exception.no.permission.project", project.getName()));
                        ctx.getErrorCollection().addError(ShareTypeValidator.DELEGATED_ERROR_KEY,
                            ctx.getI18nBean().getText("common.sharing.exception.delegated.user.no.permission.project", ctx.getLoggedInUser().getDisplayName(), project.getName()));
                    }
                }
                else
                {
                    ...
                    else
                    {
                        ...
                        if (!permissionManager.hasPermission(Permissions.BROWSE, project, ctx.getUser()))
                        {
                            ...
                            ctx.getErrorCollection().addError(ShareTypeValidator.DELEGATED_ERROR_KEY,
                                ctx.getI18nBean().getText("common.sharing.exception.delegated.user.no.permission.project.but.in.role", ctx.getLoggedInUser().getDisplayName(), project.getName()));
                        }
                        else if (!projectRoleManager.isUserInProjectRole(ctx.getUser(), role, project))
                        {
                            ctx.getErrorCollection().addError(ShareTypeValidator.ERROR_KEY,
                                ctx.getI18nBean().getText("common.sharing.exception.no.permission.role", project.getName(), role.getName()));
                            ctx.getErrorCollection().addError(ShareTypeValidator.DELEGATED_ERROR_KEY,
                                ctx.getI18nBean().getText("common.sharing.exception.delegated.user.no.permission.role", ctx.getLoggedInUser().getDisplayName(), project.getName(), role.getName()));
                        }
                    }
                }
            }
        ...
        if (projectId != null)
        {
            ...
            else
            {
                ...
                if (roleId != null)
                {
                    ...
                    else if (!projectRoleManager.isUserInProjectRole(serviceCtx.getUser(), projectRole, project))
                    {
                ...
                else
                {
                    ...
                    final boolean browsePermission = permissionManager.hasPermission(Permissions.BROWSE, project, serviceCtx.getUser());
                    if (!browsePermission)
                    {
                        errorCollection.addError(ShareTypeValidator.ERROR_KEY,
