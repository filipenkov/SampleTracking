...
public class ExportService extends AbstractService
{
    ...
    {
        super.init(props);
        boolean useDefaultDirectory = getBoolean(USE_DEFAULT_DIRECTORY);
    ...
    private boolean getBoolean(final String paramName) throws ObjectConfigurationException
    {
        return hasProperty(paramName) && Boolean.parseBoolean(getProperty(USE_DEFAULT_DIRECTORY));
    }
    ...
    public void run()
    {
        ...
        try
        {
            if (dirName == null)
            {
                log.warn("No directory specified for Export Service \"" + getName() + "\" - not exporting");
                return;
            }
    ...
    {
        final com.atlassian.jira.bc.dataimport.ExportService exportService = ComponentManager.getComponent(com.atlassian.jira.bc.dataimport.ExportService.class);
        final JiraAuthenticationContext authenticationContext = ComponentManager.getComponent(JiraAuthenticationContext.class);
    ...
    {
        return ComponentManager.getComponentInstanceOfType(I18nHelper.class);
    }
