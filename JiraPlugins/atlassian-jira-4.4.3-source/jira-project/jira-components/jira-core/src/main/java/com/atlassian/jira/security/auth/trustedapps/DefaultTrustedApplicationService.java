...
public class DefaultTrustedApplicationService implements TrustedApplicationService
{
    ...
    {
        return !notAllowed(context, Keys.NO_PERMISSION) && manager.delete(context.getUser(), id);
    }
    ...
    public TrustedApplicationInfo store(final JiraServiceContext context, final TrustedApplicationInfo info)
    {
        ...
        return manager.store(context.getUser(), info);
    }
    ...
    {
        final boolean notAllowed = !permissionCheck.check(context.getUser());
        if (notAllowed)
        {
