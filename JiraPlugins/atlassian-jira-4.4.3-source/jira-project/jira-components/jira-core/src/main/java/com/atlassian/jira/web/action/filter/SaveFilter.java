...
public class SaveFilter extends SearchDescriptionEnabledAction implements FilterOperationsAction
{
    ...
    {
        return !searchService.doesQueryFitFilterForm(getRemoteUser(), (dbSearchRequest != null) ? dbSearchRequest.getQuery() : null) ||
               !searchService.doesQueryFitFilterForm(getRemoteUser(), (getSearchRequest() != null) ? getSearchRequest().getQuery() : null);
    }
    ...
    public String getOldSearcherViewHtml(IssueSearcher searcher)
    {
        ...
        if (searchRenderer.isRelevantForQuery(getRemoteUser(), (getDbSearchRequest() != null) ? getDbSearchRequest().getQuery() : null))
        {
            return searchRenderer.getViewHtml(getRemoteUser(), dbSearchContext, dbFieldValuesHolder, EasyMap.build("currentFieldValuesHolder", getFieldValuesHolder()), this);
        }
        else
        {
            return "";
    ...
    public String getNewSearcherViewHtml(IssueSearcher searcher)
    {
        ...
        {
            return searchRenderer.getViewHtml(getRemoteUser(), searchContext, getFieldValuesHolder(), EasyMap.build("dbFieldValuesHolder", dbFieldValuesHolder), this);
        }
        else
        {
            return "";
    ...
    private void updateDbRequestState()
    {
        if (getSearchRequest() == null || getSearchRequest().getId() == null)
        {
            ...
            dbSearchRequest.setOwnerUserName(getRemoteUser() == null ? null : getRemoteUser().getName());
        }
        else
        {
            dbSearchRequest = searchRequestService.getFilter(getJiraServiceContext(), getSearchRequest().getId());
        }

        dbFieldValuesHolder = new FieldValuesHolderImpl();
        dbSearchContext = searchService.getSearchContext(getRemoteUser(), dbSearchRequest.getQuery());
        final Collection<IssueSearcher<?>> searchers = issueSearcherManager.getAllSearchers();
        for (IssueSearcher<?> searcher : searchers)
        {
            searcher.getSearchInputTransformer().populateFromQuery(getRemoteUser(), dbFieldValuesHolder, dbSearchRequest.getQuery(), dbSearchContext);
        }
    }
