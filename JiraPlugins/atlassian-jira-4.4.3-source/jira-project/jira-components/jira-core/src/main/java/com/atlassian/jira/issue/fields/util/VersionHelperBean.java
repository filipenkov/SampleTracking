    ...
    public boolean validateVersionIds(Collection versionIds, ErrorCollection errorCollection, I18nHelper i18n, String fieldId)
    {
        ...
        if (versionIds != null)
        {
            if (versionIds.size() > 1)
            {
                ...
                {
                    errorCollection.addError(fieldId, i18n.getText("issue.field.versions.noneselectedwithother"));
                    valid = false;
                }
            }
            ...
            for (Iterator iterator = versionIds.iterator(); iterator.hasNext();)
            {
                ...
                if (l < -1)
                {
                    errorCollection.addError(fieldId, i18n.getText("issue.field.versions.releasedunreleasedselected"));
                    valid = false;
                }
            }
        }
    ...
    public void validateVersionForProject(final Collection versionIds, final Project project, final ErrorCollection errorCollection, final I18nHelper i18n, String fieldId)
    {
        if (versionIds != null && project != null)
        {
            ...
            for (Object versionId : versionIds)
            {
                ...
                if (version == null)
                {
                    errorCollection.addError(fieldId, i18n.getText("issue.field.versions.invalid.version.id", id));
                    return;
                }
            ...
            if (sb != null)
            {
                errorCollection.addError(fieldId, i18n.getText("issue.field.versions.versions.not.valid.for.project", sb.toString(), project.getName()));
            }
        }
    }
