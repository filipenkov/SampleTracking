...
public class ProjectRoleActorMapperValidatorImpl implements ProjectRoleActorMapperValidator
{
    ...
    public ProjectRoleActorMapperValidatorImpl(final ApplicationProperties applicationProperties)
    {
    ...
    public MessageSet validateProjectRoleActors(final I18nHelper i18nHelper, final ProjectImportMapper projectImportMapper, final ProjectImportOptions projectImportOptions)
    {
        ...
        if (projectImportOptions.overwriteProjectDetails())
        {
            ...
            for (final Iterator iterator = projectImportMapper.getProjectRoleActorMapper().getAllProjectRoleActors().iterator(); iterator.hasNext();)
            {
                ...
                if (projectRoleActor.isUserActor())
                {
                    ...
                    if (isExternalUserManagementEnabled())
                    {
                        ...
                        if (projectImportMapper.getUserMapper().getMappedId(projectRoleActor.getRoleActor()) == null)
                        {
                            // User is missing.
                    ...
                    else
                    {
                        ...
                        if (projectImportMapper.getUserMapper().getMappedId(userName) == null)
                        {
                            // User is missing from current JIRA.
                            if (projectImportMapper.getUserMapper().getExternalUser(userName) == null)
                            {
                                // .. and we don't have the details, so they won't be created.
                                final String projectRoleName = projectImportMapper.getProjectRoleMapper().getDisplayName(projectRoleActor.getRoleId());
                                messageSet.addWarningMessage(i18nHelper.getText(
                                    "admin.errors.project.import.project.role.actor.validation.user.missing", projectRoleName, userName));
                                messageSet.addWarningMessageInEnglish("Project role '" + projectRoleName + "' contains a user '" + userName + "' that doesn't exist in the current system. This user will not be added to the project role membership.");
                            }
                        }
