...
public class EditLabels extends AbstractIssueSelectAction
{
    ...
    private LabelService.LabelsResult getLabelsResult(final Issue issue)
    {
        ...
        {
            labelsResult = labelService.getLabels(getRemoteUser(), issue.getId());
        }
        else
        {
            labelsResult = labelService.getLabels(getRemoteUser(), issue.getId(), customFieldId);
        }
        return labelsResult;
    }
    ...
    public String doExecute() throws Exception
    {
        ...
        if (customFieldId == null)
        {
            setLabelsValidation = labelService.validateSetLabels(getRemoteUser(), issue.getId(), labels);
        }
        else
        {
            setLabelsValidation = labelService.validateSetLabels(getRemoteUser(), issue.getId(), customFieldId, labels);
        }

        if (setLabelsValidation.isValid())
        {
            LabelService.LabelsResult setLabelsResult = labelService.setLabels(getRemoteUser(), setLabelsValidation,
    ...
    public String getLabelNavigatorUrl(final String label)
    {
        if (customFieldId == null)
        {
            return labelUtil.getLabelJql(getRemoteUser(), label);
        }
        return labelUtil.getLabelJql(getRemoteUser(), customFieldId, label);
    }
