...
public class OfBizWorkflowDescriptorStore implements WorkflowDescriptorStore
{
    ...
    {
        final List workflowGVs = ofBizDelegator.findAll(WORKFLOW_ENTITY_NAME);
        ...
        int i = 0;
        for (final Iterator iterator = workflowGVs.iterator(); iterator.hasNext();)
        {
            final GenericValue workflowGV = (GenericValue) iterator.next();
