...
public class GroupPickerStatisticsMapper implements StatisticsMapper
{
    ...
    public SearchRequest getSearchUrlSuffix(Object value, SearchRequest searchRequest)
    {
        ...
        {
            String clauseName = customFieldInputHelper.getUniqueClauseName(authenticationContext.getUser(), clauseNames.getPrimaryName(), customField.getName());
            JqlClauseBuilder builder = JqlQueryBuilder.newBuilder(searchRequest.getQuery()).where().defaultAnd();
            if (value != null)
            {
                builder.addStringCondition(clauseName, Operator.EQUALS, ((Group)value).getName());
            }
            else
            {
