...
public class AbstractBulkOperationAction extends IssueActionSupport
{
    ...
    public List getColumns() throws Exception
    {
        ...
        {
            columnLayout = ManagerFactory.getFieldManager().getColumnLayoutManager().getColumnLayout(getRemoteUser(), searchRequest);
        }
        else
        {
            columnLayout = ManagerFactory.getFieldManager().getColumnLayoutManager().getColumnLayout(getRemoteUser());
        }

        Query query = searchRequest.getQuery();
        final QueryContext queryContext = searchService.getQueryContext(getRemoteUser(), query);
        return columnLayout.getVisibleColumnLayoutItems(getRemoteUser(), queryContext);
    }
    ...
    {
        BulkEditBean.removeFromSession();
    }
    ...
    {
        BulkEditBean bean = BulkEditBean.getFromSession();
        if (bean == null)
        {
