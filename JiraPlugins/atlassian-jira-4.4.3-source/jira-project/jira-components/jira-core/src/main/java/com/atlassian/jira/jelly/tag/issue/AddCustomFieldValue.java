...
public class AddCustomFieldValue extends TagSupport
{
    ...
    public void doTag(XMLOutput xmlOutput) throws MissingAttributeException, JellyTagException
    {
        ...
        if (parent != null)
        {
            ...
            {
                customField = ManagerFactory.getCustomFieldManager().getCustomFieldObject(getId());
            }
            else if (StringUtils.isNotEmpty(getName()))
            {
                // Retrieve the custom field from its name
                customField = ManagerFactory.getCustomFieldManager().getCustomFieldObjectByName(getName());
            }
