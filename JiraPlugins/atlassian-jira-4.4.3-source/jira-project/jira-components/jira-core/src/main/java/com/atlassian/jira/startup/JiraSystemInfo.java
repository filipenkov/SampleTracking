...
public class JiraSystemInfo
{
    ...
    public void obtainBasicInfo(final ServletContext context)
    {
        ...
        logMsg.outputProperty("Java Version", System.getProperty("java.version", STRANGELY_UNKNOWN) + " - " + System.getProperty("java.vendor",
                STRANGELY_UNKNOWN));
        logMsg.outputProperty("Current Working Directory", System.getProperty("user.dir", STRANGELY_UNKNOWN));
    ...
    public void obtainJiraAppProperties()
    {
        ...
        final String crowdified = jiraSysInfo.isCrowdified() ? "ON" : "OFF";
        ...
        logMsg.outputProperty("CROWD Integration", crowdified);
    ...
    public void obtainDatabaseStatistics()
    {
        ...
        final long attachmentCount = delegator.getCount("FileAttachment");

        final IssueIndexManager indexManager = ComponentManager.getComponentInstanceOfType(IssueIndexManager.class);
        int commentCount = 0;
        try
        {
            commentCount = indexManager.getCommentSearcher().getIndexReader().numDocs();
        }
    ...
    {
        final UpgradeManager upgradeManager = ComponentManager.getComponentInstanceOfType(UpgradeManager.class);
    ...
    {
        obtainFilePaths(ComponentManager.getComponent(JiraHome.class));
    }
    ...
    {
        final PluginInfoProvider pluginInfoProvider = ComponentManager.getComponentInstanceOfType(PluginInfoProvider.class);
    ...
    private void showPluginInfo(String title, Collection<PluginInfoProvider.Info> userPlugins)
    {
        logMsg.outputHeader(title);
        logMsg.outputProperty("Number", String.valueOf(userPlugins.size()), 1);
        for (final PluginInfoProvider.Info pluginInfo : userPlugins)
        {
    ...
    {
        final ServiceManager serviceManager = ComponentManager.getComponentInstanceOfType(ServiceManager.class);
    ...
    {
        final TrustedApplicationManager trustedAppManager = ComponentManager.getComponentInstanceOfType(TrustedApplicationManager.class);
