...
public class DefaultProjectOperationManager implements ProjectOperationManager
{
    ...
    @Override
    public List<PluggableProjectOperation> getVisibleProjectOperations(Project project, com.opensymphony.user.User user)
    {
        return getVisibleProjectOperations(project, (User) user);
    }
    ...
    private void loadProjectOperation(final Project project, final User user, final List<? super PluggableProjectOperation> returnList, final ProjectOperationModuleDescriptor descriptor)
    {
        ...
        try
        {
            ...
            if (pluggableProjectOperation != null && pluggableProjectOperation.showOperation(project, OSUserConverter.convertToOSUser(user)))
            {
