...
public class FixForVersionsSearcher extends AbstractInitializationSearcher implements IssueSearcher<SearchableField>
{
            ...
            VelocityManager velocityManager)
    {
        ...
        final VersionSearchContextVisibilityChecker versionSearchContextVisibilityChecker = new VersionSearchContextVisibilityChecker(versionManager);
        this.searcherInformation = new GenericSearcherInformation<SearchableField>(constants.getSearcherId(), "common.concepts.fixfor", Collections.<Class<? extends FieldIndexer>>singletonList(FixForVersionsIndexer.class), fieldReference, SearcherGroupType.PROJECT);

        VersionIndexInfoResolver indexInfoResolver = new VersionIndexInfoResolver(versionResolver);

        this.searchInputTransformer = new VersionSearchInputTransformer(constants.getJqlClauseNames(), constants.getUrlParameter(), indexInfoResolver, operandResolver, fieldFlagOperandRegistry, versionSearchContextVisibilityChecker, versionResolver, versionManager);
        this.searchRenderer = new FixForVersionRenderer(projectManager, versionManager, fieldVisibilityManager,
                velocityRequestContextFactory, applicationProperties, velocityManager,
