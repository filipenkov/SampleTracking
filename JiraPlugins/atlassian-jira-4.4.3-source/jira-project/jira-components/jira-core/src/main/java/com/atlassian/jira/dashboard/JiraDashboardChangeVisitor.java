...
class JiraDashboardChangeVisitor implements DashboardChange.Visitor
{
    ...
    JiraDashboardChangeVisitor(final DashboardState updatedState, final PortletConfigurationStore portletConfigurationStore,
            final PortalPageStore portalPageStore, final LegacyGadgetUrlProvider legacyGadgetUrlProvider)
    {
    ...
    public void visit(final AddGadgetChange addGadgetChange)
    {
        ...
        for (GadgetState gadget : gadgets)
        {
            ...
            if (isAddedGadget)
            {
                ...
                //add the gadget
                if (legacyGadgetUrlProvider.isLegacyGadget(newGadgetState.getGadgetSpecUri()))
                {
                    //For legacy gadgets, make sure the portlet key is also set in the DB!
                    portletConfigurationStore.addLegacyPortlet(
                            portalPageId, toLong(newGadgetId), column.index(), row, newGadgetState.getGadgetSpecUri(), newGadgetState.getColor(),
                            newGadgetState.getUserPrefs(), legacyGadgetUrlProvider.extractPortletKey(newGadgetState.getGadgetSpecUri()));
                }
                else
                {
