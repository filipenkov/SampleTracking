...
public class CustomFieldUtils
{
    ...
    /**
     * @deprecated use {@link com.atlassian.jira.util.MapUtils#invertMap(java.util.Map)}. Since v4.3
     */
    public static MultiMap invertMap(final Map mapToInvert)
    {
        return MapUtils.invertMap(mapToInvert);
    }
    ...
    public static boolean isUserHasPermissionToProjects(final CustomField customField, final User user)
    {
        ...
        else
        {
            ...
            if ((projects != null) && !projects.isEmpty())
            {
                ...
                {
                    if (ComponentManager.getComponentInstanceOfType(PermissionManager.class).hasPermission(Permissions.BROWSE, project, user))
                    {
                        return true;
                    }
                }
     ...
     * @return true if user has permission to at least one project that falls under this custom field, false otherwise
     *
     * @deprecated Use {@link #isUserHasPermissionToProjects(com.atlassian.jira.issue.fields.CustomField, com.atlassian.crowd.embedded.api.User)} instead. Since v4.3
     */
    public final static boolean isUserHasPermissionToProjects(final CustomField customField, final com.opensymphony.user.User user)
    {
        return isUserHasPermissionToProjects(customField, (User) user);
    }
    ...
    public static String prettyPrintOptions(final Options options)
    {
        ...
        if ((options != null) && !options.isEmpty())
        {
            ...
            for (final Iterator iterator = options.iterator(); iterator.hasNext();)
            {
                ...
                sb.append("<li>").append(option.getValue()).append(option.getDisabled() ? " (" + getI18nBean().getText("admin.common.words.disabled") + ")" : "").append("</li>");
                ...
                if ((option.getChildOptions() != null) && !childOptions.isEmpty())
                {
                    ...
                    {
                        final Option childOption = (Option) iterator1.next();
                        sb.append("<li>").append(childOption.getValue()).append("</li>");
                    }
                    sb.append("</ul>");
                }
            }
            ...
            final Map customFieldValuesHolder, final Action action, final Map displayParameters)
    {
        ...
        final String language = ComponentManager.getComponentInstanceOfType(JiraAuthenticationContext.class).getLocale().getLanguage();
        params.put("hasCalendarTranslation", ComponentManager.getComponentInstanceOfType(CalendarLanguageUtil.class).hasTranslationForLanguage(language));
        params.put("calendarIncluder", new CalendarResourceIncluder());
        if (customField.isRenderable() && (fieldLayoutItem != null))
        {
            params.put("rendererDescriptor", ComponentManager.getComponentInstanceOfType(RendererManager.class).getRendererForType(fieldLayoutItem.getRendererType()).getDescriptor());
    ...
    {
        return ComponentManager.getComponentInstanceOfType(ApplicationProperties.class).getDefaultBackedString(APKeys.JIRA_DATE_PICKER_JAVASCRIPT_FORMAT);
    }

    public static String getDateTimeFormat()
    {
        return ComponentManager.getComponentInstanceOfType(ApplicationProperties.class).getDefaultBackedString(APKeys.JIRA_DATE_TIME_PICKER_JAVASCRIPT_FORMAT);
    }
     ...
     * and visible in all field schemes (using {@link com.atlassian.jira.web.FieldVisibilityManager#isFieldHiddenInAllSchemes(String,com.atlassian.jira.issue.search.SearchContext,com.opensymphony.user.User)})
     *
     ...
     * @see com.atlassian.jira.web.FieldVisibilityManager#isFieldHiddenInAllSchemes(String,com.atlassian.jira.issue.search.SearchContext,com.opensymphony.user.User)
     */
     ...
     * @see com.atlassian.jira.web.FieldVisibilityManager#isFieldHiddenInAllSchemes(String,com.atlassian.jira.issue.search.SearchContext,com.opensymphony.user.User)
     *
     * @deprecated Use {@link #isShownAndVisible(com.atlassian.jira.issue.fields.CustomField, com.atlassian.crowd.embedded.api.User, com.atlassian.jira.issue.search.SearchContext, com.atlassian.jira.web.FieldVisibilityManager)} instead. Since v4.3
     */
    public static boolean isShownAndVisible(final CustomField customField, final com.opensymphony.user.User user,
            final SearchContext searchContext, final FieldVisibilityManager fieldVisibilityManager)
    {
        return isShownAndVisible(customField, (User) user, searchContext, fieldVisibilityManager);
    }
}
