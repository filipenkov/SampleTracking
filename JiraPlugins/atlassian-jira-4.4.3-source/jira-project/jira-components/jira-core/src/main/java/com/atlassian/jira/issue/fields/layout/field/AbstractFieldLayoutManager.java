...
abstract public class AbstractFieldLayoutManager implements FieldLayoutManager, Startable
{
    ...
    private final ConcurrentMap<CacheObject<Long>, FieldLayout> fieldLayoutCache =
            new MapMaker().makeComputingMap(new Function<CacheObject<Long>, FieldLayout>()
            {
                @Override
     ...
     *
     * @param editableFieldLayout editable default field layout
     */
    public synchronized void storeEditableFieldLayout(final EditableFieldLayout editableFieldLayout)
    {
        ...
        try
        {
            ...
            {
                EntityUtils.createValue("FieldLayoutItem", EasyMap.build("fieldlayout", fieldLayoutGV.getLong("id"), "description",
                        fieldLayoutItem.getFieldDescription(), "fieldidentifier", fieldLayoutItem.getOrderableField().getId(), "ishidden",
            ...
            refreshCaches(fieldLayoutGV.getLong("id"));
        }
    ...
    protected void refreshCaches(final Long id)
    {
        ...
        fieldLayoutCache.remove(new CacheObject<Long>(id));
    ...
    public boolean hasDefaultFieldLayout()
    {
        ...
        {
            final GenericDelegator genericDelegator = CoreFactory.getGenericDelegator();
            final GenericValue fieldLayoutGV = EntityUtil.getOnly(genericDelegator.findByAnd("FieldLayout", EasyMap.build("type", TYPE_DEFAULT)));
    ...
    protected synchronized void restoreFieldLayout(final Long id)
    {
        try
        {
            ...
            final GenericDelegator genericDelegator = CoreFactory.getGenericDelegator();
            final GenericValue fieldLayoutGV = EntityUtil.getOnly(genericDelegator.findByAnd("FieldLayout", EasyMap.build("id", id)));
            if (fieldLayoutGV != null)
            {
                // Remove Field Layout Items. The removeRalted method seems to cause problems (duplicated records) on Tomcat, hence it is not used.
                final List fieldLayoutItemGVs = fieldLayoutGV.getRelated("ChildFieldLayoutItem");
    ...
    {
        return fieldLayoutCache.get(new CacheObject<Long>(id));
    }
    ...
    private FieldLayout loadFieldLayout(Long id)
    {
        try
        {
            ...
            if (id != null)
            {
                fieldLayoutGV = ofBizDelegator.findByPrimaryKey("FieldLayout", EasyMap.build("id", id));
            }
            ...
            if (fieldLayoutGV == null)
            {
                ...
                final List<CustomField> customFieldObjects = fieldManager.getCustomFieldManager().getCustomFieldObjects();
