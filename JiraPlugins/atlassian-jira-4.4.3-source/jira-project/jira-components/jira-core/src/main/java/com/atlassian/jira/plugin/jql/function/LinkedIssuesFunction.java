...
public class LinkedIssuesFunction extends AbstractJqlFunction implements ClauseSanitisingJqlFunction
{
    ...
    public MessageSet validate(final com.opensymphony.user.User searcher, final FunctionOperand operand, final TerminalClause terminalClause)
    {
        ...
        {
            messageSet.addErrorMessage(getI18n().getText("jira.jql.function.linked.issues.disabled", getFunctionName()));
            return messageSet;
        }
    ...
    {
        final List<Issue> issueList = overrideSecurity ? jqlIssueSupport.getIssues(issueArg) : jqlIssueSupport.getIssues(issueArg, searcher);
        final Collection<Issue> issues = new HashSet<Issue>(issueList);
        if (!issues.isEmpty())
        {
            return issues;
        }
        else
        {
            ...
            if (issueId != null)
            {
                final Issue issue = overrideSecurity ? jqlIssueSupport.getIssue(issueId) : jqlIssueSupport.getIssue(issueId, searcher);
                if (issue != null)
                {
