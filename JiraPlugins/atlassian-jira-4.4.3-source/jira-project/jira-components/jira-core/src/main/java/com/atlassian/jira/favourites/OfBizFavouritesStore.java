...
public class OfBizFavouritesStore implements FavouritesStore
{
    ...
    {
        final List<GenericValue> favourites = getFavouriteGVsOfEntityType(user.getName(), entity.getEntityType(), EasyList.build(Column.ID, Column.ENTITY_ID,
            Column.SEQUENCE));
        ...
        for (final Object element : favourites)
        {
            ...
            else
            {
                ...
                if ((storedSequence == null) || (storedSequence.longValue() != expectedSequence))
                {
                    favouriteGv.set(Column.SEQUENCE, new Long(expectedSequence));
    ...
    {
        final List<GenericValue> idGVs = getFavouriteGVsOfEntityType(username, entityType, EasyList.build(Column.ENTITY_ID));
    ...
    {
        delegator.removeByAnd(Table.FAVOURITE_ASSOCIATION, new PrimitiveMap.Builder().add(Column.USERNAME, user.getName()).add(Column.ENTITY_TYPE,
            entityType.getName()).toMap());
    }
    ...
    private GenericValue createAssociation(final String username, final SharedEntity entity)
    {
        ...
        final Long sequence = new Long(previousEntities.size());

        return delegator.createValue(Table.FAVOURITE_ASSOCIATION, new PrimitiveMap.Builder().add(Column.USERNAME, username).add(
            Column.ENTITY_TYPE, entity.getEntityType().getName()).add(Column.ENTITY_ID, entity.getId()).add(Column.SEQUENCE, sequence).toMap());
    }

    private GenericValue getAssociation(final String username, final SharedEntity entity)
    {
        final Map<String, Object> map = new PrimitiveMap.Builder().add(Column.USERNAME, username).add(Column.ENTITY_TYPE,
    ...
    private List<GenericValue> getFavouriteGVsOfEntityType(final String username, final SharedEntity.TypeDescriptor<?> entityType, final List<String> fieldToReturnList)
    {
        ...
        final EntityCondition joinedCondition = new EntityConditionList(EasyList.build(userCondition, typeCondition), EntityOperator.AND);
        final List<String> orderByList = EasyList.build(Column.SEQUENCE + Order.ASCENDING);
    ...
    private void storeGVListSequence(final List<GenericValue> sortedGVList)
    {
        for (int i = 0; i < sortedGVList.size(); i++)
        {
            ...
            genericValue.set(Column.SEQUENCE, new Long(i));
        }
