...
public class ManageConfigurationScheme extends JiraWebActionSupport
{
    ...
    // ---------------------------------------------------------------------------------------------------- Constructors
    public ManageConfigurationScheme(final FieldConfigManager fieldConfigManager, final CustomFieldManager customFieldManager, final FieldConfigSchemeManager fieldConfigSchemeManager, final ProjectManager projectManager, final ConstantsManager constantsManager, final JiraContextTreeManager treeManager, final SubTaskManager subTaskManager, final ReindexMessageManager reindexMessageManager, final CustomFieldContextConfigHelper customFieldContextConfigHelper)
    {
        ...
        if (configScheme != null)
        {
            ...
            if (configScheme.isBasicMode())
            {
                ...
                else
                {
                    ...
                    {
                        final FieldConfig config = (FieldConfig) iterator.next();
    ...
    protected String doExecute() throws Exception
    {
        ...
        if (isBasicMode())
        {
            ...
            {
                messageRequired = customFieldContextConfigHelper.doesAddingContextToCustomFieldAffectIssues(getRemoteUser(), getCustomField(),
            ...
            else
            {
                ...
                messageRequired = customFieldContextConfigHelper.doesChangingContextAffectIssues(getRemoteUser(), getCustomField(), configScheme,
    ...
    public String doRemove() throws Exception
    {
        ...
        if (customFieldContextConfigHelper.doesRemovingSchemeFromCustomFieldAffectIssues(getRemoteUser(), getCustomField(), configScheme))
        {
            reindexMessageManager.pushMessage(getRemoteUser(), "admin.notifications.task.custom.fields");
        }
    ...
    public Collection getAllIssueTypes() throws Exception
    {
        ...
        else
        {
            ...
            for (final Iterator iterator = intersection.iterator(); iterator.hasNext();)
            {
