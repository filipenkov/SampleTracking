 ...
 */
public class DefaultEventTypeManager implements EventTypeManager, Startable
{
    ...
    public DefaultEventTypeManager(OfBizDelegator delegator, WorkflowManager workflowManager, NotificationSchemeManager notificationSchemeManager, final EventPublisher eventPublisher)
    {
    ...
    private synchronized long getNextEventTypeId()
    {
        ...
        try
        {
            entities = CoreFactory.getGenericDelegator().findAll(EventType.EVENT_TYPE);
        }
        catch (GenericEntityException e)
        {
            log.error("Error accessing database to retrieve event type IDs", e);
