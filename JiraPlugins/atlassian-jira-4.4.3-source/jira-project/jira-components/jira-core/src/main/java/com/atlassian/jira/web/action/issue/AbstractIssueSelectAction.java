...
public abstract class AbstractIssueSelectAction extends IssueActionSupport implements IssueSummaryAware
{
    ...
    {
        this(ComponentManager.getComponent(SubTaskManager.class));
    }
    ...
    public boolean isHasEditIssuePermission(final Issue issue)
    {
        ...
        return isHasIssuePermission(Permissions.EDIT_ISSUE, issue.getGenericValue());
    }
    ...
    // This method is a hack that is associated with JRA-10205. This method is needed for trackbacks to function
    // correctly. Even when a user does not have permission to view an issue, if trackbacks have been enabled then
    // we need to display the rdf. To generate the rdf correctly we need the issue, even though we don't have the
    // correct permissions to see the issue.
    ...
    public boolean isSubTaskCreatable()
    {
        ...
        final Collection<Option> subTaskOptions = ComponentAccessor.getComponent(FieldManager.class).getIssueTypeField().
                getOptionsForIssue(getIssueObject(), true);
        return getSubTaskManager().isSubTasksEnabled() && !isSubTask() && isEditable() && isHasProjectPermission(Permissions.CREATE_ISSUE, getProject()) && (subTaskOptions != null) && !subTaskOptions.isEmpty();
    }
    ...
    protected void assertIssueIsValid() throws IssueNotFoundException, IssuePermissionException
    {
        ...
        if (!isIssueValid())
        {
            Issue issueObject = issue.object();
    ...
    {
        return getPermissionManager().hasPermission(Permissions.BROWSE, issue, getLoggedInUser());
    }
