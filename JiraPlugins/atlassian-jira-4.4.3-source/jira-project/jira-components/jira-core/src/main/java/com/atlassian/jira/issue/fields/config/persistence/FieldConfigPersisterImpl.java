...
public class FieldConfigPersisterImpl implements FieldConfigPersister
{
    ...
    public FieldConfig create(final FieldConfig config, final List configurationItemTypes)
    {
    ...
    private void removeImpl(final Long id)
    {
        ...
        {
            throw new DataAccessException("Unable to remove FieldConfig id: " + id, e);
        }
    }
    ...
    public Collection getConfigsExclusiveToConfigScheme(final Long fieldConfigSchemeId)
    {
        ...
        class ExclusiveConfigFinder
        {
            ...
            Collection /* <FieldConfig> */process()
            {
                ...
                for (final Iterator it = fieldConfigSchemeIssueTypeMappings.iterator(); it.hasNext();)
                {
                    final GenericValue fieldConfigSchemeIssueTypeGV = (GenericValue) it.next();
            ...
            private boolean fieldConfigIsExclusiveToScheme(final Long configId)
            {
                ...
                for (final Iterator it = specificConfigs.iterator(); it.hasNext();)
                {
                    final GenericValue fieldConfigIssueType = (GenericValue) it.next();
    ...
    public FieldConfig getFieldConfig(final Long configId)
    {
        ...
        final ConfigurableField field = ManagerFactory.getFieldManager().getConfigurableField(gv.getString(ENTITY_FIELD));
    ...
    private FieldConfig transformToDomainObject(final GenericValue createdGV, final List configurationItemTypes)
    {
    ...
    {
        final Map fields = EasyMap.build(ENTITY_ID, configId);
        return delegator.findByPrimaryKey(ENTITY_TABLE_NAME, fields);
    }
}