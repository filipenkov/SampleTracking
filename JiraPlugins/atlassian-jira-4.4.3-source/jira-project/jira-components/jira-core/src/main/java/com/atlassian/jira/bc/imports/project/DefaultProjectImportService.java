...
public class DefaultProjectImportService implements ProjectImportService
{
    ...
    public DefaultProjectImportService(final PermissionManager permissionManager, final ProjectImportManager projectImportManager, final ProjectManager projectManager, final IssueManager issueManager, final VersionManager versionManager, final ProjectComponentManager projectComponentManager, final PluginAccessor pluginAccessor, final ApplicationProperties applicationProperties, final AttachmentManager attachmentManager, final AttachmentPathManager attachmentPathManager, final BuildUtilsInfo buildUtilsInfo)
    {
    ...
    private void writeLogMessages(final MappingResult mappingResult)
    {
        ...
        for (final Iterator iterator = mappingResult.getSystemFieldsMessageList().iterator(); iterator.hasNext();)
        {
            final MappingResult.ValidationMessage validationMessage = (MappingResult.ValidationMessage) iterator.next();
            writeLogMessages(validationMessage.getMessageSet());
        }
        // Log messages from Custom Fields
        for (final Iterator iterator = mappingResult.getCustomFieldsMessageList().iterator(); iterator.hasNext();)
        {
            final MappingResult.ValidationMessage validationMessage = (MappingResult.ValidationMessage) iterator.next();
            writeLogMessages(validationMessage.getMessageSet());
        }
    ...
    private void writeLogMessages(final MessageSet messageSet)
    {
        ...
        // Log the errors
        for (final Iterator iterator = messageSet.getErrorMessagesInEnglish().iterator(); iterator.hasNext();)
        {
            log.error(iterator.next());
        }
        // Log the warnings
        for (final Iterator iterator = messageSet.getWarningMessagesInEnglish().iterator(); iterator.hasNext();)
        {
        ...
        try
        {
            ...
            try
            {
                ...
                projectImportManager.doImport(projectImportOptions, projectImportData, backupProject, backupSystemInformation, projectImportResults,
                    subInterval, i18n, jiraServiceContext.getUser());
    ...
    private void logImportResults(final ProjectImportResults projectImportResults)
    {
        ...
        for (final Iterator iterator = projectImportResults.getRoles().iterator(); iterator.hasNext();)
        {
            final String projectRoleName = (String) iterator.next();
    ...
    void createValidationMessageList(final MappingResult mappingResult, final ProjectImportData projectImportData, final I18nHelper i18nHelper)
    {
        ...
        for (final Iterator iterator = mappingResult.getCustomFieldValueMessageSets().keySet().iterator(); iterator.hasNext();)
        {
            final String oldCustomFieldId = (String) iterator.next();
            final MessageSet messageSet = (MessageSet) mappingResult.getCustomFieldValueMessageSets().get(oldCustomFieldId);
    ...
    {
        final Map customFieldMessageSets = new HashMap();
        final CustomFieldMapper customFieldMapper = projectImportData.getProjectImportMapper().getCustomFieldMapper();
