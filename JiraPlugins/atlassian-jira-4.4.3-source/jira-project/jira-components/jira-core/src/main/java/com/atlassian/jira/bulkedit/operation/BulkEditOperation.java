...
public class BulkEditOperation extends AbstractBulkOperation
{
    ...
    public BulkEditOperation(PermissionManager permissionManager, ProjectManager projectManager, FieldManager fieldManager, JiraAuthenticationContext authenticationContext)
    {
    ...
    public Map getCustomFieldActions(BulkEditBean bulkEditBean, User remoteUser)
    {
        ...
        List customFields = ManagerFactory.getCustomFieldManager().getCustomFieldObjects(searchContext);
    ...
    public void perform(final BulkEditBean bulkEditBean, final User remoteUser) throws Exception
    {
        for (Iterator iterator = bulkEditBean.getSelectedIssues().iterator(); iterator.hasNext();)
        {
            ...
            actionParams.put("remoteUser", remoteUser);
            for (Iterator iterator1 = bulkEditBean.getActions().values().iterator(); iterator1.hasNext();)
            {
                ...
                FieldLayoutItem fieldLayoutItem = ComponentAccessor.getFieldLayoutManager().getFieldLayout(issue.getProject(), issue.getIssueTypeObject().getId()).getFieldLayoutItem(field);
                field.updateIssue(fieldLayoutItem, issue, bulkEditBean.getFieldValuesHolder());
            }

            actionParams.put("issue", issue.getGenericValue());
            actionParams.put("issueObject", issue);

            boolean sendMail = bulkEditBean.isSendBulkNotification();
            actionParams.put(IssueEvent.SEND_MAIL, Boolean.valueOf(sendMail));
