...
public class BrowseProject extends ProjectActionSupport
{
    ...
    {
        return projectHistoryManager.getCurrentProject(Permissions.BROWSE, getRemoteUser());
    }

    public boolean isHasProjectAdminPermission()
    {
        return getPermissionManager().hasPermission(Permissions.PROJECT_ADMIN, getProject(), getRemoteUser());
    }
    ...
    public String doExecute() throws Exception
    {
        try
        {
            ...
            if (project == null)
            {
                ...
                final Collection projects = getBrowseableProjects();
                if (projects.size() == 1)
                {
                    final GenericValue onlyProject = (GenericValue) projects.iterator().next();
    ...
    {
        return getPermissionManager().hasPermission(Permissions.CREATE_ISSUE, getProject(), getRemoteUser());
    }
    ...
    private PopularIssueTypesUtil.PopularIssueTypesHolder getIssueTypesHolder()
    {
        ...
        {
            issueTypesHolder = popularIssueTypesUtil.getPopularAndOtherIssueTypesForProject(getProject(), getRemoteUser());
        }
    ...
    protected List<ProjectTabPanelModuleDescriptor> initTabPanels(final Class tabPanelClass)
    {
        ...
        try
        {
            ...
            for (final Iterator iterator = tabPanels.iterator(); iterator.hasNext();)
            {
                final AbstractTabPanelModuleDescriptor descriptor = (AbstractTabPanelModuleDescriptor) iterator.next();
     ...
     * @since v3.10
     */
    protected boolean isTabPanelHidden(final AbstractTabPanelModuleDescriptor descriptor) throws PermissionException
    {
        return !((ProjectTabPanel) descriptor.getModule()).showPanel(getBrowseContext());
    }

    private BrowseContext getBrowseContext()
    {
        return new BrowseProjectContext(getRemoteUser(), getProject());
    }
}
