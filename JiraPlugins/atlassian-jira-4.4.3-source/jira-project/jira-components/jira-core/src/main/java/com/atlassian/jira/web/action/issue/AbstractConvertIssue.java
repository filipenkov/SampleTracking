...
public abstract class AbstractConvertIssue extends JiraWebActionSupport
{
            ...
            PermissionManager permissionManager)
    {
        ...
        this.sessionSearchObjectManagerFactory = ComponentManager.getComponent(SessionSearchObjectManagerFactory.class);
    }
    ...
    public String doSetIssueType()
    {
        ...
        else
        {
            ...
            populateDefaultFieldValues(context);
    ...
    public String doConvert()
    {
        ...
        if (permissionManager.hasPermission(Permissions.BROWSE, getIssue(), context.getUser()))
        {
     ...
     * @param context jira service context
     */
    private void populateDefaultFieldValues(JiraServiceContext context)
    {
        Issue targetIssue = getTargetIssue();
        Issue origIssue = getIssue();
        Collection fieldItems = service.getFieldLayoutItems(context, origIssue, targetIssue);

        for (Iterator fieldLayoutIt = fieldItems.iterator(); fieldLayoutIt.hasNext();)
        {
            FieldLayoutItem item = (FieldLayoutItem) fieldLayoutIt.next();
    ...
    public MutableIssue getUpdatedIssue()
    {
        if (updatedIssue == null)
        {
            ...
            for (Iterator iterator = getConvertFieldLayoutItems().iterator(); iterator.hasNext();)
            {
            ...
            for (Iterator fields = removedFields.iterator(); fields.hasNext();)
            {
                OrderableField field = (OrderableField) fields.next();
    ...
    public Collection /* <FieldLayoutItem> */ getConvertFieldLayoutItems()
    {
        ...
        {
            final JiraServiceContextImpl context = createServiceContext();
            layoutItems = service.getFieldLayoutItems(context, getIssue(), getTargetIssue());
        }
        return layoutItems;
    }
    ...
    public Collection /* <OrderableField> */ getRemoveFields()
    {
        ...
        return service.getRemovedFields(context, getIssue(), getTargetIssue());
