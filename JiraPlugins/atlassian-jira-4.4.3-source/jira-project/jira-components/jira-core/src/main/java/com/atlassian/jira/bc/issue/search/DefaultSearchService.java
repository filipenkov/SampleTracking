...
public class DefaultSearchService implements SearchService
{
    ...
    // NOTE: this is a request level cache, not a long term peristent cache
    ...
    @Override
    public QueryContext getQueryContext(com.opensymphony.user.User searcher, Query query)
    {
        return getQueryContext((User) searcher, query);
    }

    @Override
    public SearchResults search(com.opensymphony.user.User searcher, Query query, PagerFilter pager)
            throws SearchException
    {
        return search((User) searcher, query, pager);
    }
    ...
    @Override
    public long searchCount(com.opensymphony.user.User searcher, Query query) throws SearchException
    {
        return searchCount((User) searcher, query);
    }
    ...
    @Override
    public String getQueryString(com.opensymphony.user.User user, Query query)
    {
        return getQueryString((User) user, query);
    }
    ...
    public String getQueryString(final User user, final Query query)
    {
        ...
        url.addParameter(IssueNavigator.JQL_QUERY_PARAMETER, getJqlString(query));
        return url.asUrlString();
    }

    @Override
    public ParseResult parseQuery(com.opensymphony.user.User searcher, String query)
    {
        return parseQuery((User) searcher, query);
    }
    ...
    @Override
    public MessageSet validateQuery(com.opensymphony.user.User searcher, Query query)
    {
        return validateQuery((User) searcher, query);
    }
    ...
    @Override
    public QueryContext getSimpleQueryContext(com.opensymphony.user.User searcher, Query query)
    {
        return getSimpleQueryContext((User) searcher, query);
    }
    ...
    @Override
    public SearchContext getSearchContext(com.opensymphony.user.User searcher, Query query)
    {
        return getSearchContext((User) searcher, query);
    }

    @Override
    public MessageSet validateQuery(final User searcher, final Query query)
    {
        notNull("query", query);
        final Clause clause = query.getWhereClause();
        ...
        com.opensymphony.user.User osUserSearcher = OSUserConverter.convertToOSUser(searcher);
        if (clause != null)
        {
            //Validate caluse.
            final ValidatorVisitor visitor = validatorVisitorFactory.createVisitor(osUserSearcher);
        ...
        {
            //Validate OrderBy.
            messageSet.addMessageSet(orderByValidator.validate(osUserSearcher, orderBy));
        }
        return messageSet;
    }

    @Override
    public boolean doesQueryFitFilterForm(com.opensymphony.user.User user, Query query)
    {
        return doesQueryFitFilterForm((User) user, query);
    }
    ...
    @Override
    public Query sanitiseSearchQuery(com.opensymphony.user.User searcher, Query query)
    {
        return sanitiseSearchQuery((User) searcher, query);
    }
    ...
    public Query sanitiseSearchQuery(final User searcher, final Query query)
    {
        ...
        final ClauseSanitisingVisitor visitor = createClauseSanitisingVisitor(searcher);
     ...
     * @param searchContext the context of the search query
     * @return the set of relevant searchers, or null if ANY of the clauses do not have a relevant searcher
     */
    Set<IssueSearcher> getRelevantQuerySearchers(final User user, Query query, SearchContext searchContext)
    {
        notNull("searhQuery", query);
        RelevantSearcherVisitor relevantSearcherVisitor = createRelevantSearcherVisitor(user, searchContext);
    ...
    boolean calculateDoesQueryFitFilterForm(final User user, final Query query)
    {
        ...
        else
        {
            ...
            Set<IssueSearcher> relevantSearchers = getRelevantQuerySearchers(user, query, simpleSearchContext);
            ...
            else if (!relevantSearchers.isEmpty())
            {
                ...
                com.opensymphony.user.User osuser = OSUserConverter.convertToOSUser(user);
                for (IssueSearcher relevantSearcher : relevantSearchers)
                {
                    ...
                    if (!searchInputTransformer.doRelevantClausesFitFilterForm(osuser, query, simpleSearchContext))
                    {
                        doesItFit = false;
                        break;
                    }
    ...
    {
        final ValidatorVisitor validatorVisitor = validatorVisitorFactory.createVisitor(OSUserConverter.convertToOSUser(user));
        ...
        {
            final Collection<IssueSearcher<?>> issueSearchers = searchHandlerManager.getSearchersByClauseName(user, clause.getName(), searchContext);
    ...
    boolean checkValidationMatches(final User user, final Query query, final SearchContext searchContext, final TerminalClause clause, final IssueSearcher searcher, final ClauseVisitor<MessageSet> validatorVisitor)
    {
        ...
        transformer.populateFromQuery(osuser, holder, query, searchContext);
        ErrorCollection searcherErrors = new SimpleErrorCollection();
        transformer.validateParams(osuser, searchContext, holder, getI18n(user), searcherErrors);

        return searcherErrors.hasAnyErrors();
    }

    ///CLOVER:OFF
    RelevantSearcherVisitor createRelevantSearcherVisitor(final User user, SearchContext searchContext)
    {
        return new RelevantSearcherVisitor(searchHandlerManager, user, searchContext);
    }
    ///CLOVER:ON
