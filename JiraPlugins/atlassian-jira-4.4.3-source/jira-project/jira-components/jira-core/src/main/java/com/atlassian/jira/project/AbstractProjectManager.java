...
public abstract class AbstractProjectManager implements ProjectManager
{
    ...
    {
        return getNextId(project.getGenericValue());
    }

    public Collection<GenericValue> getComponents(final List<Long> ids)
    {
        final List<GenericValue> components = new ArrayList<GenericValue>(ids.size());

        for (final Long id : ids)
        {
            components.add(getComponent(id));
        }
        return components;
    }
    ...
    private User getDefaultAssignee(Project project, String username) throws InvalidAssigneeException
    {
        ...
        User assignee = userManager.getUserObject(username);
        if (assignee == null)
        {
     ...
     * @param component
     */
    public com.opensymphony.user.User getDefaultAssignee(final GenericValue project, final GenericValue component)
    {
    ...
    private com.opensymphony.user.User getUser(final GenericValue entity) throws DefaultAssigneeException
    {
        com.opensymphony.user.User user = userManager.getUser(entity.getString("lead"));
        if (user == null)
        {
