...
public class DefaultSharedEntitySearcher<S extends SharedEntity> implements SharedEntitySearcher<S>
{
    static final class SharedEntitySortField extends SortField
    {
        ...
        SharedEntitySortField(final SharedEntityColumn column, final boolean reverse)
        {
            super(column.getSortColumn(), column.getSortType(), reverse);
        }

        SharedEntitySortField(final SharedEntityColumn column, final SortComparatorSource sortComparatorSource, final boolean reverse)
        {
            super(column.getSortColumn(), sortComparatorSource, reverse);
        }
    }
    ...
    public SharedEntitySearchResult<S> search(final SharedEntitySearchParameters searchParameters, final User user, final int pageOffset, final int pageWidth)
    {
        checkParametersForSanity(searchParameters, pageOffset, pageWidth);

        final Query query = queryFactory.create(searchParameters, user);
        ...
        final Searcher searcher = searcherFactory.get();
        try
        {
            return new Engine(true)
            {
                @Override
                Hits getHits() throws IOException
                {
                    return searcher.search(query, sort);
                }

                @Override
                Page page(final Hits source)
                {
    ...
    abstract class Engine
    {
        ...
        SharedEntitySearchResult<S> search(User user)
        {
            ...
            return new SharedEntitySearchResult<S>(accessor.get(user, ids), page.hasNext(), hits.length());
        }
        ...
        SharedEntitySearchResult<S> search()
        {
            ...
            final Page page = page(hits);
            final Ids ids = new Ids(transform(page.list(), Transformers.STRING_TO_LONG), preserveSort);
            return new SharedEntitySearchResult<S>(accessor.get(ids), page.hasNext(), hits.length());
        }
    }
    ...
    class Page
    {
        ...
        private Page(final Hits hits, final int pageOffset, final int pageWidth, final Searcher searcher)
        {
            final int startOffset = Math.min(hits.length(), pageOffset * pageWidth);
            final int endOffset = Math.min(hits.length(), startOffset + pageWidth);
            ...
            this.hasNext = hits.length() > endOffset;
        }
    ...
    {
        public All(final Hits hits, final Searcher searcher)
        {
    ...
    private static class SortFieldFactory
    {
        SharedEntitySortField getSortField(final SharedEntityColumn column, final boolean reverse)
        {
            ...
            if (column.isCustomSort())
            {
                return new SharedEntitySortField(column, column.createSortComparator(), reverse);
            }
            return new SharedEntitySortField(column, reverse);
        }
    }
}
