...
public class DefaultShareManager implements ShareManager
{
    ...
    @Override
    public boolean hasPermission(com.opensymphony.user.User user, SharedEntity entity)
    {
        return hasPermission((User) user, entity);
    }
    ...
    public boolean isSharedWith(User user, SharedEntity sharedEntity)
    {
        ...
        if ((sharedEntity.getOwnerUserName() != null) && userNameEqualsUtil.equals(sharedEntity.getOwnerUserName(), user))
        {
            return true;
        }
