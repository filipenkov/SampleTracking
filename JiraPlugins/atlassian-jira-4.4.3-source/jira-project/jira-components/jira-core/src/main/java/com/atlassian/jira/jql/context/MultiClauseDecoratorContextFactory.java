 ...
 * </pre>
 *
 * It does this by intercepting calls to {@link #getClauseContext(com.opensymphony.user.User, com.atlassian.query.clause.TerminalClause)}
...
public class MultiClauseDecoratorContextFactory implements ClauseContextFactory
{
    ...
    public final ClauseContext getClauseContext(final User searcher, final TerminalClause terminalClause)
    {
        ...
        else
        {
            ...
            {
                final Operand newOperand;
                if (literal.isEmpty())
                {
                    newOperand = EmptyOperand.EMPTY;
                }
                else if (literal.getLongValue() != null || literal.getStringValue() != null)
                {
                    newOperand = new SingleValueOperand(literal);
                }
                else
                {
                    //This is a really broken literal so lets skip it.
                    continue;
                }

                final TerminalClause newClause = new TerminalClauseImpl(terminalClause.getName(), convertedOperator, newOperand);
                ctxs.add(delegate.getClauseContext(searcher, newClause));
            }
