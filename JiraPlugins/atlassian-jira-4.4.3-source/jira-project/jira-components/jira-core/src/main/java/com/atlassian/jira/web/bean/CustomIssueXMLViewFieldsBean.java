...
public class CustomIssueXMLViewFieldsBean
{
    ...
    public CustomIssueXMLViewFieldsBean(final IssueViewFieldParams issueViewFieldParams, final Long projectId, final String issueTypeId)
    {
    ...
    private boolean checkFieldVisible(final String fieldId, final Long projectId, final String issueTypeId)
    {
        ...
        {
            return !fieldVisibilityManager.isFieldHidden(projectId, IssueFieldConstants.TIMETRACKING, issueTypeId);
        }
        else
        {
            if (issueViewFieldParams.getOrderableFieldIds().contains(fieldId))
            {
                return !fieldVisibilityManager.isFieldHidden(projectId, fieldId, issueTypeId);
            }
            else
            {
                return true;
