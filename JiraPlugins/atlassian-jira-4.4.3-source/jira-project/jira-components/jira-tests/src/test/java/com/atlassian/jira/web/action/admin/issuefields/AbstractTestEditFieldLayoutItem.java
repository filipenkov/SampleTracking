...
public abstract class AbstractTestEditFieldLayoutItem extends LegacyJiraMockTestCase
{
    ...
    public void testDoDefault() throws Exception
    {
        ...
        OrderableField orderableField = ManagerFactory.getFieldManager().getOrderableField(IssueFieldConstants.ISSUE_TYPE);
        FieldLayout fieldLayout =null;
        if (getFieldLayoutId() != null)
            fieldLayout = ManagerFactory.getFieldManager().getFieldLayoutManager().getFieldLayout(getFieldLayoutId());
        else
            fieldLayout = ManagerFactory.getFieldManager().getFieldLayoutManager().getFieldLayout();
        ...
        String result = getEfli().doDefault();
        assertEquals(ActionSupport.INPUT, result);
        assertEquals(ManagerFactory.getFieldManager().getField(IssueFieldConstants.ISSUE_TYPE).getNameKey(), getEfli().getFieldNameKey());
