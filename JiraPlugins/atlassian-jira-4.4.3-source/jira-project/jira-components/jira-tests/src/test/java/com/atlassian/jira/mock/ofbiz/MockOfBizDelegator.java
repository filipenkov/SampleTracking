...
public class MockOfBizDelegator implements OfBizDelegator
{
    ...
    public void store(final GenericValue gv) throws DataAccessException
    {
        ...
        // try to find current value for gv
        final GenericValue existingValue = findByPrimaryKey(gv.getEntityName(), new FieldMap("id", gv.get("id")));
        if (existingValue != null)
        {
            genericValues.remove(existingValue);
        }
