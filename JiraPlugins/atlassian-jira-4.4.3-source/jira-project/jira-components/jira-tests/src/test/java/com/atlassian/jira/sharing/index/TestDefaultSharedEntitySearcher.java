...
public class TestDefaultSharedEntitySearcher extends MockControllerTestCase
{
    ...
    private void assertBaseParams(final DefaultSharedEntitySearcher<SharedEntity> entitySearcher, final SharedEntitySearchParameters searchParameters, final User user, final int pageOffset, final int pageWidth)
    {
        try
        {
    ...
    public void testBasicSearch() throws IOException
    {
        ...
        final QueryFactory queryFactory = mockController.getMock(QueryFactory.class);
        queryFactory.create(searchParameters, user);
        mockController.setReturnValue(expectedQuery);

        final List<Long> expectedIds = EasyList.build(ID_123);
        ...
        sharedEntityAccessor.get(user, getIds(expectedIds, true));
     ...
     * A searcher with some standard data in it
     */
    Searcher getSearcher(final Map<Long, String> idMap)
    {
        try
        {
    ...
    private class DelegateSearcher extends Searcher
    {
        private final Searcher delegate;

        DelegateSearcher(final Searcher delegate)
        {
        ...
        {
            return delegate.search(query, filter, sort);
        }

        @Override
        public Hits search(final Query query, final Filter filter) throws IOException
        {
            return delegate.search(query, filter);
        }

        @Override
        public void search(final Query query, final HitCollector results) throws IOException
        {
            delegate.search(query, results);
        }

        @Override
        public Hits search(final Query query, final Sort sort) throws IOException
        {
            return delegate.search(query, sort);
        }

        @Override
        public void search(final Weight arg0, final Filter arg1, final HitCollector arg2) throws IOException
        {
            delegate.search(arg0, arg1, arg2);
        }
