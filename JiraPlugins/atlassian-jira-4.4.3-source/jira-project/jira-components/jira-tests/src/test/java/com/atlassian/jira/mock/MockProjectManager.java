...
//    }
    ...
    {
        for (Iterator iterator = getProjects().iterator(); iterator.hasNext();)
        {
    ...
    public Project getProjectObjByKey(String projectKey)
    {
        ...
        {
            GenericValue project = (GenericValue) iterator.next();
            if (projectKey.equals(project.getString("key")))
            {
    ...
    {
        Long currentId = project.getLong("counter");

        if (currentId == null)
        {
            currentId = new Long(1);
        }
        else
        {
        ...
        project.set("counter", currentId);
        getDelegator().storeAll(EasyList.build(project));
