...
public class TestReleaseNote extends LegacyJiraMockTestCase
{
    ...
    public void testGetVersionsCallsProjectManagerCorrectly() throws Exception
    {
        ...
        final GenericValue project = UtilsForTests.getTestEntity("Project", new HashMap());
        projectManagerMock.expectAndReturn("getProject", P.args(P.eq(new Long(projectId))), project);
        versionManager.expectAndReturn("getVersionsUnreleased", P.args(P.eq(project), P.IS_FALSE), unreleasedVersions);
        versionManager.expectAndReturn("getVersionsReleased", P.args(P.eq(project), P.IS_FALSE), Collections.EMPTY_LIST);
