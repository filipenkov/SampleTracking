 ...
 * @since v4.0
 */
public class TestDefaultRecentCommentManager extends ListeningTestCase
{
     ...
     */
    private void _testGetRecentComments(final List<Long> issueIds, final SearchRequest request, final User user, final boolean includeDates, final org.apache.lucene.search.Query expectedQuery) throws Exception
    {
        //the issue searcher is never called, but we need it there.
        final MockControl mockIssueSearcherControl = MockClassControl.createControl(Searcher.class);
        final Searcher mockIssueSearcher = (Searcher) mockIssueSearcherControl.getMock();
        mockIssueSearcherControl.replay();

        //this is where we check to make sure search lucene with the correct comment.
        final MockControl mockCommentSearcherControl = MockClassControl.createControl(Searcher.class);
        final Searcher mockCommentSearcher = (Searcher) mockCommentSearcherControl.getMock();

        final Sort sort = new Sort(new SortField[] { new SortField(DocumentConstants.COMMENT_UPDATED, SortField.STRING, true) });
        final ArgumentsMatcher compositeArgumentsMatcher = ArgumentsMatcherBuilder.newNaturalBuilder().addDefaultMatcher().addArgumentMatcher(
            Sort.class, new LuceneSortMatcher()).asArgumentsMatcher();

        mockCommentSearcher.search(expectedQuery, sort);
        ...
        MockIssueSearchProvider mockIssueSearchProvider = new MockIssueSearchProvider(issueIds, user, request.getQuery());
        final SearchService service = EasyMock.createMock(SearchService.class);
        EasyMock.expect(service.doesQueryFitFilterForm(user, request.getQuery())).andReturn(includeDates);
        ...
        final DefaultRecentCommentManager defaultCommentManager = new DefaultRecentCommentManager(commentService, factory, mockIssueSearchProvider, dateSupport, jqlOperandResolver, service, myLuceneQueryModifier);
    ...
    {
        private final Map<String, Searcher> searchers = new HashMap<String, Searcher>();

        public MockSearchProviderFactory addRegistration(final String name, final Searcher search)
        {
            searchers.put(name, search);
            return this;
        }

        public Searcher getSearcher(final String searcherName)
        {
    ...
    private static class MockIssueSearchProvider extends MockSearchProvider
    {
        ...
        public void search(final Query query, final User user, final HitCollector hitCollector) throws SearchException
        {
            ...
            final FieldHitCollector collector = (FieldHitCollector) hitCollector;

            for (final Long issueId : issueIds)
            {
                final Document document = new Document();
                document.add(new Field(DocumentConstants.ISSUE_ID, String.valueOf(issueId), Field.Store.YES, Field.Index.UN_TOKENIZED));
