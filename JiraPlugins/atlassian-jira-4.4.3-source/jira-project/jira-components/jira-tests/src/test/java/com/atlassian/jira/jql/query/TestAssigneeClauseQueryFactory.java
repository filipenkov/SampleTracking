 ...
 * @since v4.0
 */
public class TestAssigneeClauseQueryFactory extends MockControllerTestCase
{
    @Test
    public void testUnsupportedOperators() throws Exception
    {
        ...
        for (Operator invalidOperator : invalidOperators)
        {
            ...
            AssigneeClauseQueryFactory assigneeClauseQueryFactory = new AssigneeClauseQueryFactory(userResolver, MockJqlOperandResolver.createSimpleSupport());

            TerminalClause terminalClause = new TerminalClauseImpl("assignee", invalidOperator, singleValueOperand);

            final QueryFactoryResult result = assigneeClauseQueryFactory.getQuery(null, terminalClause);
