 ...
 * @since v3.13.2
 */
public class TestConverters extends ListeningTestCase
{
    @Test
    ...
    private void assertConversionException(final Class classUnderTest, final ParameterConverter converter, final String[] inputValues)
    {
        try
        {
            ...
            fail(new StringBuffer().append("Should have thrown an exception for input :").append(inputValues).toString());
        }
    ...
    private void assertArraysEquals(final Object expected[], final Object actual)
    {
        ...
        {
            fail(new StringBuffer().append("not the same length expected : ").append(expected.length).append(" actual : ").append(actualArr.length).toString());
        }
        for (int i = 0; i < expected.length; i++)
        {
            if (expected[i] != actualArr[i])
            {
                ...
                {
                    fail(new StringBuffer().append("Unequal values in index : ").append(i).append(" expected : ").append(expected[i]).append(" actual : ").append(actualArr[i]).toString());
                }
            }
