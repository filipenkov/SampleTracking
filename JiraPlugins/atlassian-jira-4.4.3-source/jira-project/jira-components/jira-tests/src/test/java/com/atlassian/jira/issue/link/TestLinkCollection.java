...
public class TestLinkCollection extends AbstractUsersTestCase
{
    ...
    protected void setUp() throws Exception
    {
        ...
        issueLinkManager = new DefaultIssueLinkManager(delegator, new DefaultIssueLinkCreator(null, new DefaultIssueManager(null, null, null, null, null, null)), new DefaultIssueLinkTypeManager(delegator, null), null, null, null, applicationProperties, null);
    }

    public void testEmptyLinkCollection()
    {
        GenericValue issue = UtilsForTests.getTestEntity("Issue", EasyMap.build("key", "ABC-1"));
        LinkCollection linkColl = issueLinkManager.getLinkCollection(issue, UtilsForTests.getTestUser("blah"));
    ...
    public void testLinkCollection()
    {
        ...
        try
        {
            ...
            assertTrue(linkColl.getLinkTypes().contains(new IssueLinkType(linkType1)));
            assertTrue(linkColl.getLinkTypes().contains(new IssueLinkType(linkType2)));
     ...
     */
    public void testGetOutwardIssues() throws StoreException
    {
        ...
        // give testRemoteUser permissions to browse the first project but not the second
        final User testUser = UtilsForTests.getTestUser("blah");
        PermissionManager oldPermissionManager = (PermissionManager) ManagerFactory.addService(PermissionManager.class, new DefaultPermissionManager()
            {
                public boolean hasPermission(int permissionsId, GenericValue entity, com.atlassian.crowd.embedded.api.User u)
                {
