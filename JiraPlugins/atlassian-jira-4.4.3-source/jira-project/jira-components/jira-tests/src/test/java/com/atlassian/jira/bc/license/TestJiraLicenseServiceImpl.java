...
public class TestJiraLicenseServiceImpl extends MockControllerTestCase
{
   ...
   private static final String EXPECTED_MSG = "expectedMsg";
    ...
    @Test
    public void testValidate_DifferentPartnerName()
    {
        expect(licenseManager.isDecodeable(GOOD_LIC_STRING)).andReturn(true);
        expect(licenseManager.getLicense(GOOD_LIC_STRING)).andReturn(licenseDetails);
        expect(licenseDetails.getLicenseVersion()).andReturn(2);
        expect(licenseDetails.getPartnerName()).andReturn("Sam");
        expect(buildUtilsInfo.getBuildPartnerName()).andReturn("Brad");

        expect(i18nHelper.getText("setup.error.invalidlicensekey.regional.license", "Sam")).andReturn(EXPECTED_MSG);

        JiraLicenseServiceImpl licenseService = instantiate(JiraLicenseServiceImpl.class);
        final JiraLicenseService.ValidationResult validationResult = licenseService.validate(i18nHelper, GOOD_LIC_STRING);
        assertValidationResult(validationResult, EXPECTED_MSG);
    }

    @Test
    public void testValidate_NullLicensePartnerNameButNonBlankBuildPartnerName()
    {
        expect(licenseManager.isDecodeable(GOOD_LIC_STRING)).andReturn(true);
        expect(licenseManager.getLicense(GOOD_LIC_STRING)).andReturn(licenseDetails);
        expect(licenseDetails.getLicenseVersion()).andReturn(2);
        expect(licenseDetails.getPartnerName()).andReturn(null);
        expect(buildUtilsInfo.getBuildPartnerName()).andReturn("Brad");

        expect(i18nHelper.getText("setup.error.invalidlicensekey.regional.installation", "Brad")).andReturn(EXPECTED_MSG);
    ...
    @Test
    public void testValidate_HappyPath_NullPartnerName()
    {
        expect(licenseManager.isDecodeable(GOOD_LIC_STRING)).andReturn(true);
        expect(licenseManager.getLicense(GOOD_LIC_STRING)).andReturn(licenseDetails);
        expect(licenseDetails.getLicenseVersion()).andReturn(2);
        expect(licenseDetails.getPartnerName()).andReturn(null);
        expect(buildUtilsInfo.getBuildPartnerName()).andReturn(null);
    ...
    @Test
    public void testValidate_HappyPath_NonNullPartnerName()
    {
        expect(licenseManager.isDecodeable(GOOD_LIC_STRING)).andReturn(true);
        expect(licenseManager.getLicense(GOOD_LIC_STRING)).andReturn(licenseDetails);
        expect(licenseDetails.getLicenseVersion()).andReturn(2);
        expect(licenseDetails.getPartnerName()).andReturn("Sam");
        expect(buildUtilsInfo.getBuildPartnerName()).andReturn("Sam");
