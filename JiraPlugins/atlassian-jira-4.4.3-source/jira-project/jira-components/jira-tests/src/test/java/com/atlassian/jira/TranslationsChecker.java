...
//                new RegexLineCheck("^[^#=]*=.*[^']'\\{[^']+($|('[^}])+$)", "'{ unmatched with '}", true)
    ...
    public String getProblemsDescription()
    {
        ...
        {
            problemDescription.append(problems.get(i)).append("\n");
        }
        problemDescription.append("Total problems: " + problemCount);
    ...
    public void checkFile(File propertiesFile)
    {
    ...
    void doLineChecks(final BufferedReader br, final String filename) throws IOException
    {
        ...
        while ((line = br.readLine()) != null && problemCount < MAX_PROBLEMS)
        {
            ...
            for (int i = 0; i < propertiesFileCheckRegexes.length; i++)
            {
                propertiesFileCheckRegexes[i].check(filename, line, lineNumber);
            }
        }
    }
    ...
    private class ProblemLogger
    {
        ...
        {
            final String messageBuf = new StringBuffer(mesg).append(" in filename: ").append(filename)
