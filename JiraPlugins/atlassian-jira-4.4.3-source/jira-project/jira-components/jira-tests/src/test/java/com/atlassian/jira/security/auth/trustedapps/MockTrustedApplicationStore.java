...
class MockTrustedApplicationStore implements TrustedApplicationStore
{
    ...
    MockTrustedApplicationStore(final Collection<TrustedApplicationData> datas)
    {
    ...
    private static class Cache
    {
        ...
        {
            byId.put(new Long(data.getId()), data);
            byAppId.put(data.getApplicationId(), data);
        }

        TrustedApplicationData remove(final long id)
        {
            final TrustedApplicationData data = byId.remove(new Long(id));
            if (data != null)
            {
