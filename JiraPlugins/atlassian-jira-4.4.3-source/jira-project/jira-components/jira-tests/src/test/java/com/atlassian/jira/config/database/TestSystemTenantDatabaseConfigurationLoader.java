...
/**
 * Unit test for {@link SystemTenantDatabaseConfigurationLoader}.
 *
 * @since v4.4
 */
public class TestSystemTenantDatabaseConfigurationLoader
{
    ...
    public void loadAndSave()
    {
        final String configString = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
                ...
                + "    <pool-size>99</pool-size>\n"
                + "  </jdbc-datasource>\n"
                + "</jira-database-config>\n";
        final StringReader reader = new StringReader(configString);
        final StringWriter writer = new StringWriter();
        SystemTenantDatabaseConfigurationLoader stdcl = new SystemTenantDatabaseConfigurationLoader(null)
        {
            @Override
        ...
        Assert.assertEquals((Object) 99, jdbcDatasource.getConnectionPoolInfo().getMaxSize());
