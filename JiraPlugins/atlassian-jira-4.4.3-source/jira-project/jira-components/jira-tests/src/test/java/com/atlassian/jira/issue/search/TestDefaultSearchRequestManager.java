 ...
 * @since v3.13
 */
public class TestDefaultSearchRequestManager extends ListeningTestCase
{
    ...
    private MockSharedEntityIndexer mockSharedEntityIndexer;
    ...
    public void setUp()
    {
        ...
        mockSharedEntityIndexer = new MockSharedEntityIndexer();

        user = new MockUser(USER_NAME);
        ...
        searchRequest1 = new SearchRequest(new QueryImpl(), user.getName(), "one", "one description", 1L, 0L);
        searchRequest2 = new SearchRequest(new QueryImpl(), user.getName(), "two", "two description", 2L, 0L);
        searchRequest3 = new SearchRequest(new QueryImpl(), user.getName(), "three", "three description", 3L, 0L);
    }

    private DefaultSearchRequestManager createDefaultSearchRequestManager()
    {
        return new DefaultSearchRequestManager(mockColumnLayoutManager, subscriptionManager, mockShareManager, mockSearchRequestStore,
                mockSharedEntityIndexer, mockSearchService, mockUserUtil);
    }
    ...
    private void replayMocks()
    {
        ...
        EasyMock.replay(mockSearchService, mockUserUtil);
    }
    ...
    private void verifyMocks()
    {
        ...
        EasyMock.verify(mockSearchService, mockUserUtil);
    }
    ...
    public void testSetSanitisedQuerySanitisedIsSame() throws Exception
    {
        ...
        when(mockSearchService.sanitiseSearchQuery(user, query)).thenReturn(query);
    ...
    public void testSetSanitisedQuerySanitisedIsDifferentIsNotModified() throws Exception
    {
        ...
        when(mockSearchService.sanitiseSearchQuery(user, queryOld)).thenReturn(querySanitised);
    ...
    {
        when(mockSearchRequestStore.getAllOwnedSearchRequests(null)).thenReturn(Collections.<SearchRequest>emptyList());
        final SearchRequestManager searchRequestManager = createDefaultSearchRequestManager();
        final Collection results = searchRequestManager.getAllOwnedSearchRequests(null);
    ...
    public void getAllOwnedSearchRequestsSetsTheSharePermissionsOnTheSearchRequestOwnedByTheUser()
    {
        ...
        when(mockSearchService.sanitiseSearchQuery(eq(user), isA(Query.class))).thenAnswer(byReturningArgumentNo(2));
        final SearchRequestManager searchRequestManager = createDefaultSearchRequestManager();
    ...
    public void getRequestAuthorAndNameReturnsNullForANullUser()
    {
        ...
        assertNull(searchRequestManager.getOwnedSearchRequestByName(null, NICKS_FILTER));
    }
    ...
    public void getSearchRequestByIdReturnsNullGivenThatTheRequestHasNotBeenSharedWithTheSpecifiedUser()
    {
        ...
        when(mockShareManager.isSharedWith(null, searchRequest1)).thenReturn(false);

        final SearchRequestManager searchRequestManager = createDefaultSearchRequestManager();

        final SearchRequest request = searchRequestManager.getSearchRequestById(null, searchRequest1.getId());
        assertNull(request);
    }
    ...
    public void getSearchRequestByIdReturnsASearchRequestObjectForANullUserWhenThatRequestHasBeenSharedWithThatUser()
    {
        ...
        when(mockShareManager.isSharedWith(null, searchRequest2)).thenReturn(true);
        ...
        final SearchRequest request = searchRequestManager.getSearchRequestById(null, searchRequest2.getId());
    ...
    {
        final SearchRequestManager searchRequestManager = createDefaultSearchRequestManager();
        final SearchRequest request = new SearchRequest(new QueryImpl(), user.getName(), null, "description");
    ...
    public void createShouldReturnASearchRequestWithNoPermissionsGivenASearchRequestThatIsPrivate()
    {
        ...
        when(mockUserUtil.getUserObject(USER_NAME)).thenReturn(user);
    ...
    {
        final SearchRequestManager searchRequestManager = createDefaultSearchRequestManager();
        final SearchRequest notIdRequest = new SearchRequest(new QueryImpl(), user.getName(), "name", "description");
    ...
    public void deletingAnExistingSearchRequestShouldRemoveItFromTheStore() throws Exception
    {
        ...
        when(subscriptionManager.getAllSubscriptions(searchRequest1.getId())).
        ...
        verify(subscriptionManager).deleteSubscription(subscriptionId1);
        verify(subscriptionManager).deleteSubscription(subscriptionId2);
    ...
    public void deleteDoesNotDeleteSubscriptionsWhenTheSearchRequestHasNotBeenSubscribedTo() throws Exception
    {
        ...
        when(subscriptionManager.getAllSubscriptions(searchRequest1.getId())).thenReturn(null);

        final SearchRequestManager searchRequestManager = createDefaultSearchRequestManager();

        searchRequestManager.delete(searchRequest1.getId());

        verify(subscriptionManager, never()).deleteSubscription(anyLong());
    }
    ...
    {
        when(mockSearchRequestStore.getSearchRequest(searchRequest1.getId())).thenReturn(searchRequest1);
        doThrow(new RuntimeException()).when(subscriptionManager).getAllSubscriptions(searchRequest1.getId());
    ...
    public void getSearchRequestOwnerReturnsTheOwnerStoredForTheSearchRequest()
    {
        ...
        assertEquals(user.getName(), manager.getSearchRequestOwnerUserName(searchRequest1.getId()));
    }
    ...
    public void getSearchRequestOwnerReturnsNullGivenASearchRequestThatIsNotStored()
    {
        ...
        final SearchRequestManager manager = createDefaultSearchRequestManager();
        assertNull(manager.getSearchRequestOwnerUserName(searchId));
    }
    ...
    {
        final SearchRequestManager manager = createDefaultSearchRequestManager();
        manager.getSearchRequestOwnerUserName(null);
    }
    ...
    public void getSharedEntityShouldDelegateToGetSearchRequestById()
    {
        ...
        final SearchRequestManager searchRequestManager = new DefaultSearchRequestManager(mockColumnLayoutManager, subscriptionManager, mockShareManager,
                mockSearchRequestStore, new MockSharedEntityIndexer(), mockSearchService, mockUserUtil)
        {
            @Override
            public SearchRequest getSearchRequestById(final com.atlassian.crowd.embedded.api.User actualUser, final Long id)
            {
        ...
        final SharedEntity actualSharedEntity = searchRequestManager.getSharedEntity(user, searchRequest3.getId());
    ...
    public void hasPermissionsToUseReturnsTrueWhenTheSearchHasBeenSharedWithTheSpecifiedUser()
    {
        ...
        final SearchRequestManager searchRequestManager = createDefaultSearchRequestManager();
        final boolean actualResult = searchRequestManager.hasPermissionToUse(user, searchRequest2);
    ...
    public void hasPermissionsToUseReturnsFalseWhenTheSearchHasNotBeenSharedWithTheSpecifiedUser()
    {
        ...
        final SearchRequestManager searchRequestManager = createDefaultSearchRequestManager();
        final boolean actualResult = searchRequestManager.hasPermissionToUse(user, searchRequest3);
