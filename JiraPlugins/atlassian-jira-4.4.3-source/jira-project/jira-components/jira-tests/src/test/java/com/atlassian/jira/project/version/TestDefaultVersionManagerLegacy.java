...
public class TestDefaultVersionManagerLegacy extends LegacyJiraMockTestCase
{
    ...
    {
        delegatorMock.expectAndReturn("findAll", P.args(P.eq("Version"), P.eq(EasyList.build("sequence"))), existingVersionGVs);
        ...
        versionManager.createVersion("Version 1", null, null, project1, null);
    ...
    {
        delegatorMock.expectAndReturn("findAll", P.args(P.eq("Version"), P.eq(EasyList.build("sequence"))),
        ...
        final DefaultVersionManager versionManager = getVersionManager();
        versionManager.createVersion("Version 2", null, null, project1, new Long(-1));
    ...
    public void testCreateVersionWithReleaseDateAndDescription() throws GenericEntityException, CreateException
    {
        ...
        final DefaultVersionManager versionManager = getVersionManager();
        versionManager.createVersion("Version 2", releaseDate, "The Description", project1, null);
    ...
    /**
     * @deprecated since v3.13. Manager no longer does validation
     */
    public void testValidateDeleteParams() throws GenericEntityException
    {
        // No Version
        _testDeleteValidator(VersionKeys.DELETE_ACTION, null, VersionKeys.SWAP_ACTION, version1.getLong("id"), VersionKeys.SWAP_ACTION,
            version1.getLong("id"));

        // Unknown Affects Action
        _testDeleteValidator(VersionKeys.DELETE_ACTION, version1, "Unknown Action", version1.getLong("id"), VersionKeys.SWAP_ACTION,
            version1.getLong("id"));

        // Swap AffectsVersion To version to be deleted
        _testDeleteValidator(VersionKeys.DELETE_ACTION, version1, VersionKeys.SWAP_ACTION, version1.getLong("id"), VersionKeys.SWAP_ACTION,
            version2.getLong("id"));

        // Unknown Fix Action
        _testDeleteValidator(VersionKeys.DELETE_ACTION, version1, VersionKeys.SWAP_ACTION, version1.getLong("id"), "Unknown Action",
            version1.getLong("id"));

        // Swap Fix Version to Version to be deleted
        _testDeleteValidator(VersionKeys.DELETE_ACTION, version1, VersionKeys.SWAP_ACTION, version2.getLong("id"), VersionKeys.SWAP_ACTION,
            version1.getLong("id"));
    }

    private void _testDeleteValidator(final String actionType, final Version version, final String affectsAction, final Long affectsSwapVersionId, final String fixAction, final Long fixSwapVersionId)
    {
        final DefaultVersionManager defaultVersionManager = getVersionManager();
        try
        {
            defaultVersionManager.deleteVersion(actionType, version, affectsAction, affectsSwapVersionId, fixAction, fixSwapVersionId);
    ...
    public void testOldDeleteVersionAffectingNoIssues() throws GenericEntityException
    {
        issueManagerMock.expectAndReturn("getIssuesByEntity", P.args((P.IS_NOT_NULL), P.IS_NOT_NULL), Collections.EMPTY_LIST);
        issueManagerMock.setStrict(true);

        associationManager.expectVoid("removeAssociationsFromSink", P.args(P.eq(version1.getGenericValue())));
        associationManager.setStrict(true);

        issueIndexManagerMock.expectAndReturn("reIndexIssues", P.args(P.eq(Collections.EMPTY_SET)), new Long(1));
        issueIndexManagerMock.setStrict(true);

        final DefaultVersionManager defaultVersionManager = getVersionManager();

        defaultVersionManager.deleteVersion(VersionKeys.DELETE_ACTION, version1, VersionKeys.REMOVE_ACTION, null, VersionKeys.REMOVE_ACTION, null);

        issueIndexManagerMock.verify();
        associationManager.verify();
        issueIndexManagerMock.verify();
    }

    public void testOldDeleteVersionRemoveAffectedIssues() throws GenericEntityException
    {
        issueManagerMock.expectAndReturn("getIssuesByEntity", P.args((P.IS_NOT_NULL), P.IS_NOT_NULL), affectedIssues);
        issueManagerMock.setStrict(true);

        associationManager.expectVoid("removeAssociationsFromSink", P.args(P.eq(version1.getGenericValue())));
        associationManager.setStrict(true);

        issueIndexManagerMock.expectAndReturn("reIndexIssues", P.args(P.IS_ANYTHING), new Long(1));
        issueIndexManagerMock.setStrict(true);
    ...
    public void testOldDeleteVersionSwapAffectedIssues() throws GenericEntityException
    {
        delegatorMock.expectAndReturn("findByPrimaryKey", P.args(P.eq("Version"), P.IS_NOT_NULL), version2.getGenericValue());
        delegatorMock.expectAndReturn("removeValue", P.args(P.eq(version1.getGenericValue())), new Integer(1));
        //JRA-13766: Need to check that the versions will be re-stored with corrected sequence numbers after a version is delted.
        //Since easymock is pretty stupid and can't mock two method calls with different parameters, we'll simulate that one of
        //the version's sequence numbers has already been corrected.
        version2.setSequence(new Long(1));
        delegatorMock.expectAndReturn("findAll", P.args(P.eq("Version"), P.IS_NOT_NULL), EasyList.build(version2.getGenericValue(),
            version4.getGenericValue()));
        //The delete action should restore version4 with a new sequence number of 2.
        final Version expectedVersion4 = new MyVersion(null, new MockGenericValue("Version", EasyMap.build("name", "Version 4", "id", new Long(1004),
            "sequence", new Long(2), "project", PROJECT_ONE_ID, "archived", "true", "released", "true")));
        delegatorMock.expectVoid("store", P.args(P.eq(expectedVersion4.getGenericValue())));
        delegatorMock.setStrict(true);
        ...
        associationManager.expectVoid("removeAssociationsFromSink", P.args(P.eq(version1.getGenericValue())));
        final Constraint[] swapAssociationArgs = { P.eq("Issue"), P.IS_NOT_NULL, P.IS_NOT_NULL, P.IS_NOT_NULL };
        associationManager.expectVoid("swapAssociation", swapAssociationArgs);
        associationManager.setStrict(true);

        issueIndexManagerMock.expectAndReturn("reIndexIssues", P.ANY_ARGS, new Long(2));
        issueIndexManagerMock.setStrict(true);

        final DefaultVersionManager defaultVersionManager = getVersionManager();

        defaultVersionManager.deleteVersion(VersionKeys.DELETE_ACTION, version1, VersionKeys.SWAP_ACTION, version2.getLong("id"),
            VersionKeys.SWAP_ACTION, version2.getLong("id"));
    ...
    {
        projectManager.expectAndReturn("getProjectObj", P.args(P.eq(PROJECT_ONE_ID)), project1Obj);
        projectManager.setStrict(true);
    ...
    {
        delegatorMock.expectAndReturn("findByPrimaryKey", P.args(P.eq("Version"), P.IS_NOT_NULL), version1.getGenericValue());
        delegatorMock.expectVoid("store", P.args(P.eq(version1.getGenericValue())));
        delegatorMock.setStrict(true);
    ...
    public void testGetUnarchivedVersions() throws Exception
    {
        ...
        delegatorMock.expectAndReturn("findAll", P.args(P.eq("Version"), P.eq(EasyList.build("sequence"))), unarchivedVersions);
        delegatorMock.setStrict(true);

        final DefaultVersionManager defaultVersionManager = getVersionManager();

        final Collection unarchived = defaultVersionManager.getVersionsUnarchived(project1);
    ...
    {
        delegatorMock.expectAndReturn("findAll", P.args(P.eq("Version"), P.eq(EasyList.build("sequence"))), currentVersions);
    ...
    {
        delegatorMock.expectAndReturn("findByPrimaryKey", P.args(P.eq("Version"), P.eq(EasyMap.build("id", version.getId()))),
