...
public class TestJiraAnalyzer extends ListeningTestCase
{
    ...
    public void testSqlProcessorFound() throws IOException, ParseException
    {
        ...
        assertSearchFoundInAllLanguages(exceptionLine, "ofbiz");
    ...
    {
        final Hits hits = getHitsForSearch(language, textToSearch, searchTerm);
    ...
    {
        final Hits hits = getHitsForSearch(language, textToSearch, searchTerm);
        assertEquals("Search Term '" + searchTerm + "' was found in '" + textToSearch + "'.", 0, hits.length());
    }

    private Hits getHitsForSearch(final String language, final String textToSearch, final String searchTerm) throws IOException, ParseException
    {
        ...
        final IndexWriter writer = new IndexWriter(directory, indexingAnalyzer, true);

        final Document doc = new Document();
        doc.add(new Field("term", textToSearch, Field.Store.YES, Field.Index.TOKENIZED));
        writer.addDocument(doc);
        writer.close();

        final IndexSearcher searcher = new IndexSearcher(directory);
        final QueryParser parser = new QueryParser("term", searchingAnalyzer);
