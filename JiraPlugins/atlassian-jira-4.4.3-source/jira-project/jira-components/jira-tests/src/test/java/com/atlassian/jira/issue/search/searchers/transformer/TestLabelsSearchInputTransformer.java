...
public class TestLabelsSearchInputTransformer extends ListeningTestCase
{
    @Test
    public void testPopulateFromParams()
    {
        final User user = new MockOSUser("admin");

        final LabelsSearchInputTransformer transformer = new LabelsSearchInputTransformer(null, null, null, null);

        final FieldValuesHolder fieldValuesHolder = new FieldValuesHolderImpl();
        final ActionParams params = new ActionParamsImpl(MapBuilder.<String, String[]>build("labels",
                new String[] { "label1 label2 label3" }));
        transformer.populateFromParams(user, fieldValuesHolder, params);

        assertEquals(CollectionBuilder.<String>newBuilder("label1", "label2", "label3").asList(),
                fieldValuesHolder.get("labels"));
    }
