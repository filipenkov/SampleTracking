...
public class TestWriterWrapper extends ListeningTestCase
{
    ...
    public void testOptimize() throws Exception
    {
        ...
        final IndexWriter writer = new IndexWriter(configuration.getDirectory(), configuration.getAnalyzer())
        {
            @Override
        ...
        try
        {
            final WriterWrapper wrapper = new WriterWrapper(new Supplier<IndexWriter>()
            {
                public IndexWriter get()
                {
                    return writer;
                }
            }, configuration, UpdateMode.INTERACTIVE);
    ...
    public void testUpdate() throws Exception
    {
        ...
        wrapper.updateDocuments(new Term("blah", "blah"), EasyList.build(new Document(), new Document(), new Document()));
        assertEquals(1, delete.get());
        assertEquals(3, add.get());
        assertEquals(0, update.get());
        wrapper.updateDocuments(new Term("blah", "blah"), EasyList.build(new Document(), new Document()));
        assertEquals(2, delete.get());
        assertEquals(5, add.get());
        assertEquals(0, update.get());
        wrapper.updateDocuments(new Term("blah", "blah"), EasyList.build(new Document()));
        assertEquals(2, delete.get());
        assertEquals(5, add.get());
