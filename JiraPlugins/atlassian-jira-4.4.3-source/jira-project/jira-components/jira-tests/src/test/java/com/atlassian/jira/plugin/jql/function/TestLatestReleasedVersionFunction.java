 ...
 * @since v4.3
 */
public class TestLatestReleasedVersionFunction extends ListeningTestCase
{
    ...
    public void testValidateTooFewArguments() throws Exception
    {
        ...
        FunctionOperand function = new FunctionOperand(LatestReleasedVersionFunction.FUNCTION_LATEST_RELEASED_VERSION, new ArrayList<String>());
        ...
        assertEquals("Function 'latestReleasedVersion' expected '1' arguments but received '0'.", messageSet.getErrorMessages().iterator().next());
    }
    ...
    public void testValidateTooManyArguments() throws Exception
    {
        ...
        FunctionOperand function = new FunctionOperand(LatestReleasedVersionFunction.FUNCTION_LATEST_RELEASED_VERSION, Arrays.asList("should", "not", "be", "here"));
        final MessageSet messageSet = latestreleasedversionFunction.validate(null, function, terminalClause);
        assertTrue(messageSet.hasAnyMessages());
        assertTrue(messageSet.hasAnyErrors());
        assertFalse(messageSet.hasAnyWarnings());
        assertEquals(1, messageSet.getErrorMessages().size());
        assertEquals("Function 'latestReleasedVersion' expected '1' arguments but received '4'.", messageSet.getErrorMessages().iterator().next());
    }
    ...
    public void testGetMinimumNumberOfExpectedArguments() throws Exception
    {
        ...
        final VersionManager versionManager = EasyMock.createMock(VersionManager.class);
        LatestReleasedVersionFunction latestreleasedversionFunction = new LatestReleasedVersionFunction(versionManager, projectResolver, permissionmanager);
        assertEquals(1, latestreleasedversionFunction.getMinimumNumberOfExpectedArguments());
    }
