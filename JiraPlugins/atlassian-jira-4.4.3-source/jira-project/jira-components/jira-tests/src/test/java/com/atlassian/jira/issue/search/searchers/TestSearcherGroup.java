 ...
 * @since v4.0
 */
public class TestSearcherGroup extends ListeningTestCase
{
    @Test
    ...
    private static class MockSearchRenderer implements SearchRenderer
    {
        ...
        public String getEditHtml(final com.opensymphony.user.User searcher, final SearchContext searchContext, final FieldValuesHolder fieldValuesHolder, final Map<?, ?> displayParameters, final Action action)
        {
            return null;
        }

        public boolean isShown(final com.opensymphony.user.User searcher, final SearchContext searchContext)
        {
            return shown;
        }

        public String getViewHtml(final com.opensymphony.user.User searcher, final SearchContext searchContext, final FieldValuesHolder fieldValuesHolder, final Map<?, ?> displayParameters, final Action action)
        {
            return null;
        }

        public boolean isRelevantForQuery(final com.opensymphony.user.User searcher, final Query query)
        {
            return false;
        }
    }
