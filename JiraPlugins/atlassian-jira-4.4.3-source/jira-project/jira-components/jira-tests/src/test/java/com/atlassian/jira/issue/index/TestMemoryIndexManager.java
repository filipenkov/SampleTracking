...
public class TestMemoryIndexManager extends AbstractUsersTestCase
{
    ...
    protected void setUp() throws Exception
    {
        ...
        PropertiesManager.getInstance().getPropertySet().setString(APKeys.JIRA_PATH_INDEX, "memory");
    }
    ...
    public void testLuceneMemoryIndexing() throws IOException, SearchException, ParseException
    {
        ...
        final IndexWriter writer = new IndexWriter(ramDir, JiraAnalyzer.ANALYZER_FOR_INDEXING, true);
        final Document doc = IssueDocument.getDocument(issue);
        writer.addDocument(doc);

        // check that documents has increased by 1
        assertEquals(1, writer.docCount());
        writer.close();
    ...
    public void testReindexAllAndSearch() throws IndexException, ParseException, IOException, SearchException
    {
        ...
        final Hits hits = search(issueDir);
     ...
     * @throws IOException
     */
    private Hits search(final Directory dir) throws ParseException, IOException, SearchException
    {
