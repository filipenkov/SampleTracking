...
public class TestDefaultRepositoryManager extends AbstractUsersIndexingTestCase
{
    ...
    protected void setUp() throws Exception
    {
        ...
        repositoryManager = new DefaultRepositoryManager(CoreFactory.getAssociationManager(), CoreFactory.getGenericDelegator(),
            ManagerFactory.getServiceManager(), (PermissionManager) mockPermissionManager.proxy(),
            (ChangeHistoryManager) mockChangeHistoryManager.proxy(), null, null);

        user = UtilsForTests.getTestUser("Sam");
    ...
    public void testMarkVcsServiceToRun() throws GenericEntityException
    {
        ...
        final DefaultRepositoryManager repositoryManager = new DefaultRepositoryManager(null, null, serviceManager, null, null, null, null)
        {
            @Override
    ...
    private void _testCreateRepository(final String viewCVSRootParameter) throws Exception
    {
        ...
        repositoryManager = new DefaultRepositoryManager(CoreFactory.getAssociationManager(), CoreFactory.getGenericDelegator(),
            (ServiceManager) mockServiceManager.proxy(), null, null, null, null);
    ...
    public void testRemoveRepository() throws Exception
    {
        ...
        final GenericDelegator genericDelegator = CoreFactory.getGenericDelegator();
        repositoryManager = new DefaultRepositoryManager(CoreFactory.getAssociationManager(), genericDelegator,
            (ServiceManager) mockServiceManager.proxy(), null, null, null, null);

        final Long id = psa.getLong("id");
        List versionControlGVs = genericDelegator.findByAnd("VersionControl", EasyMap.build("id", id));
        assertEquals(1, versionControlGVs.size());
        ...
        versionControlGVs = genericDelegator.findByAnd("VersionControl", EasyMap.build("id", id));
     ...
     */
    public void testGetCommitsNoPermission() throws ImmutableException, DuplicateEntityException, GenericEntityException, RepositoryException
    {
    ...
    public void testProjectWithNoRepository() throws GenericEntityException
    {
        ...
        final GenericValue issue = UtilsForTests.getTestEntity("Issue", EasyMap.build("id", new Long(2), "project", projectA.getLong("id"), "key",
            ISSUE_KEY));
        final Issue issueObj = IssueImpl.getIssueObject(issue);
        ...
        final Map commits = repositoryManager.getCommits(issueObj, user);
        assertTrue(commits.isEmpty());
    }
