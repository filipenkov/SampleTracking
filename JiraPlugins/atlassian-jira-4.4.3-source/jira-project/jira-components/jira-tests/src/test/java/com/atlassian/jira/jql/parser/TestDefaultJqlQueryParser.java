...
public class TestDefaultJqlQueryParser extends Junit3ListeningTestCase
{
    ...
    private static final Set<String> currentWords = CollectionBuilder.newBuilder("empty", "null", "and", "or", "not", "in", "is", "cf", "order", "by", "desc", "asc", "on", "before", "to", "after", "from").asSet();
    ...
    public static TestSuite suite()
    {
        ...
        test.add(String.format("pri not in func(%d)", tooBigNumber), new TerminalClauseImpl("pri", NOT_IN, new FunctionOperand("func", Collections.<String>singletonList(tooBigNumber.toString()))));
        ...
        for (String reservedWord : JqlStringSupportImpl.RESERVED_WORDS)
        {
        ...
        //Some tests for invalid parsing input.
        {
            ...
            test.addException("Bare field", "foo", JqlParseErrorMessages.badOperator(Token.EOF_TOKEN));
            test.addException("Missing operand", "foo=", JqlParseErrorMessages.badOperand(Token.EOF_TOKEN));
            ...
            test.addException("Legal subclause with missing second subclause", "foo=bar and", JqlParseErrorMessages.badFieldName(Token.EOF_TOKEN));
            ...
            test.addException("Incomplete second subclause", "foo=bar and 78foo", JqlParseErrorMessages.badOperator(Token.EOF_TOKEN));
            test.addException("Incomplete second subclause", "foo=bar and 78foo=", JqlParseErrorMessages.badOperand(Token.EOF_TOKEN));
            test.addException("Incomplete second subclause with newline", "foo=bar and \n78foo", JqlParseErrorMessages.badOperator(Token.EOF_TOKEN));
            ...
            test.addException("Second subclause missing with not", "foo=bar and not", JqlParseErrorMessages.badFieldName(Token.EOF_TOKEN));
            test.addException("Second subclause incomplete with not", "foo=bar and not foo =", JqlParseErrorMessages.badOperand(Token.EOF_TOKEN));
            test.addException("Just not", "not", JqlParseErrorMessages.badFieldName(Token.EOF_TOKEN));
            test.addException("two nots", "not not", JqlParseErrorMessages.badFieldName(Token.EOF_TOKEN));
            ...
            test.addException("just one paren", "(", JqlParseErrorMessages.badFieldName(Token.EOF_TOKEN));
            ...
            test.addException("complex nested parens don't match", "abc in ((fee, fie, foe, fum), 787, (34, (45))", JqlParseErrorMessages.expectedText(Token.EOF_TOKEN, ")"));
            test.addException("Random crap on the end1", "priority = 12345=== not p jfkff fjfjfj", JqlParseErrorMessages.needLogicalOperator(createToken("=", 1, 16)));
            ...
            test.addException("unmatched parens (open only)", "(a in a", JqlParseErrorMessages.expectedText(Token.EOF_TOKEN, ")"));
            test.addException("unmatched parens (open only)", "abc in (foo", JqlParseErrorMessages.expectedText(Token.EOF_TOKEN, ")"));
            test.addException("unmatched parens (open open)", "abc in (foo(", JqlParseErrorMessages.expectedText(Token.EOF_TOKEN, ")"));
            ...
            test.addException("unmatched parens (open open)", "abc in ((foo), a", JqlParseErrorMessages.expectedText(Token.EOF_TOKEN, ")"));
            test.addException("unmatched parens (open open close)", "(abc  =        b or not (j=k and p=l)", JqlParseErrorMessages.expectedText(Token.EOF_TOKEN, ")"));
            ...
            test.addException("Bad function argument", "q = func(a, [)", JqlParseErrorMessages.badFunctionArgument(createToken("[", 1, 12)));
            test.addException("Bad function argument", "q = func(a, [", JqlParseErrorMessages.badFunctionArgument(createToken("[", 1, 12)));
            test.addException("Bad function argument", "q = func(a, )", JqlParseErrorMessages.emptyFunctionArgument(createToken(")", 1, 12)));
            test.addException("Bad function argument", "q = func(a, ", JqlParseErrorMessages.badFunctionArgument(Token.EOF_TOKEN));
            test.addException("Bad function argument", "q = func(a", JqlParseErrorMessages.expectedText(Token.EOF_TOKEN, ")"));
            test.addException("Bad function argument", "q = func(", JqlParseErrorMessages.expectedText(Token.EOF_TOKEN, ")"));
            test.addException("Bad function argument", "q = func(good, jack!, \"really good\")", JqlParseErrorMessages.expectedText(createToken("!", 1, 19), ")", ","));

            test.addException("No such operator", "pri notin (x)", JqlParseErrorMessages.reservedWord("notin", 1, 4));
            test.addException("No such operator1", "pri isnot empty", JqlParseErrorMessages.badOperator(createToken("isnot", 1, 4)));
            test.addException("No such operator2", "pri ^ empty", JqlParseErrorMessages.reservedCharacter('^', 1, 4));
            test.addException("No such operator3", "pri is", JqlParseErrorMessages.badOperand(Token.EOF_TOKEN));
            test.addException("No such operator4", "pri not is empty", JqlParseErrorMessages.expectedText(createToken("is", 1, 8), "IN"));
            ...
            test.addException("Unmatched string quote", "priority = \"", JqlParseErrorMessages.unfinishedString(null, 1, 11));
            test.addException("Unmatched string quote", "priority = '''", JqlParseErrorMessages.unfinishedString(null, 1, 13));
            test.addException("Unmatched string quote", "priority = '", JqlParseErrorMessages.unfinishedString(null, 1, 11));
            ...
            test.addException("Empty Function Name", "b = \"\"()", JqlParseErrorMessages.emptyFunctionName(1, 4));
            ...
            test.addException("Unterminated illegal unicod10", "case order by \\u-998zzzzz", JqlParseErrorMessages.illegalEsacpe("\\u-", 1, 14));
            ...
            test.addException("Control Character String 5", "aa = order by \u0001", JqlParseErrorMessages.illegalCharacter('\u0001', 1, 14));
            ...
            test.addException("Empty Order By", "a = b order by", JqlParseErrorMessages.badFieldName(Token.EOF_TOKEN));
            test.addException("Empty Order By2", "a = b order", JqlParseErrorMessages.expectedText(Token.EOF_TOKEN, "by"));
            test.addException("Empty Order By2", "a = b order bad", JqlParseErrorMessages.expectedText(createToken("bad", 1, 12), "by"));
            test.addException("Unfinished Order By", "a = b order BY abc desc, ", JqlParseErrorMessages.badFieldName(Token.EOF_TOKEN));
            ...
            Set<String> words = new HashSet<String>(JqlStringSupportImpl.RESERVED_WORDS);
            words.removeAll(currentWords);
            for (String reservedWord : words)
            {
        ...
        test.addException("Incomplete was clause", "status was", JqlParseErrorMessages.badOperand(Token.EOF_TOKEN));
    ...
    private static class JqlParserErrorTest extends Junit3ListeningTestCase
    {
        ...
        protected void runTest()
        {
            ...
            {
                assertEquals(parseError, e.getParseErrorMessage());
            }
        }
    }
