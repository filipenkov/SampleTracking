...
{
    public TestFullCommentHandler(String s)
    {
        super(s);
    }
            ...
            throws Exception
    {
        ...
        this.message = HandlerTestUtil.createMessageFromFile(messageSourceFilename, ISSUE_KEY);
        ...
        assertEquals(true, this.handler.handleMessage(this.message));
        assertNull(this.errorHandler.getError());
        ...
        final List changeHistoryActions = changeHistoryManager.getChangeHistoriesForUser(issueObject, u1);
        List changeItems;
        if (null != attachmentFilename)
        {
            ...
            final ChangeHistory changeHistory = (ChangeHistory) changeHistoryActions.iterator().next();
            changeItems = changeHistory.getChangeItems();
            assertTrue(0 != changeItems.size());
        ...
        final List<Attachment> attachments = ManagerFactory.getAttachmentManager().getAttachments(issue);
        ...
        for (Attachment attachment : attachments)
        {
            ...
            assertEquals(mimeType, attachmentMimeType);
            assertEquals(attachmentFilename, attachment.getFilename());
        }
    }
