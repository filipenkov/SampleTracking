...
public interface JqlOperandResolver
{
     ...
     * @return a list of the values in the literal. May return null on an error.
     */
    List<QueryLiteral> getValues(com.opensymphony.user.User searcher, Operand operand, final TerminalClause terminalClause);
     ...
     * unknown, an error message will be added to the message set returned. Never null.
     */
    MessageSet validate(com.opensymphony.user.User user, Operand operand, final TerminalClause terminalClause);
     ...
     * @throws IllegalArgumentException if the operand contains more than one value.
     */
    QueryLiteral getSingleValue(com.opensymphony.user.User user, Operand operand, final TerminalClause clause);
