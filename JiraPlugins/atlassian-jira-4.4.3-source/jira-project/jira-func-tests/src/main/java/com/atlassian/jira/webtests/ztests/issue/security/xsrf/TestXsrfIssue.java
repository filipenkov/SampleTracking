...
public class TestXsrfIssue extends FuncTestCase
{
    ...
    public void testXsrfPageSessionCanBeRetried()
    {
        ...
        assertions.getTextAssertions().assertTextSequence(new WebPageLocator(tester),
                "Session Expired",
                "JIRA could not complete this action because your session has expired",
    ...
    public void testIssueLinking()
    {
        ...
        StringBuilder url = new StringBuilder().append("/secure/LinkExistingIssue.jspa?atl_token=").append("invalidtoken").
                append("&id=").append(getIssueIdWithIssueKey(issueKey)).append("&linkDesc=").append("inward").append("&currentIssueKey=").
                append(issueKey).append("&linkKey=").append(issueKey2);
        ...
        url = new StringBuilder().append("/secure/LinkExistingIssue.jspa?atl_token=").append(page.getXsrfToken()).
                append("&id=").append(getIssueIdWithIssueKey(issueKey)).append("&linkDesc=").append("inward").append("&currentIssueKey=").
                append(issueKey).append("&linkKey=").append(issueKey2);
        navigation.gotoPage(url.toString());
    ...
    {
        administration.usersAndGroups().addUser("my-user");
        administration.usersAndGroups().addUserToGroup("my-user", "jira-developers");
                ...
                new XsrfCheck(
                        ...
                        new XsrfCheck.Setup()
                        {
                            public void setup()
                            {
                                String issueKey = navigation.issue().createIssue("monkey", "Bug", "My Bug");
                                navigation.issue().viewIssue(issueKey);
                                tester.clickLinkWithText("sub-task");
