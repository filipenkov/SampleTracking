...
public class TestUserLimitedLicense extends FuncTestCase
{
    ...
    {
        _testLicenseCanSubmitSupportRequest(LicenseKeys.V2_STARTER);
    }

    public void testLicenseCanSubmitSupportRequest_CommercialLimited()
    {
        _testLicenseCanSubmitSupportRequest(LicenseKeys.V2_COMMERCIAL_LIMITED);
    }

    private void _testLicenseCanSubmitSupportRequest(final LicenseKeys.License license)
    {
        administration.switchToLicense(license);
        addUsersWhileUnderTheLimit("devman");
        addUsersWhileUnderTheLimit("prodman");
        ...
        tester.assertTextPresent("Please enter the details of your support problem here");
        final String desc = license.getDescription();
        text.assertTextSequence(new XPathLocator(tester, "//table[@id='support_req_env']"), new String[] { "License Type", desc, "User Limit", "5 (4 currently active)" });
    }

    private void addMailServerConfig()
    {
        administration.mailServers().goTo().Smtp();
