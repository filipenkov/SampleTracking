...
public class ViewIssueAssertions extends AbstractFuncTestUtil
{
    ...
    public void assertOnViewIssuePage(String issueKey)
    {
        ...
        assertions.getLinkAssertions().assertLinkPresentWithExactText("//div[@class='item-header']", issueKey);
    }
     ...
     *
     * <p>If you expect multiple versions then you should separate them with a comma and space, eg "1.0, 1.1, 2.0".
     * If you expect no affected versions, then give the String "None", or use {@link #assertAffectsVersionsNone}.
     *
     ...
     *
     * Note that the text has whitespace collapsed, so it will return values like "1.0, 1.1, 2.0".
     * If there are no affected versions set, then the value returned is whatever the page displays (currently "None").
     *
     ...
     *
     * <p>If you expect multiple versions then you should separate them with a comma and space, eg "1.0, 1.1, 2.0".
     * If you expect no fix versions, then give the String "None", or use {@link #assertFixVersionsNone}.
     *
     ...
     *
     * <p>If you expect multiple components then you should separate them with a comma and space, eg "1.0, 1.1, 2.0".
     * If you expect no components, then give the String "None", or use {@link #assertComponentsNone}.
     *
    ...
    /**
     * Finds a value within the given table, assuming that the table is a list of name-value pairs.
     *
     * @param tableId ID of the table to search.
     * @param label Expected label.
     * @return The first value found in the table with the given label.
     */
    private String getTableValueForLabel(final String tableId, final String label)
    {
        // Get the "issue_header" table
        WebTable webTable = tester.getDialog().getWebTableBySummaryOrId(tableId);
        if (webTable == null)
        {
            // Table doesn't exist. JIRA hides values that are blank :(
            return null;
        }
        // find the row with the desired label
        for (int row = 0; row < webTable.getRowCount(); row ++)
        {
            String thisLabel = webTable.getCellAsText(row, 0);
            if (thisLabel != null && thisLabel.trim().startsWith(label))
            {
