...
public class TestAdminTabs extends FuncTestCase
{
    ...
    public void testCorrectTabsShown() throws SAXException
    {
        ...
        List<AdminTabs.TabGroup> expected = createFlatTabs(new AdminTabs.Tab("Issue Types", true), new AdminTabs.Tab("Sub-Tasks", false));
        assertEquals("Issue Types", tabs.getPageHeading());
        assertEquals(expected, tabs.getCurrentTabs());
        tester.clickLink("subtasks_tab");
        expected = createFlatTabs(new AdminTabs.Tab("Issue Types", false), new AdminTabs.Tab("Sub-Tasks", true));
        assertEquals("Issue Types", tabs.getPageHeading());
        assertEquals(expected, tabs.getCurrentTabs());

        navigation.gotoAdminSection("trackbacks");
        expected = createFlatTabs(new AdminTabs.Tab("Trackbacks", true), new AdminTabs.Tab("Time Tracking", false), new AdminTabs.Tab("Issue Linking", false));
        assertEquals("Issue Features", tabs.getPageHeading());
        assertEquals(expected, tabs.getCurrentTabs());

        navigation.gotoAdminSection("timetracking");
        expected = createFlatTabs(new AdminTabs.Tab("Trackbacks", false), new AdminTabs.Tab("Time Tracking", true), new AdminTabs.Tab("Issue Linking", false));
        assertEquals("Issue Features", tabs.getPageHeading());
        assertEquals(expected, tabs.getCurrentTabs());
        ...
        expected = CollectionBuilder.newBuilder(
                ...
                new AdminTabs.TabGroup(CollectionBuilder.newBuilder(
                        new AdminTabs.Tab("People", false),
