...
public class TestLinkIssue extends JIRAWebTest
{
    ...
    /**
     * 1. Tests if the 'Link Issue' link is not available if there are NO saved links and with 'Issue Linking' activated
     * 2. Tests if the 'Link Issue' link is available if there are saved links and with 'Issue Linking' activated
     */
    public void availabilityOfIssueLinkLink(String issueKey)
    {
        ...
        // Create a link type and check for presence of 'Link Issue' link
        administration.issueLinking().enable();
    ...
    public void testVisibilityOfDeleteIssueLink() throws SAXException
    {
        ...
        navigation.issue().gotoIssue(issueKey1);
        tester.assertLinkPresent("del_10001_10000");
        ...
        navigation.issue().gotoIssue(issueKey1);
        tester.assertLinkNotPresent("del_10001_10000");
    ...
    public void confirmLinkDeletion()
    {
        ...
        tester.submit("Delete");
        text.assertTextPresent(locator.page(), issueKey1);
        text.assertTextNotPresent(locator.page(), issueKey2);
    ...
    {
        administration.restoreData("TestLinkIssueWithPriorityFieldHiddenEnterprise.xml");

        navigation.issue().gotoIssue("HSP-1");

        final WebTable outwardLinksTable = getTableWithClass("links-outward");
        assertEquals(4, outwardLinksTable.getRowCount());
        assertions.getTableAssertions().assertTableRowEquals(outwardLinksTable, 0, new Object[] { "This issue duplicates" });

        assertions.getTableAssertions().assertTableRowEquals(outwardLinksTable, 1,
                new Object[]
                        {
                                new TextCell("MKY-1"),
                                new TextCell("new feature in project monkey with no priority set on creation as it is hidden"),
                                "",
                                new ImageCell(STATUS_IMAGE_OPEN),
                                DELETE_LINK_TEXT
                        });

        assertions.getTableAssertions().assertTableRowEquals(outwardLinksTable, 2,
                new Object[]
                        {
                                new TextCell("HSP-3"),
                                new TextCell("this task is duplicated by hsp-1"),
                                new ImageCell(PRIORITY_IMAGE_MAJOR),
                                new ImageCell(STATUS_IMAGE_OPEN), DELETE_LINK_TEXT
                        });

        assertions.getTableAssertions().assertTableRowEquals(outwardLinksTable, 3,
                new Object[]
                        {
                                new TextCell("HSP-2"),
                                new TextCell("improvement on project homosapien this issue will be linked to the homosapien bug and has its priority showing"),
                                new ImageCell(PRIORITY_IMAGE_CRITICAL),
                                new ImageCell(STATUS_IMAGE_OPEN),
                                DELETE_LINK_TEXT });

        final WebTable inwardLinksTable = getTableWithClass("links-inward");
        assertEquals(4, inwardLinksTable.getRowCount());
        assertions.getTableAssertions().assertTableRowEquals(inwardLinksTable, 0, new Object[] { "This issue is duplicated by:" });

        assertions.getTableAssertions().assertTableRowEquals(inwardLinksTable, 1,
                new Object[]
                        {
                                new TextCell("HSP-4"),
                                new TextCell("this bug duplicates hsp-1"),
                                new ImageCell(PRIORITY_IMAGE_MINOR),
                                new ImageCell(STATUS_IMAGE_OPEN),
                                DELETE_LINK_TEXT
                        });

        assertions.getTableAssertions().assertTableRowEquals(inwardLinksTable, 2,
                new Object[]
                        {
                                new TextCell("MKY-1"),
                                new TextCell("new feature in project monkey with no priority set on creation as it is hidden"),
        ...
        assertions.getTableAssertions().assertTableRowEquals(inwardLinksTable, 3,
                new Object[]
                        {
                                new TextCell("HSP-2"),
                                new TextCell("improvement on project homosapien this issue will be linked to the homosapien bug and has its priority showing"),
    ...
    private WebTable getTableWithClass(final String clazz)
    {
        try
        {
    ...
    public void testChangeHistoryShowsHyperlinkCorrectly()
    {
        ...
        tester.gotoPage("/plugins/servlet/project-config/" + "MKY" + "/permissions");
    ...
    private void gotoLinkDeleteConfirmationScreen(String issueKey1, String issueKey2)
    {
        ...
        administration.issueLinking().enable();
        navigation.issue().gotoIssue(issueKey1);
        tester.clickLink("del_" + destId + "_" + linkTypeId);
