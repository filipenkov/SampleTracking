...
/**
 * Factory of web test runners. Web test runners wrap any parent runner with a transforming runner that
 * applies filtering and sorting by web test category (for given web test suite instance), as well as batching
...
public final class WebTestRunners
{
    ...
    public static <T> ParentRunner<T> newRunner(WebTestSuite suite, ParentRunner<T> runner,
            Predicate<Description> splittable) throws InitializationError
    {
        return new TransformingParentRunner<T>(nameOf(suite), runner, Lists.newArrayList(
                Transforms.fromFilter(new CategoryFilter(suite.includes(), suite.excludes())),
                SortByCategory.INSTANCE,
                new SystemPropertyBasedBatcherTransform(splittable),
                new TestOutputTransform()
        ));
    }
    ...
    public static <T> ParentRunner<T> newRunner(WebTestSuite suite, ParentRunner<T> runner) throws InitializationError
    {
        return new TransformingParentRunner<T>(nameOf(suite), runner, Lists.newArrayList(
                Transforms.fromFilter(new CategoryFilter(suite.includes(), suite.excludes())),
                SortByCategory.INSTANCE,
                new SystemPropertyBasedBatcherTransform(),
                new TestOutputTransform()
        ));
    }

    public static ParentRunner<Runner> newRunner(WebTestSuite suite, RunnerBuilder runnerBuilder,
            Predicate<Description> splittable, Class<?>... classes) throws InitializationError
    {
        return newRunner(suite, new SuiteExt(runnerBuilder, classes), splittable);
    }
