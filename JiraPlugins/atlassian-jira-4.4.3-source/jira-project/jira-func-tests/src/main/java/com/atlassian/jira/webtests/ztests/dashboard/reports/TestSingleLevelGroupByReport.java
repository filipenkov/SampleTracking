 ...
 */
@WebTest ({ Category.FUNC_TEST, Category.REPORTS, Category.USERS_AND_GROUPS })
public class TestSingleLevelGroupByReport extends FuncTestCase
{
    ...
    public void testFieldVisibility()
    {
        ...
        tester.submit("Add");
        tester.clickLink("configure-Many Hidden Fields");
    ...
    private void assertFieldsVisibleEnt()
    {
        try
        {
            ...
            {
                final TableCell priorityCell = table.getTableCell(i, 5);
                ...
                final TableCell resolutioCell = table.getTableCell(i, 3);
                if (i == 8) // last one is new feature
                {
                    assertTrue(resolutioCell.asText().indexOf("Unresolved") >= 0);
                }
                else
                {
                    assertFalse(resolutioCell.asText().indexOf("Unresolved") >= 0);
                }
            }
        }
    ...
    private void assertFieldsVisible(boolean priorityVisible, boolean resolutionVisible)
    {
        try
        {
            ...
            for (int i = 2; i < rowCount; i+=2) // every second row starting from 2
            {
                ...
                final TableCell resolutionCell = table.getTableCell(i, 3);
                if (resolutionVisible)
                {
                    assertTrue(resolutionCell.asText().indexOf("Unresolved") >= 0);
                }
                else
                {
                    assertFalse(resolutionCell.asText().indexOf("Unresolved") >= 0);
                }
            }
        }
        catch (SAXException e)
