...
public class IssueNavigationImpl extends AbstractNavigationUtil implements IssueNavigation
{
    ...
    public void gotoIssue(String issueKey)
    {
        ...
        tester.beginAt("/browse/" + issueKey);
    }
    ...
    private String createIssueDetails(final String summary, final String description)
    {
        ...
        if (issueKey == null)
        {
            Locator locator = new CssLocator(tester, ".aui-message.warning");
    ...
    private String getNoPermissionError (Locator xPathLocator)
    {
        ...
        if (tds.length >= 1)
        {
            final String fullMessage = xPathLocator.getText(tds[0]);
    ...
    private String createIssueDetails(final String summary, final String description, final Map<String,String[]> params)
    {
        ...
        else
        {
            ...
            if (issueKey == null)
            {
                Locator locator = new CssLocator(createdIssueNode, ".aui-message.warning");
                issueKey = getNoPermissionError(locator);
            }
            return issueKey;
        }
    ...
    {
        viewIssue(issueKey);
        if (tester.getDialog().getResponseText().contains("icon-watch-on"))
        {
    ...
    {
        viewIssue(issueKey);
        if (tester.getDialog().getResponseText().contains("icon-watch-off"))
        {
            tester.clickLink("toggle-watch-issue");
        }
    }
    ...
    {
        viewIssue(issueKey);
        if (tester.getDialog().getResponseText().contains("icon-vote-on"))
        {
    ...
    {
        viewIssue(issueKey);
        if (tester.getDialog().getResponseText().contains("icon-vote-off"))
        {
            tester.clickLink("toggle-vote-issue");
        }
        else
