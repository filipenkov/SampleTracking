...
public class TestIssueResourceNoPrioritySet extends RestFuncTest
{
    ...
    public void testUnassignedIssueHasNoValue() throws Exception
    {
        ...
        assertEquals(PRIORITY_BLOCKER, issue2.fields.priority.value.name);
        assertEquals(ADMIN_USERNAME, issue2.fields.reporter.value.name);
        ...
        assertNull(issueClient.get("TST-1").fields.duedate.value);
        ...
        assertTrue(issueClient.get("TST-1").fields.duedate.value.startsWith("2011-05-")); // not going into further details due to possible timezone issues (irrelevant here)
