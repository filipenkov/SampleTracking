...
public class TestViewIssue extends JIRAWebTest
{
    ...
    /**
     * Ensure that non-existant assignee and reporter users dont break the
     * view issue page - JRA-12360
     */
    ...
    /**
     * check that if the date formats end with a double quot and a space that the UI does not break and
     * the quotes are html encoded. - JRA-13104
     */
    ...
    private void _testViewIssuePageWithInvalidAssigneeAndReportersStandard()
    {
        ...
        text.assertTextPresent(new XPathLocator(tester, "//dd[@id='customfield_10000-val']/preceding-sibling::dt[1]"), "Custom User Picker");
        assertions.getLinkAssertions().assertLinkAtNodeEndsWith("//dd[@id='customfield_10000-val']//a", "/secure/ViewProfile.jspa?name=admin");
        text.assertTextPresent(new XPathLocator(tester, "//dd[@id='customfield_10000-val']"), ADMIN_FULLNAME);

        text.assertTextPresent(new XPathLocator(tester, "//dd[@id='customfield_10001-val']/preceding-sibling::dt[1]"), "Custom Multi User Picker");
        assertions.getLinkAssertions().assertLinkAtNodeEndsWith("//dd[@id='customfield_10001-val']//a", "/secure/ViewProfile.jspa?name=admin");
        text.assertTextPresent(new XPathLocator(tester, "//dd[@id='customfield_10001-val']"), ADMIN_FULLNAME);
        ...
        //check that the custom user pickers are displayed correctly
        assertElementNotPresent("customfield_10000-val");
    ...
    /**
     * Additional asserts to {@link #_testViewIssuePageWithInvalidAssigneeAndReportersStandard()} for
     ...
     * The graph should be present if *any* subtask has any timetracking data - either original estimate
     * or work logged.
     */
    ...
    public void testFieldTabs() throws Exception
    {
        ...
        text.assertTextPresent(new IdLocator(tester, "customfield_10001-val"), "Tab2CF");

        //This issue should have two tabs.
        gotoIssue("HSP-3");
        assertElementPresent("customfieldmodule");
        text.assertTextPresent(new IdLocator(tester, "tabCell1"), "Tab1");
        text.assertTextPresent(createCFValueLocator("tabCellPane1", 10000), "Tab1CF");
        text.assertTextPresent(new IdLocator(tester, "tabCell2"), "Tab2");
        text.assertTextPresent(createCFValueLocator("tabCellPane2", 10001), "Tab2CF");
    }
    ...
    public void testUserCustomFieldTabs() throws Exception
    {
        ...
        assertLinks("//*[@id='customfield_10020-val']//a/@href", "secure/ViewProfile.jspa?name=admin");
        ...
        assertLinks("//*[@id='customfield_10020-val']//a/@href", "secure/ViewProfile.jspa?name=admin");
        assertLinks("//*[@id='customfield_10022-val']//a/@href", "secure/admin/user/ViewGroup.jspa?name=jira-developers", "secure/admin/user/ViewGroup.jspa?name=jira-users");
        assertLinks("//*[@id='customfield_10021-val']//a/@href", "secure/admin/user/ViewGroup.jspa?name=jira-administrators");
        assertLinks("//*[@id='customfield_10023-val']//a/@href", "secure/ViewProfile.jspa?name=admin", "secure/ViewProfile.jspa?name=fred");
    }
    ...
    {
        XPathLocator locator = new XPathLocator(tester, xPath);
        Node[] nodes = locator.getNodes();

        assertEquals(String.format("Expected number of links %d but there where %d links.", urls.length, nodes.length),
                urls.length, nodes.length);

        for (int i = 0; i < urls.length; i++)
        {
            final String url = nodes[i].getNodeValue();
            assertFalse(String.format("URL[%d] is blank.", i), StringUtils.isBlank(url));
            assertTrue(String.format("The actual URL '%s' does not contain '%s'.", url, urls[i]), url.contains(urls[i]));
        }
    }
    ...
    private void enableDeleteAllWorklogInDefaultPermissionScheme(String groupName)
    {
        ...
        clickLink("0_edit");
        clickLink("add_perm_43"); // delete all worklogs
