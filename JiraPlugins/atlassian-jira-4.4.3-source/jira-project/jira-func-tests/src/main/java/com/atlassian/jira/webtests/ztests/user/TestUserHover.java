...
public class TestUserHover extends FuncTestCase
{
    ...
    {
        //first test getting the hover for a nonexistent user. Should just print the username & links
        gotoUserHover("unknownuser");
        text.assertTextPresent(new WebPageLocator(tester), "unknownuser");
    ...
    public void testLocalTimeShouldBeDisplayedInUserHover() throws Exception
    {
        ...
        assertThat("time of day info is present", locator.css(".time-zone-info").getNode(), not(equalTo(null)));
        assertThat("hour icon shows correct hour=" + hourOfDayAdmin, locator.css(".user-time-icon.hour-of-day-" + hourOfDayAdmin).getNode(), not(equalTo(null)));

        // now test getting the hover for a nonexistent user. should not print TZ stuff
        gotoUserHover("unknownuser");
        assertThat("time of day info is present", locator.css(".time-zone-info").getNode(), equalTo(null));

        // this guy is in a different time zone
        gotoUserHover("brad");
        final String hourOfDayBrad = hourOfDay("Pacific/Palau");
        assertThat("time of info is present", locator.css(".time-zone-info").getNode(), not(equalTo(null)));
        assertThat("hour icon shows correct hour=" + hourOfDayBrad, locator.css(".user-time-icon.hour-of-day-" + hourOfDayBrad).getNode(), not(equalTo(null)));
