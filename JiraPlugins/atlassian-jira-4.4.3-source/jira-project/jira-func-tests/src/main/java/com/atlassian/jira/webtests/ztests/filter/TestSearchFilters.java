...
public class TestSearchFilters extends FuncTestCase
{
    ...
    List<FilterItem> expectedList;
    List<FilterItem> actualList;
    ...
    public void testUserNameSearch() throws Exception
    {
        for (FilterNavigation aFilterNavigationScreen : filterNavigationScreen)
        {
            testUserNameSearch(aFilterNavigationScreen);
        }
    }
    ...
    public void testInitialScreenAppearance(FilterNavigation filterNavigation) throws Exception
    {
        ...
        // test that the user name error is shown
        text.assertTextPresent(xPathLocator("//table[@class='filterSearchInput']//span[@class='errMsg']"), "The user 'trumpet' does not exist");
    }
    ...
    {
        // TODO make filterpicker popup version
        ...
        url = StringUtils.replace(url, "groupShare=&", "groupShare=nodelonghere&");
        tester.gotoPage(url);

        text.assertTextPresent(xPathLocator("//table[@class='filterSearchInput']//span[@class='errMsg']"), "Group: 'nodelonghere' does not exist.");

        // test non existent project
        url = StringUtils.replace(baseURL, "searchShareType=&", "searchShareType=project&");
        url = StringUtils.replace(url, "projectShare=&", "projectShare=666&");
        tester.gotoPage(url);

        text.assertTextPresent(xPathLocator("//table[@class='filterSearchInput']//span[@class='errMsg']"), "The project with identifier '666' does not exist.");

        // test bad project id
        url = StringUtils.replace(baseURL, "searchShareType=&", "searchShareType=project&");
        url = StringUtils.replace(url, "projectShare=&", "projectShare=666x666&");
        tester.gotoPage(url);

        text.assertTextPresent(xPathLocator("//table[@class='filterSearchInput']//span[@class='errMsg']"), "Illegal search parameters.");

        // test non existent project role
        url = StringUtils.replace(baseURL, "searchShareType=&", "searchShareType=project&");
        url = StringUtils.replace(url, "projectShare=&", "projectShare=10000&");
        url = StringUtils.replace(url, "roleShare=&", "roleShare=666&");
        tester.gotoPage(url);

        text.assertTextPresent(xPathLocator("//table[@class='filterSearchInput']//span[@class='errMsg']"), "The project role with identifier '666' does not exist.");
    }
    ...
    public void testXss(FilterNavigation filterNavigation) throws Exception
    {
        ...
        final FilterItem groupFilterItem = new FilterItem(10040L, "xss group filter", "" , ADMIN_FULLNAME + " (admin)", 0, Collections.singletonList(groupShare), true, 0, null,1);
        ...
        final FilterItem roleFilterItem = new FilterItem(10041L, "xss project role filter", "", ADMIN_FULLNAME + " (admin)",0, ImmutableList.of(roleShare), true, 0, null, 1);
    ...
    public void testBasicNameSearch(final FilterNavigation filterNavigation) throws Exception
    {
        ...
        Closure basicNameSearchTest1 = new Closure()
        {
            public void execute(final Object userNameObj)
            {
                ...
                submitSimpleSearch("b", ADMIN_USERNAME, filterNavigation);
                ...
                submitSimpleSearch("the evil zurg", ADMIN_USERNAME, filterNavigation);
                assertInputSearchFormHasThisShape("the evil zurg", ADMIN_USERNAME);
                assertNull("Should have found nothing", parse.filter().parseFilterList(SEARCH_TABLE_ID));

                //
                // search for multiple words
                submitSimpleSearch("b m w", ADMIN_USERNAME, filterNavigation);
    ...
    public void testUserNameSearch(final FilterNavigation filterNavigation) throws Exception
    {
        administration.restoreData("BaseProfessionalFiltersWithDeveloper.xml");
        navigation.manageFilters().searchFilters();

        // search for d with a user name of anyone
        submitSimpleSearch("d*", "", filterNavigation);

        expectedList = ImmutableList.of(
                newFI('d'),
                newFI(DEV_ID_1, DEVELOPER_FILTER, DEVELOPER_THE_GREAT).setFav(false)
        );
        // check the expected versus the actual
        assertFilterItemLists();

        // search for d with a user name of anyone
        submitSimpleSearch("d*", ADMIN_USERNAME, filterNavigation);
        ...
        // search for i with a user name of anyone
        submitSimpleSearch("d*", "", filterNavigation);
        ...
        // search for d with a user name of developer
    ...
    public void testBasicPaging(final FilterNavigation filterNavigation) throws Exception
    {
        ...
        Closure basicPagingTest = new Closure()
        {
            public void execute(final Object userNameObj)
            {
                ...
                submitSimpleSearch("", ADMIN_USERNAME, filterNavigation);
                ...
                String pageURL = assertSinglePagingLinksPresent(1, 2, NEXT, "1 - 20 of 26", "", ADMIN_USERNAME);
                ...
                pageURL = assertSinglePagingLinksPresent(0, 2, PREVIOUS, "21 - 26 of 26", "", ADMIN_USERNAME);
                ...
                pageURL = assertSinglePagingLinksPresent(1, 2, NEXT, "1 - 20 of 26", "", ADMIN_USERNAME);
                ...
                assertSinglePagingLinksPresent(0, 2, PREVIOUS, "21 - 26 of 26", "", ADMIN_USERNAME);
            }
        };
    ...
    public void testBasicSorting(final FilterNavigation filterNavigation) throws Exception
    {
        ...
        String sortURL = assertSortUrls("Name", false, "", ADMIN_USERNAME);
        ...
        // check the expected versus the actual
        assertFilterItemLists();
        sortURL = assertSortUrls("Name", true, "", ADMIN_USERNAME);
        assertNotNull(sortURL);
        ...
        // check the expected versus the actual
        assertFilterItemLists();
        sortURL = assertSortUrls("Owner", false, "", ADMIN_USERNAME);
        assertNotNull(sortURL);
        ...
        // check the expected versus the actual
        assertFilterItemLists();
        sortURL = assertSortUrls("Author", true, "", ADMIN_USERNAME);
        assertNotNull(sortURL);
        ...
        // check the expected versus the actual
        assertFilterItemLists();
        sortURL = assertSortUrls("Popularity", true, "", ADMIN_USERNAME);
        assertNotNull(sortURL);
        ...
        // check the expected versus the actual
        assertFilterItemLists();
        sortURL = assertSortUrls("Popularity", false, "", ADMIN_USERNAME);
        assertNotNull(sortURL);
    ...
    private void assertDualPagingLinksPresent(int expectedNextPageOffset, int expectedLinks, String expectedXOfY, String searchName, String searchOwnerUserName)
    {
        ...
        pageLocator = xPathLocator("//p[@class='pagination']/a/span[text() = '" + NEXT + "']");
        ...
        pageLocator = xPathLocator("//p[@class='pagination']/a/span[text() = '" + PREVIOUS + "']");
        assertEquals(expectedLinks, pageLocator.getNodes().length);
        for (int i = 0; i < expectedLinks; i++)
        {
        ...
        pageLocator = xPathLocator("//p[@class='pagination']/span");
        assertEquals(expectedLinks, pageLocator.getNodes().length);
        for (int i = 0; i < expectedLinks; i++)
        {
        ...
        assertSortUrls("Name", false, searchName, searchOwnerUserName);

        pageLocator = xPathLocator("//p[@class='pagination']/a[contains(@class, 'icon-next')]/@href");
        assertEquals(expectedLinks, pageLocator.getNodes().length);
        for (int i = 0; i < expectedLinks; i++)
        {
        ...
        pageLocator = xPathLocator("//p[@class='pagination']/a[contains(@class, 'icon-previous')]/@href");
        assertEquals(expectedLinks, pageLocator.getNodes().length);
    ...
    private String assertSinglePagingLinksPresent(int expectedPageOffset, int expectedLinks, String expectedLinkText, String expectedXOfY, String searchName, String searchOwnerUserName)
    {
        ...
        XPathLocator pageLocator = xPathLocator("//p[@class='pagination']//a");
        assertEquals(expectedLinks, pageLocator.getNodes().length);
        for (int i = 0; i < expectedLinks; i++)
        {
        ...
        pageLocator = xPathLocator("//p[@class='pagination']//a/@href");
        assertEquals(expectedLinks, pageLocator.getNodes().length);
        for (int i = 0; i < expectedLinks; i++)
        {
            String hrefUrl = pageLocator.getText(pageLocator.getNodes()[i]);
    ...
    final static int[] SORT_TD_OFFSETS = new int[] { 1, 3, 6 };
    ...
    private String assertSortUrls(String selectedColumnName, final boolean selectedSortAscending, final String searchName, final String searchOwnerUserName)
    {
        ...
        for (int i = 0; i < SORT_COL_NAMES.length; i++)
        {
            ...
            text.assertTextPresent(hrefUrl, "searchOwnerUserName=" + searchOwnerUserName + "&");
     ...
     */
    private void submitSimpleSearch(final String searchName, final String searchOwnerUserName, FilterNavigation filterNavigation)
    {
        filterNavigation.findFilters(searchName, searchOwnerUserName);
        assertInputSearchFormHasThisShape(searchName, searchOwnerUserName);
    }

    private void assertInputSearchFormHasThisShape(String searchName, String searchOwnerUserName)
    {
        text.assertTextPresent(xPathLocator("//table[@class='filterSearchInput']//td"), "Search");
        ...
        text.assertTextPresent(xPathLocator("//table[@class='filterSearchInput']//td"), "Owner");
        assertTrue(xPathLocator("//table[@class='filterSearchInput']//input[@name='searchOwnerUserName']").hasNodes());
        assertEquals(searchOwnerUserName, tester.getDialog().getFormParameterValue("searchOwnerUserName"));
