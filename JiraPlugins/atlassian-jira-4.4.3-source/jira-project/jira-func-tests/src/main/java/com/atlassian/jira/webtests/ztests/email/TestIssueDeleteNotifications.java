...
public class TestIssueDeleteNotifications extends EmailFuncTestCase
{
    ...
    {
        administration.usersAndGroups().addGroup(TEST_GROUP_ONE);
        administration.usersAndGroups().addUserToGroup(BOB_USERNAME, TEST_GROUP_ONE);
        administration.usersAndGroups().addUserToGroup(FRED_USERNAME, TEST_GROUP_ONE);
        administration.usersAndGroups().addGroup(TEST_GROUP_TWO);
        administration.usersAndGroups().addUserToGroup(BOB_USERNAME, TEST_GROUP_TWO);
        administration.usersAndGroups().addUserToGroup(HENRY_FORD_USERNAME, TEST_GROUP_TWO);
    }

    private void assertGotDeleteIssueMessages(String deletedIssueKey, String... emails) throws Exception
    {
        assertEquals(emails.length, getGreenMail().getReceivedMessages().length);
