...
public class TestXmlIssueViewErrors extends JIRAWebTest
{
    ...
    {
        assert500SamplePathMessage("/si");
        assert500SamplePathMessage("/si/");
        assert500SamplePathMessage("/si/unknownplugin");
        assert500SamplePathMessage("/si/unknownplugin/");
        assert500SamplePathMessage("/si/unknownplugin/blah");
        assert500BadPlugin("/si/unknownplugin/blah/");
        assert404BadKey("/si/jira.issueviews:issue-xml/badkey/");

        // TODO: add more cases here
    }

    private void assert404BadKey(final String url)
    {
        assertResponseCode(url, 404);
        assertTextPresent("Could not find issue with issue key badkey");
    }

    private void assert500SamplePathMessage(String url)
    {
        assertResponseCode(url, 500);
        assertTextSequence(new String[] {"System Error", "Cause:", SAMPLE_PATH_MESSAGE_1, SAMPLE_PATH_MESSAGE_2, SAMPLE_PATH_MESSAGE_3 });
    }

    private void assert500BadPlugin(String url)
    {
        assertResponseCode(url, 500);
        assertTextSequence(new String[] {"System Error", "Cause:", BAD_PLUGIN_MESSAGE });
    }
