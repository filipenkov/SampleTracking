...
public class ConfigurationChecker
{
    ...
    private class Visitor implements ConfigFileWalker.ConfigVisitor
    {
        ...
        public void visitConfig(ConfigFile file)
        {
            ...
            for (ConfigurationCheck check : configurationChecks)
            {
                final ConfigurationCheck.Result checkResut = check.checkConfiguration(config, options);
                result.addErrors(file.getFile(), checkResut.getErrors());
                result.addWarnings(file.getFile(), checkResut.getWarnings());
