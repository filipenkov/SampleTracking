...
public class TestXmlIssueView extends JIRAWebTest
{
    ...
    private static final String TESTDESCRIP_CDATA = "<![CDATA[A nice description]]>";
    private static final String TESTENVI = "A test environment";
    private static final String TESTENVI_CDATA = "<![CDATA[A test environment]]>";
    private static final String i18n = "\u0126\u0118\u0139\u0139\u0150";

    public TestXmlIssueView(String name)
    {
        super(name);
    }
    ...
    private void _testSingleCharacter(String unescapedCharacter, String escapedCharacter)
    {
        ...
        assertTextPresent("(Test Issue with " + escapedCharacter + ")");
    ...
    {
        _testEscapedUsers(">", "%3E", "&gt;");
        _testEscapedUsers("<", "%3C", "&lt;");
        _testEscapedUsers("&", "%26", "&amp;");
//        _testEscapedUsers(i18n.toLowerCase(), TextUtils.htmlEncode(i18n.toLowerCase())); // non-ascii usernames don't yet work (probably due to httpunit problems)
    }

    public void _testEscapedUsers(String unEscapedCharacter, String urlEncodedCharacter, String htmlEscapedCharacter) throws IOException
    {
        administration.restoreBlankInstance();
    ...
    public void testCustomFields() throws IOException, ParseException
    {
        ...
        _hideFields("//item/version", "Affects Version/s");
        _hideFields("//item/assignee", "Assignee");
        //_hideFields("//item/attachments","Attachment"); //not meant to be hidden
        _hideFields("//item/component", "Component/s");
        _hideFields("//item/due", "Due Date");
        _hideFields("//item/environment", "Environment");
        _hideFields("//item/fixVersion", "Fix Version/s");
        _hideFields("//item/priority", "Priority");
        _hideFields("//item/reporter", "Reporter");
        _hideFields("//item/resolution", "Resolution"); //this is playing up: not showing/not hiding
        _hideFields("//item/security", "Security Level"); //this isn't displaying
    }
    ...
    public void _testTranslation(String xpathexpr, String notExpected) throws IOException, ParseException
    {
        ...
        tester.selectOption("userLocale", "German (Germany)");
        tester.submit();

        navigation.issue().goToCreateIssueForm(null,null);
        tester.setFormElement("summary", "Testing German");
        clickButtonWithValue("Erstellen");
        tester.clickLinkWithText("XML");
    ...
    private void _hideFields(String findNode, String fieldToHide) throws IOException, ParseException
    {
        setHiddenFieldsOnEnterprise(DEFAULT_FIELD_CONFIG, fieldToHide);//hide field
        ...
        setShownFieldsOnEnterprise(DEFAULT_FIELD_CONFIG, fieldToHide);
    }
