...
public class TestIssueNavigatorFullContentView extends AbstractTestIssueNavigatorView
{
    ...
    public void testFullContentViewItem3()
    {
        ...
        assertLinkPresent(new String[]{baseUrl + "/secure/admin/user/ViewGroup.jspa", "name=jira-developers"});
    ...
    private void assertCustomFieldValues(Item item, final String title, final String cfName)
    {
        ...
        {
            final List values = customField.getValues();
            if (values != null)
            {
                for (Iterator i = values.iterator(); i.hasNext();)
                {
                    CustomField.Value value = (CustomField.Value) i.next();
...
//        assertCustomFieldValues(item, "NumberField", CF_NUMBER_FIELD);
    ...
    {
        final List attachments = item.getAttachments();
    ...
    {
        final List comments = item.getComments();
        if (comments.isEmpty())
        {
    ...
    {
        final List components = item.getComponents();
