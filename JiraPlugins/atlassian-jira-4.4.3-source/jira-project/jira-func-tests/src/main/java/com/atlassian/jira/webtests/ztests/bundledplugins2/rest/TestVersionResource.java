...
{
    final static String HONOLULU = "Pacific/Honolulu";
    ...
    private Version getInitialVersion()
    {
        ...
        Version actualVersion = versionClient.get(versionID);
        Version expectedVersion = new Version().self(createSelfLink(Long.valueOf(versionID)))
    ...
    {
        Version expectedVersion = getInitialVersion();
    ...
    public void testVersionReleaseDateShouldBeInSystemTimeZone() throws Exception
    {
        ...
        Response r = versionClient.loginAs(BOB_USERNAME).putResponse(updatedVersion);
    ...
    public void testViewVersion() throws Exception
    {
        ...
        expectedVersion = new Version().self(createSelfLink(10011)).name("Two").description("Two")
                .releaseDate("01/Mar/11").archived(false).released(false).overdue(true).id(10011L);

        assertThat(expectedVersion, equalTo(actualVersion));

        actualVersion = versionClient.get("10012");
        expectedVersion = new Version().self(createSelfLink(10012)).name("Three").description("Three")
                .releaseDate("09/Mar/11").archived(false).released(true).id(10012L);

        assertThat(expectedVersion, equalTo(actualVersion));

        actualVersion = versionClient.get("10013");
        expectedVersion = new Version().self(createSelfLink(10013)).name("Four").archived(true).released(false).id(10013L);

        assertThat(expectedVersion, equalTo(actualVersion));

        actualVersion = versionClient.get("10014");
        expectedVersion = new Version().self(createSelfLink(10014)).name("Five").description("Five")
                .archived(true).released(true).id(10014L);
        ...
        expectedVersion = new Version().self(createSelfLink(10110)).name("Hidden")
                .archived(false).released(false).id(10110L);
    ...
    public void testCreateVersion() throws Exception
    {
        ...
        assertThat(newVersion, equalTo(addNewAttributes(inputVersion, newVersion)));
        ...
        newVersion = versionClient.create(inputVersion);
        assertThat(newVersion, equalTo(addNewAttributes(inputVersion, newVersion).overdue(true).releaseDate("12/Oct/81")));
        ...
        newVersion = versionClient.create(inputVersion);
        assertThat(newVersion, equalTo(addNewAttributes(inputVersion, newVersion).overdue(true).releaseDate(tenDaysAgo).userReleaseDate(tenDaysAgo.toString("dd/MMM/yy"))));
        ...
        newVersion = versionClient.create(inputVersion);
        assertThat(newVersion, equalTo(addNewAttributes(inputVersion, newVersion).releaseDate(tenDaysFromNow).overdue(false).userReleaseDate(tenDaysFromNow.toString("dd/MMM/yy"))));
...
//        assertEquals(new Errors().addError("Could not find version with id '10000' for project 'MKY'"), response.entity);
    ...
    public void testDeleteVersion() throws Exception
    {
        ...
        assertThat(resp1.statusCode, equalTo(404));
        assertTrue(resp1.entity.errorMessages.contains("Could not find version for id '10,010'"));
        ...
        assertThat(resp1.statusCode, equalTo(404));
        assertTrue(resp1.entity.errorMessages.contains("Could not find version for id '10,011'"));

        // Search for fixVersion = "Four" should return one issue
        SearchResult fixFourResults = searchClient.postSearch(new SearchRequest().jql("fixVersion = Four"));
        ...
        assertThat(resp1.statusCode, equalTo(404));
        assertTrue(resp1.entity.errorMessages.contains("Could not find version for id '10,013'"));

        // Search for fixVersion = "Four" should return one issue
        fixFourResults = searchClient.postSearch(new SearchRequest().jql("fixVersion = Four"));
        assertThat(fixFourResults.total, equalTo(0));
        assertThat(fixFourResults.issues.size(), equalTo(0));
        ...
        assertThat(resp1.statusCode, equalTo(404));
        assertTrue(resp1.entity.errorMessages.contains("Could not find version for id '10,014'"));
        ...
        // Search for fixVersion = "Five" should return one issue
        affectedFive = searchClient.postSearch(new SearchRequest().jql("affectedVersion = Five"));
        assertThat(affectedFive.total, equalTo(0));
        assertThat(affectedFive.issues.size(), equalTo(0));
    ...
    public void testDeleteVersionErrorConditions() throws Exception
    {
        ...
        assertThat(resp1.statusCode, equalTo(404));
        assertTrue(resp1.entity.errorMessages.contains("Could not find version for id '99,010'"));
    ...
    {
        Response resp1 = versionClient.getVersionIssueCountsResponse("1");
        assertThat(resp1.statusCode, equalTo(404));
        assertTrue(resp1.entity.errorMessages.contains("Could not find version for id '1'"));

        Response respZbing = versionClient.getVersionIssueCountsResponse("zbing");
        assertThat(respZbing.statusCode, equalTo(404));
        assertTrue(respZbing.entity.errorMessages.contains("Could not find version for id 'zbing'"));
    }
    ...
    {
        Response resp1 = versionClient.getVersionUnresolvedIssueCountResponse("1");
        assertThat(resp1.statusCode, equalTo(404));
        assertTrue(resp1.entity.errorMessages.contains("Could not find version for id '1'"));

        Response respZbing = versionClient.getVersionUnresolvedIssueCountResponse("zbing");
        assertThat(respZbing.statusCode, equalTo(404));
        assertTrue(respZbing.entity.errorMessages.contains("Could not find version for id 'zbing'"));
    }
    ...
    {
        List<Version> versions = projectClient.getVersions(project);
