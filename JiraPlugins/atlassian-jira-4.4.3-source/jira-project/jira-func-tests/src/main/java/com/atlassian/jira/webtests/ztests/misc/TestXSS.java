...
public class TestXSS extends FuncTestCase
{
    ...
    private static final String XSS_ALERT_RAW = "\"><script>alert('" + XSS_ALERT_CONTENTS + "')</script>";
    ...
    public void testXSSFieldTypeInCreateCustomField()
    {
        ...
        tester.gotoPage("http://localhost:8090/jira/secure/admin/CreateCustomField.jspa?currentStep=1&global=true&&issuetypes=-1&fieldType=INJECT&searcher=&description=&fieldName=&nextBtn=Next+%3e%3e&atl_token=" + atl_token);
        // Any non-valid custom-field type should be completely ignored:
        assertions.getTextAssertions().assertTextNotPresent("INJECT");
    }

    // JRA-22953
    public void testRequestURI()
    {
        administration.restoreData("TestBrowseVersionsAndComponentsPaging.xml");
        navigation.issue().createIssue("homosapien", null, "Test issue");
        tester.gotoPage("/secure\"><script>alert(1)</script>/IssueNavigator.jspa?reset=true");
        assertions.getTextAssertions().assertTextNotPresent("<script>alert(1)</script>");
    }
    ...
    public void testXSSPickers()
    {
        ...
        //runportlet error
        tester.gotoPage("/secure/views/runportleterror.jsp?portletKey=%3Cscript%3Ealert%28%27moosies%27%29%3C/script%3E");
        tester.assertTextNotPresent("&lt;script&gt;alert(&#39;moosies&#39;);&lt;/script&gt;");
        tester.assertTextNotPresent("<script>alert('moosies');</script>");
    }
    ...
    private void assertXssEscaped(final String responseHtml)
    {
        assertTrue(responseHtml.contains(XSS_ALERT_ENCODED));
        assertFalse(responseHtml.contains(XSS_ALERT_RAW));
        assertFalse(responseHtml.contains(XSS_TAG));
    }
