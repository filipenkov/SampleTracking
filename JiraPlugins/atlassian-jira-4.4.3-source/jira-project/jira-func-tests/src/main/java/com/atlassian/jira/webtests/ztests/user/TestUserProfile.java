...
public class TestUserProfile extends FuncTestCase
{
    ...
    {
        administration.project().addProject("<iframe src=\"http://www.google.com\"></iframe>", "FRAME", "admin");
        navigation.userProfile().gotoRoadmap("admin");
        final CssLocator cssLocator = locator.css("a[title=FRAME]");
        assertions.assertNodeHasText(cssLocator, "<iframe src=\"http://www.google.com\"></iframe>");
    }
    ...
    public void testUserChangePassword()
    {
        ...
        assertEquals(ADMIN_FULLNAME, locator.xpath("//div[@id='header-details-user']/a").getText());
    }
    ...
    public void testShowsLanguageListInDefaultLanguage()
    {
        ...
        text.assertTextPresent(locator.id("up-p-locale"), "Deutsch (Deutschland) [Default]");

        tester.clickLink("edit_prefs_lnk");

        // assert that the list of languages has German selected by default
        assertions.getJiraFormAssertions().assertSelectElementHasOptionSelected("userLocale", "Deutsch (Deutschland) [Default]");
    }
