...
public class IssueParserImpl extends AbstractFuncTestUtil implements IssueParser
{
    ...
    private Map<String, String> parseCustomFields()
    {
        ...
        int tabNum = 1;
        while (new XPathLocator(tester, "//ul[@id='tabCellPane" + tabNum + "']").getNodes().length != 0)
        {
            XPathLocator fieldLabels = new XPathLocator(tester, "//ul[@id='tabCellPane" + tabNum + "']//strong");
            final Node[] labelNodes = fieldLabels.getNodes();
            XPathLocator fieldValues = new XPathLocator(tester, "//ul[@id='tabCellPane" + tabNum + "']//div[@class='wrap']/div");
...

        {
            ...
            final XPathLocator ddLocator = new XPathLocator(tester, "//*[@id='datesmodule' or @id='peopledetails']//dd[contains(@id, 'customfield')]");
            if (ddLocator.getNodes().length > 0)
            {
                final Node[] valueNodes = ddLocator.getNodes();
                for (Node valueNode : valueNodes)
                {
                    String name = StringUtils.trimToNull(findPreviousElementValue(valueNode, "dt"));
    ...
    {
//        final String key = getValue("key-val");
        return getValue("issue_header_summary");
//        return fullSummary.substring(key.length() + 3);
    }
