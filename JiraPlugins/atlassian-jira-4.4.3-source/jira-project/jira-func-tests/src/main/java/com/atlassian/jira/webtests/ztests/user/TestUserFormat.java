...
public class TestUserFormat extends FuncTestCase
{
    ...
    public void testWorkflowInfoBox()
    {
        ...
        administration.usersAndGroups().addUserToGroup(FRED_USERNAME, "jira-administrators");
        //create a draft
        ...
        //view the draft workflow
        tester.gotoPage("/secure/admin/workflows/ViewWorkflowSteps.jspa?workflowMode=draft&workflowName=Workflow1");
        assertions.assertProfileLinkPresent("draft_workflow_edited_admin", ADMIN_FULLNAME);

        tester.clickLink("view_live_workflow");
        assertions.assertProfileLinkPresent("workflow_edited_admin", ADMIN_FULLNAME);
    }

    public void testCommentPanel()
    {
        //need to grant anyone edit comment permissions.
        tester.gotoPage("/secure/admin/AddPermission!default.jspa?permissions=34&schemeId=0");
        tester.checkCheckbox("type", "group");
        tester.submit(" Add ");
    ...
    //panels/project/summary.jsp
    public void testProjectSummary()
    {
        tester.gotoPage("/plugins/servlet/project-config/" + "HSP" + "/people");
    ...
    public void testBrowseComponent()
    {
        ...
        tester.gotoPage("/browse/HSP?selectedTab=com.atlassian.jira.plugin.system.project:components-panel");
    ...
    public void testFullProfile()
    {
        ...
        tester.assertTextPresent("Create New Dashboard");
        assertFullProfilePresent(ADMIN_USERNAME, ADMIN_FULLNAME, "admin@example.com", EasyList.build("jira-administrators", "jira-users", "jira-developers"));

        //editportalpage.jsp
        tester.gotoPage("/secure/EditPortalPage!default.jspa?pageId=10000");
        tester.assertTextPresent("Edit and Share Dashboard");
        assertFullProfilePresent(ADMIN_USERNAME, ADMIN_FULLNAME, "admin@example.com", EasyList.build("jira-administrators", "jira-users", "jira-developers"));

        //restoredefaultdashboardconfirm.jsp
        tester.gotoPage("/secure/RestoreDefaultDashboard!default.jspa?destination=manageportal");
        tester.assertTextPresent("Restore System Dashboard");
        assertFullProfilePresent(ADMIN_USERNAME, ADMIN_FULLNAME, "admin@example.com", EasyList.build("jira-administrators", "jira-users", "jira-developers"));

        //viewuserissuecolumns.jsp
        tester.gotoPage("/secure/ViewUserIssueColumns!default.jspa");
        tester.assertTextPresent("Issue Navigator Columns");
        assertFullProfilePresent(ADMIN_USERNAME, ADMIN_FULLNAME, "admin@example.com", EasyList.build("jira-administrators", "jira-users", "jira-developers"));

        //also check that e-mail & full name are escaped properly
    ...
    public void testIssueSummary()
    {
        ...
        tester.gotoPage("/secure/admin/AddPermission!default.jspa?permissions=17&schemeId=0");
        tester.checkCheckbox("type", "group");
        tester.submit(" Add ");
        ...
        //assignee link
        assertions.assertProfileLinkPresent("issue_summary_assignee_fred", FRED_FULLNAME);
        //reporter link
        assertions.assertProfileLinkPresent("issue_summary_reporter_admin", ADMIN_FULLNAME);
    }
    ...
    //user-searcher-view.vm
    public void testUserSearcherView() throws SAXException
    {
        tester.clickLink("find_link");
        final String linkText = "Current User";
        tester.selectOption("reporterSelect", linkText);
        tester.submit("show");
        tester.clickLink("viewfilter");


        final String id = "searcher-profile-link";
        final WebLink link = tester.getDialog().getResponse().getLinkWithID(id);
        assertNotNull("No link with id '" + id + "' present.", link);
        assertEquals("Link text for link with id '" + id + "' does not match '" + linkText + "'.", linkText, link.asText());

        tester.clickLink(id);
        text.assertTextPresent(new WebPageLocator(tester), "User Profile : " + ADMIN_FULLNAME);
    }
    ...
    public void testCustomFields()
    {
        ...
        assertions.assertProfileLinkPresent("user_cf_fred", FRED_FULLNAME);
        assertions.assertProfileLinkPresent("multiuser_cf_fred", FRED_FULLNAME);
        assertions.assertProfileLinkPresent("multiuser_cf_admin", ADMIN_FULLNAME);
    }
