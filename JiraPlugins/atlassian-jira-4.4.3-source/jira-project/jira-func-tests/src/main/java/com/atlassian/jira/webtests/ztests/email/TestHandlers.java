...
public class TestHandlers extends EmailFuncTestCase
{
    public void setUpTest()
    {
        ...
        setupJiraMailServer("admin@example.com", "PRE", String.valueOf(JIRAServerSetup.SMTP.getPort()));
    }

    public void testPortsInService()
    {
        setupPopService();
        String popPort = String.valueOf(getGreenMail().getPop3().getPort());
        String imapPort = String.valueOf(getGreenMail().getImap().getPort());

        tester.assertTextPresent("port:</strong> " + popPort);

        setupImapService();

        tester.assertTextPresent("port:</strong> " + imapPort);
    }
