...
public class TestMoveIssue extends JIRAWebTest
{
    ...
    public void testMoveIssue()
    {
        ...
        moveIssueWithAssignableUser(ISSUE_KEY_NORMAL);
    ...
    /** Tests the ability to move an issue with the 'Assignable User' permission removed */
    public void moveIssueWithAssignableUser(String issueKey)
    {
        log("Move Operation: Moving issue with no valid assignees.");
        grantGroupPermission(ASSIGNABLE_USER, Groups.DEVELOPERS);
        gotoIssue(issueKey);
        clickLink(MOVE_ISSUE_LINK);
        selectOption(PID_REQUEST_PARAM, PROJECT_MONKEY);
        submit();
    ...
    public void testMoveIssueWithSubtaskIssueTypesBetweenProjects()
    {
        ...
        gotoIssue("AL-1");
        tester.clickLink("move-issue");
        tester.selectOption("pid", "Baloney");
        tester.selectOption("issuetype", "Issue Type Bentley");
        tester.submit("Next >>");

        assertTextPresent("Move Issues: Select Issue Types for Sub-Tasks");

        // Assert the table 'issuetypechoices'
        final TableLocator tableLocator = new TableLocator(tester, "issuetypechoices");
        text.assertTextPresent(tableLocator, "Select New Issue Types for Sub-Tasks");
        text.assertTextPresent(tableLocator, "Subtask Apple");
        text.assertTextPresent(tableLocator, "Subtask Asterisk");
        tester.selectOption("subtaskissuetype_6", "Subtask Bacon");
        tester.selectOption("subtaskissuetype_9", "Subtask Butter");
        tester.submit("Next >>");

        assertTextPresent("Workflow Brown"); // assert that the correct destination workflow is being displayed
        assertTextPresent("1 task with status");
        tester.selectOption("subtaskstatus_6_10000", "B-Status-2");
        tester.submit("Next >>");
        ...
        assertLastChangeHistoryIs("BA-2", "Assignee", "User A", ADMIN_USERNAME);
    ...
    public void testMoveIssueWithSubtasksBetweenProjectsWithSecurityLevel() throws SAXException
    {
        ...
        assertCollapsedTextSequence(new String[] { "Project", "Rattus", "Porcine",
                                                   "Type", "Bug", "Bug",
                                                   "Security Level", "Level Mouse", "None" });
        submit("Move");
        ...
        submit("Next >>");
        assertCollapsedTextSequence(new String[] { "Project", "Porcine", "Canine",
                                                   "Type", "Bug", "Bug",
                                                   "Security Level", "None", "Level Green" });
        submit("Move");
    ...
    public void testMoveIssueWithRequiredLabels()
    {
        ...
        tester.assertTextPresent("MKY-9");
    }
