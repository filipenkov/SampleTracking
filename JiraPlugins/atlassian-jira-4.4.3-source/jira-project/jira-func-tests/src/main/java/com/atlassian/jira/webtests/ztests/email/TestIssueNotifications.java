...
public class TestIssueNotifications extends EmailFuncTestCase
{
    ...
    public void testDeleteUserBeforeCommentNotification() throws InterruptedException, MessagingException, IOException
    {
        ...
        assertTrue("henry.ford commented", mimeMessages[0].getContent().toString().indexOf("henry.ford commented on COW-2") >= 0);
        assertTrue("updated reporter", mimeMessages[1].getContent().toString().indexOf("admin updated COW-1") >= 0);
    }
    ...
    private void assertCommentAndDeleteMessages(List<MimeMessage> mimeMessages) throws IOException, MessagingException
    {
        ...
        assertEmailBodyContains(mimeMessages.get(0), "henry.ford commented on COW-1");
        assertEmailBodyContains(mimeMessages.get(1), "admin deleted COW-1");
    }
...
//        }
    ...
    public void testAnonymousUser() throws InterruptedException, MessagingException, IOException
    {
        ...
        {
            assertEmailBodyContains(messagesForAdmin.get(1), "henry.ford commented on COW-1");
        }
        else
        {
            assertEmailBodyContains(messagesForAdmin.get(0), "henry.ford commented on COW-1");
            assertEmailBodyContains(messagesForAdmin.get(1), "Anonymous commented on COW-1");
        }
    }
    ...
    public void testAssignIssueNotificationCurrentAndPreviousAssignee() throws Exception
    {
        ...
        assertEmailBodyContains(message, "reassigned");
    ...
    private void assertMailProperties(MimeMessage message, String issueId, String subject) throws MessagingException
    {
        ...
        assertTrue(body.indexOf("admin deleted " + issueId) >= 0);
    }

    private String getPrefUrl(String body)
    {
        final String path = "/secure/ViewProfile.jspa";
        assertTrue("Could not find link to User Preferences in email body: \n-----\n" + body + "\n-----\n", body.indexOf(path) >= 0);
        return path;
    }
}
