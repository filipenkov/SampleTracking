...
public class TestCommentSecurityLevel extends JiraSeleniumTest
{
    ...
    {
        super.onSetUp();
        restoreData("TestCommentSecurityLevel.xml");
    }
    ...
    public void testCommentSecurityLevel()
    {
        ...
        client.click("comment-issue", false);
        ...
        //test submit with no comment to make sure the change is persisted across request
        getNavigator().gotoIssue("HSP-1");
        client.click("comment-issue", false);
        assertThat.visibleByTimeout("css=.security-level", DROP_DOWN_WAIT);
        ...
        assertThat.textPresent("Comment body can not be empty");
        assertThat.elementContainsText("css=.security-level .current-level", "Restricted to Administrators");
