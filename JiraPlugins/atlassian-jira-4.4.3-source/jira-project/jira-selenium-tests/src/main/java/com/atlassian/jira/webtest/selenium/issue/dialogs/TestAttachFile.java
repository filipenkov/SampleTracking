...
public class TestAttachFile extends AbstractIssueDialogTest
{
    ...
    public void testAttachFile() throws Exception
    {
        ...
        assertThat.elementPresentByTimeout("css=h2#issue_header_summary", DEFAULT_TIMEOUT);
        assertThat.elementContainsText("css=h2#issue_header_summary", "test");
    ...
    public void testFileSizesForSingleAttachment()
    {
        ...
        //back on issue
        assertThat.elementContainsText("css=h2#issue_header_summary", "A summary");
    }
    ...
    public void testAttachFileWithWeirdCharacters()
    {
        ...
        assertTrue(href.endsWith("a+file.txt"));

        gotoAttachment(fileNameSpace);
        assertTrue(client.getLocation().endsWith("a+file.txt"));
    ...
    public void testAttachFileFromSubTaskQuickCreateForm() throws Exception
    {
        ...
        //pop open the stqc form if it's not already there!
        if (client.isElementPresent("id=stqc_show"))
        {
            client.click("id=stqc_show");
            assertThat.visibleByTimeout("id=stqc_submit", 5000);
        }
        client.type("summary", "My subtask");
        getNavigator().issue().attachFileInExistingForm("tempFilename", fileName, 1024);
        client.click("id=stqc_submit", true);
    ...
    public void testFileNotFoundWhenAttachmentNotOnDisk() throws IOException
    {
        ...
        {
            //check we got a 500 error!
            assertTrue("Expected attachment not found message but got '" + e.getMessage() + "'", e.getMessage().contains("500"));
        }
    }
    ...
    public void testIssueResourceAttachmentsWithThumbnail() throws Exception
    {
        ...
        getWebUnitTest().gotoPage("/rest/api/2.0.alpha1/issue/MKY-1");
        JSONObject issue = new JSONObject(getWebUnitTest().getDialog().getResponseText());
        final JSONArray attachmentItems = issue.getJSONObject("fields").getJSONObject("attachment").getJSONArray("value");
        for (int i = 0; i < attachmentItems.length(); i++)
        {
            JSONObject item = attachmentItems.getJSONObject(i);
            if (item.getString("self").endsWith("rest/api/2.0.alpha1/attachment/10000"))
            {
                assertTrue(item.has("thumbnail"));
                assertTrue(item.getString("thumbnail").endsWith("secure/thumbnail/10000/_thumb_10000.png"));
                return;
            }
        }
    ...
    public void testCancelDeleteAttachment()
    {
        final String fileName = "screenshot.txt";
        ...
        assertThat.linkPresentWithText(fileName);
        client.click("id=del_10000", true);
        assertThat.textPresent("Remove Attachment");
        client.click("id=cancelButton", true);
    ...
    public void testConfirmDeleteAttachment()
    {
        final String fileName = "screenshot5.txt";
        ...
        client.click("id=del_10000", true);
        assertThat.textPresent("Remove Attachment");
        client.click("id=delete_submit", true);
        assertThat.textNotPresent("Remove Attachment");
        assertThat.linkNotPresentWithText(fileName);
        getNavigator().issue().viewIssue("MKY-1");
        assertThat.linkNotPresentWithText(fileName);
    ...
    public void testWikiRendererInlineThumbnail()
    {
        ...
        client.type("id=comment", "!picture-attachment.jpg|thumbnail!");
        ...
        client.click("id=footer-comment-button");
        assertThat.visibleByTimeout("id=comment", 5000);
        client.type("id=comment", "!picture-attachment.jpg!");
        client.click("id=issue-comment-add-submit", true);
    ...
    private void createIssueWithSummary(final String summary)
    {
        ...
        assertThat.visibleByTimeout("id=quick-create-button", 5000);
