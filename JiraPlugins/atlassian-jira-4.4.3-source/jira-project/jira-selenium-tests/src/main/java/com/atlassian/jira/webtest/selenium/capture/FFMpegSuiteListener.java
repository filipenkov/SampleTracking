package com.atlassian.jira.webtest.selenium.capture;
...
public class FFMpegSuiteListener implements WebTestListener
{
    ...
    private FFMpegCommand startFFMpeg()
    {
        ...
        builder.addOutput(createOutputFile(OUTPUT_CAPTURE).getAbsolutePath()).setOutputCodec("libx264")
                .setPreset("libx264-normal").setSize(512, 384).setOverwrite(true).setGop(40);
        builder.setListener(new ProcessListener(createOutputFile("ffmpeg.log")));
    ...
    {
        return new File(EnvironmentUtils.getMavenAwareOutputDir(), name);
    }

    private static boolean shouldRecord(final WebTestDescription test)
    {
        return test.categories().contains(Category.SELENIUM_TEST) || test.categories().contains(Category.WEBDRIVER_TEST)
                || JiraSeleniumTest.class.isAssignableFrom(test.testClass());   // TODO remove after marking selenium tests with category
    }
    ...
    private static FudgerTimedTestListener createTimedListener()
    {
        Collection<TimedTestListener> listeners = new ArrayList<TimedTestListener>(5);
        try
        {
            listeners.add(new SubRipSubtitleListener(createOutputFile(OUTPUT_SUBTITLES)));
        }
        catch (IOException e)
        {
        ...
        try
        {
            listeners.add(new MkvChapterTimedListener(createOutputFile(OUTPUT_CHAPTERS_XML)));
        }
        catch (IOException e)
        {
        ...
        try
        {
            listeners.add(new CommonChapterTimedListener(createOutputFile(OUTPUT_CHAPTERS_COMMON)));
        }
        catch (IOException e)
        {
            FuncTestOut.log("Unable to report chapters");
