...
public class TestMultiSelectInteractions extends JiraSeleniumTest
{
    ...
    private void _testExactMatch()
    {
        ...
        assertThat.elementPresentByTimeout("linkKey-textarea", DROP_DOWN_WAIT);
        client.typeWithFullKeyEvents("linkKey-textarea", "HSP-2");
    ...
    private void _testInlineErrorMessages()
    {
        getNavigator().editIssue("HSP-1");

        // Suggestions should still work when you leave behind an incomplete entry and come back.
        fixVersionsPicker.insertQuery("Ne");
        fixVersionsPicker.suggestions().assertOpen();
        fixVersionsPicker.suggestions().assertContains("New Version 1");
        client.focus("css=#environment");
        client.fireEvent(CSS_FIX_VERSIONS_TEXTAREA, "blur");
        assertThat.elementPresentByTimeout(VERSIONS_PICKER_ERROR);
        client.focus(fixVersionsPicker.inputAreaLocator());
        client.simulateKeyPressForCharacter(fixVersionsPicker.inputAreaLocator(), 'w');
        fixVersionsPicker.suggestions().assertOpen();
        fixVersionsPicker.suggestions().assertContains("New Version 1");

        // Error message should be present when invalid entry is left in textarea after blur.
        fixVersionsPicker.insertQuery("Bad Version");
        client.fireEvent(CSS_FIX_VERSIONS_TEXTAREA, "blur");
        assertThat.elementPresent(VERSIONS_PICKER_ERROR);

        // Error message should be updated if its new value is also an error.
        fixVersionsPicker.clearInputArea().insertQuery("Worse Version");
        client.fireEvent(CSS_FIX_VERSIONS_TEXTAREA, "blur");
        assertThat.elementDoesNotContainText(VERSIONS_PICKER_ERROR, "Bad Version");
        assertThat.elementContainsText(VERSIONS_PICKER_ERROR, "Worse Version");

        // No error message should be present when a valid entry is left in textarea after blur.
        fixVersionsPicker.clearInputArea().insertQuery("New Version 1").awayFromInputArea();
        assertThat.elementNotPresent(VERSIONS_PICKER_ERROR);

        // No error message should be present when a valid entry is added explicitly.
        fixVersionsPicker.insertQuery("Bad Version").awayFromInputArea();
        assertThat.elementPresent(VERSIONS_PICKER_ERROR);
        fixVersionsPicker.clearInputArea().insertQuery("New Version 4");
        client.fireEvent(CSS_FIX_VERSIONS_TEXTAREA, "blur");
        assertThat.elementNotPresent(VERSIONS_PICKER_ERROR);

        // No error message should be present when the textarea is empty.
        fixVersionsPicker.insertQuery("Bad Version").awayFromInputArea();
        assertThat.elementPresent(VERSIONS_PICKER_ERROR);
        fixVersionsPicker.clearInputArea().awayFromInputArea();
        assertThat.elementNotPresent(VERSIONS_PICKER_ERROR);
    }
