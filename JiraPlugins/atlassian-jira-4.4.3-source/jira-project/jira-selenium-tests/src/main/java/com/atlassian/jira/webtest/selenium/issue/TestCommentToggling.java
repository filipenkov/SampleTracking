...
public class TestCommentToggling extends AbstractAuiDialogTest
{
    ...
    private void _testFormsSubmit() {
        triggerCommentFromFooter();
        client.type("jquery=" + COMMENT_VISIBLE, DUMMY_TEXT_FROM_FOOTER);
        client.click(SUBMIT_SELECTOR, true);
        assertThat.textPresent(DUMMY_TEXT_FROM_FOOTER);

        triggerCommentFromStalkerBar();
        client.type("jquery=" + COMMENT_VISIBLE, DUMMY_TEXT_FROM_STALKER);
        client.click(SUBMIT_SELECTOR, true);
        assertThat.textPresent(DUMMY_TEXT_FROM_STALKER);
    }
