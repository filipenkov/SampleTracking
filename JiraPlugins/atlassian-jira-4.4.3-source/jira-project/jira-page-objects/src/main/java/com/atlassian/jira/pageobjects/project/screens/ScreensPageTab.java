...
public class ScreensPageTab extends AbstractProjectConfigPageTab
{
    ...
    public ConfigureIssueTypeScreenScheme gotoScheme()
    {
        final String schemeId = schemeEditLink.getAttribute("data-id");
        return dropDown.openAndClick(By.id(EDIT_LINK_ID), ConfigureIssueTypeScreenScheme.class, Long.valueOf(schemeId));
    }

    public SelectIssueTypeScreenScheme gotoSelectScheme()
    {
        final String projectId = schemeChangeLink.getAttribute("data-id");
        return dropDown.openAndClick(By.id(CHANGE_LINK_ID), SelectIssueTypeScreenScheme.class, Long.valueOf(projectId));
    }
    ...
    public List<ScreenSchemeInfo> getScreenSchemes()
    {
        ...
        for (PageElement section : sections)
        {
            ...
            if (sharedByLink.isPresent())
            {
                ...
                String id = "inline-dialog-" + sharedByLink.getAttribute("href").substring(1);
    ...
    public class ScreenSchemeInfo
    {
        ...
        public ConfigureFieldScreenScheme gotoEditScheme()
        {
            Long schemeId = getId();
            assertTrue("No link present to change the issue type screen scheme.", editLink.isPresent());

            return pageBinder.navigateToAndBind(ConfigureFieldScreenScheme.class, schemeId);
        }
