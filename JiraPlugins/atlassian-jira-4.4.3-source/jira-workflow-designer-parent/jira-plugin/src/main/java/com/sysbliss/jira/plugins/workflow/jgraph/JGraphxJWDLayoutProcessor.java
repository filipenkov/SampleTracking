...
public class JGraphxJWDLayoutProcessor {
    ...
    public static Logger log = Logger.getLogger(JGraphxJWDLayoutProcessor.class);
    public static String ICON_PREFIX = "/images/icons/status_";
    public static String ROOTS = "roots";
    public static String GRAPH = "graph";
    private HashMap<String, mxCell> jgraphVertexCache;
    private HashMap<String, mxCell> jgraphEdgeCache;
    private HashMap<String, NodeLayout> nodeLayoutCache;
    private HashMap<String, EdgeLayout> edgeLayoutCache;
    private List<mxCell> rootCells;
    private ConstantsManager constantsManager;
    private boolean showLabels;

    public JGraphxJWDLayoutProcessor(ConstantsManager constantsManager) {
    ...
    public synchronized Map<String,Object> calculateLayout(final List<NodeLayout> roots, JiraWorkflow workflow) {
        Map<String,Object> returnVals = runDefaultLayoutLayout(roots,workflow);
        //returnVals = runDefaultLayoutLayout((List<NodeLayout>)returnVals.get(ROOTS), workflow);
        //returnVals = runDefaultLayoutLayout((List<NodeLayout>)returnVals.get(ROOTS), workflow);

        return returnVals;
    }

    public synchronized Map<String,Object> runDefaultLayoutLayout(final List<NodeLayout> roots, JiraWorkflow workflow) {
        ...
        mxGraph graph = mxGraphFromJWDLayout(layer, roots, workflow, -1, true);
    ...
    public mxGraph mxGraphFromJWDLayout(List<NodeLayout> roots, JiraWorkflow workflow, int stepId, boolean showLabels) {
        return mxGraphFromJWDLayout(null, roots, workflow, stepId, showLabels);
    }

    public mxGraph mxGraphFromJWDLayout(Object parent, List<NodeLayout> roots, JiraWorkflow workflow, int stepId, boolean showLabels) {

        this.jgraphVertexCache = new HashMap<String, mxCell>();
        this.jgraphEdgeCache = new HashMap<String, mxCell>();
        this.nodeLayoutCache = new HashMap<String, NodeLayout>();
        this.edgeLayoutCache = new HashMap<String, EdgeLayout>();
        this.rootCells = new ArrayList<mxCell>();
        ...
        mxGraph graph = new mxGraph() {
            ...
            public String convertValueToString(Object cell) {
                if (cell instanceof mxCell) {
                    ...
                    if(((mxCell) cell).getValue() instanceof LayoutObject) {
                        LayoutObject value = (LayoutObject) ((mxCell) cell).getValue();
            ...
            //override to make sure layers don't show a collapse button
    ...
    private mxCell createNode(mxGraph graph, Object parent, NodeLayout nodeLayout, JiraWorkflow workflow, int stepId) {
        ...
        if (!jgraphVertexCache.containsKey(nodeLayout.getId())) {
            ...
            int myStepId = nodeLayout.getStepId();
    ...
    public List<NodeLayout> createReturnLayout(mxGraph graph, Object parent) {
    ...
    private void createReturnEdges(mxGraph graph, Object parent, final mxCell startNode) {
        ...
        for (int e = 0; e < edgeCount; e++) {
            ...
            if (!edgeLayoutCache.containsKey(edgeLayout.getId())) {
                ...
                final mxCell endCell = (mxCell) edgeCell.getTarget();
                final NodeLayout endNode = createReturnNode(graph, parent, endCell);
