<%--
  ...
  @author Franck WOLFF
--%><%
    boolean isErrorClass = false;
    if(jClass.type.getSuperclass() != null && jClass.type.getSuperclass().getName() == "java.lang.Exception") {
      isErrorClass = true;
    }
    ...
    public class ${jClass.as3Type.name}Base<%
        ...
        for (jInterface in jClass.interfaces) {
            if (!implementsWritten) {
                %> implements ${jInterface.as3Type.name}Base<%

                implementsWritten = true;
            } else {
                %>, ${jInterface.as3Type.name}Base<%
            }
        }
...
<%
    ...
    for (jProperty in jClass.properties) {%>
     ...
     ///////////////////////////////////////////////////////////////////////////
    // Write Error constructor if needed
    if(isErrorClass) {%>

        public var stackTrace:Array;

        public function ${jClass.as3Type.name}Base(message:String="", id:int=0){
			super(message, id);
		}<%
