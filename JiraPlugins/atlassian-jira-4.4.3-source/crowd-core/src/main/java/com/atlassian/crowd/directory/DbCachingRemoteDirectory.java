...
public class DbCachingRemoteDirectory implements RemoteDirectory, SynchronisableDirectory
{
    ...
    public void synchroniseCache(SynchronisationMode mode, SynchronisationStatusManager synchronisationStatusManager) throws OperationFailedException
    {
        ...
        try
        {
            ...
            {
                // Only sync the delta
                synchronisationStatusManager.syncStatus(getDirectoryId(), "directory.caching.sync.incremental");
            ...
            // Full sync
            syncType = "full";
            synchronisationStatusManager.syncStatus(getDirectoryId(), "directory.caching.sync.full");
            cacheRefresher.synchroniseAll(directoryCache);
        }
        finally
        {
            log.info(TimerStack.pop(syncType + " synchronisation complete in [ {0} ]"));
        }
    }
