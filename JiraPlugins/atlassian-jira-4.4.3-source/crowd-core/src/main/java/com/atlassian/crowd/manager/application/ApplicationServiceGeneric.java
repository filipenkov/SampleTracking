...
public class ApplicationServiceGeneric implements ApplicationService
{
    ...
    {
        return searchUsers(application, query, EntityComparator.of(User.class));
    }

    private <T> List<T> searchUsers(final Application application, final EntityQuery<T> query, final Comparator<? super T> comparator)
    {
        final boolean isAllResults = (query.getMaxResults() == EntityQuery.ALL_RESULTS);
        ...
        // we want directoryId in the comparison

        final Set<T> results = new TreeSet<T>(comparator);
        ...
        for (final Directory directory : getActiveDirectories(application))
        {
            try
            {
                final EntityQuery<T> totalQuery = QueryBuilder.queryFor(query.getReturnType(), query.getEntityDescriptor(),
                    query.getSearchRestriction(), 0, totalResults);

                // perform the search
                final List<T> users = directoryManager.searchUsers(directory.getId(), totalQuery);
        ...
        return SearchResultsUtil.constrainResults(new ArrayList<T>(results), query.getStartIndex(), query.getMaxResults());
    }
    ...
    public <T> List<T> searchGroups(final Application application, final EntityQuery<T> query)
    {
        ...
        final Set<T> results = new TreeSet<T>(getAggregatingAndSortingComparatorFor(query.getReturnType()));
    ...
    public <T> List<T> searchDirectGroupRelationships(final Application application, final MembershipQuery<T> query)
    {
        ...
        if (query.getEntityToMatch().equals(EntityDescriptor.user()))
        {
            ...
            try
            {
                ...
                results.addAll(doDirectDirectoryMembershipQuery(query, totalResults, user.getDirectoryId()));
            }
            catch (UserNotFoundException e)
            {
        ...
        else
        {
            for (final Directory directory : getActiveDirectories(application))
            {
                results.addAll(doDirectDirectoryMembershipQuery(query, totalResults, directory.getId()));
                if ((query.getMaxResults() != EntityQuery.ALL_RESULTS) && (results.size() >= query.getMaxResults()))
                {
                    break;
                }
            }
    ...
    public <T> List<T> searchNestedGroupRelationships(final Application application, final MembershipQuery<T> query)
    {
        ...
        if (query.getEntityToMatch().equals(EntityDescriptor.user()))
        {
            ...
            {
                User user = findUserByName(application, query.getEntityNameToMatch());
                results.addAll(doNestedDirectoryMembershipQuery(query, totalResults, user.getDirectoryId()));
            }
            catch (UserNotFoundException e)
            {
                // Don't worry.  No results.
        ...
        else
        {
            for (final Directory directory : getActiveDirectories(application))
            {
                results.addAll(doNestedDirectoryMembershipQuery(query, totalResults, directory.getId()));
                if ((query.getMaxResults() != EntityQuery.ALL_RESULTS) && (results.size() >= query.getMaxResults()))
                {
                    break;
                }
     ...
     */
    private <T> Comparator<T> getAggregatingAndSortingComparatorFor(final Class<T> type)
    {
