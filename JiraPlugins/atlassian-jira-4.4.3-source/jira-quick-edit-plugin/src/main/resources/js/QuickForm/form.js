 ...
 * @class AbstractForm
 */
JIRA.QuickForm.AbstractForm = AJS.Control.extend({
    ...
    submit: function () {
        ...
        instance.getForm().find(":submit").attr("disabled", "disabled");

        instance.getForm().animate({
            opacity: 0.2
        }, "fast");


        // Make sure that we atleast wait 700ms to fade form back in
        ...
        return JIRA.SmartAjax.makeRequest({
            ...
            complete: function (xhr, textStatus, smartAjaxResult) {
                ...
                instance.getForm().find(":submit").removeAttr("disabled");
                ...
                // if we haven't already faded the form back in - do so.
                if (fadeIn) {
                    fadeIn();
                }
    ...
    /**
     * Serializes form into paramater string
     * @return {String}
     */
    serialize: function () {

        var postBody = this.getForm().serialize();
    ...
    createField: function () {
        throw new Error("JIRA.QuickForm.AbstractForm: You must implement [createField] method in subclass.");
    },
    ...
    render: function (values) {
        ...
        if (values) {
            ...
            this.model.refresh(values).done(function () {
                instance._render().done(function () {
                    instance.triggerEvent("rendered", [instance.$element]);
 ...
 * using a picker (@see JIRA.QuickForm.FieldPicker). Users can also configure the order of these fields using
 * drag and drop.
 *
 * @class AbstractConfigurableForm
 */
JIRA.QuickForm.AbstractConfigurableForm = JIRA.QuickForm.AbstractForm.extend({

    DRAGGING_FIELD_CLASS: "qf-field-dragging",
    TRANSITIONING_FIELD_CLASS: "qf-field-transitioning",
    SORTABLE_ITEM_SELECTOR: ".qf-field-active:not(.qf-required)",
    DRAG_HANDLE_SELECTOR: ".drag-handle",
    SORTABLE_PLACEHOLDER_CLASS: "qf-placeholder",

    /**
     * Gets all fields
     *
     * @param Array<JIRA.QuickForm.Field> fields
     * @return jQuery Promise
     */
    getFields: function () {
    ...
    serialize: function () {
        ...
        var instance = this,
            postBody = this._super.apply(this, arguments);
        ...
        if (this.model.hasRetainFeature()) {
            ...
            jQuery.each(this.getActiveFieldIds(), function (i, fieldId) {
                instance.model.addFieldToRetainValue(fieldId);
            });
     ...
     *
     * @param descriptor
     * @return {JIRA.QuickForm.ConfigurableField}
     */
    createField: function (descriptor) {
        ...
        var instance = this,
            field = new JIRA.QuickForm.ConfigurableField(descriptor);
        ...
        instance.getFields().done(function (fields) {
            ...
            instance.model.getActiveFieldIds().done(function (activeIds) {
                ...
                jQuery.each(fields, function () {
                    instance.getFormContent().append(this.render());
                });
                ...
                instance.hasNoVisibleFields().done(function (answer) {
                    ...
                    deferred.resolveWith(this, [instance.$element]);
                });
            });
        });
 ...
 * @class AbstractConfigurableForm
 */
JIRA.QuickForm.AbstractUnconfigurableForm =  JIRA.QuickForm.AbstractForm.extend({
 ...
 * @class Error
 */
JIRA.QuickForm.Error = AJS.Control.extend({
