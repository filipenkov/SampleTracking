...
public class JiraHelper
{
    ...
    public StreamsEntry.Parameters<HasId, HasPostedDate, HasAlternateLinkUri, HasApplicationType, NeedsRenderer, NeedsActivityObject, NeedsVerb, NeedsAuthors> newBuilder(JiraActivityItem item)
    {
        return entryBuilderFactory.newParams(item, getIssueUri(item));
    }

    public StreamsEntry.Parameters<HasId, HasPostedDate, HasAlternateLinkUri, HasApplicationType, NeedsRenderer, NeedsActivityObject, NeedsVerb, NeedsAuthors> newCommentBuilder(JiraActivityItem item)
    {
        return entryBuilderFactory.newParams(item, uriProvider.getIssueCommentUri(item.getComment().get()));
    }

    public StreamsEntry.Parameters<HasId, HasPostedDate, HasAlternateLinkUri, HasApplicationType, NeedsRenderer, NeedsActivityObject, NeedsVerb, NeedsAuthors> newLinkedIssueBuilder(JiraActivityItem item)
    {
    ...
    {
        return getValues(transform(filter(changeItems, isAttachment()), getAttachment()));
    }
    ...
    {
        final int oldStatusId = Integer.parseInt(changeItem.getString("oldvalue"));
        final String newStatusIdString = changeItem.getString("newvalue");
        final int newStatusId = Integer.parseInt(newStatusIdString);
