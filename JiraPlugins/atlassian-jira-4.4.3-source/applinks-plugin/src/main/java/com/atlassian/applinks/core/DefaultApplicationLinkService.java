...
public class DefaultApplicationLinkService implements MutatingApplicationLinkService
{
    ...
    private String findSuitableName(final String name)
    {
        ...
        if (!isNameInUse(name, allApplicationLinks))
        {
            return name;
        }
        ...
        while (isNameInUse(proposedName, allApplicationLinks));
        return proposedName;
    }

    private boolean isNameInUse(final String name, final Iterable<ApplicationLink> allApplicationLinks)
    {
        try
        {
            Iterables.find(allApplicationLinks, new Predicate<ApplicationLink>()
            {
                ...
                {
                    return appLink.getName().equals(name);
                }
            });
        }
    ...
    private InternalApplicationLink retrieveApplicationLink(final ApplicationId id) throws TypeNotInstalledException
    {
        ...
        if (type == null)
        {
            LOG.warn(String.format("Couldn't load type %s for application link with id %s. Type is not installed?", properties.getType(), id.get()));
    ...
    public void createReciprocalLink(final URI remoteRpcUrl, final URI customLocalRpcUrl, final String username, final String password) throws ReciprocalActionException {
        ...
        final String url;
        try {
            url = restUrlBuilder.getUrlFor(RestUtil.getBaseRestUri(remoteRpcUrl), ApplicationLinkResource.class)
                    .updateApplicationLink(internalHostApplication.getId().toString(), null)
                    .toString();
        } catch (TypeNotInstalledException e) {
